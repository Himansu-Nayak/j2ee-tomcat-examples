...
/**
 * flow: - copy file - start the server - stop the server - remove the file
 */
public class FullRestartContainer extends AbstractContainers implements Containers {
    ...
    public boolean deploy(final InputStream archive, final String name) throws IOException {
        ...
        if (port > 0) {
            ...
            throw new OpenEJBTCKRuntimeException(
                    "Please set the tomee port using the system property 'server.http.port'");
        }
