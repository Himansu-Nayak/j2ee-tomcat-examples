...
public class RemoteTomEEEJBContainer extends EJBContainer {
    ...
    public static class Provider implements EJBContainerProvider {
        private static final List<String> CONTAINER_NAMES = Arrays.asList(RemoteTomEEEJBContainer.class.getName(),
                "tomee-remote", "remote-tomee");
        ...
        public EJBContainer createEJBContainer(final Map<?, ?> properties) {
            ...
            try {
                ejbContainerProviders = ProviderLocator.getServices(EJBContainerProvider.class.getName(),
                        EJBContainer.class, Thread.currentThread().getContextClassLoader()).size();
            } catch (final Exception e) {
                // no-op
            }

            if ((provider == null && ejbContainerProviders > 1) || (!RemoteTomEEEJBContainer.class.equals(provider)
            ...
            try {
                ...
                try {
                    instance.container.start(Arrays.asList(
                            ...
                            "-Dopenejb.system.apps=true", "-Dtomee.remote.support=true"), "start", true);
                } catch (final Exception e) {
                    instance.container.destroy();
                    throw e;
                }
                ...
                if (deployer == null) {
                    throw new TomEERemoteEJBContainerException(
                            "Can't lookup deployer, eother increse retries or setup it correctly",
