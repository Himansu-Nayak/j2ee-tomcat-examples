...
public class TomEEMyFacesContainerInitializer implements ServletContainerInitializer {
    ...
    public void onStartup(final Set<Class<?>> classes, final ServletContext ctx) throws ServletException {
        ...
        abstractInitializerLogger.setFilter(new RemoveLogMessage(
                new RemoveLogMessage(abstractInitializerLogger.getFilter(), Level.WARNING,
        ...
        if (facesServletPresent || isFacesConfigPresent(ctx)) {
            ...
            if (ctx instanceof ApplicationContextFacade) {
                try {
                    ...
                    if (!Arrays.asList(tomcatCtx.findApplicationListeners())
                            .contains(StartupServletContextListener.class.getName())) {
                        addListener(ctx);
                    }
    ...
    private boolean isFacesConfigPresent(final ServletContext servletContext) {
        try {
            ...
            final Collection<URL> metaInfFacesConfigUrls = new TomEEFacesConfigResourceProvider()
                    .getMetaInfConfigurationResources(null);
