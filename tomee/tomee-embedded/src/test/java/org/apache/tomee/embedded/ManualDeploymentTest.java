...
public class ManualDeploymentTest {
    ...
    public void run() throws IOException {
        ...
        configuration
                .setDir(Files.mkdirs(new File("target/" + getClass().getSimpleName() + "-tomcat")).getAbsolutePath());
        ...
        try (final Container container = new Container(configuration)) {
            ...
            final File war = ShrinkWrap.create(WebArchive.class, "the-webapp").addClass(Foo.class)
                    .addAsWebInfResource(EmptyAsset.INSTANCE, "beans.xml") // activate CDI
                    .as(ExplodedExporter.class).exportExploded(parent);
            ...
            assertEquals("Awesome",
                    IO.slurp(new URL("http://localhost:" + configuration.getHttpPort() + "/awesome")).trim());
