...
public class ClasspathAsWebappTest {
    ...
    public void run() throws MalformedURLException {
        ...
        MyBean.VALUE = null;
        try (final Container container = new Container(new Configuration().http(NetworkUtil.getNextAvailablePort())
                .property("openejb.container.additional.exclude", "org.apache.tomee.embedded.")
                .property("openejb.additional.include", "tomee-").user("tomee", "tomeepwd")
                .loginConfig(new LoginConfigBuilder().basic())
                .securityConstaint(new SecurityConstaintBuilder().addAuthRole("**").authConstraint(true)
                        .addCollection("api", "/api/resource2/")))
                                .deployPathsAsWebapp(JarLocation.jarLocation(MyInitializer.class)).inject(this)) {
            ...
            try {
                assertEquals("Servlet!",
                        IO.slurp(new URL("http://localhost:" + container.getConfiguration().getHttpPort() + "/s")));
            } catch (final IOException e) {
                fail(e.getMessage());
            }
            try {
                assertEquals("WebServlet",
                        IO.slurp(new URL("http://localhost:" + container.getConfiguration().getHttpPort() + "/w")));
            } catch (final IOException e) {
                fail(e.getMessage());
            }
            ...
            try {
                assertEquals("JSP",
                        IO.slurp(new URL(
                                "http://localhost:" + container.getConfiguration().getHttpPort() + "/?test=JSP"))
                        .trim());
            } catch (final IOException e) {
                fail(e.getMessage());
            }
            ...
            // JAXRS
            try {
                assertEquals("jaxrs", IO.slurp(
                        new URL("http://localhost:" + container.getConfiguration().getHttpPort() + "/api/resource")));
            } catch (final IOException e) {
                fail(e.getMessage());
            }
            ...
            try {
                final URL url = new URL(
                        "http://localhost:" + container.getConfiguration().getHttpPort() + "/api/resource2/");
            ...
            try {
                assertEquals("tomee", IO.slurp(new URL("http://tomee:tomeepwd@localhost:"
                        + container.getConfiguration().getHttpPort() + "/api/resource2/")));
            ...
            try {
                ...
                final Session session = webSocketContainer.connectToServer(webSocketClient,
                        new URI("ws://localhost:" + container.getConfiguration().getHttpPort() + "/ws"));
    ...
    public static class WebSocketEndpoint {
        ...
        public void open(final Session session) {
            try {
                ...
                try {
                    session.close(
                            new CloseReason(CloseReason.CloseCodes.PROTOCOL_ERROR, "Should be able to send a message"));
