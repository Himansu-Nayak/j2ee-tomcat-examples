...
public class Container implements AutoCloseable {
    ...
    private static void addCallersAsEjbModule(final ClassLoader loader, final AppModule app,
            final String... additionalCallers) {
        final Set<String> callers = new HashSet<>(NewLoaderLogic.callers(
                Filters.classes(Container.class.getName(), "org.apache.openejb.maven.plugins.TomEEEmbeddedMojo")));
    ...
    public Container deployClasspathAsWebApp(final String context, final File docBase,
            final boolean singleClassLoader) {
    ...
    public Container deployClasspathAsWebApp(final String context, final File docBase, final List<String> callers,
        ...
        final List<URL> jarList = new DeploymentsResolver.ClasspathSearcher()
                .loadUrls(Thread.currentThread().getContextClassLoader()).getUrls();
        if (dependencies != null) {
            for (final String dep : dependencies) {
                final Set<String> strings = SystemInstance.get().getComponent(ProvisioningResolver.class)
                        .realLocation(dep);
        ...
        try {
            return deployPathsAsWebapp(context,
                    ...
                    NewLoaderLogic
                            .applyBuiltinExcludes(new UrlSet(jarList),
                                    NewLoaderLogic.ADDITIONAL_INCLUDE == null ? null
                                            : Filters.prefixes(
                                                    NewLoaderLogic.ADDITIONAL_INCLUDE.split("[ \t\n\n]*,[ \t\n\n]*")))
                            .getUrls(),
    ...
    public Container deployPathsAsWebapp(final String context, final List<URL> jarList, final File docBase,
        ...
        final WebModule webModule = new WebModule(new WebApp(), contextRoot, loader, jarLocation.getAbsolutePath(),
                contextRoot.replace("/", ""));
        if (docBase == null) {
        ...
        try {
            final String filterContainerClasses = SystemInstance.get()
                    .getProperty("tomee.embedded.filter-container-classes");
            webModule.setFinder(new FinderFactory.OpenEJBAnnotationFinder(
        ...
        try {
            ...
            DeploymentLoader.addWebModuleDescriptors(new File(webModule.getJarLocation()).toURI().toURL(), webModule,
                    app);
        } catch (final Exception e) {
            throw new IllegalStateException(e);
        }
        ...
        try {
            ...
            systemInstance.getComponent(Assembler.class).createApplication(appInfo,
                    loader /* don't recreate a classloader */);
        } catch (final Exception e) {
            throw new IllegalStateException(e);
        }
    ...
    public void start() throws Exception {
        ...
        /*
         * if we use our config (Logger.configure()) don't override it copyFileTo(conf, "logging.properties");
         * System.setProperty("java.util.logging.manager", "org.apache.juli.ClassLoaderLogManager"); final File logging
         * = new File(conf, "logging.properties"); if (logging.exists()) {
    ...
    public AppContext deploy(final String name, final File file, final boolean overrideName)
