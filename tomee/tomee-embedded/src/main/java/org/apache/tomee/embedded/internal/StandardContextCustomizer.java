...
public class StandardContextCustomizer {
    ...
    public void customize(@Observes final LifecycleEvent event) {
            ...
            for (final URL url : module.getScannableUrls()) {
                ...
                if (file.isDirectory()) {
                    ...
                    if (new File(file, "META-INF/resources").exists()) {
                        resources.createWebResourceSet(WebResourceRoot.ResourceSetType.RESOURCE_JAR, "/", absolutePath,
                                "", "/META-INF/resources");
                    }
                } else {
                    if (absolutePath.endsWith(".jar")
                            || Boolean.getBoolean("tomee.embedded.resources.add-war-as-jar")) {
                        resources.createWebResourceSet(WebResourceRoot.ResourceSetType.CLASSES_JAR, "/WEB-INF/lib",
                                absolutePath, null, "/");
                        resources.createWebResourceSet(WebResourceRoot.ResourceSetType.RESOURCE_JAR, "/", url,
                                "/META-INF/resources");
        ...
        case Lifecycle.CONFIGURE_START_EVENT:
            SystemInstance.get().getComponent(TomcatWebAppBuilder.class).setFinderOnContextConfig(context,
                    module.appModule());
            break;
        default:
        }
