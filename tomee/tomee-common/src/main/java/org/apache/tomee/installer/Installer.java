...
public class Installer implements InstallerInterface {
    ...
    static {
        ...
        agentInstalled = InstallerTools.invokeStaticNoArgMethod("org.apache.openejb.javaagent.Agent",
    ...
    public void addTomEEAdminConfInTomcatUsers(final boolean securityActivated) {
        ...
        // add our listener
        final String roleUserTags = "  <role rolename=\"tomee-admin\" />\n"
                + "  <user username=\"tomee\" password=\"tomee\" roles=\"tomee-admin,manager-gui\" />\n";
        final String content;
        if (!securityActivated) {
            content = "  <!-- Activate those lines to get access to TomEE GUI -->\n" + "  <!--\n" + roleUserTags
                    + "  -->\n" + "</tomcat-users>\n";
        } else {
            content = "  <!-- Activate those lines to get access to TomEE GUI\n -->" + roleUserTags
                    + "</tomcat-users>\n";
    ...
    private void workaroundOnBat() {
        ...
        if (bat == null || bat
                .contains(" NOT DEFINED ") /* already done, tomcat doesnt use yet this new Windows NT 4 syntax */) {
            return;
        }
    ...
    /*
     * private void addJavaeeInEndorsed() { final File endorsed = new File(paths.getCatalinaHomeDir(), "endorsed"); if
     * (!endorsed.mkdir()) { alerts.addWarning("can't create endorsed directory"); }
     * 
     * final File jaxbApi = paths.findOpenEJBJar("geronimo-jaxb_2.2_spec"); copyClasses(paths.getJavaEEAPIJar(),
     * jaxbApi, new File(endorsed, "jaxb-api.jar"), "javax/xml/bind/.*",
     * Arrays.asList("javax/xml/bind/ContextFinder.class", "javax/xml/bind/DatatypeConverter.class"));
     * removeJar(jaxbApi);
     * 
     * // don't put jaxb-impl in endorsed since it relies on the jvm itself final File jaxbImpl = new
     * File(paths.getCatalinaLibDir(), "jaxb-impl.jar"); if (!jaxbImpl.exists()) { try {
     * Installers.copyFile(paths.getJAXBImpl(), jaxbImpl); } catch (final IOException e) { alerts.addError("can't copy "
     * + paths.getJAXBImpl().getPath() + " to " + endorsed.getPath() + "/jaxb-impl.jar"); } } }
     * 
     * 
     * private void copyClasses(final File javaEEAPIJar, final File sourceJar, final File destinationJar, final String
     * pattern, final List<String> exceptions) { if (javaEEAPIJar == null) { throw new
     * NullPointerException("javaEEAPIJar"); } if (sourceJar == null) { throw new NullPointerException("sourceJar"); }
     * if (destinationJar == null) { throw new NullPointerException("destinationJar"); } if (pattern == null) { throw
     * new NullPointerException("pattern"); } if (exceptions == null) { throw new NullPointerException("exceptions"); }
     * 
     * if (destinationJar.exists()) { return; }
     * 
     * try { final ByteArrayOutputStream destinationBuffer = new ByteArrayOutputStream(524288); final ZipOutputStream
     * destination = new ZipOutputStream(destinationBuffer); final ZipInputStream source = new
     * ZipInputStream(IO.read(sourceJar)); for (ZipEntry entry; (entry = source.getNextEntry()) != null; ) { final
     * String entryName = entry.getName(); if (!entryName.matches(pattern) || exceptions.contains(entryName)) {
     * continue; } destination.putNextEntry(new ZipEntry(entryName)); IO.copy(source, destination); } IO.close(source);
     * final ZipInputStream source2 = new ZipInputStream(IO.read(javaEEAPIJar)); for (ZipEntry entry; (entry =
     * source2.getNextEntry()) != null; ) { final String entryName = entry.getName(); if (!entryName.matches(pattern) ||
     * !exceptions.contains(entryName)) { continue; }
     * 
     * destination.putNextEntry(new ZipEntry(entryName));
     * 
     * IO.copy(source2, destination); } IO.close(source2); IO.close(destination);
     * IO.copy(destinationBuffer.toByteArray(), destinationJar); } catch (final IOException e) {
    ...
    public void installListener(final String listener) {
        ...
        if (copyOpenEJBLoader) {
            try {
                ...
                alerts.addError(
                        "Unable to copy OpenEJB Tomcat loader jar to Tomcat lib directory.  This will need to be performed manually.",
                        e);
            }
        }
        ...
        //Add TomEE header
        try {
            newServerXml = Installers.replace(newServerXml, "<Connector port=\"8080\"", "<Connector port=\"8080\"",
                    "/>", "xpoweredBy=\"false\" server=\"Apache TomEE\" />");

            newServerXml = Installers.replace(newServerXml, "<Connector port=\"8443\"", "<Connector port=\"8443\"",
                    "/>", "xpoweredBy=\"false\" server=\"Apache TomEE\" />");
        } catch (final IOException e) {
    ...
    public void installJavaagent() {
        ...
        if (copyJavaagentJar) {
            try {
                ...
                alerts.addError(
                        "Unable to copy OpenEJB javaagent jar to Tomcat lib directory.  This will need to be performed manually.",
                        e);
            }
        }
    ...
    /**
     * Installs conf/tomee.xml and conf/logging.properties files. This method retrieves the tomee.xml and
     * logging.properties files from openejb core jar file and installs them under the conf directory of tomcat. if
     * there is already a conf/logging.properties file available then this method appends the contents of openejb
     * logging.properties file to the exisiting properties file.
     *
     * Replace web.xml to set jasper in production mode instead of dev mode.
     *
     * NOTE:- If the existing conf/logging.properties file already has some openejb specific configuration, then this
     * method will just leave the logging.properties file alone
     * 
     * @param builtIn
     */
        ...
        if (!openEjbXmlFile.exists()) {
            ...
            if (openEjbXml != null) {
                if (Installers.writeAll(openEjbXmlFile,
                        openEjbXml.replace("<openejb>", "<tomee>").replace("</openejb>", "</tomee>"), alerts)) {
        ...
        if (!openejbSystemProperties.exists()) {
            ...
            try {
                ...
                systemPropertiesWriter
                        .write("# for more information please see http://tomee.apache.org/properties-listing.html\n");

                systemPropertiesWriter.write("\n");
                systemPropertiesWriter
                        .write("# allowed packages to be deserialized, by security we denied all by default, "
                ...
                systemPropertiesWriter.write("\n");
                systemPropertiesWriter.write(
                        "# Activate EE default resources (ManagedExecutorService, JMSConnectionFactory if JMS is there...)");
                ...
                systemPropertiesWriter
                        .write("# openejb.jndiname.format = {deploymentId}{interfaceType.annotationName}\n");
                ...
                systemPropertiesWriter.write("#\n");
                systemPropertiesWriter
                        .write("# These properties are only for cxf service (SOAP webservices) and TomEE+\n");
                systemPropertiesWriter.write(
                ...
                systemPropertiesWriter
                        .write("# javax.xml.soap.MetaFactory = com.sun.xml.messaging.saaj.soap.SAAJMetaFactoryImpl\n");
            } catch (final IOException e) {
    ...
    private void installTomEEJuli(final Alerts alerts, final File loggingPropsFile, final String newLoggingProps) {
        if (newLoggingProps != null
                && Installers.writeAll(loggingPropsFile, newLoggingProps.replace("java.util.logging.ConsoleHandler",
                        "org.apache.tomee.jul.formatter.AsyncConsoleHandler"), alerts)) {
