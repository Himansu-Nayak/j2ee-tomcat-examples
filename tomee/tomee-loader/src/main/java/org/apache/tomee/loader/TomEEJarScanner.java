...
public class TomEEJarScanner extends StandardJarScanner {
    ...
    private void embeddedSurefireScanning(final JarScanType scanType, final ServletContext context,
        ...
        if (isScanClassPath() && System.getProperty("surefire.real.class.path") != null) {
            ...
            try {
                ...
                for (final URL url : urls) {
                    ...
                    if ((cpe.getName().endsWith(".jar") || scanType == JarScanType.PLUGGABILITY || scanAllDirectories)
                            && getJarScanFilter().check(scanType, cpe.getName())) {
                        try {
                            if (process == null) {
                                process = StandardJarScanner.class.getDeclaredMethod("process", JarScanType.class,
    ...
    private static class TomEEFilter implements JarScanFilter {
        private static final Filter INCLUDE = Filters.tokens("javax.faces-2.", "spring-security-taglibs",
                "spring-webmvc");
        ...
        public boolean check(final JarScanType jarScanType, final String jarName) {
            if (jarScanType == JarScanType.TLD) {
                ...
                if (jarName.startsWith("tomcat-websocket") || jarName
                        .startsWith("myfaces-impl") /* see org.apache.tomee.jasper.TomEETldScanner.scanPlatform */) {
                    return false;
                }
            }
