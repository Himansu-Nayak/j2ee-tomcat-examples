 ...
 *
 * With that information this class finds the openejb-loader jar in the tomee.war essentially creates a "mini-webapp"
 * which is to say it creates a new WebappClassloader that contains the openejb-loader jar and then uses that
 * classloader to reflectively call the {@link TomcatHook#hook} method which does all the work to load OpenEJB into
 * Tomcat
 *
 * This messing around is required so that it doesn't matter if the {@link OpenEJBListener}, which does not execute in a
 * webapp classloader, or the {@link LoaderServlet}, which does, calls the TomcatEmbedder. Either way the embedding
 * process starts inside a WebappClassloader and keeps that very complex code just a little simpler.
 */
public class TomcatEmbedder {
     ...
     * 
     * @param properties
     *            this instance contains all System properties as well as all initialization parameters of the
     *            LoaderServlet
     * @param catalinaCl
     *            The ClassLoader which loaded the ServletConfig class
     */
    public static void embed(final Properties properties, final ClassLoader catalinaCl) {
        ...
        try {
            final ClassLoader childCl = new URLClassLoader(new URL[] { getThisJar().toURI().toURL(),
    ...
    /**
     * Gets path to jar file that has namePrefix and in the tomee.war/lib location.
     * 
     * @param tomeeWar
     *            path to tomee.war
     * @param namePrefix
     *            prefix of the jar file
     * @return path to file
     */
