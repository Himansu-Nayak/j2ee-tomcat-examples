 ...
 * <p/>
 * This is an alternate way to load the Tomcat integration This approach is mutually exclussive to the
 * {@link LoaderServlet}
 * <p/>
 * This class does nothing more than scrape around in Tomcat and look for the tomee.war so it can call the embedder
 * <p/>
 * This class can be installed in the Tomcat server.xml as an alternate way to bootstrap OpenEJB into Tomcat. The
 * benefit of this is that OpenEJB is guaranteed to start before all webapps.
 */
    ...
    private static boolean isTomEEWar(final File file) {
        ...
        try {
            ...
            return jarFile.getEntry("lib") != null
                    && (name.startsWith("tomee") || name.startsWith("openejb") && name.endsWith(".war"));
