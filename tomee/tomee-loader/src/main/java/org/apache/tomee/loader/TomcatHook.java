 ...
 *
 * Everything that happens up to the point of calling this particular method (except setting tomee.war) ultimately means
 * nothing and does not matter to the integration.
 *
 * Requires tomee.war to be set, the sets the following properties:
 *
 * System properties: set openejb.home -> catalina.home set openejb.base -> catalina.base set tomee.war -> $tomee.war
 * set tomcat.version if not set set tomcat.built if not set
 *
 * Local properties: set openejb.loader -> tomcat-system set openejb.libs -> $tomee.war/lib
 *
 * With these properties setup, this class with construct an {@link Embedder} using the
 * "org.apache.tomee.catalina.TomcatLoader" as the loader.
 *
 * The Embedder will use the openejb.libs property to find all the jars to be loaded then it will use the openejb.loader
 * property to find out *how* to add them into the classpath of the right classloader. Once all the jars are in the
 * required class loader, it loads the {@link org.apache.openejb.loader.Loader} implementation and calls it's
 * {@link org.apache.openejb.loader.Loader#init} method.
 *
 * See org.apache.tomee.catalina.TomcatLoader for the next part of the story
 */
class TomcatHook {
    /**
     * Using tomee.war path, it sets several required system properties and init {@link SystemInstance#init(Properties)}
     * 
     * <p>
     * This method is called from {@link TomcatEmbedder#embed(Properties, ClassLoader)} method from classloader that
     * contains openejb-loader and tomee-loader with CatalinaClassLoader as the parent.
     * </p>
     * 
     * @param properties
     *            properties file
