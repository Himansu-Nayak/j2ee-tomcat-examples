...
public class TomcatHelper {
    ...
    public static StandardServer getServer() {
        ...
        try {
            final Class<?> systemInstanceClass = Thread.currentThread().getContextClassLoader()
            ...
            server = (StandardServer) systemInstanceClass.getDeclaredMethod("getComponent", Class.class)
                    .invoke(instance, StandardServer.class);
        ...
        try {
            ...
            if (result.getClass().isEnum()) {
                ...
                if ("FAILED".equals(e.toString())) {
                    return 4;
                } else if ("STOPPING".equals(e.toString()) || "STOPPING_PREP".equals(e.toString())
                        || "MUST_STOP".equals(e.toString()) || "MUST_DESTROY".equals(e.toString())) {
                    return 2;
    ...
    public static void configureJarScanner(final Context standardContext) {
        try { // override only if default
            if ("true".equalsIgnoreCase(SystemInstance.get().getProperty("tomee.tomcat.override.jar-scanner", "true"))
                ...
                final String scanBootstrap = properties
                        .getProperty(TomEEJarScanner.class.getName() + ".scanBootstrapClassPath");
