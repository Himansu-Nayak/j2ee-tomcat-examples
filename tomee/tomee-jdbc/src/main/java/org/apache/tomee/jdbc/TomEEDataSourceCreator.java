...
public class TomEEDataSourceCreator extends PoolDataSourceCreator {
    ...
    private SuperProperties createProperties(final String name, final Properties properties) {
        ...
        converted.setProperty("driverClassName",
                properties.getProperty("driverClassName", (String) properties.remove("JdbcDriver")));
        ...
        if (passwordCipher != null && !"PlainText".equals(passwordCipher)) {
            converted.setProperty("password", PasswordCipherFactory.getPasswordCipher(passwordCipher)
                    .decrypt(converted.getProperty("Password").toCharArray()));
        }
    ...
    private static class TomEEConnectionPool extends ConnectionPool {
        ...
        public TomEEConnectionPool(final PoolConfiguration poolProperties, final ClassLoader creationLoader)
                throws SQLException {
            ...
            if (getPoolProperties().getDataSource() == null) { // using driver
                ...
                try {
                    Reflections.set(con, "driver",
                            Class.forName(getPoolProperties().getDriverClassName(), true, cl).newInstance());
                } catch (final ClassNotFoundException cnfe) {
                    try { // custom resource classloader
                        Reflections.set(con, "driver", Class
                                .forName(getPoolProperties().getDriverClassName(), true, creationLoader).newInstance());
                    } catch (final Exception e) {
    ...
    public static class TomEEPoolProperties extends PoolProperties {
        ...
        public void setXaDataSource(final String jndi) {
            ...
            try {
                setDataSource(SystemInstance.get().getComponent(ContainerSystem.class).getJNDIContext()
                        .lookup("openejb:Resource/" + jndi));
