...
public class TomcatRsRegistry implements RsRegistry {
    ...
    private static String removeWebContext(final String webContext, final String completePath) {
        ...
        return completePath
                .substring((webContext.length() > 0 && !webContext.startsWith("/") ? 1 : 0) + webContext.length());
    }
    ...
    public AddressInfo createRsHttpListener(final String appId, final String webContext, final HttpListener listener,
            final ClassLoader classLoader, final String completePath, final String virtualHost, final String auth,
            final String realm) {
        ...
        if (host == null) {
            ...
            if (host == null) {
                throw new IllegalArgumentException("Invalid virtual host '" + virtualHost
                        + "'.  Do you have a matching Host entry in the server.xml?");
            }
        } else {
        ...
        if (context == null) {
            throw new IllegalStateException(
                    "Invalid context '" + webContext + "'.  Cannot find context in host " + host.getName());
        }
