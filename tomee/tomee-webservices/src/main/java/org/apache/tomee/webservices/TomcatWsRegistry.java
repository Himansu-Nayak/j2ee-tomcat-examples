...
public class TomcatWsRegistry implements WsRegistry {
    private static final String WEBSERVICE_SUB_CONTEXT = forceSlash(
            SystemInstance.get().getOptions().get("tomee.jaxws.subcontext", "/webservices"));

    private static final boolean WEBSERVICE_OLDCONTEXT_ACTIVE = SystemInstance.get().getOptions()
            .get("tomee.jaxws.oldsubcontext", false);
    ...
    private static Context createNewContext(final ClassLoader classLoader, String authMethod, String transportGuarantee,
        ...
        if (authMethod == null || "NONE".equals(authMethod)) { //NOPMD
            ...
            final String securityRole = SystemInstance.get().getProperty(TOMEE_JAXWS_SECURITY_ROLE_PREFIX + name,
                    "default");
    ...
    @Override
    public List<String> setWsContainer(final HttpListener httpListener, final ClassLoader classLoader,
            String contextRoot, String virtualHost, final ServletInfo servletInfo, final String realmName,
        ...
        if (host == null) {
            throw new IllegalArgumentException("Invalid virtual host '" + virtualHost
        ...
        if (context == null) {
            throw new IllegalArgumentException(
                    "Could not find web application context " + contextRoot + " in host " + host.getName());
        }

        final Wrapper wrapper = (Wrapper) context.findChild(servletInfo.servletName);
        if (wrapper == null) {
            throw new IllegalArgumentException("Could not find servlet " + servletInfo.servletName
                    + " in web application context " + context.getName());
        }
        ...
        for (final Connector connector : connectors) {
            for (final String mapping : wrapper.findMappings()) {
                final URI address = new URI(connector.getScheme(), null, host.getName(), connector.getPort(),
                        (contextRoot.startsWith("/") ? "" : "/") + contextRoot + mapping, null, null);
    ...
    @Override
    public List<String> addWsContainer(final HttpListener httpListener, final ClassLoader classLoader,
            final String context, String virtualHost, String path, final String realmName,
            final String transportGuarantee, final String authMethod, final String moduleId) throws Exception {
        if (path == null) {
        ...
        if (WEBSERVICE_OLDCONTEXT_ACTIVE) {
            deployInFakeWebapp(path, classLoader, authMethod, transportGuarantee, realmName, host, httpListener,
                    addresses, context);
        }
    ...
    private void deployInFakeWebapp(final String path, final ClassLoader classLoader, final String authMethod,
            final String transportGuarantee, final String realmName, final Container host,
            final HttpListener httpListener, final List<String> addresses, final String name) {
    ...
    private void addServlet(final Container host, final Context context, final String mapping,
            final HttpListener httpListener, final String path, final List<String> addresses,
            final boolean fakeDeployment, final String moduleId) {
        // build the servlet
        ...
        final String webServicecontainerID = wrapper.getName() + WsServlet.WEBSERVICE_CONTAINER
        ...
        for (final Connector connector : connectors) {
            ...
            try {
                final URI address = new URI(connector.getScheme(), null, host.getName(), connector.getPort(),
                        fullContextpath.toString(), null, null);
                addresses.add(address.toString());
    ...
    private static class Key {
        ...
        public String toString() {
            return "Key{" + "moduleId='" + moduleId + '\'' + ", path='" + path + '\'' + '}';
        }
    }
}
