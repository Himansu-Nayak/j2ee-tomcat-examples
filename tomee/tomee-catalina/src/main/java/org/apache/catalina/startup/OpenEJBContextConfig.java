...
public class OpenEJBContextConfig extends ContextConfig {
    ...
    private static final String ADJUST_DATASOURCE_JNDI_NAMES = SystemInstance.get()
            .getProperty("tomee.resources.adjust-web-xml-jndi-name", "true");
    ...
    private void addAddedJAXWsServices() {
        ...
        if (webAppInfo == null || appInfo == null || "false"
                .equalsIgnoreCase(appInfo.properties.getProperty("openejb.jaxws.add-missing-servlets", "true"))) {
            return;
        }
    ...
    private void cleanUpRestServlets() {
        ...
        if (webAppInfo == null || appInfo == null
                || "false".equalsIgnoreCase(appInfo.properties.getProperty("openejb.jaxrs.on", "true"))) {
            return;
        }
    ...
    private URL replaceKnownRealmsByTomEEOnes(final URL contextXml) throws MalformedURLException {
        return new URL(contextXml.getProtocol(), contextXml.getHost(), contextXml.getPort(), contextXml.getFile(),
            ...
            for (final ContextResource resource : foundResources) {
                if ("javax.sql.DataSource".equals(resource.getType())
                    ...
                    if (ids == null) {
                        ...
                        final OpenEjbConfiguration runningConfig = SystemInstance.get()
                                .getComponent(OpenEjbConfiguration.class);
                        ...
                        if (runningConfig != null) {
                            for (final ResourceInfo resourceInfo : runningConfig.facilities.resources) {
                                if (ConfigurationFactory.isResourceType(resourceInfo.service, resourceInfo.types,
    ...
    protected void processServletContainerInitializers() {
        try {
            ...
            final Iterator<Map.Entry<ServletContainerInitializer, Set<Class<?>>>> iterator = initializerClassMap
                    .entrySet().iterator();
            while (iterator.hasNext()) {
                ...
                if (classname.equals("org.apache.myfaces.ee6.MyFacesContainerInitializer")
                    ...
                    for (final Map.Entry<Class<?>, Set<ServletContainerInitializer>> scanning : typeInitializerMap
                            .entrySet()) {
            ...
            try {
                final Class<?> initializer = Class.forName("org.springframework.web.SpringServletContainerInitializer",
                        true, loader);
                final ServletContainerInitializer instance = (ServletContainerInitializer) initializer.newInstance();
                typeInitializerMap.put(Class.forName("org.springframework.web.WebApplicationInitializer", true, loader),
                        Collections.singleton(instance));
            ...
            if (typeInitializerMap.size() > 0 && finder != null) {
                for (final Map.Entry<Class<?>, Set<ServletContainerInitializer>> entry : typeInitializerMap
                        .entrySet()) {
                    ...
                    for (final ServletContainerInitializer sci : entry.getValue()) {
                        if (annotation.isAnnotation()) {
                            try {
                                final Class<? extends Annotation> reloadedAnnotation = Class.class
                                        .cast(tempLoader.loadClass(annotation.getName()));
    ...
    protected void processAnnotationsWebResource(final WebResource webResource, final WebXml fragment,
            final boolean handlesTypesOnly, final Map<String, JavaClassCacheEntry> javaClassCache) {
        final WebAppInfo webAppInfo = info.get();
    ...
    @Override
    protected void processAnnotationsStream(final InputStream is, final WebXml fragment, final boolean handlesTypesOnly,
            final Map<String, JavaClassCacheEntry> javaClassCache) throws ClassFormatException, IOException {
        // no-op
    }
    ...
    private void internalProcessAnnotations(final File currentUrlAsFile, final WebAppInfo webAppInfo,
    ...
    private void addClassesWithRightLoader(final ClassLoader loader, final ServletContainerInitializer sci,
            final List<Class<?>> implementations) throws ClassNotFoundException {
    ...
    @Override
    protected void processAnnotationWebServlet(final String className, final AnnotationEntry ae,
            final WebXml fragment) {
        try {
