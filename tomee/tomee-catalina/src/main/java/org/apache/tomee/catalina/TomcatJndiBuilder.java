...
import org.apache.tomee.common.*;
...
public class TomcatJndiBuilder {
    ...
    public TomcatJndiBuilder(final StandardContext standardContext, final WebAppInfo webAppInfo,
    ...
    public static void mergeJava(final StandardContext standardContext) {
        ...
        final TomcatWebAppBuilder builder = (TomcatWebAppBuilder) SystemInstance.get()
                .getComponent(WebAppBuilder.class);
            ...
            if (webContext == null && contextInfo != null && contextInfo.appInfo != null) { // can happen if deployed from apps/
                for (final WebAppInfo webAppInfo : contextInfo.appInfo.webApps) {
                    if (webAppInfo.path != null
                            && webAppInfo.path.replace(File.separatorChar, '/').equals(standardContext.getDocBase())) {
        ...
        try {
            ...
            if (webContext != null && webContext.getWebbeansContext() != null) {
                comp.rebind("BeanManager",
                        new InjectableBeanManager(webContext.getWebbeansContext().getBeanManagerImpl()));
            } else if (contextInfo != null) {
                final WebBeansContext webBeansContext = cs.getAppContext(contextInfo.appInfo.appId)
                        .getWebBeansContext();
    ...
    public static void importOpenEJBResourcesInTomcat(final Collection<ResourceInfo> resources,
    ...
    public void mergeRef(final NamingResourcesImpl naming, final PersistenceContextReferenceInfo ref,
            final URI moduleUri) {
        if (isLookupRef(naming, ref)) {
            return;
        }
        ...
        if (ref.location != null) {
            ...
            try {
                ...
                throw new IllegalStateException("PersistenceUnit '" + ref.unitId + "' not found for EXTENDED ref '"
            ...
            setResource(resource, new JtaEntityManager(ref.persistenceUnitName, jtaEntityManagerRegistry, factory,
    ...
    @SuppressWarnings({ "UnusedDeclaration" })
    public void mergeRef(final NamingResourcesImpl naming, final PersistenceUnitReferenceInfo ref,
            final URI moduleUri) {
        if (isLookupRef(naming, ref)) {
            return;
        }
    ...
    public void mergeRef(final NamingResourcesImpl naming, final ServiceReferenceInfo ref) {
        ...
        if (ref.location != null) {
            ...
            if (!ref.handlerChains.isEmpty()) {
                try {
                    final List<HandlerChainData> handlerChains = WsBuilder.toHandlerChainData(ref.handlerChains,
                    ...
                    throw new IllegalArgumentException("Error creating handler chain for web service-ref "
                            + ref.referenceName.replaceAll("^comp/env/", ""));
                }
            }
        }
    ...
    private URL getWsdlUrl(final ServiceReferenceInfo ref) {
        ...
        if (wsdlUrl == null) {
            throw new IllegalArgumentException("WSDL file " + ref.wsdlFile + " for web service-ref "
                    + ref.referenceName.replaceAll("^comp/env/", "") + " not found");
        }
