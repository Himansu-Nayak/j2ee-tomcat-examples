...
public class TomcatWebAppBuilder implements WebAppBuilder, ContextListener, ParentClassLoaderFinder {
     ...
     * Logger instance
     */
    private static final Logger logger = Logger.getInstance(LogCategory.OPENEJB.createChild("tomcat"),
            "org.apache.openejb.util.resources");
    private static final boolean FORCE_RELOADABLE = SystemInstance.get().getOptions().get("tomee.force-reloadable",
            false);
    ...
    /**
     * OpenEJB assembler instance TODO can we use the SPI interface instead?
     */
    private Assembler assembler;
    /**
     * OpenEJB container system TODO can we use the SPI interface instead?
     */
    ...
    private static File rootPath(final File file) {
        if (file.isDirectory() && file.getName().equals("classes") && file.getParentFile() != null
    ...
    private static void eagerInitOfLocalBeanProxies(final Collection<BeanContext> beans,
    ...
    private static Reference createReference(final ResourceBase resource) {
        ...
        if (resource instanceof ContextResource) {
            ...
            ref = new ResourceRef(resource.getType(), resource.getDescription(), cr.getScope(), cr.getAuth(),
                    cr.getSingleton());
        } else {
    ...
    private static void updateInjections(final Collection<Injection> injections, final ClassLoader classLoader,
            final boolean keepInjection) {
    ...
    private static boolean undeploy(final StandardContext standardContext, final Container host) {
        ...
        final TomEEWebappClassLoader tomEEWebappClassLoader = lazyClassLoader(
                org.apache.catalina.Context.class.isInstance(child) ? org.apache.catalina.Context.class.cast(child)
                        : null);
    ...
    private static boolean isIgnored(final StandardContext standardContext) {
        ...
        return standardContext.getServletContext().getAttribute(IGNORE_CONTEXT) != null
                ...
                || standardContext instanceof IgnoredStandardContext || isExcludedBySystemProperty(standardContext);
    }
    ...
    private static boolean isExcludedBySystemProperty(final StandardContext standardContext) {
        ...
        return "true".equalsIgnoreCase(
                systemInstance.getProperty(name + ".tomcat-only", systemInstance.getProperty("tomcat-only", "false")));
    }
    ...
    private static DeploymentLoader.ExternalConfiguration configuredClasspath(final StandardContext standardContext) {
        ...
        if (loader != null) {
            ...
            if (webResources != null) { // to enhance
                for (final WebResourceSet[] sets : asList(webResources.getPreResources(),
                        webResources.getPostResources(), webResources.getJarResources())) {
                    for (final WebResourceSet wr : sets) {
                        ...
                        if (base != null) {
                            ...
                            } else if (baseFile.exists() && baseFile.getName().endsWith(".jar")
                                    && wr.getResource("/WEB-INF/classes/").exists()) {
                                try {
    ...
    private void setComponentsUsedByCDI() {
        ...
        if (systemInstance.getComponent(HttpServletRequest.class) == null) {
            systemInstance.setComponent(HttpServletRequest.class,
                    Proxys.threadLocalProxy(HttpServletRequest.class, OpenEJBSecurityListener.requests, null));
        }
        if (systemInstance.getComponent(HttpSession.class) == null) {
            systemInstance.setComponent(javax.servlet.http.HttpSession.class,
                    Proxys.threadLocalRequestSessionProxy(OpenEJBSecurityListener.requests, null));
        }
        if (systemInstance.getComponent(ServletContext.class) == null) {
            systemInstance.setComponent(ServletContext.class, Proxys.handlerProxy(servletContextHandler,
                    ServletContext.class, CdiAppContextsService.FiredManually.class));
        }
    }
    ...
    public void start(final StandardServer server) {
        if (SystemInstance.get().isDefaultProfile()) { // add user tomee is no user are specified
            try {
                ...
                final UserDatabase db = (UserDatabase) server.getGlobalNamingContext()
        ...
        try {
            for (final WebAppInfo webApp : appInfo.webApps) {
                ...
                if (war.isDirectory()) {
                    ...
                    if (entry != null) {
                        contextXmlUrl = new URL("jar:" + war.getAbsoluteFile().toURI().toURL().toExternalForm() + "!/"
                ...
                StandardContext standardContext;
                {
                    ...
                    if (StandardHost.class.isInstance(host) && !StandardContext.class.getName()
                            .equals(StandardHost.class.cast(host).getContextClass())) {
                        try {
                            standardContext = StandardContext.class.cast(containerLoader
                                    .loadClass(StandardHost.class.cast(host).getContextClass()).newInstance());
                        } catch (final Throwable th) {
                ...
                standardContext
                        .setUnpackWAR(!"false".equalsIgnoreCase(appInfo.properties.getProperty("tomcat.unpackWar")));
                if (contextXml != null) {
                ...
                if (!isAlreadyDeployed(appInfo, webApp)) {
                    ...
                    if (standardContext.getDocBase() != null && standardContext.getDocBase().endsWith(".war")) {
                        ...
                        standardContext.setDocBase(
                                standardContext.getDocBase().substring(0, standardContext.getDocBase().length() - 4));
                    }
    ...
    private boolean isAlreadyDeployed(final AppInfo appInfo, final WebAppInfo webApp) {
        ...
        if (contextInfo != null && contextInfo.standardContext != null
                && contextInfo.standardContext.getState() == LifecycleState.FAILED) {
            synchronized (infos) {
    ...
    public synchronized ContextInfo standaAloneWebAppInfo(final File file) {
        for (final ContextInfo info : infos.values()) {
            if (info.appInfo != null && info.appInfo.webAppAlone && ((file.equals(new File(info.appInfo.path))
                    || file.equals(new File(info.appInfo.path + ".war"))))) {
                return info;
            }
            if (info.standardContext != null && (file.equals(new File(info.standardContext.getDocBase()))
                    || file.equals(new File(info.standardContext.getDocBase() + ".war")))) {
                return info;
            }
        }
    ...
    public void init(final StandardContext standardContext) {
        ...
        String sessionManager = SystemInstance.get().getOptions().get(OPENEJB_SESSION_MANAGER_PROPERTY + "." + name,
                (String) null);
    ...
    public void beforeStart(final StandardContext standardContext) {
        if (standardContext.getResources() != null
                && LazyStopStandardRoot.class.isInstance(standardContext.getResources())) {
            // reset after reload
            Reflections.set(standardContext, "resources",
                    LazyStopStandardRoot.class.cast(standardContext.getResources()).getDelegate());
        }
    ...
    private void initContextLoader(final StandardContext standardContext) {
        ...
        if (standardContextLoader != null && ((!TomEEWebappLoader.class.equals(standardContextLoader.getClass())
                && !WebappLoader.class.equals(standardContextLoader.getClass()))
                || (WebappLoader.class.equals(standardContextLoader.getClass()) && !WebappLoader.class
                        .cast(standardContextLoader).getLoaderClass().startsWith("org.apache.tom")))) {
    ...
    public void configureStart(final LifecycleEvent event, final StandardContext standardContext) {
        ...
        // breaks cdi
        standardContext.setTldValidation(
                Boolean.parseBoolean(SystemInstance.get().getProperty("tomee.tld.validation", "false")));
        // breaks jstl
        standardContext.setXmlValidation(
                Boolean.parseBoolean(SystemInstance.get().getProperty("tomee.xml.validation", "false")));
    }
    ...
    private void startInternal(final StandardContext standardContext) {
        ...
        if (a == null) {
            logger.warning("OpenEJB has not been initialized so war will not be scanned for nested modules "
        ...
        if (contextInfo == null) {
            ...
            if (standardContext.getNamingResources() instanceof OpenEJBNamingResource) {
                ...
                for (final ResourceBase resource : nr.getTomcatResources()) {
                    ...
                    boolean found = false;
                    for (final ResourceInfo r : SystemInstance.get()
                            .getComponent(OpenEjbConfiguration.class).facilities.resources) {
                        if (r.id.equals(name)) {
                            nr.removeResource(name);
                            found = true;
                            logger.warning(
                                    name + " resource was defined in both tomcat and tomee so removing tomcat one");
                            break;
                        }
                    }
            ...
            if (appModule != null) {
                try {
                    ...
                    if (System.getProperty(TOMEE_EAT_EXCEPTION_PROP) == null) {
                        ...
                        final DeploymentExceptionManager dem = SystemInstance.get()
        ...
        if (contextInfo.appInfo != null) {
            for (final WebAppInfo w : contextInfo.appInfo.webApps) {
                if (id.equals(getId(w.host, w.contextRoot, contextInfo.version))
                        || id.equals(getId(w.host, w.moduleId, contextInfo.version))) {
                    if (webAppInfo == null) {
                        webAppInfo = w;
            ...
            try {
                ...
                for (final PersistenceUnitInfo unitInfo : contextInfo.appInfo.persistenceUnits) {
                    if (unitInfo.webappName != null && unitInfo.webappName.equals(webAppInfo.moduleId)) {
                        try {
                            final ReloadableEntityManagerFactory remf = (ReloadableEntityManagerFactory) SystemInstance
                                    .get().getComponent(ContainerSystem.class).getJNDIContext()
                                    .lookup(Assembler.PERSISTENCE_UNIT_NAMING_CONTEXT + unitInfo.id);
                ...
                if (!contextInfo.appInfo.webAppAlone) {
                    final List<BeanContext> beanContexts = assembler.initEjbs(classLoader, contextInfo.appInfo,
                            appContext, injections, new ArrayList<BeanContext>(), webAppInfo.moduleId);
                ...
                webContext.getBindings()
                        .putAll(getJndiBuilder(classLoader, webAppInfo, injections, appContext.getProperties())
                                .buildBindings(JndiEncBuilder.JndiScope.comp));
                ...
                logger.error("Error merging Java EE JNDI entries in to war " + standardContext.getPath()
    ...
    private JndiEncBuilder getJndiBuilder(final ClassLoader classLoader, final WebAppInfo webAppInfo,
            final Set<Injection> injections, final Properties props) throws OpenEJBException {
        return new JndiEncBuilder(webAppInfo.jndiEnc, injections, webAppInfo.moduleId, "Bean", null,
                webAppInfo.uniqueId, classLoader, props);
    }
    ...
    public void afterStart(final StandardContext standardContext) {
        ...
        try {
                ...
                try {
                    ...
                    standardContext.getServletContext()
                            .setAttribute("javax.faces.validator.beanValidator.ValidatorFactory",
                                    openejbContext.lookup(
                                            Assembler.VALIDATOR_FACTORY_NAMING_CONTEXT.replaceFirst("openejb", "")
            ...
            final TransactionSynchronizationRegistry synchronizationRegistry = SystemInstance.get()
                    .getComponent(TransactionSynchronizationRegistry.class);
        ...
        try {
            ...
            if (webBeansContext != null) {
                ...
                {
                    WebBeansThreadBindingListener webBeansThreadBindingListener = new WebBeansThreadBindingListener(
                            webBeansContext, standardContext.getThreadBindingListener());
                ...
                if (CdiAppContextsService.class.isInstance(contextsService)) { // here ServletContext is usable
                    CdiAppContextsService.class.cast(contextsService)
                            .applicationStarted(standardContext.getServletContext());
                }
            } else {
    ...
    private void ensureMyFacesDontLooseFacesContext(final StandardContext standardContext) {
        for (final Container w : standardContext.findChildren()) {
            ...
            if ("FacesServlet".equals(wrapper.getName())
                    && "javax.faces.webapp.FacesServlet".equals(wrapper.getServletClass())) {
                final ClassLoader loader = standardContext.getLoader().getClassLoader();
                try {
                    if (Files.toFile(loader.getResource("javax/faces/webapp/FacesServlet.class")).getName()
                            .startsWith("myfaces")) {
    ...
    private void addConfiguredDocBases(final StandardContext standardContext, final ContextInfo contextInfo) {
        if (contextInfo.appInfo.path != null) { // add external web resources
            ...
            final String webResources = SystemInstance.get().getProperty("tomee." + name + ".docBases",
                    contextInfo.appInfo.properties.getProperty("docBases"));
    ...
    public void beforeStop(final StandardContext standardContext) {
        ...
        if (!LazyStopStandardRoot.class.isInstance(standardContext.getResources())
                && TomEEWebappClassLoader.class.isInstance(classLoader)
                && !TomEEWebappClassLoader.class.cast(classLoader).isForceStopPhase()) {
    ...
    public void afterStop(final StandardContext standardContext) {
        ...
        if (destroyFromTomcat && isUnDeployable(contextInfo)) {
            ...
            try {
                ...
                logger.error("Unable to stop web application " + standardContext.getPath() + ": Exception: "
                        + e.getMessage(), e);
            }
        } else {
    ...
    public synchronized void checkHost(final StandardHost standardHost) {
        if (standardHost.getAutoDeploy()) {
            ...
            for (final Iterator<Map.Entry<String, DeployedApplication>> iterator = deployedApps.entrySet()
            ...
            if (null != files) {
                for (File file : files) {
                    ...
                    // ignore war files
                    if (name.toLowerCase().endsWith(".war") || isRoot(name) || name.equalsIgnoreCase("META-INF")
    ...
    /**
     * Returns true if given application is deployed false otherwise.
     *
     * @param file
    ...
    /**
     * Creates an openejb {@link AppModule} instance from given tomcat context.
     *
    ...
    /**
     * Creates a new {@link WebModule} instance from given tomcat context instance.
     *
     * @param standardContext
     *            tomcat context instance
     */
    ...
    /**
     * Gets id of the context. Context id is host name + context root name.
     *
     * @param standardContext
     *            context instance
    ...
    private String getId(final StandardContext standardContext) {
        return getId(Contexts.getHostname(standardContext), standardContext.getPath(),
    ...
    private String getId(final String host, final String context, final String version) {
        ...
        return (host == null ? hosts.getDefaultHost() : host) + contextRoot
                + (version == null || version.isEmpty() ? "" : (version.startsWith("##") ? version : "##" + version));
    }
    ...
    public ContextInfo addContextInfo(final String host, final StandardContext standardContext) {
        ...
        synchronized (infos) {
            ...
            if (contextInfo == null) {
                ...
                contextInfo.version = webappVersion != null && !webappVersion.isEmpty() ? "##" + webappVersion
                        : webappVersion;
                contextInfo.standardContext = standardContext;
    ...
    public static class ContextInfo {
        ...
        public String toString() {
            return "ContextInfo{" + "appInfo = " + appInfo + ", " + "deployer = " + deployer + ", " + "host = " + host
                    + "}";
        }
    }
    ...
    private static class DeployedApplication {
        ...
        public boolean isModified() {
            for (final Map.Entry<File, Long> entry : watchedResource.entrySet()) {
                ...
                if ((!file.exists() && lastModified != 0L) || (file.lastModified() != lastModified)) {
                    return true;
                }
            }
            return false;
    ...
    public class StandardContextInfo {
        ...
        public WebAppInfo get() {
            ...
            for (final WebAppInfo webApp : contextInfo.appInfo.webApps) {
                if (webApp == null) {
                    logger.debug(
                            "ContextInfo.appInfo.webApps entry is null StandardContext " + standardContext.getName());
                    continue;
                }
        ...
        public String toString() {
            ...
            return "StandardContextInfo{" + "standardContext=" + standardContext + '}';
        }
    }
}
