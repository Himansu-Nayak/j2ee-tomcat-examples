 ...
 * <p/>
 * <ul>
 * <li/>Setup ServiceJar: set openejb.provider.default -> org.apache.tomee We therefore will load this file:
...
public class TomcatLoader implements Loader {
    ...
    public static void destroy() {
        ...
        final TomcatWebAppBuilder tomcatWebAppBuilder = (TomcatWebAppBuilder) SystemInstance.get()
    ...
    /**
     * this method will be split in two to be able to use SystemInstance in between both invocations ie use
     * configuration before effective boot
     */
    @Deprecated
    ...
    public void initialize(final Properties properties) throws Exception {
        ...
        try {// in embedded mode we can easily remove it so check we can use it before setting it
            final Class<?> creatorClass = TomcatLoader.class.getClassLoader()
                    .loadClass("org.apache.tomee.jdbc.TomEEDataSourceCreator");
        ...
        {
            final String jarToSkipProp = CatalinaProperties
        ...
        {
            final String jarToSkipProp = CatalinaProperties
                    .getProperty("org.apache.catalina.startup.ContextConfig.jarsToSkip");
            if (jarToSkipProp != null && !jarToSkipProp.isEmpty()) {
                for (final String s : jarToSkipProp.split(",")) {
                    final String sanitize = NewLoaderLogic.sanitize(s.trim());
                    if (!sanitize.isEmpty()) {
        ...
        TomcatWebAppBuilder tomcatWebAppBuilder = (TomcatWebAppBuilder) SystemInstance.get()
                .getComponent(WebAppBuilder.class);
        if (tomcatWebAppBuilder == null) {
        ...
        TomcatJndiBuilder.importOpenEJBResourcesInTomcat(
        ...
        if (SystemInstance.get().getOptions().get("openejb.servicemanager.enabled", true)) {
            ...
            try {
                manager = clazz == null ? new TomEEServiceManager()
                        : (ServiceManager) cl.loadClass(clazz).newInstance();
            ...
            // WS
            try {
                final ServerService cxfService = (ServerService) cl
                        .loadClass("org.apache.openejb.server.cxf.CxfService").newInstance();
            ...
            // REST
            try {
                final ServerService restService = (ServerService) cl
                        .loadClass("org.apache.openejb.server.cxf.rs.CxfRSService").newInstance();
        ...
        if (SystemInstance.get().getOptions().get(TOMEE_NOSHUTDOWNHOOK_PROP, (String) null) != null) {
            ...
            try {
                ...
                if (daemon != null) {
                    ...
                    try {
                        Catalina.class.getMethod("setUseShutdownHook", boolean.class).invoke(catalinaField.get(daemon),
                                false);
                    } finally {
     ...
     *
     * @param tomcatWebAppBuilder
     *            tomcat web app builder instance
     * @param standardServer
     *            tomcat server instance
     */
    private void processRunningApplications(final TomcatWebAppBuilder tomcatWebAppBuilder,
            final StandardServer standardServer) {
                            ...
                            if (hostChild instanceof StandardContext) {
                                ...
                                if (state == 0) {
                                    ...
                                    Thread.currentThread()
                                            .setContextClassLoader(standardContext.getLoader().getClassLoader());
                                    try {
                                        tomcatWebAppBuilder.init(standardContext);
