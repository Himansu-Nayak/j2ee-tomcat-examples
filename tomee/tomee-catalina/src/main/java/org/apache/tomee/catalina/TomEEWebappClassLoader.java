...
public class TomEEWebappClassLoader extends ParallelWebappClassLoader {
    ...
    private static final Logger LOGGER = Logger.getInstance(LogCategory.OPENEJB,
            TomEEWebappClassLoader.class.getName());
    ...
    private boolean forceStopPhase = Boolean
            .parseBoolean(SystemInstance.get().getProperty("tomee.webappclassloader.force-stop-phase", "false"));
        ...
        synchronized (this) { // TODO: rework it to avoid it and get aligned on Java 7 classloaders (but not a big issue)
            if (isEar) {
                ...
                if (!filter) {
                    ...
                    return loadWithDelegate(getResource(name.replace('.', '/') + CLASS_EXTENSION) == null, resolve,
                            name);
                }
            }
            return super.loadClass(name, resolve);
    ...
    private Class<?> loadWithDelegate(final boolean delegate, final boolean resolve, final String name)
    ...
    public synchronized void initAdditionalRepos() {
        ...
        if (CONTEXT.get() != null) {
            ...
            final String externalRepositories = SystemInstance.get()
                    .getProperty("tomee." + name + ".externalRepositories");
    ...
    private boolean jarIsAccepted(final File file) {
        ...
        try {
            if (!configurer.accept(file.toURI().toURL())) {
                LOGGER.warning("jar '" + file.getAbsolutePath() + "' is excluded: " + file.getName()
                        + ". It will be ignored.");
                return false;
            }
    ...
    private static final class PremptiveDirResourceSet extends DirResourceSet {
        ...
        public PremptiveDirResourceSet(final WebResourceRoot resources, final String s, final String absolutePath,
                final String s1) {
