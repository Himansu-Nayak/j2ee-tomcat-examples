...
/**
 * NOTE: for simplicity the prefix `org.apache.tomee.jul.handler.rotating.LocalFileHandler.` has been removed of name
 * columns.
 * <p/>
 * |=== | Name | Default Value | Description | filenamePattern | ${catalina.base}/logs/logs.%s.%03d.log | where log
 * files are created, it uses String.format() and gives you the date and file number - in this order. | limit | 10
 * Megabytes | limit size indicating the file should be rotated | dateCheckInterval | 5 seconds | how often the date
 * should be computed to rotate the file (don't do it each time for performances reason, means you can get few records
 * of next day in a file name with current day) | bufferSize | -1 bytes | if positive the in memory buffer used to store
 * data before flushing them to the disk | encoding | - | file encoding | level | ALL | level this handler accepts |
 * filter | - | filter used to check if the message should be logged | formatter | java.util.logging.SimpleFormatter |
 * formatter used to format messages | archiveDirectory | ${catalina.base}/logs/archives/ | where compressed logs are
 * put. | archiveFormat | gzip | zip or gzip. | archiveOlderThan | -1 days | how many days files are kept before being
 * compressed | purgeOlderThan | -1 days | how many days files are kept before being deleted, note: it applies on
 * archives and not log files so 2 days of archiving and 3 days of purge makes it deleted after 5 days. |
 * compressionLevel | -1 | In case of zip archiving the zip compression level (-1 for off or 0-9). | formatterPattern |
 * - | SimpleFormatter pattern (ignored if formatter is provided). | formatterLocale | - | Locale to use. |===
 * <p/>
 * NOTE: archiving and purging are done only when a file is rotated, it means it can be ignored during days if there is
 * no logging activity.
 * <p/>
 * NOTE: archiving and purging is done in a background thread pool, you can configure the number of threads in thanks to
 * `org.apache.tomee.jul.handler.rotating.BackgroundTaskRunner.threads` property in `conf/logging.properties`. Default
 * is 2 which should be fine for most applications.
 */
/*
 * Open point/enhancements: - date pattern/filename pattern instead of hardcoded String.format? - write another async
 * version? ensure it flushed well, use disruptor? -> bench seems to show it is useless
 */
public class LocalFileHandler extends Handler {
    ...
    private static void copyStream(final InputStream inputStream, final OutputStream outputStream, final byte[] buffer)
            throws IOException {
        int n;
    ...
    private void configure() {
        ...
        dateCheckInterval = new Duration(
                getProperty(className + ".dateCheckInterval", String.valueOf(dateCheckInterval))).asMillis();
        ...
        { // file rotation index
            ...
            if (indexIdxStart >= 0) {
                ...
                if (indexIdxEnd >= 0) {
                    fileNameReg = fileNameReg.substring(0, indexIdxStart) + "\\d*"
                            + fileNameReg.substring(indexIdxEnd + 1, fileNameReg.length());
                }
            }
        }
        ...
        compressionLevel = Integer
                .parseInt(getProperty(className + ".compressionLevel", String.valueOf(Deflater.DEFAULT_COMPRESSION)));
        archiveExpiryDuration = new Duration(getProperty(className + ".archiveOlderThan", String.valueOf("-1 days")))
                .asMillis();
        ...
        purgeExpiryDuration = new Duration(getProperty(className + ".purgeOlderThan", String.valueOf("-1 days")))
                .asMillis();
    ...
    private Formatter newSimpleFormatter(final String className) {
        final String defaultFormat = System.getProperty("java.util.logging.SimpleFormatter.format",
        ...
        try {
            ...
            try {
                if (writer != null) {
                    ...
                    reportError(getClass().getSimpleName() + " is closed or not yet initialized, unable to log ["
        ...
        if (purgeExpiryDuration > 0) { // purging archives
            ...
            if (archives != null) {
                for (final File archive : archives) {
                    try {
                        final BasicFileAttributes attr = Files.readAttributes(archive.toPath(),
                        ...
                        if (now - attr.creationTime().toMillis() > purgeExpiryDuration) {
                            if (!Files.deleteIfExists(archive.toPath())) {
                                ...
                                reportError("Can't delete " + archive.getAbsolutePath() + ".", null,
            ...
            if (logs != null) {
                for (final File file : logs) {
                    try {
                        ...
                        if (attr.creationTime().toMillis() < now
                                && now - attr.lastModifiedTime().toMillis() > archiveExpiryDuration) {
                            createArchive(file);
                        }
                    } catch (final IOException e) {
                        throw new IllegalStateException(e);
    ...
    public static class PatternFormatter extends Formatter {
        ...
        public String format(final LogRecord record) {
            ...
            return String.format(locale, format, date, source, record.getLoggerName(),
