 ...
 * 
 * @Rule public TomEEMavenPluginRule TMPRule = new TomEEMavenPluginRule();
 * 
 * @Url // get the base http url injected private String url;
 * 
 * @Config private String tomeeHost = "localhost";
 * 
 * @Test public void simpleStart() throws Exception { assertThat(IO.slurp(new URL(url + "/docs")), containsString(
 *       "Apache Tomcat")); } }
 *
                        ...
                        new Class<?>[] { ArtifactFactory.class }, new InvocationHandler() {
                            ...
                            public Object invoke(final Object proxy, final Method method, final Object[] args)
                                ...
                                return new DefaultArtifact(String.class.cast(args[0]), String.class.cast(args[1]),
                                        ...
                                        String.class.cast(args[3]), args[4] == null ? "" : String.class.cast(args[4]),
                                        null) {
                                    @Override
                                    public File getFile() {
                                        return new File(tomEEMojo.settings.getLocalRepository(),
                                                getGroupId().replace('.', '/') + '/' + getArtifactId().replace('.', '/')
                                                        + '/' + getVersion() + '/' + getArtifactId().replace('.', '/')
                                                        + '-' + getVersion()
