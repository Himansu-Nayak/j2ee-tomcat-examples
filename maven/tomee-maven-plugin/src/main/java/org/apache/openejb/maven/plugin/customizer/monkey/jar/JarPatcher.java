...
public class JarPatcher {
    ...
    private String jarName(final String name) {
        return name.substring(0, name.length() - ".jar".length()) + "tomee-monkey-"
                + new SimpleDateFormat("yyyyMMdd").format(new Date()) + ".jar";
    }

    private void replace(final PrintStream stream, final String name, final File exploded, final String path,
            final File file) {
