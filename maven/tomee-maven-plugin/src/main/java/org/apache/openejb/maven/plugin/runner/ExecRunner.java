...
public class ExecRunner {
    ...
    public static void main(final String[] rawArgs) throws Exception {
        ...
        if (forceDelete || !timestampFile.exists()
                || Long.parseLong(IO.slurp(timestampFile).replace(System.getProperty("line.separator"), "")) < Long
        ...
        if (cmd.endsWith(SH_BAT_AUTO)) {
            ...
            if (lastSlash > 0) {
                ...
                final String script = cmd.substring(lastSlash + 1, cmd.length() - SH_BAT_AUTO.length()).replace('/',
                        isWin ? '\\' : '/') + (isWin ? ".bat" : ".sh");
        ...
        try {
            ...
            if ("java".equals(cmd)) {
                ...
                final ProcessBuilder builder = new ProcessBuilder(params.toArray(new String[params.size()])).inheritIO()
