...
public abstract class UpdatableTomEEMojo extends AbstractTomEEMojo {
    ...
    protected void run() {
        ...
        if (removeTomeeWebapp && !ejbRemote) {
            getLog().warn(
                    "TomEE webapp is asked to be removed (<ejbRemote>true> or <removeTomeeWebapp>true</removeTomeeWebapp>) so you can use reload feature");
        }
    ...
    private void initSynchronization(final Synchronization synchronization) {
        ...
        if (synchronization.getTargetBinariesDir() == null) {
            synchronization
                    .setTargetBinariesDir(new File(catalinaBase, webappDir + "/" + destination + "/WEB-INF/classes"));
        }
    ...
    private Deployer deployer() {
        if (removeTomeeWebapp && !ejbRemote) {
            throw new OpenEJBRuntimeException(
                    "Can't use reload feature without TomEE Webapp, please set removeTomeeWebapp to false or ejbRemote to true");
        }
    ...
    private class Synchronizer implements Callable<Integer> {
        ...
        public Synchronizer(final AbstractSynchronizable synch) {
            ...
            if (synchronization.getRegex() != null) {
                fileFilter = new SuffixesAndRegexFileFilter(synchronization.getExtensions(),
