...
public abstract class AbstractTomEEMojo extends AbstractAddressMojo {
    ...
    /**
     * supported formats: --> groupId:artifactId:version... --> unzip:groupId:artifactId:version... --> remove:prefix
     * (often prefix = artifactId)
     */
    @Parameter
    ...
    /**
     * the actual path used in server.xml for the https keystore if relevant. Common usage will be to put in
     * src/main/tomee/conf a keystore foo.jks and set this value to ${catalina.base}/foo.jks.
     * <p/>
                        ...
                        try {
                            ...
                            try {
                                clazz.getMethod("main", String[].class).invoke(null,
                                        new String[] { catalinaBase.getAbsolutePath() });
                                ...
                                try {
                                    ...
                                    try {
                                        ...
                                        throw new MojoExecutionException("can't create customizer: " + currentLoader,
                                                e);
                                    }
            ...
            try {
                ...
                if (!content.contains("java.util.logging.ConsoleHandler.formatter")) {
                    content += System.getProperty("line.separator")
                            + "java.util.logging.ConsoleHandler.formatter = org.apache.tomee.jul.formatter.SimpleTomEEFormatter";
                } else {
                    content = content.replace(SimpleFormatter.class.getName(),
                            "org.apache.tomee.jul.formatter.SimpleTomEEFormatter");
                }
        ...
        if (webapps.isDirectory()) {
            ...
            if (null != files) {
                for (final File webapp : files) {
                    ...
                    if (webapp.isDirectory() && !providedWebapps.contains(name)
                            && (removeTomee || !name.equals("tomee"))) {
                        try {
        ...
        if (lib.startsWith(REMOVE_PREFIX)) {
            ...
            try {
                ...
                if (!unzip && !isExplodedWar) {
                    ...
                    if (isExplodedWar) {
                        destParent = Files.mkdirs(new File(rawDestParent,
                                extractedName != null ? extractedName : file.getName().replace(".war", "")));
                    }
    ...
    private File mvnToFile(final String lib, final String defaultType)
            throws ArtifactResolutionException, ArtifactNotFoundException {
        final String[] infos = lib.split(":");
        final String classifier;
        final String type;
        if (infos.length < 3) {
            throw new TomEEException(
    ...
    private File resolve(final String group, final String artifact, final String version, final String classifier,
            final String type) throws ArtifactResolutionException, ArtifactNotFoundException {
        final Artifact dependencyArtifact = factory.createDependencyArtifact(group, artifact,
                createFromVersion(version), type, classifier, SCOPE_COMPILE);
    ...
    private void overrideServerXml() {
        ...
        if (tomeeHttpsPort != null && tomeeHttpsPort.length() > 0 && parser.value("HTTPS", null) == null) {
                ...
                if (conf.isDirectory()) {
                    ...
                    if (jks != null && jks.length == 1) {
                        ...
                        throw new IllegalArgumentException(
                                "Ambiguous jks in conf/,please use <keystore /> to force it.");
                    }
                }
            }
            ...
            value = value.replace("<Service name=\"Catalina\">",
    ...
    protected void run() {
        ...
        if (TOM_EE.equals(container)) {
            ...
            getLog().info(
                    "Running '" + getClass().getSimpleName().replace("TomEEMojo", "").toLowerCase(Locale.ENGLISH));
        }
        ...
        if (persistJavaagents) {
            try {
                ...
                }
                {
                    ...
                    final String content = IO.slurp(catalinaBat).replace(
                            "\"-javaagent:%CATALINA_HOME%\\lib\\openejb-javaagent.jar\"",
                            javaagentString.toString().replace('\'', '"').replace('/', '\\').replace("$CATALINA_HOME",
                                    "%CATALINA_HOME%"));
    ...
    private void forceDefaultForNiceWebAppDevelopment() {
        if (!deployOpenEjbApplication) {
            getLog().info(
                    "Forcing deployOpenEjbApplication=true to be able to type 'reload[ENTER]' when classes are updated");
            deployOpenEjbApplication = true;
        }
        if (!forceReloadable) {
        ...
        try {
            ...
            while (entries.hasMoreElements()) {
                ...
                if (!dest.exists()) {
                    ...
                    if ((!parent.exists() && !parent.mkdirs()) || (!parent.canWrite() && !parent.setWritable(true))
    ...
    private void ensureAppsFolderExistAndIsConfiguredByDefault(final File file) throws IOException {
        if ("openejb".equals(container.toLowerCase(Locale.ENGLISH)) || (file.exists()
                && ((apps != null && !apps.isEmpty()) || (!"pom".equals(packaging) && !"war".equals(packaging))))) { // webapps doesn't need apps folder in tomee
