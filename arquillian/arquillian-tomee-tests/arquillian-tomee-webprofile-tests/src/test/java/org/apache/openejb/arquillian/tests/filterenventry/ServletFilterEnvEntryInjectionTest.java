...
public class ServletFilterEnvEntryInjectionTest {
    ...
    public static WebArchive createDeployment() {
        WebAppDescriptor descriptor = Descriptors.create(WebAppDescriptor.class).version(WebAppVersionType._3_0)
                .createFilter().filterName("filter").filterClass(PojoServletFilter.class.getName()).up()
                .createFilterMapping().filterName("filter").urlPattern("/" + TEST_NAME).up();
        ...
        WebArchive archive = ShrinkWrap.create(WebArchive.class, TEST_NAME + ".war").addClass(Code.class)
                .addClass(PojoServletFilter.class).addClass(Car.class).addClass(CompanyLocal.class)
                .addClass(Company.class).addClass(DefaultCompany.class).addClass(SuperMarket.class)
    ...
    private static void addEnvEntry(WebAppDescriptor descriptor, String name, String type, String value) {
        ...
        appNode.createChild("/env-entry").createChild("env-entry-name").text(name).getParent()
                 ...
                 * "org.apache.openejb.arquillian.ServletPojoInjectionTest$PojoServletFilter") .parent()
