...
import org.apache.tomee.catalina.realm.event.*;
...
public class CdiEventRealmTest {
    ...
    public void digest() {
        final GenericPrincipal gp = getGenericPrincipal(
                new CdiEventRealm().authenticate("ryan", "md5", "nonce", "nc", "cnonce", "qop", "realm", "md5a2"));
    ...
    public void gss() {
        final GenericPrincipal gp = getGenericPrincipal(
                new CdiEventRealm().authenticate(mock(GSSContext.class), false));
    ...
    public void ssl() {
        ...
        final GenericPrincipal gp = getGenericPrincipal(
                new CdiEventRealm().authenticate(new X509Certificate[] { cert }));
    ...
    @Test
    public void find() {
        final SecurityConstraint[] securityConstraints = new CdiEventRealm()
                .findSecurityConstraints(mock(Request.class), mock(Context.class));
