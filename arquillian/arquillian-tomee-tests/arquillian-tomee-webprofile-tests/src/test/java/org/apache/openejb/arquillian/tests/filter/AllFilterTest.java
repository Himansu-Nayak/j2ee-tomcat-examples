...
public class AllFilterTest {
    ...
    public static Archive<?> war() throws Exception {
        ...
        return ShrinkWrap.create(WebArchive.class, "filter.war").addClass(MyFilter.class)
                ...
                .addAsWebInfResource(new StringAsset(Descriptors.create(WebAppDescriptor.class)
                        .version(WebAppVersionType._2_5).metadataComplete(true).createFilter().filterName("My")
                        .filterClass(MyFilter.class.getName()).up().createFilterMapping().filterName("My")
                        .urlPattern("/*").up().exportAsString()), "web.xml");
    }
    ...
    private static String httpPort() throws IOException {
        ...
        return org.apache.openejb.loader.IO
                .slurp(Thread.currentThread().getContextClassLoader().getResourceAsStream("port.conf"));
    }
    ...
    private static String findPort() throws Exception {
        ...
        final ObjectName on = server.queryMBeans(new ObjectName("Tomcat:type=ProtocolHandler,port=*"), null).iterator()
                .next().getObjectName();
