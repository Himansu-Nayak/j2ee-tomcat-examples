...
public class TomEEWebappContainer extends TomEEContainer<TomEEWebappConfiguration> {
        ...
        try {
            ...
            if (openejbHome == null) {
                ...
                System.setProperty("openejb.deploymentId.format",
                        System.getProperty("openejb.deploymentId.format", "{appId}/{ejbJarId}/{ejbName}"));
            ...
            final URL logging = Thread.currentThread().getContextClassLoader()
                    .getResource("default.remote.logging.properties");
            if (logging != null) {
            ...
            container.start(Arrays.asList("-Dopenejb.system.apps=true", "-Dtomee.remote.support=true",
    ...
    protected void downloadOpenEJBWebapp(final File targetDirectory, final String defaultTempDir)
    ...
    protected void downloadTomcat(final File catalinaDirectory, final String tomcatVersion, final String defaultTempDir)
            throws LifecycleException {
        String source = null;

        try {
            int v = Integer.parseInt(tomcatVersion.substring(0, tomcatVersion.indexOf('.')));
            source = "http://archive.apache.org/dist/tomcat/tomcat-" + v + "/v" + v + "/bin/apache-tomcat-"
                    + tomcatVersion + ".zip";
        } catch (final Exception e) {
            // no-op
        }
