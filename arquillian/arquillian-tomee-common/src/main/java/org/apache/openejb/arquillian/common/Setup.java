 ...
 */
public class Setup {
    public static final String TOMEE_BEAN_DISCOVERER_JAR = "lib" + File.separator
            + "xx-arquillian-tomee-bean-discoverer.jar"; // starts with xx to not be filtered
    private static final Logger LOGGER = Logger.getLogger(Setup.class.getName()); // JUL is used by arquillian so that's fine
    private static final String DEFAULT_MEM_CONFIG = (javaVersion() >= 1.8 ? ""
            : "-XX:PermSize=64m -XX:MaxPermSize=256m ") + "-Xmx512m -Xms256m -XX:ReservedCodeCacheSize=64m";
    ...
    public static void updateServerXml(final File tomeeHome, final TomEEConfiguration configuration)
            throws IOException {
    ...
    public static File downloadAndUnpack(final File dir, final String artifactID, final String defaultTempDir)
    ...
    public static File downloadFile(final String artifactName, final String altUrl, final String defaultTempDir) {
        final String cache = SystemInstance.isInitialized() ? SystemInstance.get().getOptions()
                .get(ProvisioningResolver.OPENEJB_DEPLOYER_CACHE_FOLDER, (String) null)
    ...
    public static void replace(final Map<String, String> replacements, final File file, final boolean escape)
            throws IOException {
    ...
    public static void configureServerXml(final File tomeeHome, final TomEEConfiguration configuration)
            throws IOException {
        ...
        if (configuration.getProperties() != null) {
            try {
                ...
                LOGGER.log(Level.SEVERE,
                        "Can't parse <property name=\"properties\"> value '" + configuration.getProperties() + "'", e);
            }
        }
    ...
    public static void installArquillianBeanDiscoverer(final File home) {
        ...
        ShrinkWrap.create(JavaArchive.class, destination.getName()).addManifest() // avoid NPE from org.apache.tomcat.util.scan.StandardJarScanner.processManifest()
                .addClasses(BeanDicovererInstaller.class, TestClassDiscoverer.class, ArquillianFilterRunner.class,
                        Discover.class)
                .addAsManifestResource(new StringAsset(BeanDicovererInstaller.class.getName()),
    ...
    public static Map<File, String> addTomEELibraries(final File libFolder, final String additionalLibs,
            final boolean parseArgs) {
