...
public abstract class TomEEContainer<Configuration extends TomEEConfiguration>
    ...
    protected boolean isTestable(final Archive<?> archive, final DeploymentDescription deploymentDescription) {
        return deploymentDescription != null && deploymentDescription.isArchiveDeployment()
    ...
    protected void addArquillianServlet(final Archive<?> archive, final AppInfo appInfo, final String archiveName,
        ...
        if (archiveName.endsWith(".war")) {
            ...
            final String contextRoot = System.getProperty("tomee.arquillian.ear.context",
    ...
    public void addServlets(final HTTPContext httpContext, final AppInfo appInfo) {
        for (final WebAppInfo webApps : appInfo.webApps) {
            for (final ServletInfo servlet : webApps.servlets) {
                ...
                /*
                 * for (String mapping : servlet.mappings) { httpContext.add(new Servlet(servlet.servletClass,
    ...
    protected Dump dumpFile(final Archive<?> archive) {
        ...
        if (size > 0 && size != file.length()) {
            LOGGER.warning("\nFile overwritten but size doesn't match: (now) " + file.length() + "/(before) " + size
                    + " name=" + file.getName()
    ...
    protected Assignable archiveWithTestInfo(final Archive<?> archive) {
        ...
        return archive.add(new StringAsset(testClass.get().getJavaClass().getName() + '#' + name),
    ...
    protected void doUndeploy(DeployedApp deployed)
            throws UndeployException, NoSuchApplicationException, NamingException {
