...
public final class Mvn {
    ...
    public static class Builder {
        ...
        public <T extends Archive<?>> T build(final Class<T> type) {
            ...
            add(webArchive, classes, root).add(webArchive, resources, root).add(webArchive, webapp, "/");
            ...
            if (LibraryContainer.class.isInstance(webArchive)) {
                try {
                    final File[] deps = Maven.configureResolver().workOffline()
                            .loadPomFromFile(new File(basedir, "pom.xml")).importDependencies(scopes).resolve()
        ...
        private Builder add(final Archive<?> webArchive, final File dir, final String root) {
            ...
            filter.update(webArchive.merge(ShrinkWrap.create(GenericArchive.class).as(ExplodedImporter.class)
