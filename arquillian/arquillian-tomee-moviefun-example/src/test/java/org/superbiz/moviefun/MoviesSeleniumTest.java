...
public class MoviesSeleniumTest {
    ...
    public static WebArchive createDeployment() {
        WebArchive archive = ShrinkWrap.create(WebArchive.class, "moviefun.war")
                .addClasses(ActionServlet.class, SetupServlet.class, Movie.class, MovieController.class, Movies.class,
                ...
                .addAsLibraries(new File("target/test-libs/commons-beanutils.jar"),
                        ...
                        new File("target/test-libs/commons-logging.jar"), new File("target/test-libs/jstl.jar"),
                        new File("target/test-libs/log4j.jar"), new File("target/test-libs/standard.jar"));
            ...
            if (files != null) {
                for (File file : files) {
                    if (file.getName().startsWith("."))
                        continue;
                    addResources(source + File.separator + file.getName(), target + File.separator + file.getName(),
                            archive);
                }
            }
        }
