...
public class OpenEJBDeployableContainer implements DeployableContainer<OpenEJBConfiguration> {
    ...
    public void start() throws LifecycleException {
        ...
        configurationFactory = (ConfigurationFactory) SystemInstance.get()
                .getComponent(OpenEjbConfigurationFactory.class);
    ...
    public ProtocolMetaData deploy(final Archive<?> archive) throws DeploymentException {
        ...
        try {
            ...
            if (archive.getName().endsWith(".war")) {
                ...
                if (beanContexts.size() > 1) {
                    ...
                    while (it.hasNext()) {
                        ...
                        if (ModuleTestContext.class.isInstance(next.getModuleContext())
            ...
            final ClassLoader loader = info.appCtx.getWebContexts().isEmpty() ? info.appCtx.getClassLoader()
                    : info.appCtx.getWebContexts().iterator().next().getClassLoader();
            ...
            for (final WebAppInfo webapp : info.appInfo.webApps) {
                ...
                for (final ClassListInfo classListInfo : webapp.webAnnotatedClasses) {
                    for (final String path : classListInfo.list) {
                        ...
                        http.add(new Servlet(
                                path.substring(path.lastIndexOf('!') + 2).replace(".class", "").replace("/", "."),
                                webapp.contextRoot));
                    }
                }
            }
    ...
    private DeploymentInfo quickDeploy(final Archive<?> archive, final TestClass testClass, final Closeables cls)
        ...
        if (info == null) {
            try {
                ...
                final boolean isEmbeddedWebAppBuilder = webAppBuilder != null
                        && LightweightWebAppBuilder.class.isInstance(webAppBuilder);
                if (isEmbeddedWebAppBuilder) {
                    ...
                    final LightweightWebAppBuilder lightweightWebAppBuilder = LightweightWebAppBuilder.class
                            .cast(webAppBuilder);
                ...
                if (isEmbeddedWebAppBuilder && PROPERTIES.containsKey(OpenEjbContainer.OPENEJB_EMBEDDED_REMOTABLE)
                        && !appCtx.getWebContexts().isEmpty()) {
                    cls.add(new Closeable() {
                        @Override
                        public void close() throws IOException {
                            try {
                                final SessionManager sessionManager = SystemInstance.get()
    ...
    private static final class DeploymentInfo {
        ...
        private DeploymentInfo(final ServletContext appServletContext, final HttpSession appSession,
                final AppInfo appInfo, final AppContext appCtx) {
