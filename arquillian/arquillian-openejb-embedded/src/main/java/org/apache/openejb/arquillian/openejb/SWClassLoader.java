...
public class SWClassLoader extends ClassLoader implements Closeable {
            ...
            if (isWar) { // add dependencies - file and url - to be able to lookup them. ArchiveAssets are provided normally
                for (final Node n : a.getContent(new IncludeRegExpPaths("/WEB-INF/lib/.*\\.jar")).values()) {
                    ...
                    if (FileAsset.class.isInstance(asset)) {
                        final JavaArchive jar = ShrinkWrap.createFromZipFile(JavaArchive.class,
                                get(File.class, "file", asset));
                        this.archives.add(jar);
                        ArchiveStreamHandler.set(jar, closeables);
                    } else if (UrlAsset.class.isInstance(asset)) {
                        final JavaArchive jar = ShrinkWrap.createFromZipFile(JavaArchive.class,
                                URLs.toFile(get(URL.class, "url", asset)));
                        this.archives.add(jar);
                        ArchiveStreamHandler.set(jar, closeables);
                    }
                }
        ...
        if (cdiExtensions || !name.contains("META-INF/services/javax")) {
            ...
            if (!node.isEmpty()) {
                ...
                for (final Archive<?> i : node) {
                    urls.add(new URL(null, "archive:" + i.getName() + (!name.startsWith("/") ? "/" : "") + name,
                            new ArchiveStreamHandler()));
                }
                if (cdiExtensions && !"true".equalsIgnoreCase(
                        SystemInstance.get().getProperty("openejb.arquillian.cdi.extension.skip-externals", "false"))) {
    ...
    public URL getWebResource(final String name) {
        for (final Archive<?> a : archives) {
            ...
            if (node != null) {
                try {
                    return new URL(null, "archive:" + a.getName() + (!name.startsWith("/") ? "/" : "") + name,
                            new ArchiveStreamHandler());
    ...
    protected URL findResource(final String name) {
        ...
        if (!node.isEmpty()) {
            ...
            try {
                return new URL(null, "archive:" + i.getName() + (!name.startsWith("/") ? "/" : "") + name,
                        new ArchiveStreamHandler());
            } catch (final MalformedURLException e) {
    ...
    private static class ArchiveStreamHandler extends URLStreamHandler {
        ...
        protected URLConnection openConnection(final URL u) throws IOException {
            ...
            final String path = path(archive.getName(), WebArchive.class.isInstance(archive) ? "/WEB-INF/classes/" : "",
                    u);
            ...
            if (UrlAsset.class.isInstance(asset)) {
                ...
                return ClassLoader.class.cast(Reflections.get(asset, "classLoader"))
                        .getResource(String.class.cast(Reflections.get(asset, "resourceName"))).openConnection();
            }
