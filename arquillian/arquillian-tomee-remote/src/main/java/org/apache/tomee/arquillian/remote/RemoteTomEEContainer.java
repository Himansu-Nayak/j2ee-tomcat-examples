...
import org.apache.openejb.arquillian.common.*;
...
public class RemoteTomEEContainer extends TomEEContainer<RemoteTomEEConfiguration> {
    ...
    private static final String ARQUILLIAN_FILTER = "-Dorg.apache.openejb.servlet.filters="
            + ArquillianFilterRunner.class.getName() + "=" + ServletMethodExecutor.ARQUILLIAN_SERVLET_MAPPING;
    ...
    private void configure() throws LifecycleException, IOException {
        ...
        Setup.exportProperties(tomeeHome, configuration,
                opts == null || (!opts.contains("-Xm") && !opts.matches(".*-XX:[^=]*Size=.*")));
        ...
        if (configuration.isSimpleLog() && noLoggingConfigProvided()) {
            ...
            logging.put("java.util.logging.ConsoleHandler.formatter",
    ...
    private boolean noLoggingConfigProvided() {
        ...
        return !(conf.exists() && (new File(conf, "logging.properties").exists()
                || new File(conf, "log4j.properties").exists() || new File(conf, "log4j.xml").exists()));
    }
            ...
            if (Boolean.getBoolean("openejb.arquillian.debug")) {
                ...
                try {
                    LOGGER.info("Here is the server.xml:\n"
                            + IO.slurp(new File(Setup.findHome(new File(configuration.getDir()).getAbsoluteFile()),
                                    "conf/server.xml")));
