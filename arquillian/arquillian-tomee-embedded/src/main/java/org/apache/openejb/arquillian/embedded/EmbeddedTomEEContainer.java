...
public class EmbeddedTomEEContainer extends TomEEContainer<EmbeddedTomEEConfiguration> {
    ...
    /*
     * Not exactly as elegant as I'd like. Maybe we could have the EmbeddedServer configuration in openejb-core so all
     * the adapters can use it. Depending on tomee-embedded is fine in this adapter, but less desirable in the others,
     * as we'd get loads of stuff in the classpath we don't need.
     */
    private Configuration convertConfiguration(final EmbeddedTomEEConfiguration tomeeConfiguration) {
            ...
            for (final String s : tomeeConfiguration.getConfigurationCustomizers().split(",")) {
                ...
                if (!trim.isEmpty()) {
                    try {
                        ...
                        configuration
                                .addCustomizer(Configuration.ConfigurationCustomizer.class.cast(type.newInstance()));
        ...
        if (tomeeConfiguration.getRoles() != null) {
            configuration
                    .setRoles(new HashMap<String, String>(Map.class.cast(tomeeConfiguration.getRolesAsProperties())));
        }
        if (tomeeConfiguration.getUsers() != null) {
            configuration
                    .setUsers(new HashMap<String, String>(Map.class.cast(tomeeConfiguration.getUsersAsProperties())));
        }
    ...
    public void start() throws LifecycleException {
        try {
            ...
            SystemInstance.get().setProperty("org.apache.openejb.servlet.filters",
    ...
    public ProtocolMetaData deploy(final Archive<?> archive) throws DeploymentException {
        try {
            /*
             * don't do it since it should be configurable final File tempDir = Files.createTempDir(); final File file =
            ...
            final HTTPContext httpContext = new HTTPContext(this.configuration.getHost(),
            ...
            classLoader.set(SystemInstance.get().getComponent(ContainerSystem.class).getAppContext(info.appId)
                    .getClassLoader());
    ...
    private void stopCdiContexts(final String name) {
        try {
            ...
            if (session != null) {
                final WebBeansContext wbc = container.getAppContexts(container.getInfo(name).appId)
                        .getWebBeansContext();
                if (wbc != null && wbc.getBeanManagerImpl().isInUse()) {
                    wbc.getContextsService().startContext(RequestScoped.class, null);
                    wbc.getContextsService().startContext(SessionScoped.class, session);
