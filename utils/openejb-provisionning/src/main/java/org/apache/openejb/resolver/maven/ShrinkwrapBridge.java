...
public class ShrinkwrapBridge {
    ...
    public File resolve(final String rawLocation) throws Exception {
        final Class<?> mvn = Thread.currentThread().getContextClassLoader()
                .loadClass("org.jboss.shrinkwrap.resolver.api.maven.Maven");
        /*
         * return Maven.configureResolver().workOffline() .resolve(toSwFormat(rawLocation)) .withoutTransitivity()
         * .asSingleFile() .getAbsolutePath()
         */
        return File.class.cast(Reflections.invokeByReflection(
                Reflections
                        .invokeByReflection(Reflections.invokeByReflection(
                                Reflections.invokeByReflection(mvn.getMethod("configureResolver").invoke(null),
                                        "workOffline", NO_PARAM, NO_ARG),
                                "resolve", STRING_PARAM, new Object[] { toSwFormat(rawLocation) }),
                "withoutTransitivity", NO_PARAM, NO_ARG), "asSingleFile", NO_PARAM, NO_ARG));
    }
