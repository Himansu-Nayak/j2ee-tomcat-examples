...
public abstract class JtaTransactionPolicy implements TransactionPolicy {
    ...
    protected static final Logger txLogger = Logger.getInstance(LogCategory.TRANSACTION,
    ...
    protected void fireNonTransactionalCompletion() {
        for (final TransactionSynchronization synchronization : new ArrayList<TransactionSynchronization>(
                synchronizations)) {
            try {
                synchronization.beforeCompletion();
            } catch (final Throwable e) {
                logger.error(
                        "Exception thrown from beforeCompletion() of TransactionSynchronization " + synchronization);
            }
        }
        final TransactionSynchronization.Status status = isRollbackOnly() ? TransactionSynchronization.Status.ROLLEDBACK
        ...
        for (final TransactionSynchronization synchronization : new ArrayList<TransactionSynchronization>(
            ...
            try {
                ...
                logger.error("Exception thrown from afterCompletion(" + status + ") of TransactionSynchronization "
                        + synchronization);
            }
        }
    }
    ...
    protected Transaction getTransaction() throws SystemException {
        try {
            ...
            txLogger.error(
                    "The Transaction Manager has encountered an unexpected error condition while attempting to obtain current transaction: {0}",
                    e.getMessage());
    ...
    protected Transaction beginTransaction() throws SystemException {
        ...
        try {
            ...
            txLogger.error(
                    "The Transaction Manager has encountered an unexpected error condition while attempting to begin a new transaction: {0}",
                    e.getMessage());
            throw new SystemException(e);
        }
    ...
    protected void resumeTransaction(final Transaction tx) throws SystemException {
        try {
            ...
            txLogger.error("Could not resume the client's transaction, the transaction is no longer valid: {0}",
                    ite.getMessage());
            ...
            txLogger.error(
                    "Could not resume the client's transaction: The transaction reported a system exception: {0}",
                    e.getMessage());
            throw new SystemException(e);
        }
    }
    ...
    protected void completeTransaction(final Transaction tx) throws SystemException, ApplicationException {
        ...
        try {
            ...
            txLogger.error(
                    "The Transaction Manager has encountered an unexpected error condition while attempting to obtain transaction status: {0}",
                    e.getMessage());
            throw new SystemException(e);
        }
        ...
        try {
            ...
            final Throwable txe = new TransactionRolledbackException(
            ...
            txLogger.debug(
                    "A heuristic decision was made, some relevant updates have been committed while others have been rolled back: {0}",
                    e.getMessage());
            throw new ApplicationException(new RemoteException(
                    "A heuristic decision was made, some relevant updates have been committed while others have been rolled back",
                    e));

        } catch (final HeuristicRollbackException e) {

            txLogger.debug(
                    "A heuristic decision was made while commiting the transaction, some relevant updates have been rolled back: {0}",
                    e.getMessage());
            throw new ApplicationException(new RemoteException(
                    "A heuristic decision was made while commiting the transaction, some relevant updates have been rolled back",
                    e));
            ...
            txLogger.error(
                    "The Transaction Manager has encountered an unexpected error condition while attempting to commit the transaction: {0}",
                    e.getMessage());
    ...
    protected void rollbackTransaction(final Transaction tx) throws SystemException {
        try {
            ...
            logger.error("The TransactionManager reported an exception while attempting to rollback the transaction: "
                    + e.getMessage());
            throw new SystemException(e);
