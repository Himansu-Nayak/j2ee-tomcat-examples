...
public abstract class BaseSessionContext extends BaseContext implements SessionContext {
    ...
    public boolean wasCancelCalled() {
        ...
        throw new IllegalStateException("Current running method " + runningMethod.getName()
                + " is not an asynchronous method :" + di.getDestinationId());
    }
    ...
    public EJBLocalObject getEJBLocalObject() throws IllegalStateException {
        ...
        return (EJBLocalObject) EjbObjectProxyHandler.createProxy(di, threadContext.getPrimaryKey(),
                InterfaceType.EJB_LOCAL, di.getLocalInterface());
    }
    ...
    public EJBObject getEJBObject() throws IllegalStateException {
        ...
        return (EJBObject) EjbObjectProxyHandler.createProxy(di, threadContext.getPrimaryKey(),
                InterfaceType.EJB_OBJECT, di.getRemoteInterface());
    }
    ...
    public Object getBusinessObject(final Class interfce) {
        ...
        if (!interfaceType.isBusiness()) {
            throw new IllegalStateException(
                    "Interface is not a business interface for this bean: " + interfce.getName());
        }
        ...
        try {
            ...
            case STATEFUL: {
                handler = new StatefulEjbObjectHandler(di, threadContext.getPrimaryKey(), interfaceType,
                        new ArrayList<Class>(), interfce);
                break;
            }
            case STATELESS: {
                handler = new StatelessEjbObjectHandler(di, threadContext.getPrimaryKey(), interfaceType,
                        new ArrayList<Class>(), interfce);
                break;
            }
            case SINGLETON: {
                handler = new SingletonEjbObjectHandler(di, threadContext.getPrimaryKey(), interfaceType,
                        new ArrayList<Class>(), interfce);
                break;
            }
            case MANAGED: {
                handler = new ManagedObjectHandler(di, threadContext.getPrimaryKey(), interfaceType,
                        new ArrayList<Class>(), interfce);
                break;
            }
            default:
            ...
            throw new InternalErrorException("Could not create IVM proxy for " + interfce.getName() + " interface",
                    iae);
        }
    }
    ...
    public Class getInvokedBusinessInterface() {
        ...
        if (!type.isBusiness()) {
            throw new IllegalStateException(
                    "The EJB spec requires us to cripple the use of this method for anything but business interface proxy.  But FYI, your invoked interface is: "
                            + invokedInterface.getName());
        }
