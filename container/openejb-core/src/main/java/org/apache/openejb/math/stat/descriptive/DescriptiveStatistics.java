...
import org.apache.openejb.math.stat.descriptive.moment.*;
 ...
 * {@link #getWindowSize() windowSize} property sets a limit on the number of values that can be stored in the dataset.
 * The default value, INFINITE_WINDOW, puts no limit on the size of the dataset. This value should be used with caution,
 * as the backing store will grow without bound in this case. For very large datasets,
 * {@link org.apache.commons.math.stat.descriptive.SummaryStatistics}, which does not store the dataset, should be used
 * instead of this class. If <code>windowSize</code> is not INFINITE_WINDOW and more values are added than can be stored
 * in the dataset, new values are added in a "rolling" manner, with new values replacing the "oldest" values in the
 * dataset.
 * <p/>
 * <p>
...
public class DescriptiveStatistics implements StatisticalSummary, Serializable {
     ...
     * the number of data values that can be stored in the dataset.
     */
    ...
    /**
     * Adds the value to the dataset. If the dataset is at the maximum size (i.e., the number of stored elements equals
     * the currently configured windowSize), the first (oldest) element in the dataset is discarded to make room for the
     * new value.
     *
     * @param v
    ...
    /**
     * Replaces the most recently stored value with the given value. There must be at least one element stored to call
     * this method.
     *
     * @param v
     ...
     *
     * @return The geometricMean, Double.NaN if no values have been added, or if the product of the available values is
    ...
    /**
     * Returns the maximum number of values that can be stored in the dataset, or INFINITE_WINDOW (-1) if there is no
     * limit.
     *
    ...
    /**
     * WindowSize controls the number of values which contribute to the reported statistics. For example, if windowSize
     * is set to 3 and the values {1,2,3,4,5} have been added <strong> in that order</strong> then the <i>available
     * values</i> are {3,4,5} and all reported statistics will be based on these values
     *
     * @param windowSize
    ...
    public void setWindowSize(final int windowSize) {
        if (windowSize < 1) {
            if (windowSize != INFINITE_WINDOW) {
                throw MathRuntimeException.createIllegalArgumentException("window size must be positive ({0})",
                        windowSize);
            }
        }
    ...
    /**
     * Returns the current set of values in an array of double primitives. The order of addition is preserved. The
     * returned array is a fresh copy of the underlying data -- i.e., it is not a reference to the stored data.
     *
    ...
    /**
     * Returns the current set of values in an array of double primitives, sorted in ascending order. The returned array
     * is a fresh copy of the underlying data -- i.e., it is not a reference to the stored data.
     *
     ...
     * <ul>
     * <li><code>0 &lt; p &le; 100</code> (otherwise an <code>IllegalArgumentException</code> is thrown)</li>
     ...
     *             if percentile implementation has been overridden and the supplied implementation does not support
     *             setQuantile values
     */
    ...
    /**
     * Generates a text report displaying univariate statistics from values that have been added. Each statistic is
     ...
     *
     * @param meanImpl
     *            the UnivariateStatistic instance to use for computing the mean
     * @since 1.2
     */
     ...
     *            the UnivariateStatistic instance to use for computing the geometric mean
     * @since 1.2
     */
     ...
     *
     * @param kurtosisImpl
     *            the UnivariateStatistic instance to use for computing the kurtosis
     * @since 1.2
     */
     ...
     *
     * @param maxImpl
     *            the UnivariateStatistic instance to use for computing the maximum
     * @since 1.2
     */
     ...
     *
     * @param minImpl
     *            the UnivariateStatistic instance to use for computing the minimum
     * @since 1.2
     */
    ...
    /**
     * Sets the implementation to be used by {@link #getPercentile(double)}. The supplied
     * <code>UnivariateStatistic</code> must provide a <code>setQuantile(double)</code> method; otherwise
     * <code>IllegalArgumentException</code> is thrown.
     *
     * @param percentileImpl
     *            the percentileImpl to set
     * @throws IllegalArgumentException
     *             if the supplied implementation does not provide a <code>setQuantile</code> method
     * @since 1.2
     */
    public synchronized void setPercentileImpl(final UnivariateStatistic percentileImpl) {
        try {
            ...
            throw MathRuntimeException.createIllegalArgumentException(
                    "percentile implementation {0} does not support setQuantile", percentileImpl.getClass().getName());
        } catch (final IllegalAccessException e2) {
            throw MathRuntimeException.createIllegalArgumentException(ILLEGAL_ACCESS_MESSAGE, SET_QUANTILE_METHOD_NAME,
     ...
     *
     * @param skewnessImpl
     *            the UnivariateStatistic instance to use for computing the skewness
     * @since 1.2
     */
     ...
     *
     * @param varianceImpl
     *            the UnivariateStatistic instance to use for computing the variance
     * @since 1.2
     */
     ...
     *
     * @param sumsqImpl
     *            the UnivariateStatistic instance to use for computing the sum of squares
     * @since 1.2
     */
     ...
     *
     * @param sumImpl
     *            the UnivariateStatistic instance to use for computing the sum
     * @since 1.2
     */
