...
public class WsDeployer implements DynamicDeployer {
    ...
    public static Definition readWsdl(final URL url) throws OpenEJBException {
        ...
        try {
            ...
            final WsdlResolver wsdlResolver = new WsdlResolver(new URL(url, ".").toExternalForm(),
                    new InputSource(IO.read(url)));
    ...
    public static HandlerChains getHandlerChains(Class<?> declaringClass, final String serviceEndpoint,
                ...
                try {
                    ...
                    if (serviceQNames.size() == 1) {
                        ...
                        logger.error("The service-ref " + serviceRef.getName()
                                + " must define service-qname because the wsdl-file " + serviceRef.getWsdlFile()
                                + " does not constain any service definitions ");
                    } else {
                        logger.error("The service-ref " + serviceRef.getName()
                                + " must define service-qname because the wsdl-file " + serviceRef.getWsdlFile()
                                + " constain more then one service definitions " + serviceQNames);
                    }
                } catch (final Exception e) {
            ...
            try {
                ...
                if (JaxWsUtils.isWebService(clazz)) {
                    ...
                    if (portComponent.getId() == null) {
                        portComponent.setId(
                                webModule.getHost() + "." + webModule.getModuleId() + "." + servlet.getServletName());
                    }
                    if (webserviceDescription.getId() == null) {
                        webserviceDescription.setId(
                                webModule.getHost() + "." + webModule.getModuleId() + "." + servlet.getServletName());
                    }
                    ...
                    if (webserviceDescription.getWsdlFile() == null) {
                        webserviceDescription
                                .setWsdlFile(JaxWsUtils.getServiceWsdlLocation(clazz, webModule.getClassLoader()));
                    }
                    ...
                    if (portComponent.getHandlerChains() == null) {
                        final HandlerChains handlerChains = getHandlerChains(clazz,
                                portComponent.getServiceEndpointInterface(), webModule.getClassLoader());
    ...
    private void processPorts(final EjbModule ejbModule) throws OpenEJBException {
        ...
        for (final EnterpriseBean enterpriseBean : ejbModule.getEjbJar().getEnterpriseBeans()) {
            ...
            if (portComponent == null) {
                portComponent = new PortComponent();
                if (webserviceDescription.getPortComponentMap()
                        .containsKey(JaxWsUtils.getPortQName(ejbClass).getLocalPart())) {
            ...
            if (JaxWsUtils.isWebService(ejbClass)) {
                ...
                if (webserviceDescription.getWsdlFile() == null) {
                    webserviceDescription
                            .setWsdlFile(JaxWsUtils.getServiceWsdlLocation(ejbClass, ejbModule.getClassLoader()));
                }
                if (portComponent.getWsdlService() == null) {
                ...
                // handlers
                if (portComponent.getHandlerChains() == null) {
                    final HandlerChains handlerChains = getHandlerChains(ejbClass, sessionBean.getServiceEndpoint(),
        ...
        try {
            ...
            if (module.getClassLoader() != null) {
                final URL wsdlUrl = module.getClassLoader().getResource(
                        wsdlFile.startsWith("classpath:") ? wsdlFile.substring("classpath:".length()) : wsdlFile);
                try {
                    final Definition definition = readWsdl(wsdlUrl);
                    module.getAltDDs().put(wsdlFile, definition);
                    return definition;
