...
public final class ValidatorBuilder {
    ...
    public static ValidationInfo getInfo(final ValidationConfigType config) {
        ...
        if (config != null) {
            ...
            if (executableValidation != null) {
                ...
                final DefaultValidatedExecutableTypesType executableTypes = executableValidation
        ...
        try {
            ...
            if (config == null) {
                ...
                try {
                    ...
                    logger.warning("Unable create validator factory with config " + config + " (" + ve.getMessage()
    ...
    private static Configuration<?> getConfig(final ValidationInfo info) {
        ...
        if (providerClassName != null) {
            try {
                ...
                logger.warning("Unable create validator factory with provider " + providerClassName + " ("
                        + ve.getMessage() + ")." + " Default one will be used.");
            }
        }
        if (target == null) {
        ...
        final OpenEjbBootstrapConfig bootstrapConfig = new OpenEjbBootstrapConfig(providerClassName,
                info.constraintFactoryClass, info.messageInterpolatorClass, info.traversableResolverClass,
                info.parameterNameProviderClass, new HashSet<>(info.constraintMappings),
                info.executableValidationEnabled, types, props);
        ...
        if (messageInterpolatorClass != null) {
            try {
                @SuppressWarnings("unchecked")
                final Class<MessageInterpolator> clazz = (Class<MessageInterpolator>) classLoader
        ...
        if (traversableResolverClass != null) {
            try {
                @SuppressWarnings("unchecked")
                final Class<TraversableResolver> clazz = (Class<TraversableResolver>) classLoader
        ...
        if (constraintFactoryClass != null) {
            try {
                @SuppressWarnings("unchecked")
                final Class<ConstraintValidatorFactory> clazz = (Class<ConstraintValidatorFactory>) classLoader
        ...
        for (final String mappingFileName : info.constraintMappings) {
            ...
            if (in == null) {
                logger.warning(
                        "Unable to open input stream for mapping file " + mappingFileName + ". It will be ignored");
            } else {
        ...
        if (info.parameterNameProviderClass != null) {
            try {
                final Class<ParameterNameProvider> clazz = (Class<ParameterNameProvider>) classLoader
                        .loadClass(info.parameterNameProviderClass);
    ...
    private static final class OpenEjbBootstrapConfig implements BootstrapConfiguration, Serializable {
        ...
        public OpenEjbBootstrapConfig(final String providerClassName, final String constraintFactoryClass,
                final String messageInterpolatorClass, final String traversableResolverClass,
                final String parameterNameProviderClass, final Set<String> constraintMappings,
                final boolean executableValidationEnabled, final Set<ExecutableType> validatedTypes,
    ...
    private static final class OpenEJBValidatorFactory implements ValidatorFactory {
        ...
        public OpenEJBValidatorFactory(final ValidatorFactory validatorFactory,
                final Collection<Releasable<?>> releasables) {
