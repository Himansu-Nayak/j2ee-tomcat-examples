...
public class CdiEjbBean<T> extends BaseEjbBean<T>
        implements InterceptedMarker, DeploymentValidationService.BeanInterceptorInfoProvider {
    ...
    public CdiEjbBean(final BeanContext beanContext, final WebBeansContext webBeansContext, final AnnotatedType<T> at,
        ...
        this(beanContext, webBeansContext, beanContext.getManagedClass(), at,
                new EjbInjectionTargetFactory<T>(beanContext, at, webBeansContext), attributes);
        EjbInjectionTargetImpl.class.cast(getInjectionTarget()).setCdiEjbBean(this);
    }

    public CdiEjbBean(final BeanContext bc, final WebBeansContext webBeansContext, final Class beanClass,
            final AnnotatedType<T> at, final InjectionTargetFactoryImpl<T> factory,
            final BeanAttributes<T> attributes) {
        super(webBeansContext, toSessionType(bc.getComponentType()), at, new EJBBeanAttributesImpl<T>(bc, attributes),
                beanClass, factory);
        this.beanContext = bc;
        ...
        if (webBeansContext.getBeanManagerImpl().isPassivatingScope(getScope()) && stateful) {
            if (!getBeanContext().isPassivable()) {
                throw new DefinitionException(
                        getBeanContext().getBeanClass() + " is a not apssivation-capable @Stateful with a scope "
        ...
        if (getAnnotatedType().isAnnotationPresent(Interceptor.class)
                || getAnnotatedType().isAnnotationPresent(Decorator.class)) {
    ...
    protected T createEjb(final CreationalContext<T> creationalContext) {
        ...
        try {
            ...
            if (homeLocalBean != null) {
                ...
                throw new IllegalStateException("no interface to proxy for ejb " + beanContext.getEjbName()
                        + ", is this is a MDB maybe you shouldn't use a scope?");
            }
    ...
    private void destroyScopedStateful(final T instance, final CreationalContext<T> cc) {
        try {
            ...
            logger.log(Level.FINE, "The stateful instance " + instance + " can't be removed since it was invalidated",
                    e);
            return;
        }
    ...
    public void initInternals() {
        ...
        if (beanContext.getComponentType().isMessageDriven()) {
            ...
            final List<Class> interfaces = ProxyInterfaceResolver.getInterfaces(beanContext.getBeanClass(),
            ...
            final List<Class> interfaces = ProxyInterfaceResolver.getInterfaces(beanContext.getBeanClass(),
                    mainInterface, beanContext.getBusinessRemoteInterfaces());
    ...
    public static class EJBBeanAttributesImpl<T> extends BeanAttributesImpl<T> { // TODO: move it in its own class
        ...
        public void initTypes() {
            ...
            /*
             * not in EJB types - 3.2.2 of cdi 1.2 final List<Class> clRemote =
             * beanContext.getBusinessRemoteInterfaces(); if (clRemote != null && !clRemote.isEmpty()) { for (final
             * Class<?> c : clRemote) { ejbTypes.add(c); // parentInterfaces(c), but is it useful in practise? } }
             */
    ...
    public static class EjbInjectionTargetFactory<T> extends InjectionTargetFactoryImpl<T> {
        ...
        public EjbInjectionTargetFactory(final BeanContext bc, final AnnotatedType<T> annotatedType,
        ...
        public InjectionTarget<T> createInjectionTarget(final Bean<T> bean) {
            final EjbInjectionTargetImpl<T> injectionTarget = new EjbInjectionTargetImpl<T>(getAnnotatedType(),
                    createInjectionPoints(bean), getWebBeansContext());
            final InjectionTarget<T> it = getWebBeansContext().getWebBeansUtil()
                    .fireProcessInjectionTargetEvent(injectionTarget, getAnnotatedType()).getInjectionTarget();
    ...
    public static class EjbInjectionTargetImpl<T> extends InjectionTargetImpl<T> {
        ...
        public EjbInjectionTargetImpl(final AnnotatedType<T> annotatedType, final Set<InjectionPoint> points,
                final WebBeansContext webBeansContext) {
            super(annotatedType, points, webBeansContext, Collections.<AnnotatedMethod<?>> emptyList(),
        ...
        public EjbInjectionTargetImpl(final EjbInjectionTargetImpl<T> original, final InjectionTarget<T> delegate) {
            super(original.annotatedType, original.getInjectionPoints(), original.webBeansContext,
        ...
        private static boolean isDynamicBean(final Bean<?> bean) {
            return CdiEjbBean.class.isInstance(bean)
                    && CdiEjbBean.class.cast(bean).beanContext.isDynamicallyImplemented();
        }
        ...
        @Override
        protected void defineLifecycleInterceptors(final Bean<T> bean, final AnnotatedType<T> annotatedType,
                final WebBeansContext webBeansContext) {
