...
public class MethodTransactionBuilder {
    ...
    public static void applyTransactionAttributes(final BeanContext beanContext,
            List<MethodTransactionInfo> methodTransactionInfos) throws OpenEJBException {
        ...
        final Map<MethodInfoUtil.ViewMethod, MethodAttributeInfo> attributes = resolveViewAttributes(
                methodTransactionInfos, beanContext);
        ...
        for (final Map.Entry<MethodInfoUtil.ViewMethod, MethodAttributeInfo> entry : attributes.entrySet()) {
            ...
            beanContext.setMethodTransactionAttribute(method, TransactionType.get(transactionInfo.transAttribute),
                    view);
        }
    }

    /**
     * This method splits the MethodTransactionInfo objects so that there is exactly one MethodInfo per
     * MethodTransactionInfo. A single MethodTransactionInfo with three MethodInfos would be expanded into three
     * MethodTransactionInfo with one MethodInfo each.
     * <p/>
    ...
    public void build(final HashMap<String, BeanContext> deployments,
            final List<MethodTransactionInfo> methodTransactions) throws OpenEJBException {
