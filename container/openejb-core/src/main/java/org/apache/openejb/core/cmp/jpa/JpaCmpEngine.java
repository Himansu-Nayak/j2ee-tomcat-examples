...
public class JpaCmpEngine implements CmpEngine {
    ...
    private EntityManager getEntityManager(final BeanContext beanContext) {
        ...
        if (entityManager == null) {
            throw new EJBException(
                    "Entity manager not found at \"openejb/cmp\" in jndi ejb " + beanContext.getDeploymentID());
        }
    ...
    private synchronized void registerListener(final EntityManager entityManager) {
        if (entityManager instanceof OpenJPAEntityManagerSPI) {
            ...
            final OpenJPAEntityManagerFactorySPI openjpaEMF = (OpenJPAEntityManagerFactorySPI) openjpaEM
    ...
    public List<Object> queryBeans(final ThreadContext callContext, final Method queryMethod, final Object[] args)
    ...
    public List<Object> queryBeans(final BeanContext beanContext, final String signature, final Object[] args)
            throws FinderException {
        final EntityManager entityManager = getEntityManager(beanContext);
    ...
    public int executeUpdateQuery(final BeanContext beanContext, final String signature, Object[] args)
            throws FinderException {
        final EntityManager entityManager = getEntityManager(beanContext);
    ...
    private void commitTransaction(final String operation, final ThreadContext callContext,
