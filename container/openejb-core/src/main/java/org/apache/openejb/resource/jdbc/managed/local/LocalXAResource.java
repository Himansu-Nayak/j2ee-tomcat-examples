...
public class LocalXAResource implements XAResource {
        ...
        if (flag == XAResource.TMNOFLAGS) {
            ...
            if (xid != currentXid) {
                throw new XAException(
                        "Attempting to resume in different transaction: expected " + currentXid + ", but was " + xid);
            }
        } else {
    ...
    private RuntimeException cantGetLock() {
        return new IllegalStateException("can't get lock on resource with Xid " + currentXid + " from thread "
