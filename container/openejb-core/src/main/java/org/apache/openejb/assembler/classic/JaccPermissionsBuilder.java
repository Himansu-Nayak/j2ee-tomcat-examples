...
public class JaccPermissionsBuilder {
    ...
    private static final Logger log = Logger.getInstance(LogCategory.OPENEJB_STARTUP.createChild("attributes"),
            JaccPermissionsBuilder.class);
    ...
    public PolicyContext build(final EjbJarInfo ejbJar, final HashMap<String, BeanContext> deployments)
    ...
    private void addDeclaredEjbPermissions(final EjbJarInfo ejbJar, final EnterpriseBeanInfo beanInfo,
            final String defaultRole, PermissionCollection notAssigned, final PolicyContext policyContext)
                    throws OpenEJBException {
        ...
        for (final MethodPermissionInfo methodPermission : ejbJar.methodPermissions) {
            ...
            for (final MethodInfo method : methodPermission.methods) {
                ...
                final EJBMethodPermission permission = new EJBMethodPermission(ejbName, methodName, methodIntf,
                        methodParams);
         ...
         * <code>exclude-list</code> element. In this case, it is the responsibility of the Deployer to assign method
         * permissions for all of the unspecified methods, either by assigning them to security roles, or by marking
         * them as <code>unchecked</code>.
         */
     ...
     * <p/>
     * Method permissions are defined in the deployment descriptor as a binary relation from the set of security roles
     * to the set of methods of the home, component, and/or web service endpoint interfaces of session and entity beans,
     * including all their superinterfaces (including the methods of the <code>EJBHome</code> and <code>EJBObject</code>
     * interfaces and/or <code>EJBLocalHome</code> and <code>EJBLocalObject</code> interfaces).
     *
     * @param permissions
     ...
     */
    public void addPossibleEjbMethodPermissions(final PermissionCollection permissions, final String ejbName,
            final String methodInterface, final Class clazz) throws OpenEJBException {
        if (clazz == null) {
            return;
        }
        for (final Method method : clazz.getMethods()) {
            final String methodIface = "LocalBean".equals(methodInterface) || "LocalBeanHome".equals(methodInterface)
    ...
    /**
     * Removes permissions from <code>toBeChecked</code> that are implied by <code>permission</code>.
     *
     * @param toBeChecked
     *            the permissions that are to be checked and possibly culled
     * @param permission
     *            the permission that is to be used for culling
