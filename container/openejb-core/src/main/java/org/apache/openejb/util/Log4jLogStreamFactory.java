...
public class Log4jLogStreamFactory implements LogStreamFactory {
    ...
    private void preprocessProperties(final Properties properties) {
        ...
        if (missing.size() > 0) {
            ...
            logger.error(
                    "Logging may not operate as expected.  The directories for the following files do not exist so no file can be created.  See the list below.");
    ...
    private org.apache.log4j.Logger getFallabckLogger() {
         ...
         *
         * final SimpleLayout simpleLayout = new SimpleLayout(); final ConsoleAppender newAppender = new
         * ConsoleAppender(simpleLayout); logger.addAppender(newAppender);
         */
        return logger;
    }
