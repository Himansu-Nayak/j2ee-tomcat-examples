...
public class CheckAsynchronous extends ValidationBase {
    ...
    public void validate(final EjbModule module) {
        ...
        for (final ApplicationException applicationException : module.getEjbJar().getAssemblyDescriptor()
        ...
        for (final EnterpriseBean bean : module.getEjbJar().getEnterpriseBeans()) {
            ...
            if (bean instanceof SessionBean) {
                ...
                for (final AsyncMethod asyncMethod : session.getAsyncMethod()) {
                    ...
                    if (method == null) {
                        fail(bean, "asynchronous.missing", asyncMethod.getMethodName(), ejbClass.getName(),
                ...
                for (final Method method : classFinder.findAnnotatedMethods(Asynchronous.class)) {
                    ignoredMethodAnnotation("Asynchronous", bean, bean.getEjbClass(), method.getName(),
    ...
    private void checkAsynchronousMethod(final SessionBean bean, final Class<?> ejbClass, final Method method,
            final Set<String> applicationExceptions) {
    ...
    private String checkThrowCauses(final Class<?>[] exceptionTypes, final Set<String> applicationExceptions) {
        ...
        for (final Class<?> exceptionType : exceptionTypes) {
            if (applicationExceptions.contains(exceptionType.getName())
                    || !Exception.class.isAssignableFrom(exceptionType)
