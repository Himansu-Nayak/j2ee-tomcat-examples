    ...
    public static String searchForConfiguration(final String rawPath) throws OpenEJBException {
        ...
        if (rawPath != null) {
            for (final String path : deducePaths(rawPath)) {
                /*
                 * [1] Try finding the file relative to the current working directory
                 */
                file = new File(path);
                ...
                /*
                 * [4] Consider path as a URL resource - file: and jar: accepted by JaxbOpenejb.readConfig(String
                 * configFile)
                 */
                try {
        ...
        try {
            /*
             * [4] Try finding the standard openejb.xml file relative to the openejb.base directory
             */
            ...
            if (EnvProps.extractConfigurationFiles()) {
                ...
                /*
                 * [6] No config found! Create a config for them using the default.openejb.conf file from the
                 * openejb-x.x.x.jar
                 */
                ...
                if (confDir != null && confDir.exists()) {
                    ...
                    logger.info("Cannot find the configuration file [conf/openejb.xml].  Creating one at "
                            + config.getAbsolutePath());
                    file = createConfig(config);
                } else {
                    logger.info(
                            "Cannot find the configuration file [conf/openejb.xml].  Will attempt to create one for the beans deployed.");
                }
            }
        ...
        for (final Deployments d : config.getDeployments()) {
            ...
            if (d.getFile() != null) {
                try {
                    ...
                    /*
                     * If the jar entry is already there, no need to add it to the config or go any futher.
                     */
                    ...
                    /*
                     * No handling needed. If there is a problem resolving a config file path, it is better to just add
                     * this jars path explicitly.
                     */
                }
                ...
                try {
                    ...
                    /*
                     * If a dir entry is already there, the jar will be loaded automatically. No need to add it
                     * explicitly to the config or go any futher.
                     */
