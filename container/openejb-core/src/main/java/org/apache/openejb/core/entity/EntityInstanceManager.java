...
public class EntityInstanceManager {
    ...
    /**
     * contains a collection of LinkListStacks indexed by deployment id. Each indexed stack represents the method ready
     * pool of for that class.
     */
    private final Map<Object, LinkedListStack> poolMap;

    private final SecurityService securityService;

    public EntityInstanceManager(final EntityContainer container, final SecurityService securityService,
            final int poolSize) {
        ...
        if (callContext.getPrimaryKey() != null && txPolicy != null && txPolicy.isTransactionActive()) {
            ...
            if (wrapper != null) {// if true, the requested bean instance is already enrolled in a transaction
                ...
                if (!wrapper.isAssociated()) {// is NOT associated
                    /*
                     * If the bean identity was removed (via ejbRemove()) within the same transaction, then it's
                     * SynchronizationWrapper will be in the txReady pool but marked as disassociated. This allows us to
                     * prevent a condition where the caller removes the bean and then attempts to call a business method
                     * on that bean within the same transaction. After a bean is removed any subsequent invocations on
                     * that bean with the same transaction should throw a NoSuchEntityException. its likely that the
                     * application server would have already made the reference invalid, but this bit of code is an
                     * extra precaution.
                     */
                    throw new InvalidateReferenceException(
                            new NoSuchObjectException("Entity not found: " + primaryKey));
                } else if (callContext.getCurrentOperation() == Operation.REMOVE) {
                    /*
                     * To avoid calling ejbStore( ) on a bean that after its removed, we can not delegate the wrapper is
                     * marked as disassociated from the transaction to avoid processing the beforeCompletion( ) method
                     * on the SynchronizationWrapper object.
                     */
                ...
                /*
                 * If no synchronized wrapper for the key exists Then the bean entity is being access by this
                 * transaction for the first time, so it needs to be enrolled in the transaction.
                 */
                final EntityBean bean = getPooledInstance(callContext);
                wrapper = new SynchronizationWrapper(callContext.getBeanContext(), primaryKey, bean, false, key,
                        txPolicy);
                ...
                if (callContext.getCurrentOperation() == Operation.REMOVE) {
                     ...
                     * on the SynchronizationWrapper object.
                     *
                     * We have to still use a wrapper so we can detect when a business method is called after a
                     * ejbRemove() and act to prevent it from being processed.
                     */
                    wrapper.disassociate();
                }
                ...
                try {
                    ...
                    wrapper.disassociate();
                    throw new InvalidateReferenceException(
                            new NoSuchObjectException("Entity not found: " + primaryKey, e));
    ...
    protected EntityBean getPooledInstance(final ThreadContext callContext) throws OpenEJBException {
        ...
        if (bean == null) {
            ...
            try {
                /*
                 * setEntityContext executes in an unspecified transactional context. In this case we choose to allow it
                 * to have what every transaction context is current. Better then suspending it unnecessarily.
                 *
                 * We also chose not to invoke EntityContainer.invoke( ) method, which duplicate the exception handling
                 * logic but also attempt to manage the begining and end of a transaction. It its a container managed
                 * transaciton we don't want the TransactionScopeHandler commiting the transaction in afterInvoke()
                 * which is what it would attempt to do.
                 */
        ...
        if (callContext.getCurrentOperation() == Operation.BUSINESS
                || callContext.getCurrentOperation() == Operation.REMOVE) {
            /*
             * When a bean is retrieved from the bean pool to service a client's business method request it must be
             * notified that its about to enter service by invoking its ejbActivate( ) method. A bean instance does not
             * have its ejbActivate() invoked when: 1. Its being retreived to service an ejbCreate()/ejbPostCreate(). 2.
             * Its being retrieved to service an ejbFind method. 3. Its being retrieved to service an ejbRemove()
             * method. See section 9.1.4 of the EJB 1.1 specification.
             */
            final Operation currentOp = callContext.getCurrentOperation();
            ...
            try {
                /*
                 * In the event of an exception, OpenEJB is required to log the exception, evict the instance, and mark
                 * the transaction for rollback. If there is a transaction to rollback, then the a
                 * javax.transaction.TransactionRolledbackException must be throw to the client. See EJB 1.1
                 * specification, section 12.3.2
                 */
                bean.ejbActivate();
                ...
                if (txPolicy != null && txPolicy.isTransactionActive()) {
                    ...
                    throw new ApplicationException(new TransactionRolledbackException(
                            "Reflection exception thrown while attempting to call ejbActivate() on the instance", e));
                }
                throw new ApplicationException(new RemoteException(
                        "Exception thrown while attempting to call ejbActivate() on the instance. Exception message = "
                                + e.getMessage(),
                        e));
            } finally {
                callContext.setCurrentOperation(currentOp);
    ...
    public void poolInstance(final ThreadContext callContext, final EntityBean bean, final Object primaryKey)
        ...
        if (primaryKey != null && txPolicy != null && txPolicy.isTransactionActive()) {
            ...
            if (wrapper != null) {
                if (callContext.getCurrentOperation() == Operation.REMOVE) {
                    /*
                     * The bean is being returned to the pool after it has been removed. Its important at this point to
                     * mark the bean as disassociated to prevent it's ejbStore method from bean called (see
                     * SynchronizationWrapper.beforeCompletion() method) and that subsequent methods can not be invoked
                     * on the bean identity (see obtainInstance() method).
                     */
                    wrapper.disassociate();
                    /*
                     * If the bean has been removed then the bean instance is no longer needed and can return to the
                     * methodReadyPool to service another identity.
                     */
                ...
                /*
                 * A wrapper will not exist if the bean is being returned after a create operation. In this case the
                 * transaction scope is broader then the create method itself; its a client initiated transaction, so
                 * the bean must be registered with the tranaction and moved to the tx ready pool
                 */

                wrapper = new SynchronizationWrapper(callContext.getBeanContext(), primaryKey, bean, true, key,
                        txPolicy);
            ...
            /*
             * If there is no transaction associated with the thread OR if the operation was a find or home method
             * (PrimaryKey == null) Then the bean instance is simply returned to the methodReady pool
             */

            if (primaryKey != null && callContext.getCurrentOperation() != Operation.REMOVE) {
                /*
                 * If the bean has a primary key; And its not being returned following a remove operation; then the bean
                 * is being returned to the method ready pool after successfully executing a business method or create
                 * method. In this case we need to call the bean instance's ejbPassivate before returning it to the pool
                 * per EJB 1.1 Section 9.1.
                 */
                final Operation currentOp = callContext.getCurrentOperation();
                ...
                try {
                    /*
                     * In the event of an exception, OpenEJB is required to log the exception, evict the instance, and
                     * mark the transaction for rollback. If there is a transaction to rollback, then the a
                     * javax.transaction.TransactionRolledbackException must be throw to the client. See EJB 1.1
                     * specification, section 12.3.2
                     */
                    bean.ejbPassivate();
                    ...
                    if (txPolicy.isTransactionActive()) {
                        txPolicy.setRollbackOnly(e);
                        throw new ApplicationException(new TransactionRolledbackException(
                                "Reflection exception thrown while attempting to call ejbPassivate() on the instance",
                                e));
                    }
                    throw new ApplicationException(new RemoteException(
                            "Reflection exception thrown while attempting to call ejbPassivate() on the instance. Exception message = "
                                    + e.getMessage(),
                            e));
                } finally {
                    callContext.setCurrentOperation(currentOp);
            ...
            /*
             * The bean is returned to the method ready pool if its returned after servicing a find, ejbHome, business
             * or create method and is not still part of a tx. While in the method ready pool the bean instance is not
             * associated with a primary key and may be used to service a request for any bean of the same class.
             */
            final Stack methodReadyPool = poolMap.get(callContext.getBeanContext().getDeploymentID());
            methodReadyPool.push(bean);
        }
    ...
    public void freeInstance(final ThreadContext callContext, final EntityBean bean) throws SystemException {
        ...
        try {
            /*
             * unsetEntityContext executes in an unspecified transactional context. In this case we choose to allow it
             * to have what every transaction context is current. Better then suspending it unnecessarily.
             *
             * We also chose not to invoke EntityContainer.invoke( ) method, which duplicate the exception handling
             * logic but also attempt to manage the begining and end of a transaction. It its a container managed
             * transaciton we don't want the TransactionScopeHandler commiting the transaction in afterInvoke() which is
             * what it would attempt to do.
             */
    ...
    public void discardInstance(final ThreadContext callContext, final EntityBean bean) throws SystemException {
        ...
        if (wrapper != null) {
            /*
             * It's not possible to deregister a wrapper with the transaction, but it can be removed from the tx pool
             * and made inoperative by calling its disassociate method. The wrapper will be returned to the wrapper pool
             * after the transaction completes (see SynchronizationWrapper.afterCompletion( ) method). The wrapper must
             * be returned after the transaction completes so that it is not in the service of another bean when the
             * TransactionManager calls its Synchronization methods.
             * 
             * In addition, the bean instance is dereferenced so it can be garbage collected.
             */
            wrapper.disassociate();
        }
    }
    ...
    /*
     * Instances of this class are used as keys for storing bean instances in the tx method ready pool. A compound key
     * composed of the transaction, primary key, and deployment id identifiers is required to uniquely identify a bean
     * in the tx method ready pool.
     */
    ...
    /*
     * Instances of this class are used to wrap entity instances so that they can be registered with a tx. When the
     * Synchronization.beforeCompletion is called, the bean's ejbStore method is invoked. When the
     * Synchroniztion.afterCompletion is called, the bean instance is returned to the method ready pool. Instances of
     * this class are not recycled anymore, because modern VMs (JDK1.3 and above) perform better for objects that are
     * short lived.
     */
    protected class SynchronizationWrapper implements TransactionSynchronization {
        ...
        /*
         * <tt>isAvailable<tt> determines if the wrapper is still associated with a bean. If the bean identity is
         * removed (ejbRemove) or if the bean instance is discarded, the wrapper will not longer be associated with a
         * bean instances and therefore its beforeCompletion method will not process the ejbStore method.
         */
        private boolean available;
        private boolean associated;

        public SynchronizationWrapper(final BeanContext beanContext, final Object primaryKey, final EntityBean bean,
                final boolean available, final Key readyPoolKey, final TransactionPolicy txPolicy) {
            if (bean == null) {
