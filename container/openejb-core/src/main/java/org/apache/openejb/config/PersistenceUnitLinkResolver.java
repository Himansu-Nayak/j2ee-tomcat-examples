...
public class PersistenceUnitLinkResolver extends UniqueDefaultLinkResolver<PersistenceUnit> {
    ...
    @Override
    protected Collection<PersistenceUnit> tryToResolveForEar(final Collection<PersistenceUnit> values,
    ...
    private boolean isIn(final PersistenceUnit value, final WebModule war) {
        final Collection<URL> urls = (Collection<URL>) war.getAltDDs()
    ...
    private WebModule extractWebApp(final URI moduleUri) {
        ...
        for (final WebModule webModule : module.getWebModules()) {
            if (webModule.getJarLocation() != null
                    && isParent(new File(webModule.getJarLocation()), moduleFile, appModuleFile)) {
                return webModule;
            }
        }
