...
public class BeanContext extends DeploymentContext {
     ...
     */
    private BeanContext(final String id, final Context jndiContext, final ModuleContext moduleContext,
            final BeanType componentType, final boolean localBean, final Class beanClass, final boolean passivable) {
            ...
            for (final String interceptor : interceptorArray) {
                if (interceptor != null && !interceptor.isEmpty()) {
                    ...
                    try {
                        ...
                        logger.warning(
                                "interceptor " + interceptor + " not found, are you sure the container can load it?");
                        continue;
                    }
    ...
    public BeanContext(final String id, final Context jndiContext, final ModuleContext moduleContext,
            final Class beanClass, final Class homeInterface, final Class remoteInterface,
            final Class localHomeInterface, final Class localInterface, final Class proxy,
            final Class serviceEndpointInterface, final List<Class> businessLocals, final List<Class> businessRemotes,
            final Class pkClass, final BeanType componentType, final boolean localBean, final boolean passivable)
    ...
    public BeanContext(final String id, final Context jndiContext, final ModuleContext moduleContext,
            final Class beanClass, final Class mdbInterface, final Map<String, String> activationProperties)
                    throws SystemException {
    ...
    public void mergeOWBAndOpenEJBInfo() {
        ...
        final InjectionTargetImpl<?> injectionTarget = InjectionTargetImpl.class
                .cast(get(CdiEjbBean.class).getInjectionTarget());
        final InterceptorResolutionService.BeanInterceptorInfo info = injectionTarget.getInterceptorInfo();
        if (info == null) {
            return;
        }

        final Collection<Interceptor<?>> postConstructInterceptors = Collection.class
                .cast(Reflections.get(injectionTarget, "postConstructInterceptors"));
        final Collection<Interceptor<?>> preDestroyInterceptors = Collection.class
                .cast(Reflections.get(injectionTarget, "preDestroyInterceptors"));
        ...
        for (final Map.Entry<Method, InterceptorResolutionService.BusinessMethodInterceptorInfo> entry : info
                .getBusinessMethodsInfo().entrySet()) {
        ...
        if (info.getSelfInterceptorBean() != null) {
            try {
                final Field field = InterceptorResolutionService.BeanInterceptorInfo.class
                        .getDeclaredField("selfInterceptorBean");
    ...
    private InterceptorData createInterceptorData(final Interceptor<?> i) {
        ...
        if (CdiInterceptorBean.class.isInstance(i)) {
            ...
            data.getAroundInvoke()
                    .addAll(getInterceptionMethodAsListOrEmpty(cdiInterceptorBean, InterceptionType.AROUND_INVOKE));
            data.getPostConstruct()
                    .addAll(getInterceptionMethodAsListOrEmpty(cdiInterceptorBean, InterceptionType.POST_CONSTRUCT));
            data.getPreDestroy()
                    .addAll(getInterceptionMethodAsListOrEmpty(cdiInterceptorBean, InterceptionType.PRE_DESTROY));
            data.getPostActivate()
                    .addAll(getInterceptionMethodAsListOrEmpty(cdiInterceptorBean, InterceptionType.POST_ACTIVATE));
            data.getPrePassivate()
                    .addAll(getInterceptionMethodAsListOrEmpty(cdiInterceptorBean, InterceptionType.PRE_PASSIVATE));
            data.getAroundTimeout()
                    .addAll(getInterceptionMethodAsListOrEmpty(cdiInterceptorBean, InterceptionType.AROUND_TIMEOUT));
            /*
    ...
    private List<Method> getInterceptionMethodAsListOrEmpty(final CdiInterceptorBean cdiInterceptorBean,
            final InterceptionType aroundInvoke) {
    ...
    /**
     * DMB: This is a not so reliable way to determine the proxy type The proxy type really should come with the call in
     * the invoke.
     *
     * @param interfce
     *            Class
    ...
    public void setExtendedEntityManagerFactories(
            final Index<EntityManagerFactory, EntityManagerConfiguration> extendedEntityManagerFactories) {
    ...
    public EJBLocalHome getEJBLocalHome() {
        ...
        if (getLegacyView().ejbLocalHomeRef == null) {
            getLegacyView().ejbLocalHomeRef = (EJBLocalHome) EjbHomeProxyHandler.createHomeProxy(this,
                    InterfaceType.EJB_LOCAL_HOME);
        }
    ...
    public BusinessLocalBeanHome getBusinessLocalBeanHome() {
        ...
        return (BusinessLocalBeanHome) EjbHomeProxyHandler.createHomeProxy(this, InterfaceType.BUSINESS_LOCALBEAN_HOME,
                interfaces, this.beanClass);
    }
    ...
    public BusinessLocalHome getBusinessLocalHome(final Class mainInterface) {
        ...
        final List<Class> interfaces = ProxyInterfaceResolver.getInterfaces(this.getBeanClass(), mainInterface,
                localInterfaces);
     ...
     * TODO: Move to MethodContext
     */
    public void setMethodTransactionAttribute(final Method method, final TransactionType transactionType)
     ...
     */
    public void setMethodTransactionAttribute(final Method method, final TransactionType transactionType,
            final String view) throws OpenEJBException {
    ...
    /**
     * When an instance of an EJB is instantiated, everything in this list is also instatiated and tied to the bean
     * instance. Per spec, interceptors are supposed to have the same lifecycle as the bean they wrap.
     * <p/>
     * OpenEJB has the concept of interceptors which do not share the same lifecycle as the bean instance -- they may be
     * instantiated elsewhere and simply applied to the bean. The impact is that these interceptors must be
     * multithreaded. It also means we do not add these interceptors to this list and expose them via different means.
     *
                ...
                if (SessionBean.class.isAssignableFrom(beanClass)) {
                    ...
                    if (beanMethod == null && (getHomeInterface() != null || getLocalHomeInterface() != null)) {
                        throw new IllegalStateException(
                                "Bean class has no @Remove methods to match EJBObject.remove() or EJBLocalObject.remove().  A no-arg remove method must be added: beanClass="
                                        + beanClass.getName());
                    }
                }
        ...
        for (final Method method : homeMethods) {
            ...
            try {
                ...
                if (method.getName().startsWith("create")) {
                    ...
                    /*
                     * Entity beans have a ejbCreate and ejbPostCreate methods with matching parameters. This code maps
                     * that relationship.
                     */
                    if (this.componentType == BeanType.BMP_ENTITY || this.componentType == BeanType.CMP_ENTITY) {
                        ...
                        final Method postCreateMethod = clazz.getMethod(ejbCreateName.toString(),
                                method.getParameterTypes());
                        getCmp().postCreateMethodMap.put(getLegacyView().createMethod, postCreateMethod);
                    }
                    /*
                     * Stateless session beans only have one create method. The getCreateMethod is used by instance
                     * manager of the core.stateless.StatelessContainer as a convenience method for obtaining the
                     * ejbCreate method.
                     */
                    ...
                    final String beanMethodName = "ejbHome" + method.getName().substring(0, 1).toUpperCase()
    ...
    private void mapObjectInterface(final Class intrface) {
        if (intrface == BusinessLocalHome.class || intrface == BusinessRemoteHome.class
                || intrface == ServiceEndpoint.class) {
            return;
        }
        ...
        for (final Method method : interfaceMethods) {
            ...
            try {
                ...
                throw new OpenEJBRuntimeException(
                        "Invalid method [" + method + "]. Not declared by " + beanClass.getName() + " class");
            }
        }
    }
    ...
    public InstanceContext newInstance() throws Exception {
        ...
        try {
            ...
            CreationalContext<Object> creationalContext = currentCreationalContext != null
                    ? currentCreationalContext.get() : null;
            ...
            if (!CreationalContextImpl.class.isInstance(creationalContext) && webBeansContext != null) {
                if (creationalContext == null) {
                    ...
                    creationalContext = webBeansContext.getCreationalContextFactory()
                            .wrappedCreationalContext(creationalContext, cdiEjbBean);
                }
            }

            final Object rootInstance;
            if (cdiEjbBean != null && !dynamicallyImplemented
                    && CdiEjbBean.EjbInjectionTargetImpl.class.isInstance(cdiEjbBean.getInjectionTarget())) {
            ...
            if (!dynamicallyImplemented) {
                injectionProcessor = new InjectionProcessor(rootInstance, getInjections(),
                ...
                for (final Injection injection : getInjections()) {
                    if (beanClass.equals(injection.getTarget())) {
                        final Injection updated = new Injection(injection.getJndiName(), injection.getName(),
                                proxyClass);
            ...
            for (final InterceptorData interceptorData : this.getInstanceScopedInterceptors()) {
                ...
                if (webBeansContext != null) {
                    ConstructorInjectionBean interceptorConstructor = interceptorData
                            .get(ConstructorInjectionBean.class);
                    if (interceptorConstructor == null) {
                        synchronized (this) {
                            interceptorConstructor = interceptorData.get(ConstructorInjectionBean.class);
                            if (interceptorConstructor == null) {
                                interceptorConstructor = new ConstructorInjectionBean(webBeansContext, clazz,
                ...
                final InjectionProcessor interceptorInjector = new InjectionProcessor(iInstance, this.getInjections(),
                        InjectionProcessor.unwrap(ctx));
                try {
            ...
            final InterceptorStack postConstruct = new InterceptorStack(beanInstance, null, Operation.POST_CONSTRUCT,
            ...
            if (componentType == BeanType.SINGLETON || componentType == BeanType.STATEFUL) {
                final Set<Method> callbacks = callbackInterceptors.get(callbackInterceptors.size() - 1)
                        .getPostConstruct();
                ...
                transactionType = isBeanManagedTransaction() ? TransactionType.BeanManaged
                        : TransactionType.NotSupported;
            }
            final TransactionPolicy transactionPolicy = EjbTransactionUtil.createTransactionPolicy(transactionType,
                    callContext);
            try {
                //Call the chain
            ...
            if (cdiEjbBean != null) {
                final Class<?> proxyClass = Class.class
                        .cast(Reflections.get(cdiEjbBean.getInjectionTarget(), "proxyClass"));
                if (proxyClass != null) { // means interception
                    final InterceptorResolutionService.BeanInterceptorInfo interceptorInfo = cdiEjbBean.getBeanContext()
                            .get(InterceptorResolutionService.BeanInterceptorInfo.class);
                    if (interceptorInfo.getDecorators() != null && !interceptorInfo.getDecorators().isEmpty()) {
                        final InterceptorDecoratorProxyFactory pf = webBeansContext
                        ...
                        for (int i = decorators.size(); i > 0; i--) {
                            ...
                            final Object decoratorInstance = decorator
                                    .create(CreationalContext.class.cast(creationalContext));
                            instances.put(decorator, decoratorInstance);
                            beanInstance = pf.createProxyInstance(proxyClass, instance, new DecoratorHandler(
                                    interceptorInfo, decorators, instances, i - 1, instance, cdiEjbBean.getId()));
                        }
                    }
                }
            }
    ...
    private ConstructorInjectionBean<Object> createConstructorInjectionBean(final WebBeansContext webBeansContext) {
        ...
        synchronized (this) { // concurrentmodificationexception because of annotatedtype internals otherwise
            if (constructorInjectionBean == null) {
                constructorInjectionBean = new ConstructorInjectionBean<Object>(webBeansContext, getManagedClass(),
    ...
    /**
     * This ManagedBean is added to all EjbModules to effectively create a sharable "comp" namespace which can be used
     * in for components with undefined namespace such as CDI
     */
    ...
    public static final class ProxyClass {
        ...
        public ProxyClass(final BeanContext beanContext, final Class<?>[] interfaces) {
            Class<?> clazz;
            try {
                clazz = LocalBeanProxyFactory.createProxy(beanContext.getBeanClass(), beanContext.getClassLoader(),
                        interfaces);
