...
public class MethodConcurrencyBuilder {
    ...
    public static void applyConcurrencyAttributes(final BeanContext beanContext,
            final List<MethodConcurrencyInfo> methodConcurrencyInfos) throws OpenEJBException {

        if (beanContext.isBeanManagedConcurrency()) {
            return;
        }

        final Logger log = Logger.getInstance(LogCategory.OPENEJB_STARTUP.createChild("attributes"),
                MethodConcurrencyBuilder.class);
        ...
        for (final Map.Entry<Method, MethodAttributeInfo> entry : attributes.entrySet()) {
            final MethodConcurrencyInfo value = (MethodConcurrencyInfo) entry.getValue();
            final MethodContext methodContext = beanContext.getMethodContext(entry.getKey());
            final Duration accessTimeout = new Duration(value.accessTimeout.time,
                    TimeUnit.valueOf(value.accessTimeout.unit));
            methodContext.setAccessTimeout(accessTimeout);
        }
    }

    /**
     * This method splits the MethodConcurrencyInfo objects so that there is exactly one MethodInfo per
     * MethodConcurrencyInfo. A single MethodConcurrencyInfo with three MethodInfos would be expanded into three
     * MethodConcurrencyInfo with one MethodInfo each.
     * <p/>
     ...
     */
    public static void normalize(final List<MethodConcurrencyInfo> infos, final List<MethodConcurrencyInfo> lockInfos,
    ...
    public void build(final HashMap<String, BeanContext> deployments,
            final List<MethodConcurrencyInfo> methodConcurrencys) throws OpenEJBException {
