...
public class IntervalTimerData extends TimerData {
    ...
    public IntervalTimerData(final long id, final EjbTimerServiceImpl timerService, final String deploymentId,
            final Object primaryKey, final Method timeoutMethod, final TimerConfig timerConfig,
            final Date initialExpiration, final long intervalDuration) {
    ...
    public String toString() {
        return TimerType.Interval.name() + " initialExpiration = ["
                + DateFormat.getDateTimeInstance().format(initialExpiration) + "] intervalDuration = ["
                + intervalDuration + "]";
    }
}
