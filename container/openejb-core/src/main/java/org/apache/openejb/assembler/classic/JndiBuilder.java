...
public class JndiBuilder {
    ...
    private static final boolean USE_OLD_JNDI_NAMES = SystemInstance.get().getOptions()
            .get("openejb.use-old-jndi-names", false);
    ...
    public static String format(final String deploymentId, final String interfaceClassName,
            final InterfaceType interfaceType) {
        return deploymentId + "/" + interfaceClassName
                + (interfaceType == null ? "" : "!" + interfaceType.getSpecName());
    }
    ...
    public void bind(final EjbJarInfo ejbJarInfo, final BeanContext bean, final EnterpriseBeanInfo beanInfo,
            final JndiNameStrategy strategy) {
        ...
        try {
            if (bean.isLocalbean()) {
                ...
                optionalBind(bindings, ref,
                        "openejb/Deployment/" + format(id, beanClass.getName(), InterfaceType.LOCALBEAN));

                // if the user inject the EJB using a parent class
                if (!bean.getBeanClass().isInterface()) {
                    for (Class<?> clazz = bean.getBeanClass().getSuperclass(); !clazz
                            .equals(Object.class); clazz = clazz.getSuperclass()) {
                        optionalBind(bindings, ref,
                                "openejb/Deployment/" + format(id, clazz.getName(), InterfaceType.LOCALBEAN));
                    }
                }

                final String internalName = "openejb/Deployment/"
                        + format(id, beanClass.getName(), InterfaceType.BUSINESS_LOCALBEAN_HOME);
        ...
        try {
            ...
            for (final Class interfce : bean.getBusinessLocalInterfaces()) {
                ...
                final String internalName = "openejb/Deployment/"
                        + format(id, interfce.getName(), InterfaceType.BUSINESS_LOCAL);
                bind(internalName, ref, bindings, beanInfo, interfce);

                final String name = strategy.getName(interfce, DEFAULT_NAME_KEY,
        ...
        try {
            ...
            for (final Class interfce : bean.getBusinessRemoteInterfaces()) {
                ...
                final String internalName = "openejb/Deployment/"
                        + format(id, interfce.getName(), InterfaceType.BUSINESS_REMOTE);
                bind(internalName, ref, bindings, beanInfo, interfce);
        ...
        try {
            ...
            if (localHomeInterface != null) {
                ...
                optionalBind(bindings, ref,
                        "openejb/Deployment/" + format(id, localHomeInterface.getName(), InterfaceType.EJB_LOCAL_HOME));
        ...
        try {
            ...
            if (homeInterface != null) {
                ...
                optionalBind(bindings, ref,
                        "openejb/Deployment/" + format(id, homeInterface.getName(), InterfaceType.EJB_HOME));
                ...
                name = "openejb/Deployment/"
                        + format(id, bean.getRemoteInterface().getName(), InterfaceType.EJB_OBJECT);
                bind(name, ref, bindings, beanInfo, homeInterface);
    ...
    private void bind(final String name, final Reference ref, final Bindings bindings,
            final EnterpriseBeanInfo beanInfo, final Class intrface) throws NamingException {

        if (name.startsWith("openejb/local/") || name.startsWith("openejb/remote/")
                || name.startsWith("openejb/localbean/") || name.startsWith("openejb/global/")) {
            ...
            try {
                ...
                if (deployment != null) {
                    logger.error("Jndi(name=" + externalName + ") cannot be bound to Ejb(deployment-id="
                            + beanInfo.ejbDeploymentId + ").  Name already taken by Ejb(deployment-id="
                            + deployment.getDeploymentID() + ")");
                } else {
                    logger.error("Jndi(name=" + externalName + ") cannot be bound to Ejb(deployment-id="
                            + beanInfo.ejbDeploymentId + ").  Name already taken by another object in the system.");
                }
    ...
    private void bindJava(final BeanContext cdi, final Class intrface, final Reference ref, final Bindings bindings,
            final EnterpriseBeanInfo beanInfo) throws NamingException {
        final ModuleContext module = cdi.getModuleContext();
        final AppContext application = module.getAppContext();
    ...
    /**
     * This may not be that performant, but it's certain to be faster than the user having to track down which
     * deployment is using a particular jndi name
     *
     * @param name
     * @return .
     */
    ...
    public interface JndiNameStrategy {
        ...
        enum Interface {
            ...
            REMOTE_HOME(InterfaceType.EJB_HOME, "RemoteHome", "home", ""), LOCAL_HOME(InterfaceType.EJB_LOCAL_HOME,
                    "LocalHome", "local-home", "Local"), BUSINESS_LOCAL(InterfaceType.BUSINESS_LOCAL, "Local",
                            "business-local", "BusinessLocal"), LOCALBEAN(InterfaceType.LOCALBEAN, "LocalBean",
                                    "localbean", "LocalBean"), BUSINESS_REMOTE(InterfaceType.BUSINESS_REMOTE, "Remote",
                                            "business-remote",
                                            "BusinessRemote"), SERVICE_ENDPOINT(InterfaceType.SERVICE_ENDPOINT,
            ...
            Interface(final InterfaceType type, final String annotatedName, final String xmlName,
                    final String openejbLegacy) {
                this.type = type;
    ...
    public static class TemplatedStrategy implements JndiNameStrategy {
        ...
        public TemplatedStrategy(final EjbJarInfo ejbJarInfo, final Map<String, BeanContext> deployments) {
            ...
            { // illegal format check
                ...
                if (index > -1) {
                    logger.error("Illegal " + JNDINAME_FORMAT
                            + " contains a colon ':'.  Everything before the colon will be removed, '" + format + "' ");
        ...
        private Map<String, StringTemplate> addTemplate(final Map<String, StringTemplate> map, final String key,
        ...
        public void begin(final BeanContext bean) {
            ...
            for (final JndiNameInfo nameInfo : beanInfo.jndiNamess) {
                ...
                templates.put(intrface, addTemplate(templates.get(intrface), getType(nameInfo.name),
                        new StringTemplate(nameInfo.name)));
            }
