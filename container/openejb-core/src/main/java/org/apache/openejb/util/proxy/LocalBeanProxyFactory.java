...
public class LocalBeanProxyFactory implements Opcodes {
    ...
    public static Object newProxyInstance(final ClassLoader classLoader, final InvocationHandler handler,
            final Class classToSubclass, final Class... interfaces) throws IllegalArgumentException {
        try {
    ...
    public static Object constructProxy(final Class clazz, final InvocationHandler handler)
    ...
    private static Field getDeclaredField(final Class clazz, final String fieldName) {
        try {
            ...
            final String message = String.format("Proxy class does not contain expected field \"%s\": %s", fieldName,
                    clazz.getName());
    ...
    public static Class createProxy(final Class<?> classToProxy, final ClassLoader cl, final String proxyName,
    ...
    public static byte[] generateProxy(final Class<?> classToProxy, final String proxyName,
            final Class<?>... interfaces) throws ProxyGenerationException {
    ...
    public static void processMethod(final ClassWriter cw, final Method method, final String proxyName,
            final String handlerName) throws ProxyGenerationException {
    ...
    public static MethodVisitor visit(final ClassWriter cw, final Method method, final String proxyName,
            final String handlerName) throws ProxyGenerationException {
        ...
        for (int i = 0; i < parameterTypes.length; i++) {
            ...
            if (parameterType.isPrimitive()) {
                ...
                mv.visitMethodInsn(INVOKESTATIC, wrapperType, "valueOf",
                        "(" + getPrimitiveLetter(parameterType) + ")L" + wrapperType + ";", false);
                mv.visitInsn(AASTORE);
        ...
        if (returnType.isPrimitive() && !Void.TYPE.equals(returnType)) {
            ...
            mv.visitMethodInsn(INVOKEVIRTUAL, getWrapperType(returnType), getPrimitiveMethod(returnType),
                    "()" + getPrimitiveLetter(returnType), false);
        }
    ...
    /**
     * Returns the name of the Java method to call to get the primitive value from an Object - e.g. intValue for
     * java.lang.Integer
     *
     * @param type
     *            Type whose primitive method we want to lookup
    ...
    /**
     * Gets the string to use for CHECKCAST instruction, returning the correct value for any type, including primitives
     * and arrays
     *
     * @param returnType
     *            The type to cast to with CHECKCAST
    ...
    /**
     * pushes an array of the specified size to the method visitor. The generated bytecode will leave the new array at
    ...
    public static class Unsafe {
        ...
        static {
            ...
            putObject = AccessController.doPrivileged(new PrivilegedAction<Method>() {
                ...
                public Method run() {
                    try {
                        final Method mtd = unsafeClass.getDeclaredMethod("putObject", Object.class, long.class,
                                Object.class);
                        mtd.setAccessible(true);
                        return mtd;
                    } catch (final Exception e) {
            ...
            defineClass = AccessController.doPrivileged(new PrivilegedAction<Method>() {
                ...
                public Method run() {
                    try {
                        final Method mtd = unsafeClass.getDeclaredMethod("defineClass", String.class, byte[].class,
        ...
        private static void setValue(final Field field, final Object object, final Object value) {
            ...
            try {
                ...
                throw new IllegalStateException(
                        "Failed putting field=" + field.getName() + "  class=" + field.getDeclaringClass().getName(),
                        e);
            }
        }
        ...
        public static Class defineClass(final ClassLoader loader, final Class<?> clsToProxy, final String proxyName,
                final byte[] proxyBytes) throws IllegalAccessException, InvocationTargetException {
            return (Class<?>) defineClass.invoke(unsafe, proxyName, proxyBytes, 0, proxyBytes.length, loader,
