...
import org.apache.openejb.core.timer.*;
...
public class Assembler extends AssemblerTool implements org.apache.openejb.spi.Assembler, JndiConstants {
    ...
    public Assembler(final JndiFactory jndiFactory) {
        ...
        skipLoaderIfPossible = "true".equalsIgnoreCase(
                SystemInstance.get().getProperty("openejb.classloader.skip-app-loader-if-possible", "true"));
        resourceDestroyTimeout = SystemInstance.get().getProperty("openejb.resources.destroy.timeout");
        threadStackOnTimeout = "true"
                .equals(SystemInstance.get().getProperty("openejb.resources.destroy.stack-on-timeout", "false"));
    ...
    private static boolean hasBatchEE() {
        try {
            Class.forName("org.apache.batchee.container.services.ServicesManager", true,
    ...
    private static List<CommonInfoObject> listCommonInfoObjectsForAppInfo(final AppInfo appInfo) {
        final List<CommonInfoObject> vfs = new ArrayList<CommonInfoObject>(
                appInfo.clients.size() + appInfo.connectors.size() + appInfo.ejbJars.size() + appInfo.webApps.size());
    ...
    private static void logUnusedProperties(final Map<String, ?> unsetProperties, final ServiceInfo info) {
        ...
        for (final String property : unsetProperties.keySet()) {
            ...
            if (ignoreJdbcDefault && ("JdbcUrl".equals(property) || "UserName".equals(property)
                    || "Password".equals(property) || "PasswordCipher".equals(property))) {
                continue;
            }
        ...
        if ("true".equals(SystemInstance.get().getProperty("openejb.cdi.bval.filter", "true"))) {
            try { // bval jars are optional so do it by reflection
                ...
                loader.loadClass("org.apache.bval.cdi.BValExtension")
                        .getMethod("setAnnotatedTypeFilter",
    ...
    private void installExtensions() {
        try {
            final Collection<URL> urls = NewLoaderLogic
                    .applyBuiltinExcludes(new UrlSet(Assembler.class.getClassLoader()).excludeJvm()).getUrls();
    ...
    public void build() throws OpenEJBException {
        ...
        try {
            ...
            /*
             * OpenEJBExceptions contain useful information and are debbugable. Let the exception pass through to the
             * top and be logged.
             */
            throw ae;
        } catch (final Exception e) {
            /*
             * General Exceptions at this level are too generic and difficult to debug. These exceptions are considered
             * unknown bugs and are fatal. If you get an error at this level, please trap and handle the error where it
             * is most relevant.
             */
    ...
    /**
     * When given a complete OpenEjbConfiguration graph this method will construct an entire container system and return
     * a reference to that container system, as ContainerSystem instance.
     * <p/>
     * This method leverage the other assemble and apply methods which can be used independently.
     * <p/>
     * Assembles and returns the {@link CoreContainerSystem} using the information from the {@link OpenEjbConfiguration}
     * object passed in.
     *
     * <pre>
    ...
    public void buildContainerSystem(final OpenEjbConfiguration configInfo) throws Exception {
        ...
        for (final AppInfo appInfo : containerSystemInfo.applications) {
            ...
            try {
                ...
                final DeploymentExceptionManager exceptionManager = systemInstance
                        .getComponent(DeploymentExceptionManager.class);
    ...
    public AppContext createApplication(final EjbJarInfo ejbJar, final ClassLoader classLoader)
            throws NamingException, IOException, OpenEJBException {
        return createEjbJar(ejbJar, classLoader);
    }

    public AppContext createEjbJar(final EjbJarInfo ejbJar, final ClassLoader classLoader)
            throws NamingException, IOException, OpenEJBException {
        final AppInfo appInfo = new AppInfo();
        appInfo.path = ejbJar.path;
        appInfo.appId = ejbJar.moduleName;
    ...
    public AppContext createClient(final ClientInfo clientInfo, final ClassLoader classLoader)
            throws NamingException, IOException, OpenEJBException {
        final AppInfo appInfo = new AppInfo();
        appInfo.path = clientInfo.path;
        appInfo.appId = clientInfo.moduleId;
    ...
    public AppContext createConnector(final ConnectorInfo connectorInfo)
            throws NamingException, IOException, OpenEJBException {
        final AppInfo appInfo = new AppInfo();
    ...
    public AppContext createConnector(final ConnectorInfo connectorInfo, final ClassLoader classLoader)
            throws NamingException, IOException, OpenEJBException {
        final AppInfo appInfo = new AppInfo();
        appInfo.path = connectorInfo.path;
        appInfo.appId = connectorInfo.moduleId;
    ...
    public AppContext createWebApp(final WebAppInfo webAppInfo, final ClassLoader classLoader)
            throws NamingException, IOException, OpenEJBException {
        final AppInfo appInfo = new AppInfo();
        appInfo.path = webAppInfo.path;
        appInfo.appId = webAppInfo.moduleId;
    ...
    public AppContext createApplication(final AppInfo appInfo, final ClassLoader classLoader)
            throws OpenEJBException, IOException, NamingException {
        return createApplication(appInfo, classLoader, true);
    }

    private AppContext createApplication(final AppInfo appInfo, ClassLoader classLoader, final boolean start)
            throws OpenEJBException, IOException, NamingException {
        try {
            try {
            ...
            final JndiEncBuilder globalBuilder = new JndiEncBuilder(appInfo.globalJndiEnc, injections, appInfo.appId,
                    null, GLOBAL_UNIQUE_ID, classLoader, appInfo.properties);
            ...
            try {
                    ...
                    for (final CommonInfoObject info : vfs) {
                        ...
                        if (factory == null) {
                            try { // lazy cause of CDI :(
                                final LazyValidatorFactory handler = new LazyValidatorFactory(classLoader,
                                        info.validationInfo);
                                factory = (ValidatorFactory) Proxy.newProxyInstance(appContext.getClassLoader(),
                    ...
                    for (final Entry<String, ValidatorFactory> validatorFactory : validatorFactories.entrySet()) {
                        ...
                        try {
                            ...
                            try {
                                ...
                                validator = (Validator) Proxy.newProxyInstance(appContext.getClassLoader(),
                                        VALIDATOR_INTERFACES, lazyValidator);
                ...
                final List<BeanContext> allDeployments = initEjbs(classLoader, appInfo, appContext, injections,
                        new ArrayList<BeanContext>(), null);

                if ("true".equalsIgnoreCase(SystemInstance.get().getProperty(PROPAGATE_APPLICATION_EXCEPTIONS,
                ...
                if (!isGeronimo) {
                    for (final Entry<String, LazyValidator> lazyValidator : lazyValidators.entrySet()) {
                        ...
                        final ValidatorFactory factory = lazyValidatorFactories.get(lazyValidator.getKey())
                                .getFactory();
                        try {
                ...
                for (final ClientInfo clientInfo : appInfo.clients) {
                    ...
                    // build the enc
                    final JndiEncBuilder jndiEncBuilder = new JndiEncBuilder(clientInfo.jndiEnc, clientInjections,
                            "Bean", clientInfo.moduleId, null, clientInfo.uniqueId, classLoader, new Properties());
                ...
                for (final EjbJarInfo ejbJarInfo : appInfo.ejbJars) {
                    for (final String mbean : ejbJarInfo.mbeans) {
                        deployMBean(appContext.getWebBeansContext(), classLoader, mbean, appInfo.jmx,
    ...
    private void validateCdiResourceProducers(final AppContext appContext, final AppInfo info) {
        ...
        if (appContext.isStandaloneModule()
            ...
            final Map<String, Object> bindings = appContext.getWebContexts().isEmpty() ? appContext.getBindings()
                    : appContext.getWebContexts().iterator().next().getBindings();
            if (bindings != null && appContext.getWebBeansContext() != null
                    && appContext.getWebBeansContext().getBeanManagerImpl().isInUse()) {
                for (final Bean<?> bean : appContext.getWebBeansContext().getBeanManagerImpl().getBeans()) {
                    if (ResourceBean.class.isInstance(bean)) {
                        ...
                        if (lookup == null && reference.getAnnotation(EJB.class) != null) {
                            final CdiPlugin plugin = CdiPlugin.class
                                    .cast(appContext.getWebBeansContext().getPluginLoader().getEjbPlugin());
                            if (!plugin.isSessionBean(reference.getResourceType())) { // local beans are here and access is O(1) instead of O(n)
                                ...
                                if (!ok) {
                                    throw new DefinitionException(
                                            "EJB " + reference.getJndiName() + " in " + reference.getOwnerClass()
                                                    + " can't be cast to " + reference.getResourceType());
                                }
                            }
                        }
                        if (Reference.class.isInstance(lookup)) {
                            try {
                                ...
                                if (JndiUrlReference.class.isInstance(lookup)) {
                                    checkBuiltInResourceTypes(reference,
                                            JndiUrlReference.class.cast(lookup).getJndiName());
                                }
                                continue;
                            }
                                    ...
                                    for (final WebAppInfo w : info.webApps) {
                                        for (final EnvEntryInfo e : w.jndiEnc.envEntries) {
                                            if (name.equals(e.referenceName)) {
                                                if (e.type != null
                                                        && !reference.getResourceType().getName().equals(e.type)) {
                                                    throw new DefinitionException("Env Entry " + reference.getJndiName()
                                                            + " in " + reference.getOwnerClass() + " can't be cast to "
                        ...
                        if (lookup != null && !reference.getResourceType().isInstance(lookup)) {
                            throw new DefinitionException("Resource " + reference.getJndiName() + " in "
                                    + reference.getOwnerClass() + " can't be cast, instance is " + lookup);
                        }
                    }
                }
            }
    ...
    private void checkBuiltInResourceTypes(final ResourceReference reference, final String jndi) {
        ...
        if ("java:comp/BeanManager".equals(jndi) && resourceType != BeanManager.class) {
            throw new DefinitionException("Resource " + reference.getJndiName() + " in " + reference.getOwnerClass()
                    + " can't be cast to a BeanManager");
        } else if ("java:comp/TransactionSynchronizationRegistry".equals(jndi)
                && resourceType != TransactionSynchronizationRegistry.class) {
            throw new DefinitionException("Resource " + reference.getJndiName() + " in " + reference.getOwnerClass()
                    + " can't be cast to a TransactionSynchronizationRegistry");
        } else if ("java:comp/TransactionManager".equals(jndi) && resourceType != TransactionManager.class) {
            throw new DefinitionException("Resource " + reference.getJndiName() + " in " + reference.getOwnerClass()
                    + " can't be cast to a TransactionManager");
        } else if ("java:comp/ValidatorFactory".equals(jndi) && resourceType != ValidatorFactory.class) {
            throw new DefinitionException("Resource " + reference.getJndiName() + " in " + reference.getOwnerClass()
                    + " can't be cast to a ValidatorFactory");
        } else if ("java:comp/Validator".equals(jndi) && resourceType != Validator.class) {
            throw new DefinitionException("Resource " + reference.getJndiName() + " in " + reference.getOwnerClass()
                    + " can't be cast to a Validator");
        }
    }

    private void postConstructResources(final Set<String> resourceIds, final ClassLoader classLoader,
            final Context containerSystemContext, final AppContext appContext)
                    throws NamingException, OpenEJBException {
            ...
            for (final ResourceInfo resourceInfo : resourceList) {
                ...
                try {
                    ...
                    try {
                        ...
                        clazz = containerSystemContext.lookup(OPENEJB_RESOURCE_JNDI_PREFIX + resourceInfo.id)
                                .getClass();
                    }

                    final boolean initialize = "true".equalsIgnoreCase(
                            String.valueOf(resourceInfo.properties.remove("InitializeAfterDeployment")));
                    final AnnotationFinder finder = Proxy.isProxyClass(clazz) ? null
                            : new AnnotationFinder(new ClassesArchive(ancestors(clazz)));
                    final List<Method> postConstructs = finder == null ? Collections.<Method> emptyList()
                            : finder.findAnnotatedMethods(PostConstruct.class);
                    final List<Method> preDestroys = finder == null ? Collections.<Method> emptyList()
                            : finder.findAnnotatedMethods(PreDestroy.class);

                    CreationalContext<?> creationalContext = null;
                    Object originalResource = null;
                    if (!postConstructs.isEmpty() || initialize) {
                        originalResource = containerSystemContext
                                .lookup(OPENEJB_RESOURCE_JNDI_PREFIX + resourceInfo.id);
                        ...
                        try {
                            ...
                            if (appContext != null && appContext.getWebBeansContext() != null) {
                                final BeanManagerImpl beanManager = appContext.getWebBeansContext()
                                        .getBeanManagerImpl();
                    ...
                    if (!"none".equals(resourceInfo.preDestroy)) {
                        ...
                        if (!preDestroys.isEmpty() || creationalContext != null) {
                            ...
                            this.bindResource(resourceInfo.id,
                                    new ResourceInstance(name, originalResource, preDestroys, creationalContext), true);
                        }
                    }
                    ...
                    logger.fatal("Resource " + resourceInfo.id
                            + " could not be initialized. Application will be undeployed.");
                    throw new OpenEJBException(e);
                }
            }
        } finally {
    ...
    private void propagateApplicationExceptions(final AppInfo appInfo, final ClassLoader classLoader,
    ...
    public List<BeanContext> initEjbs(final ClassLoader classLoader, final AppInfo appInfo, final AppContext appContext,
            final Set<Injection> injections, final List<BeanContext> allDeployments, final String webappId)
        ...
        for (final EjbJarInfo ejbJar : appInfo.ejbJars) {
            ...
            final TransactionPolicyFactory transactionPolicyFactory = createTransactionPolicyFactory(ejbJar,
                    classLoader);
            ...
            for (final BeanContext beanContext : deployments.values()) {
                if (beanContext.getComponentType() != BeanType.STATEFUL) {
                    ...
                    for (final Iterator<Map.Entry<Method, MethodContext>> it = beanContext.iteratorMethodContext(); it
                            .hasNext();) {
                    ...
                    if (timerServiceRequired && "true"
                            .equalsIgnoreCase(appInfo.properties.getProperty(OPENEJB_TIMERS_ON, globalTimersOn))) {
                        // Create the timer
                        final EjbTimerServiceImpl timerService = new EjbTimerServiceImpl(beanContext,
                                newTimerStore(beanContext));
                        //Load auto-start timers
                        final TimerStore timerStore = timerService.getTimerStore();
                        for (final Iterator<Map.Entry<Method, MethodContext>> it = beanContext
                                .iteratorMethodContext(); it.hasNext();) {
                            final Map.Entry<Method, MethodContext> entry = it.next();
                            final MethodContext methodContext = entry.getValue();
                            for (final ScheduleData scheduleData : methodContext.getSchedules()) {
                                timerStore.createCalendarTimer(timerService, (String) beanContext.getDeploymentID(),
                                        null, entry.getKey(), scheduleData.getExpression(), scheduleData.getConfig(),
                                        true);
                            }
                        }
                ...
                //TODO ???
                for (final Iterator<Entry<Method, MethodContext>> it = beanContext.iteratorMethodContext(); it
                        .hasNext();) {
                ...
                if (beanContext.isLocalbean() && !beanContext.getComponentType().isMessageDriven()
                    ...
                    beanContext.set(BeanContext.ProxyClass.class, new BeanContext.ProxyClass(beanContext,
        ...
        if (!appInfo.webAppAlone) {
            if (webappId == null) {
                skip = ejbJar.webapp; // we look for the lib part of the ear so deploy only if not a webapp
                    ...
                    && !ejbJar.properties.getProperty("openejb.ejbmodule.webappId", "-").equals(webappId))) {
    ...
    private TimerStore newTimerStore(final BeanContext beanContext) {
        for (final DeploymentContext context : Arrays.asList(beanContext, beanContext.getModuleContext(),
            ...
            if (timerStoreClass != null) {
                ...
                try {
                    ...
                    try {
                        ...
                        return TimerStore.class
                                .cast(constructor.newInstance(EjbTimerServiceImpl.getDefaultTransactionManager()));
    ...
    private void deployMBean(final WebBeansContext wc, final ClassLoader cl, final String mbeanClass,
            final Properties appMbeans, final String id) {
        if (LocalMBeanServer.isJMXActive()) {
            ...
            if (bean == null) {
                try {
                    ...
                    logger.error("the mbean " + mbeanClass + " can't be registered because it can't be instantiated",
                            e);
                    return;
            ...
            try {
                ...
                final ObjectName leaf = annotation == null || annotation.objectName().isEmpty()
                        ? new ObjectNameBuilder("openejb.user.mbeans").set("application", id)
                                .set("group", clazz.getPackage().getName()).set("name", clazz.getSimpleName()).build()
                        : new ObjectName(annotation.objectName());
    ...
    private TransactionPolicyFactory createTransactionPolicyFactory(final EjbJarInfo ejbJar,
        ...
        if (value instanceof TransactionPolicyFactory) {
            ...
            try {
                ...
                final Map<String, Class<? extends TransactionPolicyFactory>> plugins = finder
                        .mapAvailableImplementations(TransactionPolicyFactory.class);
    ...
    private Collection<DestroyingResource> destroyResourceTree(final String base,
            final NamingEnumeration<Binding> namingEnumeration) {
        ...
        while (namingEnumeration != null && namingEnumeration.hasMoreElements()) {
            ...
            if (Context.class.isInstance(object)) {
                try {
                    resources
                            .addAll(destroyResourceTree(
                                    IvmContext.class.isInstance(object)
                                            ? IvmContext.class.cast(object).mynode.getAtomicName() : "",
                ...
                resources.add(
                        new DestroyingResource((base == null || base.isEmpty() ? "" : (base + '/')) + binding.getName(),
        ...
        Collections.sort(resources, new Comparator<DestroyingResource>() { // end by destroying RA after having closed CF pool (for jms for instance)
            ...
            private boolean isRa(final Object instance) {
                return ResourceAdapter.class.isInstance(instance)
                        || ResourceAdapterReference.class.isInstance(instance);
            }
        });
    ...
    private void destroyResource(final String name, final String className, final Object inObject) {
        ...
        if (resourceDestroyTimeout != null) {
            ...
            final ExecutorService es = Executors
                    .newSingleThreadExecutor(new DaemonThreadFactory("openejb-resource-destruction-" + name));
            ...
            try {
                ...
                if (threadStackOnTimeout) {
                    ...
                    for (final ThreadInfo info : dump) {
                        stream.println('"' + info.getThreadName() + "\" suspended=" + info.isSuspended() + " state="
                                + info.getThreadState());
    ...
    public void destroyApplication(final AppInfo appInfo) throws UndeployException {
        ...
        try {
            ...
            if (null == appContext) {
                logger.warning("Application id '" + appInfo.appId + "' not found in: "
                        + Arrays.toString(containerSystem.getAppContextKeys()));
                return;
            } else {
                ...
                if (webBeansContext != null) {
                    ...
                    Thread.currentThread().setContextClassLoader(classLoader);
                    try {
                        final ServletContext context = appContext.isStandaloneModule()
            ...
            final UndeployException undeployException = new UndeployException(
            ...
            for (final CommonInfoObject jar : listCommonInfoObjectsForAppInfo(appInfo)) {
                try {
                    ...
                    if (EjbJarInfo.class.isInstance(jar)) {
                        undeployException.getCauses()
                                .add(new Exception("validator: " + jar.uniqueId + ": " + e.getMessage(), e));
            ...
            for (final String id : appInfo.resourceAliases) {
                ...
                try {
                    ...
                    if (object instanceof ContextualJndiReference) {
                        final ContextualJndiReference contextualJndiReference = ContextualJndiReference.class
                                .cast(object);
            ...
            for (final ConnectorInfo connector : appInfo.connectors) {
                ...
                for (final MdbContainerInfo container : connector.inbound) {
                    try {
                        ...
                        this.containerSystem.getJNDIContext()
                                .unbind(JAVA_OPENEJB_NAMING_CONTEXT + container.service + "/" + container.id);
                    } catch (final Exception e) {
                        // no-op
                    }
                }
    ...
    private void destroyLookedUpResource(final Context globalContext, final String id, final String name)
    ...
    public ClassLoader createAppClassLoader(final AppInfo appInfo) throws OpenEJBException, IOException {
        ...
        final ClassLoaderConfigurer configurer1 = QuickJarsTxtParser
                .parse(new File(appInfo.path, prefix + QuickJarsTxtParser.FILE_NAME));
        ...
        if (skipLoaderIfPossible) { // TODO: maybe use a boolean to know if all urls comes from the classpath to avoid this validation
            ...
            for (final URL url : filtered) {
                try {
                    ...
                    allIsIntheClasspath = false;
                    if (logger.isDebugEnabled()) {
                        logger.debug(url.toExternalForm() + " (" + URLs.toFile(url) + ") is not in the classloader",
                                ignored);
                    }
                    break;
                }
    ...
    public void createExternalContext(final JndiContextInfo contextInfo) throws OpenEJBException {
        logger.getChildLogger("service").info("createService", contextInfo.service, contextInfo.id,
        ...
        try {
            ...
            throw new OpenEJBException(String.format(
                    "JndiProvider(id=\"%s\") could not be created.  Failed to create the InitialContext using the supplied properties",
                    contextInfo.id), ne);
        }
        ...
        logger.getChildLogger("service").debug("createService.success", contextInfo.service, contextInfo.id,
                contextInfo.className);
    }
    ...
    public void createContainer(final ContainerInfo serviceInfo) throws OpenEJBException {
        ...
        logger.getChildLogger("service").debug("createService.success", serviceInfo.service, serviceInfo.id,
        ...
        if (Container.class.isInstance(service) && LocalMBeanServer.isJMXActive()) {
            ...
            try {
                LocalMBeanServer.get().registerMBean(
    ...
    private void bindService(final ServiceInfo serviceInfo, final Object service) throws OpenEJBException {
        try {
            this.containerSystem.getJNDIContext()
                    .bind(JAVA_OPENEJB_NAMING_CONTEXT + serviceInfo.service + "/" + serviceInfo.id, service);
        } catch (final NamingException e) {
            throw new OpenEJBException(
                    messages.format("assembler.cannotBindServiceWithId", serviceInfo.service, serviceInfo.id), e);
        }
    }
    ...
    public void removeContainer(final String containerId) {
        ...
        // Update the config tree
        for (final Iterator<ContainerInfo> iterator = config.containerSystem.containers.iterator(); iterator
                .hasNext();) {
            ...
            if (containerInfo.id.equals(containerId)) {
                ...
                try {
                    this.containerSystem.getJNDIContext()
                            .unbind(JAVA_OPENEJB_NAMING_CONTEXT + containerInfo.service + "/" + containerInfo.id);
                } catch (final Exception e) {
    ...
    private void replaceResourceAdapterProperty(final ObjectRecipe serviceRecipe) throws OpenEJBException {
        ...
        if (resourceAdapterId instanceof String) {
            ...
            if (!(resourceAdapter instanceof ResourceAdapter)) {
                throw new OpenEJBException(
                        messages.format("assembler.resourceAdapterNotResourceAdapter", id, resourceAdapter.getClass()));
            }
    ...
    public void createResource(final Collection<ServiceInfo> infos, final ResourceInfo serviceInfo)
            throws OpenEJBException {
        ...
        final Object service = "true".equalsIgnoreCase(String.valueOf(serviceInfo.properties.remove("Lazy")))
                || usesCdiPwdCipher ? newLazyResource(infos, serviceInfo) : doCreateResource(infos, serviceInfo);
        ...
        if (serviceInfo.originAppName != null && !serviceInfo.originAppName.isEmpty()
                && !"/".equals(serviceInfo.originAppName) && !serviceInfo.id.startsWith("global")) {
    ...
    private Object doCreateResource(final Collection<ServiceInfo> infos, final ResourceInfo serviceInfo)
            throws OpenEJBException {
        ...
        if (!"true".equalsIgnoreCase(skipPropertiesFallback)) {
            serviceRecipe.setProperty("properties", new UnsetPropertiesRecipe() {
                @Override
                protected Object internalCreate(final Type expectedType, final boolean lazyRefAllowed)
        ...
        if (customLoader) {
            ...
            if (apis.size() - (apis.contains(Serializable.class) ? 1 : 0)
                ...
                service = Proxy.newProxyInstance(loader, apis.toArray(new Class<?>[apis.size()]),
                        new ClassLoaderAwareHandler(null, service, loader));
        ...
        if (service instanceof ResourceAdapter) {
            ...
            if (threadPoolSize <= 0) {
                logger.warning("Thread pool for '" + serviceInfo.id
                        + "' is (unbounded), consider setting a size using: " + serviceInfo.id + ".QueueSize=[size]");
                threadPool = Executors.newCachedThreadPool(new DaemonThreadFactory(serviceInfo.id + "-worker-"));
            } else {
                threadPool = new ExecutorBuilder().size(threadPoolSize).prefix(serviceInfo.id)
            ...
            if (GeronimoTransactionManager.class.isInstance(transactionManager)) {
                ...
                final TransactionContextHandler txWorkContextHandler = new TransactionContextHandler(
            ...
            service = new ResourceAdapterReference(resourceAdapter, threadPool,
                    OPENEJB_RESOURCE_JNDI_PREFIX + serviceInfo.id);
            ...
            final ObjectRecipe connectionManagerRecipe = new ObjectRecipe(GeronimoConnectionManagerFactory.class,
                    "create");
            ...
            logger.getChildLogger("service").info("createResource.createConnectionManager", serviceInfo.id,
                    service.getClass().getName());

            // create the connection manager
            final ConnectionManager connectionManager = (ConnectionManager) connectionManagerRecipe.create();
            if (connectionManager == null) {
                throw new OpenEJBRuntimeException(
                        messages.format("assembler.invalidConnectionManager", serviceInfo.id));
            }
    ...
    private void bindResource(final String id, final Object service, final boolean canReplace) throws OpenEJBException {
        ...
        try {
            ...
            logger.warning("unbounding resource " + name
                    + " can happen because of a redeployment or because of a duplicated id");
            try {
                jndiContext.unbind(name);
                jndiContext.bind(name, service);
            } catch (final NamingException e) {
    ...
    private String getStringProperty(final Properties properties, final String propertyName,
    ...
    private static class PersistenceClassLoaderHandlerImpl implements PersistenceClassLoaderHandler {
        ...
        @Override
        public void addTransformer(final String unitId, final ClassLoader classLoader,
    ...
    public static class ResourceInstance extends Reference implements Serializable, DestroyableResource {
        ...
        public ResourceInstance(final String name, final Object delegate, final Collection<Method> preDestroys,
