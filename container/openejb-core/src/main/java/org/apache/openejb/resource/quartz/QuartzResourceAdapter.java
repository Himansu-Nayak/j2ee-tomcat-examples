...
public class QuartzResourceAdapter implements ResourceAdapter {
    ...
    public void start(final BootstrapContext bootstrapContext) throws ResourceAdapterInternalException {
        ...
        if (null != this.bootstrapContext.getAndSet(bootstrapContext)) {
            Logger.getInstance(LogCategory.OPENEJB, "org.apache.openejb.util.resources")
                    .warning("QuartzResourceAdapter is already starting");
            return;
        }
        ...
        startThread.set(new Thread("Quartz Scheduler Start") {
            ...
            public void run() {
                ...
                try {
                    QuartzResourceAdapter.this.scheduler.get().getListenerManager()
                            .addSchedulerListener(new SchedulerListenerSupport() {
                        @Override
        ...
        if (started) {
            Logger.getInstance(LogCategory.OPENEJB, "org.apache.openejb.util.resources")
                    .info("Started Quartz Scheduler");
        } else {
            Logger.getInstance(LogCategory.OPENEJB, "org.apache.openejb.util.resources")
                    .warning("Failed to start Quartz Scheduler within defined timeout, status unknown");
        }
    }
    ...
    public void stop() {
                    ...
                    stopThread = new Thread("Quartz Scheduler Forced Stop") {
                        ...
                        public void run() {
                            try {
                                ...
                                s.shutdown(false);
                                Logger.getInstance(LogCategory.OPENEJB, "org.apache.openejb.util.resources")
                                        .warning("Forced Quartz stop - Jobs may be incomplete");
                            } catch (final Throwable e) {
                                QuartzResourceAdapter.this.ex.set(e);
                            }
                        }
        ...
        if (null != ex.get()) {
            Logger.getInstance(LogCategory.OPENEJB, "org.apache.openejb.util.resources")
                    .warning("Error stopping Quartz Scheduler", ex.get());
        } else {
            Logger.getInstance(LogCategory.OPENEJB, "org.apache.openejb.util.resources")
                    .info("Stopped Quartz Scheduler");
        }
    }
    ...
    /**
     * A private inner class is used so the key and value are not publicly visible. This is standard OpenEJB practice
     * for all "public storage" maps as it prevents outside code from becoming dependent on or tampering with the
     * private data.
     */
