...
public class ResettableDataSourceHandler implements DelegatableHandler {
    private static final Logger LOGGER = Logger.getInstance(LogCategory.OPENEJB,
            ResettableDataSourceHandler.class.getName());
    ...
    public ResettableDataSourceHandler(final CommonDataSource ds, final String value, final String methods) {
        ...
        if (!"*".equals(methods)) {
            this.retryMethods.addAll(asList(
                    methods == null ? new String[] { "getConnection", "getXAConnection" } : methods.split(" *, *")));
        }
        ...
        if (value.equals("true")) {
            ...
            tmp = new CountRetryStrategy(recreate,
                    Integer.parseInt(value.substring("retry(".length(), value.length() - 1)));
        } else {
            try {
