...
public class EJBCronTriggerPersistenceDelegate extends CronTriggerPersistenceDelegate {
    ...
    @Override
    public TriggerPropertyBundle loadExtendedTriggerProperties(final Connection conn, final TriggerKey triggerKey)
            throws SQLException {
        ...
        try {
            ...
            if (rs.next()) {
                ...
                try {
                    final EJBCronTrigger cb = new EJBCronTrigger(new ScheduleExpression().year(parts[0]).month(parts[1])
                            .dayOfMonth(parts[2]).dayOfWeek(parts[3]).hour(parts[4]).minute(parts[5]).second(parts[6])
                            .timezone(timeZoneId));
                    ...
                    throw new IllegalStateException("Can't build the Trigger with key: '" + triggerKey
                            + "' and statement: " + Util.rtp(SELECT_CRON_TRIGGER, tablePrefix, schedNameLiteral));
                }
            }
    ...
    @Override
    public int insertExtendedTriggerProperties(final Connection conn, final OperableTrigger trigger, final String state,
    ...
    @Override
    public int updateExtendedTriggerProperties(final Connection conn, final OperableTrigger trigger, final String state,
            final JobDetail jobDetail) throws SQLException, IOException {
        final EJBCronTrigger cronTrigger = (EJBCronTrigger) trigger;
        PreparedStatement ps = null;
        try {
