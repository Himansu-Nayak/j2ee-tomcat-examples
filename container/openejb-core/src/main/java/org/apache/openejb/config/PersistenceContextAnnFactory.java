...
public class PersistenceContextAnnFactory {
    ...
    public PersistenceContextAnn create(final PersistenceContext persistenceContext,
            final AnnotationDeployer.Member member) throws OpenEJBException {
        if (useAsm) {
            if (member != null) {
    ...
    private static class AsmPersistenceContext implements PersistenceContextAnn {
        ...
        public String toString() {
            return "@PersistenceContext(name = \"" + name + "\", unitName = \"" + unitName
                    + "\", type = PersistenceContextType." + type + ", " + properties + ")";
        }
    }
    ...
    private static class PersistenceContextVisitor extends AnnotationVisitor {
        ...
        public PersistenceContextVisitor(final String className, final String memberName,
                final Map<String, AsmPersistenceContext> contexts) {
            super(Opcodes.ASM5);
            this.contexts = contexts;
