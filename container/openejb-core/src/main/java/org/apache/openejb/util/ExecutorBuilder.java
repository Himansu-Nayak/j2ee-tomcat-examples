...
public class ExecutorBuilder {
    ...
    public ThreadPoolExecutor build(final Options options) {
        ...
        final BlockingQueue<Runnable> queue = options.get(prefix + ".QueueType", defaultQueueType).create(options,
                prefix, qsize);
        ...
        if (handler == null) {
            ...
            if (rejectedExecutionHandlerClass == null) {
                ...
                try {
                    handler = RejectedExecutionHandler.class.cast(Thread.currentThread().getContextClassLoader()
                            .loadClass(rejectedExecutionHandlerClass).newInstance());
        ...
        final ThreadPoolExecutor threadPoolExecutor = new ThreadPoolExecutor(corePoolSize, maximumPoolSize,
                keepAliveTime.getTime(), keepAliveTime.getUnit() != null ? keepAliveTime.getUnit() : TimeUnit.SECONDS,
    ...
    public static enum QueueType {
        ARRAY, LINKED, PRIORITY, SYNCHRONOUS;
