...
public class Logger {
    ...
    /**
     * Computes the parent of a resource name. E.g. if we pass in a key of a.b.c, it returns the value a.b
     */
    ...
    public Logger(final LogCategory category, final LogStream logStream, final String baseName) {
        ...
        ("true".equals(SystemInstance.get().getProperty("openejb.log.async", "true"))
                && System.getProperty("catalina.home") == null) ? new LogStreamAsync(logStream) : logStream;
    }
    ...
    public static synchronized void configure(final Properties config) {
        ...
        String factoryName = config.getProperty("openejb.log.factory", SystemInstance.isInitialized()
        ...
        if ("jul".equalsIgnoreCase(factoryName) || "juli".equalsIgnoreCase(factoryName)) {
            ...
            if (exists("org.apache.log4j.Logger")) {
                ...
                System.out.println(
                        "Cannot respect 'openejb.log.factory=log4j' setting as Log4j is not in the classpath.");
        ...
        for (int i = 0; i < specialKeys.length; i++) {
            ...
            final String property = config.getProperty(specialKeys[i], SystemInstance.isInitialized()
     ...
     * @param baseName
     *            - The baseName for the ResourceBundle
     * @return Logger
     */
     ...
     *
     * @param message
     *            - This could be a plain message or a key in Messages.properties
    ...
    /**
     * Given a key and a baseName, this method computes a message for a key. if the key is not found in this
     * ResourceBundle for this baseName, then it recursively looks up its parent to find the message for a key. If no
     * message is found for a key, the key is returned as is and is logged by the logger.
     *
     * @param key
     *            String
     * @param baseName
