...
public class CheckDescriptorLocation extends ValidationBase {
    ...
    private static Map<String, URL> retrieveDescriptors(final List<String> descriptorsToSearch,
            final URL... locationsToSearch) {
    ...
    private void validateWebModule(final DeploymentModule webModule) {
        ...
        if (file != null) {
            try {
                ...
                final Map<String, URL> incorrectlyLocatedDescriptors = retrieveDescriptors(descriptorsToSearch,
    ...
    public void validateEjbModule(final DeploymentModule deploymentModule) {
        ...
        final List<String> descriptorsToSearch = Arrays.asList("beans.xml", "ejb-jar.xml", "openejb-jar.xml",
    ...
    private void warnIncorrectLocationOfDescriptors(final Map<String, URL> descriptorsPlacedInWrongLocation,
            final String expectedLocation) {
        for (final Map.Entry<String, URL> map : descriptorsPlacedInWrongLocation.entrySet()) {

            warn(this.module.toString(), "descriptor.incorrectLocation", map.getValue().toExternalForm(),
                    expectedLocation);
        }
    }
}