...
public class IvmContext implements Context, Serializable {
    ...
    public static ObjectFactory[] getFederatedFactories() throws NamingException {
        if (federatedFactories == null) {
            ...
            for (final StringTokenizer tokenizer = new StringTokenizer(urlPackagePrefixes, ":"); tokenizer
                    .hasMoreTokens();) {
                ...
                try {
                    ...
                    final NamingException ne = new NamingException(
                            "Federation failed: Cannot instantiate " + className);
                    ne.setRootCause(e);
                    throw ne;
                }
            }
    ...
    public Object lookup(final String compositName) throws NamingException {
        ...
        if (index > -1) {
            ...
            /*
             * the resolve method always starts with the comparison assuming that the first component of the name is a
             * context of a peer node or the same node, so we have to prepend the current context name to the relative
             * lookup path.
             */
        ...
        /*
         * If the object has been resolved in the past from this context and the specified path (name) it will be in the
         * fastCache which is significantly faster then peruse the Node graph. 80 ms compared to 300 ms for a full node
         * path search.
         */
        ...
        if (obj.getClass() == IvmContext.class) {
             ...
             * org.apache.openejb.core.ivm.naming.Reference types that check to see if the current operation is allowed
             * access to the entry (See EJB 1.1/2.0 Allowed Operations) If the operation is not allowed, a
    ...
    public void bind(String name, final Object obj) throws NamingException {
        ...
        if (indx > -1) {
            /*
             * The ':' character will be in the path if its an absolute path name starting with the schema 'java:'. We
             * strip the schema off the path before passing it to the node.resolve method.
             */
    ...
    public String toString() {
        return "IvmContext{" + "mynode=" + mynode.getAtomicName() + '}';
    }
