...
public class CmpJpaConversion implements DynamicDeployer {
    ...
    private static final Set<String> ENHANCED_FIELDS = Collections
            .unmodifiableSet(new TreeSet<String>(Arrays.asList("pcInheritedFieldCount", "pcFieldNames", "pcFieldTypes",
                    "pcFieldFlags", "pcPCSuperclass", "pcStateManager", "class$Ljava$lang$String",
                    "class$Ljava$lang$Integer", "class$Lcom$sun$ts$tests$common$ejb$wrappers$CMP11Wrapper",
                    "pcDetachedState", "serialVersionUID")));

    /**
     * Tests if an EJB is an entity bean using container managed persistence.
     *
     * @param bean
     *            The source bean.
    ...
    public AppModule deploy(final AppModule appModule) throws OpenEJBException {
        ...
        for (final EjbModule ejbModule : appModule.getEjbModules()) {
            ...
            for (final Entity mapping : userMappings.getEntity()) {
                logger.warning("openejb-cmp-orm.xml mapping ignored: module=" + ejbModule.getModuleId()
                        + ":  <entity class=\"" + mapping.getClazz() + "\">");
            }

            for (final MappedSuperclass mapping : userMappings.getMappedSuperclass()) {
                logger.warning("openejb-cmp-orm.xml mapping ignored: module=" + ejbModule.getModuleId()
                        + ":  <mapped-superclass class=\"" + mapping.getClazz() + "\">");
            }
    ...
    private PersistenceUnit getCmpPersistenceUnit(final AppModule appModule) {
        ...
        if (persistenceUnit == null) {
            ...
            final String property = SystemInstance.get().getProperty("openejb.cmp.openjpa.jdbc.SynchronizeMappings",
            ...
            final PersistenceModule persistenceModule = new PersistenceModule(appModule,
                    getPersistenceModuleId(appModule), persistence);
    ...
    /**
     * Test if a module contains CMP entity beans that will need a JPA mapping generated.
     *
     * @param appModule
     *            The source application module.
     * @return true if the module contains any entity beans using container managed persistence.
     */
    ...
    private void processRelationship(final Map<String, Entity> entitiesByEjbName, final EjbRelation relation)
    ...
    private <R extends RelationField> R addRelationship(final R relationship, final Map<String, RelationField> existing,
            final List<R> relationships) {
        R r = null;
    ...
    /**
     * Generate the CMP mapping data for an individual EntityBean.
     *
     * @param ejbModule
     *            The module containing the bean.
     * @param entityMappings
     *            The accumulated set of entity mappings.
     * @param bean
     *            The been we're generating the mapping for.
     */
    private void processEntityBean(final EjbModule ejbModule, final EntityMappings entityMappings,
        ...
        if (ejbDeployment != null) {
            for (final org.apache.openejb.jee.oejb3.Query query : ejbDeployment.getQuery()) {
                ...
                if (queryMethod.getMethodParams() != null
                        && !queryMethod.getMethodParams().getMethodParam().isEmpty()) {
                    name.append('(');
                    boolean first = true;
                    for (final String methodParam : queryMethod.getMethodParams().getMethodParam()) {
                        if (!first) {
    ...
    /**
     * Check the user-defined entity mappings for a superclass mapping for a given named class. If the user mappings
     * exist, remove them from the user list and return them for inclusion in our generated mappings.
     *
     * @param userMappings
     *            The current user mapping set.
     * @param className
     *            The name of the class of interest.
     * @return Returns the superclass mapping for the named class, or null if the class is not in the mapping set.
     */
    ...
    /**
     * Generate the JPA mapping for a CMP 2.x bean. Since the field accessors are all defined as abstract methods and
     * the fields will not be defined in the implementation class, we don't need to deal with mapped superclasses. All
     * of the fields and concrete methods will be implemented by the generated subclass, so from a JPA standpoint, there
     * are no mapped superclasses required.
     *
     * @param mapping
     *            The mapping information we're updating.
     * @param bean
     *            The entity bean meta data
     * @param classLoader
     *            The classloader for resolving class references and primary key classes.
     */
    private Collection<MappedSuperclass> mapClass2x(final Mapping mapping, final EntityBean bean,
            ...
            try {
                ...
                for (final Field pkField : pkClass.getFields()) {
                    ...
                    if (Modifier.isPublic(modifiers) && !Modifier.isStatic(modifiers)
                            && allFields.contains(pkFieldName)) {
                ...
                throw (IllegalStateException) new IllegalStateException(
                        "Could not find entity primary key class " + bean.getPrimKeyClass()).initCause(e);
            }
        }
    ...
    /**
     * Create the class mapping for a CMP 1.x entity bean. Since the fields for 1.x persistence are defined in the
     * objects directly, we need to create superclass mappings for each of the defined fields to identify which classes
     * implement each of the managed fields.
     *
     * @param ejbClassName
     ...
     * @param classLoader
     *            The classloader used to load the bean class for inspection.
     * @return The set of mapped superclasses used in this bean mapping.
     */
    private Collection<MappedSuperclass> mapClass1x(final String ejbClassName, final Mapping mapping,
            final EntityBean bean, final ClassLoader classLoader) {
        ...
        if (bean.getPrimkeyField() != null) {
            ...
            final MappedSuperclass superclass = superclassByField.get(fieldName);
            if (superclass == null) {
                throw new IllegalStateException("Primary key field " + fieldName + " is not defined in class "
                        + ejbClassName + " or any super classes");
            }
            superclass.addField(new Id(fieldName));
            mapping.addField(new AttributeOverride(fieldName));
            primaryKeyFields.add(fieldName);
            ...
            try {
                ...
                for (final Field pkField : pkClass.getFields()) {
                    ...
                    if (Modifier.isPublic(modifiers) && !Modifier.isStatic(modifiers)
                        ...
                        if (superclass == null) {
                            throw new IllegalStateException("Primary key field " + fieldName
                                    + " is not defined in class " + ejbClassName + " or any super classes");
                        }
        ...
        for (final CmpField cmpField : bean.getCmpField()) {
            ...
            if (!primaryKeyFields.contains(fieldName)) {
                final MappedSuperclass superclass = superclassByField.get(fieldName);
                if (superclass == null) {
                    throw new IllegalStateException("CMP field " + fieldName + " is not defined in class "
                            + ejbClassName + " or any super classes");
                }
                superclass.addField(new Basic(fieldName));
                mapping.addField(new AttributeOverride(fieldName));
    ...
    /**
     * Handle the potential situation where the fields of a complex primary key are defined at different levels of the
     * class hierarchy. We want to define the idClass as the most derived class (i.e., the one that will contain ALL of
     * the defined fields).
     * <p/>
     * In practice, most ejbs will define all of the primary key fields at the same subclass level, so this should
     * return quickly.
     *
     * @param idclass
     *            The currently defined id class (will be null if this is the first call).
     * @param current
     *            The current superclass being processed.
     * @param ejbClass
     *            The ejbClass we're creating the mapping for.
     * @return Either idClass or current, depending on which is the most derived of the classes.
     */
    private MappedSuperclass resolveIdClass(final MappedSuperclass idclass, final MappedSuperclass current,
    ...
    /**
     * Build a mapping between a bean's CMP fields and the particular subclass in the inheritance hierarchy that defines
     * the field.
     *
     * @param clazz
    ...
    /**
     * Add a persistence context reference for the CMP persistence contexts to this EntityBean definition.
     *
     * @param bean
     *            The bean we're updating.
     * @return Returns true if the context was added. Returns false if the bean already is associated with the CMP
     *         persistence context.
     */
