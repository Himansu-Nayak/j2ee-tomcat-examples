...
public class ReportValidationResults implements DynamicDeployer {
    ...
    private static final Logger logger = Logger.getInstance(LogCategory.OPENEJB_STARTUP_VALIDATION,
    ...
    public AppModule deploy(final AppModule appModule) throws OpenEJBException {
        ...
        if (level != Level.VERBOSE) {
            ...
            logger.info("Set the '" + VALIDATION_LEVEL + "' system property to " + join(" or ", levels)
                    + " for increased validation details.");
        }

        throw new ValidationFailedException(
                "Module failed validation. " + uberContext.getModuleType() + "(name=" + uberContext.getName() + ")",
    ...
    private void logResults(final ValidationContext context, final Level level) {
        ...
        if (context.hasErrors() || context.hasFailures()) {
            ...
            logger.error(String.format("Invalid %s(name=%s, path=%s)", context.getModuleType(), module.getModuleId(),
                    module.getFile()));
            ...
            if (context.getWarnings().length == 1) {
                logger.warning(context.getWarnings().length + " warning for " + context.getModuleType() + "(path="
                        + context.getName() + ")");
            } else {
                logger.warning(context.getWarnings().length + " warnings for " + context.getModuleType() + "(path="
                        + context.getName() + ")");
            }
        }
    }
