...
public class TransactionContext extends AbstractContext implements Synchronization {
    ...
    protected void checkActive() {
        if (!isActive()) {
            throw new ContextNotActiveException(
                    "Context with scope annotation @" + getScope().getName() + " is not active");
        }
    }
    ...
    protected void setComponentInstanceMap() {
        ...
        componentInstanceMap = Map.class.cast(
                Proxy.newProxyInstance(Thread.currentThread().getContextClassLoader(), MAP, new TransactionalMapHandler(
    ...
    private static class TransactionalMapHandler implements InvocationHandler {
        ...
        public TransactionalMapHandler(final TransactionContext transactionContext,
