...
public class InitContextFactory implements InitialContextFactory {
    ...
    public Context getInitialContext(final Hashtable env) throws javax.naming.NamingException {
        ...
        if (user != null && pass != null) {
            try {
                ...
                throw (AuthenticationException) new AuthenticationException("User could not be authenticated: " + user)
                        .initCause(e);
            }
        }
    ...
    private void initializeOpenEJB(final Hashtable env) throws javax.naming.NamingException {
        try {
            ...
            /*
             * DMB: We should get the defaults from the functionality Alan is working on. This is temporary. When that
             * logic is finished, this block should probably just be deleted.
             */
