...
public final class TimeWatcherExecutor {
        ...
        for (final StackTraceElement elt : t.getStackTrace()) {
            ...
            for (final String p : acceptedPackages) {
                if (className.startsWith(p)) {
                    inlinedStack.append(" -> ").append(className).append('.').append(elt.getMethodName()).append(':')
    ...
    public static TimerWatcherResult execute(final Method mtd, final Object instance, final Object[] args,
            final boolean watch) {
