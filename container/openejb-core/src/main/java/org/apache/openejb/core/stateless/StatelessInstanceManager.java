...
public class StatelessInstanceManager {
    ...
    public StatelessInstanceManager(final SecurityService securityService, final Duration accessTimeout,
        ...
        this.executor = new ThreadPoolExecutor(callbackThreads, callbackThreads * 2, 1L, TimeUnit.MINUTES,
                new LinkedBlockingQueue<Runnable>(qsize), threadFactory);
     ...
     * <p/>
     * If the pool is at it's limit the StrictPooling flag will cause this thread to wait.
     * <p/>
     * If StrictPooling is not enabled this method will create a new stateless bean instance performing all required
     * injection and callbacks before returning it in a method ready state.
     *
     * @param callContext
     *            ThreadContext
     * @return Object
    ...
    public Instance getInstance(final ThreadContext callContext) throws OpenEJBException {
        ...
        try {
            ...
            final String msg = "No instances available in Stateless Session Bean pool.  Waited "
                    + data.accessTimeout.toString();
    ...
    private Instance createInstance(final ThreadContext callContext, final BeanContext beanContext)
        ...
        try {
            ...
            if (context.getBean() instanceof SessionBean) {
                ...
                try {
                    ...
                    final InterceptorStack ejbCreate = new InterceptorStack(context.getBean(),
                            beanContext.getCreateMethod(), Operation.CREATE, new ArrayList<InterceptorData>(),
    ...
    /**
     * All instances are removed from the pool in getInstance(...). They are only returned by the StatelessContainer via
     * this method under two circumstances.
     * <p/>
     * 1. The business method returns normally 2. The business method throws an application exception
     * <p/>
     * Instances are not returned to the pool if the business method threw a system exception.
     *
     * @param callContext
     *            ThreadContext
     * @param bean
    ...
    private void freeInstance(final ThreadContext callContext, final Instance instance) {
        try {
            ...
            final InterceptorStack interceptorStack = new InterceptorStack(instance.bean, remove, Operation.PRE_DESTROY,
    ...
    public void deploy(final BeanContext beanContext) throws OpenEJBException {
        ...
        final Duration accessTimeout = getDuration(options, "AccessTimeout",
    ...
    private Duration getDuration(final Options options, final String property, final Duration defaultValue,
    ...
    public void undeploy(final BeanContext beanContext) {
        ...
        try {
            if (!data.closePool()) {
                logger.error("Timed-out waiting for stateless pool to close: for deployment '"
    ...
    private final class InstanceCreatorRunnable implements Runnable {
        ...
        private InstanceCreatorRunnable(final long maxAge, final long iteration, final long min,
                final double maxAgeOffset, final Data data, final StatelessSupplier supplier) {
            this.maxAge = maxAge;
