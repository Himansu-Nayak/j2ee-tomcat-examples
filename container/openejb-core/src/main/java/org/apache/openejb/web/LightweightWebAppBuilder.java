...
public class LightweightWebAppBuilder implements WebAppBuilder {
    ...
    static {
        try {
            final Class<?> utilClass = Class.forName("org.apache.openejb.server.httpd.util.HttpUtil",
                    true/* setFactory() */, LightweightWebAppBuilder.class.getClassLoader());
            ...
            addFilterMethod = utilClass.getMethod("addFilter", String.class, WebContext.class, String.class,
                    FilterConfig.class);
    ...
    private static Class<?> loadFromUrls(final ClassLoader loader, final String url, final String path)
    ...
    public void deployWebApps(final AppInfo appInfo, final ClassLoader appClassLoader) throws Exception {
        ...
        for (final WebAppInfo webAppInfo : appInfo.webApps) {
            ...
            if (!appInfo.webAppAlone) { // add module bindings in app
                ...
                beanContexts = assembler.initEjbs(classLoader, appInfo, appContext, injections,
            ...
            bindings.putAll(new JndiEncBuilder(webAppInfo.jndiEnc, injections, webAppInfo.moduleId, "Bean", null,
                    webAppInfo.uniqueId, classLoader, appInfo.properties).buildBindings(JndiEncBuilder.JndiScope.comp));

            final WebContext webContext = new WebContext(appContext);
            webContext.setBindings(bindings);
            webContext.getBindings()
                    .putAll(new JndiEncBuilder(webAppInfo.jndiEnc, injections, webAppInfo.moduleId, "Bean", null,
                            webAppInfo.uniqueId, classLoader, appInfo.properties)
            ...
            if (webContext.getWebBeansContext() != null) {
                OpenEJBLifecycle.class.cast(webContext.getWebBeansContext().getService(ContainerLifecycle.class))
                        .startServletContext(sce.getServletContext());
            }
                ...
                switchServletContextIfNeeded(sce.getServletContext(), new Runnable() {
                    ...
                    public void run() {
                        for (final String mapping : info.mappings) {
                            final FilterConfig config = new SimpleFilterConfig(sce.getServletContext(), info.name,
                                    info.initParams);
                            try {
    ...
    public static class EmbeddedServletContextCreated {
        ...
        public String toString() {
            return "EmbeddedServletContextCreated{" + "context=" + context + '}';
        }
    }
