...
public class JSonConfigReader {
    ...
    public static <T> T read(final Class<T> clazz, final InputStream is) throws OpenEJBException {
        if (Openejb.class.equals(clazz) || Tomee.class.equals(clazz)) {
            final SaxOpenejb handler = read(is, "openejb",
                    Arrays.asList("System-Property", "Resource", "Container", "JndiProvider", "TransactionManager",
                            "ConnectionManager", "ProxyFactory", "Connector", "Deployments", "Import", "Service",
                            "SecurityService"),
                    new SaxOpenejb());
    ...
    private static <T extends DefaultHandler> T read(final InputStream is, final String mainRoot,
            final Collection<String> roots, final T handler) throws OpenEJBException {
        try {
                ...
                if (resources != null) {
                    ...
                    for (final Map.Entry<String, Map<String, Map<String, String>>> resource : resources.entrySet()) {
                        ...
                        if ("resources".equals(currentRoot) && attributes.getIndex("type") == -1
                                && attributes.getIndex("class-name") == -1 && attributes.getIndex("provider") == -1) {
