...
public class EndpointFactory implements MessageEndpointFactory {
    ...
    public EndpointFactory(final ActivationSpec activationSpec, final MdbContainer container,
            final BeanContext beanContext, final MdbInstanceFactory instanceFactory,
        ...
        if (proxyClass == null) {
            proxy = LocalBeanProxyFactory.createProxy(beanContext.getBeanClass(), beanContext.getClassLoader(),
                    interfaces);
    ...
    public MessageEndpoint createEndpoint(XAResource xaResource) throws UnavailableException {
        ...
        final EndpointHandler endpointHandler = new EndpointHandler(container, beanContext, instanceFactory,
                xaResource);
        try {
            ...
            try {
                return MessageEndpoint.class
                        .cast(LocalBeanProxyFactory.newProxyInstance(Thread.currentThread().getContextClassLoader(),
                                endpointHandler, beanContext.getBeanClass(), interfaces));
            } catch (final InternalError ie) {
                try {
                    return MessageEndpoint.class.cast(LocalBeanProxyFactory.newProxyInstance(classLoader,
                            endpointHandler, beanContext.getBeanClass(), interfaces));
