...
/**
 * This security service chooses a UUID as its token as this can be serialized to clients, is mostly secure, and can be
 * deserialized in a client vm without addition openejb-core classes.
 */
public abstract class AbstractSecurityService implements DestroyableResource, SecurityService<UUID>,
        ThreadContextListener, BasicPolicyConfiguration.RoleResolver {
    ...
    protected static void installJacc() {
        ...
        try {
            ...
            throw new IllegalStateException(
                    "Could not install JACC Policy Configuration Factory: " + System.getProperty(providerKey), e);
        } finally {
            Thread.currentThread().setContextClassLoader(contextClassLoader);
        }

        final String policyProvider = SystemInstance.get().getOptions().get("javax.security.jacc.policy.provider",
