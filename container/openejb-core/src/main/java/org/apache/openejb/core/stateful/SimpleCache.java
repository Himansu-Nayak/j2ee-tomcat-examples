...
public class SimpleCache<K, V> implements Cache<K, V> {
    ...
    /**
     * When the LRU is exceeded, this is the is the number of beans stored. This helps to avoid passivating a bean at a
     * time.
     */
    private int bulkPassivate;

    /**
     * A bean may be destroyed if it isn't used in this length of time (in milliseconds). A time out of value -1 means a
     * bean will never be destroyed due to time out. A time out of value 0 means a bean can be immediately destroyed.
     */
    ...
    public SimpleCache(final CacheListener<V> listener, final PassivationStrategy passivator, final int capacity,
            final int bulkPassivate, final Duration timeOut) {
    ...
    public synchronized void setPassivator(final Class<? extends PassivationStrategy> passivatorClass)
            throws Exception {
    ...
    public V checkOut(final K key, final boolean loadEntryIfNotFound) throws Exception {
        ...
        throw new OpenEJBRuntimeException(
                "Cache is corrupted: the entry " + key + " in the Map 'cache' is in state PASSIVATED");
    }
