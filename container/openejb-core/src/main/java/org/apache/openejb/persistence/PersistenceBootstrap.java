 ...
 * environments like JUnit where all the entity classes are right on the system classpath and likely to be loaded before
 * OpenEJB boots. The org.apache.openejb.javaagent.Agent class calls the bootstrap() method of this class the first time
 * it finds a classloader that has the openejb-core jar. We then do a quick scan of the classpath looking for
 * persistence.xml files and attempt to get the related persistence providers to setup the correct byte code enhancing
 * for the classes listed in the persistence.xml files.
 *
 * @version $Rev$ $Date$
 */
public class PersistenceBootstrap {
        ...
        try {
            ...
            for (final Unit unit : units.values()) {
                ...
                try {
                    ...
                    final EntityManagerFactory emf = persistenceProvider.createContainerEntityManagerFactory(info,
                            new HashMap());
                    emf.close();
    ...
    private static Set<URL> findUrls(final ClassLoader classLoader, final Properties args) throws IOException {
        if ("true".equals(args.getProperty("fast-scan", "true"))) {
            try {
                return new HashSet<>(NewLoaderLogic
                        .applyBuiltinExcludes(new UrlSet(ClassLoaders.findUrls(classLoader)).excludeJvm()).getUrls());
    ...
    private static void collectUnits(final InputStream in, final Map<String, Unit> units, final Properties args)
        ...
        parser.parse(inputSource, new DefaultHandler() {
            ...
            public void startPersistenceUnit(final String uri, final String localName, final String qName,
                    final Attributes attributes) {
    ...
    public static class Transformer implements ClassFileTransformer {
        ...
        public byte[] transform(final ClassLoader loader, final String className, final Class<?> classBeingRedefined,
            ...
            try {
                final byte[] bytes = transformer.transform(loader, className, classBeingRedefined, protectionDomain,
                        classfileBuffer);
                if (bytes != null) {
