...
public class EJBCronTrigger extends CronTriggerImpl {
    ...
    private static final Pattern LIST = Pattern
            .compile("(([A-Za-z0-9]+)(-[A-Za-z0-9]+)?)?((1ST|2ND|3RD|4TH|5TH|LAST)([A-za-z]+))?(-([0-7]+))?(LAST)?"
    ...
    private static final Pattern VALID_DAYS_OF_MONTH = Pattern.compile(
            "((1ST|2ND|3RD|4TH|5TH|LAST)(SUN|MON|TUE|WED|THU|FRI|SAT))|(([1-9])|(0[1-9])|([12])([0-9]?)|(3[01]?))|(LAST)|-([0-7])|[*]");
    ...
    private static final int[] ORDERED_CALENDAR_FIELDS = { Calendar.YEAR, Calendar.MONTH, Calendar.DAY_OF_MONTH,
    ...
    public EJBCronTrigger(final ScheduleExpression expr) throws ParseException {
        ...
        rawValue = expr.getYear() + DELIMITER + expr.getMonth() + DELIMITER + expr.getDayOfMonth() + DELIMITER
                + expr.getDayOfWeek() + DELIMITER + expr.getHour() + DELIMITER + expr.getMinute() + DELIMITER
                + expr.getSecond();
    }
     ...
     *             when there is a syntax error in the expression, or its values are out of range
     */
    ...
    private void validateSingleToken(final int field, final String token) throws ParseException {
        ...
        case Calendar.MONTH: {
            ...
            if (!(m.matches() || MONTHS_MAP.containsKey(token))) {
                throw new ParseException(field, token,
                        "Valid MONTH is 1-12 or {'Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', Dec'}");
            }
            break;
        }
        case Calendar.DAY_OF_MONTH: {
            ...
            if (!m.matches()) {
                throw new ParseException(field, token,
                        "Valid DAYS_OF_MONTH is 0-7 or {'Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'} ");
            }
            break;
        }
        case Calendar.DAY_OF_WEEK: {
            ...
            if (!(m.matches() || WEEKDAYS_MAP.containsKey(token))) {
                throw new ParseException(field, token,
                        "Valid DAYS_OF_WEEK is 1-31  -(1-7) or {'1st', '2nd', '3rd', '4th',  '5th', 'Last'} + {'Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'} ");
            }
            break;
        }
        case Calendar.HOUR_OF_DAY: {
    ...
    public Date getFireTimeAfter(final Date afterTime) {
        ...
        while (currentFieldIndex <= 6 && calendar.before(stopCalendar)) {
            ...
            /*
             * 18.2.1.2 Expression Rules If dayOfMonth has a non-wildcard value and dayOfWeek has a non-wildcard value,
             * then either the dayOfMonth field or the dayOfWeek field must match the current day (even though the other
            ...
            if (currentFieldIndex == 2 && !(expressions[3] instanceof AsteriskExpression)) {
                ...
                if (nextDayOfWeek != null) {
                    ...
                    if (value == null) {
                        ...
                        if (clonedCalendarDayOfWeek.get(expressions[1].field) == calendar.get(expressions[1].field)) {
            ...
            if (currentFieldIndex >= 1 && value == null) {
                ...
                if (currentFieldIndex == 3 && !(expressions[2] instanceof AsteriskExpression)) {
                    /*
                     * 18.2.1.2 Expression Rules, the day has been resolved when dayOfMonth expression is not
                     * AsteriskExpression.
                     */
                    currentFieldIndex++;
                } else {
    ...
    /**
     * reset those sub field values, we need to configure from the end to begin, as getActualMaximun consider other
     * fields' values
     *
     * @param calendar
     * @param currentField
    ...
    private void resetFields(final Calendar calendar, final int currentField, final boolean max) {
        for (int index = ORDERED_CALENDAR_FIELDS.length - 1; index >= 0; index--) {
            ...
            if (calendarField > currentField) {
                final int value = max ? calendar.getActualMaximum(calendarField)
                        : calendar.getActualMinimum(calendarField);
    ...
    private abstract static class FieldExpression implements Serializable {
        ...
        protected boolean isValidResult(final Calendar calendar, final Integer result) {
            return result != null && result >= calendar.getActualMinimum(field)
                    && result <= calendar.getActualMaximum(field);
        }

        /**
         * Returns the next allowed value in this calendar for the given field.
         *
         * @param calendar
         *            a Calendar where all the more significant fields have been filled out
         * @return the next value allowed by this expression, or <code>null</code> if none further allowed values are
         *         found
         */
         ...
         * @param calendar
         *            a Calendar where all the more significant fields have been filled out
         * @return the last value allowed by this expression, or <code>null</code> if none further allowed values are
         *         found
         */
    ...
    private static class RangeExpression extends FieldExpression {
        ...
        public RangeExpression(final Matcher m, final int field) throws ParseException {
            ...
            if (field == Calendar.DAY_OF_MONTH) {
                ...
                if (startWeekdayExpr != null || endWeekdayExpr != null || startDaysFromLastDayExpr != null
                        || endDaysFromLastDayExpr != null || startWeekDay.equals(LAST_IDENTIFIER)
                        || endWeekDay.equals(LAST_IDENTIFIER)) {
        ...
        public Integer getNextValue(final Calendar calendar) {
            ...
            if (isDynamicRangeExpression) {
                ...
                final Integer nextStartWeekday = startWeekdayExpr == null ? start
                        : startWeekdayExpr.getWeekdayInMonth(calendar);

                final Integer nextendWeekday = endWeekdayExpr == null ? end
                        : endWeekdayExpr.getWeekdayInMonth(calendar);
    ...
    /*
     * Just find that it is hard to keep those ranges in the list are not overlapped. The easy way is to list all the
     * values, also we keep a range expression if user defines a LAST expression, e.g. 12-LAST
     */
