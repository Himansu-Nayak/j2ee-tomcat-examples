...
public abstract class TimerData implements Serializable {
    ...
    /**
     * Has this timer been cancelled? A canceled timer must be rescheduled with the java.util.Timer if the transaction
     * is rolled back
     */
    private boolean cancelled;
    private boolean stopped;
    /**
     * Has this timer been registered with the transaction for callbacks? We remember when we are registered to avoid
     * multiple registrations.
     */
    private boolean synchronizationRegistered;
    /**
     * Used to set timer to expired state after the timeout callback method has been successfully invoked. only apply to
     * 1, Single action timer 2, Calendar timer there are no future timeout.
     */
    private boolean expired;

    public TimerData(final long id, final EjbTimerServiceImpl timerService, final String deploymentId,
            final Object primaryKey, final Method timeoutMethod, final TimerConfig timerConfig) {
        this.id = id;
    ...
    protected void doReadObject(final ObjectInputStream in) throws IOException {
        ...
        final BeanContext beanContext = SystemInstance.get().getComponent(ContainerSystem.class)
        ...
        for (final Iterator<Map.Entry<Method, MethodContext>> it = beanContext.iteratorMethodContext(); it.hasNext();) {
            ...
            /*
             * this doesn't work in all cases if (methodContext.getSchedules().isEmpty()) { continue; }
             */
