...
public class BasicManagedDataSource extends org.apache.commons.dbcp2.managed.BasicManagedDataSource
    ...
    /**
     * The password codec to be used to retrieve the plain text password from a ciphered value.
     * <p/>
     * <em>The default is no codec.</em>. In other words, it means password is not ciphered. The
     * {@link org.apache.openejb.cipher.PlainTextPasswordCipher} can also be used.
     */
    ...
    private void setJndiXaDataSource(final String xaDataSource) {
        setXaDataSourceInstance( // proxy cause we don't know if this datasource was created before or not the delegate
                XADataSourceResource.proxy(getDriverClassLoader() != null ? getDriverClassLoader()
                        : Thread.currentThread().getContextClassLoader(), xaDataSource));
