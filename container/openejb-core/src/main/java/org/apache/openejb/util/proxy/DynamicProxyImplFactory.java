...
public class DynamicProxyImplFactory {
    ...
    public static boolean isKnownDynamicallyImplemented(final Annotated<?> metaClass, final Class<?> clazz) {
        return clazz.isInterface() && (metaClass.getAnnotation(PersistenceContext.class) != null
                ...
                if (QueryProxy.class.equals(injection.getTarget())) {
                    try {
                        ...
                        throw new OpenEJBRuntimeException(
                                "a dynamic bean should reference at least one correct PersistenceContext", e);
                    }
                }
            }
            if (em == null) {
                throw new OpenEJBRuntimeException(
                        "can't find the entity manager to use for the dynamic bean " + context.getEjbName());
            }
