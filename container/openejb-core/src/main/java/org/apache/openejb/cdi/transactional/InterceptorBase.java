...
public abstract class InterceptorBase implements Serializable {
    ...
    private static final boolean HANDLE_EXCEPTION_ONLY_FOR_CLIENT = SystemInstance.get().getOptions()
            .get("openejb.cdi.jta.exception.client-only", false);
            ...
            if (error != null && (!HANDLE_EXCEPTION_ONLY_FOR_CLIENT || policy.isNewTransaction())) {
                ...
                if (doRollback != null) {
                    ...
                    final AnnotatedType<?> annotatedType = CDI.current().getBeanManager()
                            .createAnnotatedType(method.getDeclaringClass());
                    ...
                    if (tx != null) {
                        doRollback = new ExceptionPriotiryRules(tx.rollbackOn(), tx.dontRollbackOn()).accept(error,
    ...
    private Exception unwrap(Exception e) {
        Exception error = e;
        while (error != null && (ApplicationException.class.isInstance(error) || SystemException.class.isInstance(error)
