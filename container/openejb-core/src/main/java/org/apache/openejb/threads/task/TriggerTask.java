...
public abstract class TriggerTask<T> extends CUTask<T> {
    ...
    public T invoke() throws Exception {
        return invoke(new Callable<T>() {
            ...
            public T call() throws Exception {
                ...
                final ScheduledFuture<T> future = executorService.schedule(this,
                        trigger.getNextRunTime(lastExecution, scheduledTime).getTime()
                                - ManagedScheduledExecutorServiceImpl.nowMs(),
    ...
    private static class LastExecutionImpl implements LastExecution {
        ...
        public String toString() {
            return "LastExecutionImpl{" + (identityName != null ? "identityName='" + identityName + "\', " : "")
                    + "result=" + result + ", scheduledStart=" + scheduledStart + ", runStart=" + runStart + ", runEnd="
                    + runEnd + '}';
        }
    }
