...
public class WebInitialContext implements InvocationHandler {
    ...
    public static Context create(final Map<String, Object> bindings, final Context fallback) {
        return (Context) Proxy.newProxyInstance(Thread.currentThread().getContextClassLoader(), INTERFACES,
                new WebInitialContext(bindings, fallback));
    }

    @Override
    public Object invoke(final Object proxy, final Method method, final Object[] args) throws Throwable {
        if ("lookup".equals(method.getName()) && method.getParameterTypes().length == 1
