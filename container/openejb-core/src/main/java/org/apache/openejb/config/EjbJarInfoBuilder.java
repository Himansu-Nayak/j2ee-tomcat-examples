...
public class EjbJarInfoBuilder {
    ...
    public EjbJarInfo buildInfo(final EjbModule jar) throws OpenEJBException {
        ...
        if (beansInEjbJar != beansDeployed) {
            ...
            final String message = messages.format("conf.0008", jar.getJarLocation(), String.valueOf(beansInEjbJar),
        ...
        for (final EnterpriseBean bean : jar.getEjbJar().getEnterpriseBeans()) {
            ...
            if (deploymentIds.contains(beanInfo.ejbDeploymentId)) {
                final String message = messages.format("conf.0100", beanInfo.ejbDeploymentId, jar.getJarLocation(),
                        beanInfo.ejbName);
                logger.warning(message);
                throw new OpenEJBException(message);
            }
                ...
                for (final Beans.Scan.Exclude exclude : beans.getScan().getExclude()) {
                    ...
                    for (final Object config : exclude.getIfClassAvailableOrIfClassNotAvailableOrIfSystemProperty()) {
                        if (Beans.Scan.Exclude.IfAvailableClassCondition.class.isInstance(config)) {
                            exclusionInfo.availableClasses
                                    .add(Beans.Scan.Exclude.ClassCondition.class.cast(config).getName());
                        } else if (Beans.Scan.Exclude.IfNotAvailableClassCondition.class.isInstance(config)) {
                            exclusionInfo.notAvailableClasses
                                    .add(Beans.Scan.Exclude.ClassCondition.class.cast(config).getName());
                        } else if (Beans.Scan.Exclude.IfSystemProperty.class.isInstance(config)) {
                            final Beans.Scan.Exclude.IfSystemProperty systemProperty = Beans.Scan.Exclude.IfSystemProperty.class
                                    .cast(config);
    ...
    private void merge(final CompositeBeans composite, final URL key, final BeansInfo.BDAInfo bdaInfo,
    ...
    private CmrFieldInfo initRelationshipRole(final EjbRelationshipRole role, final EjbRelationshipRole relatedRole,
            final Map<String, EnterpriseBeanInfo> infos) throws OpenEJBException {
        ...
        if (role.getCmrField() != null) {
            ...
            cmrFieldInfo.fieldName = relatedEntity.abstractSchemaName + "_"
                    + relatedRole.getCmrField().getCmrFieldName();
    ...
    private void initMethodTransactions(final EjbModule jar, final Map ejbds, final EjbJarInfo ejbJarInfo) {
        final List<ContainerTransaction> containerTransactions = jar.getEjbJar().getAssemblyDescriptor()
    ...
    private void initMethodConcurrency(final EjbModule jar, final Map ejbds, final EjbJarInfo ejbJarInfo) {
        final List<ContainerConcurrency> containerConcurrency = jar.getEjbJar().getAssemblyDescriptor()
    ...
    private void initApplicationExceptions(final EjbModule jar, final EjbJarInfo ejbJarInfo) {
        for (final ApplicationException applicationException : jar.getEjbJar().getAssemblyDescriptor()
    ...
    private EnterpriseBeanInfo initSessionBean(final SessionBean s, final EjbJarInfo ejbJar, final Map m)
            throws OpenEJBException {
        ...
        if (d == null) {
            throw new OpenEJBException("No deployment information in openejb-jar.xml for bean " + s.getEjbName()
    ...
    private EnterpriseBeanInfo initMessageBean(final MessageDrivenBean mdb, final Map m) throws OpenEJBException {
        ...
        if (d == null) {
            throw new OpenEJBException("No deployment information in openejb-jar.xml for bean " + mdb.getEjbName()
                    + ". Please redeploy the jar");
        }
        bean.ejbDeploymentId = d.getDeploymentId();
        bean.containerId = d.getContainerId();
    ...
    private EnterpriseBeanInfo initEntityBean(final EntityBean e, final Map m) throws OpenEJBException {
        ...
        if (d == null) {
            throw new OpenEJBException("No deployment information in openejb-jar.xml for bean " + e.getEjbName()
                    + ". Please redeploy the jar");
        }
        bean.ejbDeploymentId = d.getDeploymentId();
        bean.containerId = d.getContainerId();
