...
public class JndiEncInfoBuilder {
    ...
    public void build(final JndiConsumer jndiConsumer, final String ejbName, final String moduleId, final URI moduleUri,
            final JndiEncInfo moduleJndiEnc, final JndiEncInfo compJndiEnc) throws OpenEJBException {
    ...
    private void buildEjbRefs(final JndiConsumer jndiConsumer, final URI moduleUri, final String moduleId,
            final String ejbName, final JndiEncInfo moduleJndiEnc, final JndiEncInfo compJndiEnc)
        ...
        for (final EjbReference ref : references) {
            ...
            if (info.location != null) {
                if (ref.getRefType() == EjbReference.Type.LOCAL) {
                    insert(toLocal(info), appInfo.globalJndiEnc.ejbLocalReferences,
                            appInfo.appJndiEnc.ejbLocalReferences, moduleJndiEnc.ejbLocalReferences,
                            compJndiEnc.ejbLocalReferences);
                } else {
                    insert(info, appInfo.globalJndiEnc.ejbReferences, appInfo.appJndiEnc.ejbReferences,
                            moduleJndiEnc.ejbReferences, compJndiEnc.ejbReferences);
                }
                continue;
            }
            ...
            if (ref.getRefType() == EjbReference.Type.LOCAL) {
                insert(toLocal(info), appInfo.globalJndiEnc.ejbLocalReferences, appInfo.appJndiEnc.ejbLocalReferences,
                        moduleJndiEnc.ejbLocalReferences, compJndiEnc.ejbLocalReferences);
    ...
    private void buildServiceRefInfos(final JndiConsumer jndiConsumer, final JndiEncInfo moduleJndiEnc,
            final JndiEncInfo compJndiEnc) {
        for (final ServiceRef ref : jndiConsumer.getServiceRef()) {
            ...
            info.location = buildLocationInfo(ref);
            info.targets.addAll(buildInjectionInfos(ref));
            insert(info, appInfo.globalJndiEnc.serviceRefs, appInfo.appJndiEnc.serviceRefs, moduleJndiEnc.serviceRefs,
    ...
    private void buildPersistenceUnitRefInfos(final JndiConsumer jndiConsumer, final JndiEncInfo moduleJndiEnc,
            final JndiEncInfo compJndiEnc) {
        for (final PersistenceUnitRef puRef : jndiConsumer.getPersistenceUnitRef()) {
            ...
            insert(info, appInfo.globalJndiEnc.persistenceUnitRefs, appInfo.appJndiEnc.persistenceUnitRefs,
                    moduleJndiEnc.persistenceUnitRefs, compJndiEnc.persistenceUnitRefs);
        }
    }

    private void buildPersistenceContextRefInfos(final JndiConsumer jndiConsumer, final JndiEncInfo moduleJndiEnc,
            final JndiEncInfo compJndiEnc) {
        for (final PersistenceContextRef contextRef : jndiConsumer.getPersistenceContextRef()) {
            ...
            info.synchronizationType = contextRef.getPersistenceContextSynchronization() != null
            ...
            insert(info, appInfo.globalJndiEnc.persistenceContextRefs, appInfo.appJndiEnc.persistenceContextRefs,
                    moduleJndiEnc.persistenceContextRefs, compJndiEnc.persistenceContextRefs);
        }
    }

    private void buildResourceRefInfos(final JndiConsumer item, final JndiEncInfo moduleJndiEnc,
            final JndiEncInfo compJndiEnc) {
        for (final ResourceRef res : item.getResourceRef()) {
            ...
            insert(info, appInfo.globalJndiEnc.resourceRefs, appInfo.appJndiEnc.resourceRefs,
                    moduleJndiEnc.resourceRefs, compJndiEnc.resourceRefs);
        }
    }

    private void buildResourceEnvRefInfos(final JndiConsumer item, final JndiEncInfo moduleJndiEnc,
            final JndiEncInfo compJndiEnc) {
        for (final ResourceEnvRef res : item.getResourceEnvRef()) {
            ...
            insert(info, appInfo.globalJndiEnc.resourceEnvRefs, appInfo.appJndiEnc.resourceEnvRefs,
                    moduleJndiEnc.resourceEnvRefs, compJndiEnc.resourceEnvRefs);
        }
    ...
    private void buildEnvEntryInfos(final JndiConsumer item, final JndiEncInfo moduleJndiEnc,
            final JndiEncInfo compJndiEnc) {
        for (final EnvEntry env : item.getEnvEntry()) {
            ...
            if (env.getEnvEntryValue() == null && env.getLookupName() == null
                    ...
                    && !CompManagedBean.class.isInstance(item) /*
                                                                * in this case we can desire to contextually lookup
                                                                * webapp value from an ear lib
                                                                */) {
                continue;
            }
            ...
            insert(info, appInfo.globalJndiEnc.envEntries, appInfo.appJndiEnc.envEntries, moduleJndiEnc.envEntries,
    ...
    public void buildDependsOnRefs(final EnterpriseBean enterpriseBean, final EnterpriseBeanInfo beanInfo,
            final String moduleId) throws OpenEJBException {
    ...
    public <I extends InjectableInfo> void insert(final I i, final List<I> global, final List<I> app,
            final List<I> module, final List<I> comp) {
    ...
    /**
     * The assembler package cannot have a dependency on org.apache.openejb.jee so we simply have a trimmed down copy of
     * the org.apache.openejb.jee.EjbReference interface and we adapt to it here.
     */
