...
public abstract class JaxbOpenejb {
            ...
            for (final String p : paths) {
                try {
                    ...
                    throw new OpenEJBException(
                            String.format("Unable to scan for service-jar.xml file: getResource('%s')", p), e);
                }
            }
            ...
            for (final URL url : sources) {
                try {
                    ...
                    try {
                        ...
                        throw new OpenEJBException(
                                "Unable to parse service-jar.xml file for provider " + providerPath + " at " + url, e);
                    } finally {
                        IO.close(read);
                    }
                } catch (final IOException e) {
                    throw new OpenEJBException(
                            "Unable to read service-jar.xml file for provider " + providerPath + " at " + url, e);
                }
            }
        }
    ...
    public static ServicesJar parseServicesJar(final InputStream in)
    ...
    public static Openejb readConfig(final InputSource in)
    ...
    public static void writeConfig(final String configFile, final Openejb openejb) throws OpenEJBException {
        ...
        try {
            ...
            /*
             * TODO: Implement informative error handling here. The exception will say "X doesn't match the regular
             * expression Y" This should be checked and more relevant information should be given -- not everyone
             * understands regular expressions.
             */
            /*
             * NOTE: This doesn't seem to ever happen. When the object graph is invalid, the MarshalException is thrown,
             * not this one as you would think.
             */
    ...
    @SuppressWarnings({ "unchecked" })
    public static <T> T unmarshal(final Class<T> type, final InputStream in)
            throws ParserConfigurationException, SAXException, JAXBException {
    ...
    private static class OpenEJBHandler extends DefaultHandler {
        ...
        public void startElement(final String uri, final String localName, final String qName, final Attributes att)
                throws SAXException {
