...
public class AsynchronousPool {
    ...
    public static AsynchronousPool create(final AppContext appContext) {
        ...
        final ExecutorBuilder builder = new ExecutorBuilder().prefix("AsynchronousPool")
                .size(options.get("AsynchronousPool.Size", 5))
                .threadFactory(new DaemonThreadFactory("@Asynchronous", appContext.getId()));

        return new AsynchronousPool(builder.build(options),
    ...
    private class FutureAdapter<T> implements Future<T> {
        ...
        public boolean cancel(final boolean mayInterruptIfRunning) {
             ...
             * method must return true. b. the meaning of parameter mayInterruptIfRunning is changed. So, we should
             * never call cancel(true), or the underlying Future object will try to interrupt the target thread.
             */
            /**
        ...
        private void handleException(Throwable e) throws ExecutionException {
            ...
            /*
             * StatefulContainer.obtainInstance(Object, ThreadContext, Method) will return NoSuchObjectException instead
             * of NoSuchEJBException * when it can't obtain an instance. Actually, the async client is expecting a
             * NoSuchEJBException. Wrap it here as a workaround.
             */
