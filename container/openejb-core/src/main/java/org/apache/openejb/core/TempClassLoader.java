...
/**
 * ClassLoader implementation that allows classes to be temporarily loaded and then thrown away. Useful for verifying
 * and inspecting a class without first loading(and thus polluting) the parent ClassLoader.
 * </p>
 * This class is a proper subclass of URLClassLoader. This class will locally load any class except for those defined in
 * the java.*, javax.* and sun.* packages and annotations all of which are loaded by with
...
public class TempClassLoader extends URLClassLoader {
    ...
    /**
     * Fast-parse the given class bytecode to determine if it is an enum class.
     */
    ...
    /**
     * Fast-parse the given class bytecode to determine if it is an annotation class.
     */
    ...
    protected synchronized Class loadClass(final String name, final boolean resolve) throws ClassNotFoundException {
        ...
        /*
         * FIX for openejb-tomcat JSF support . Added the following to the if statement below:
         * !name.startsWith("javax.faces") We want to use this TempClassLoader to also load the classes in the
         * javax.faces package. If not, then our AnnotationDeployer will not be able to load the
         * javax.faces.FacesServlet class if this class is in a jar which is in the WEB-INF/lib directory of a web app.
         * see AnnotationDeployer$ProcessAnnotatedBeans.deploy(WebModule webModule) Here is what happened before this
         * fix was applied: 1. The AnnotationDeployer tries to load the javax.faces.FacesServlet using this classloader
         * (TempClassLoader) 2. Since this class loader uses Class.forName to load classes starting with java, javax or
         * sun, it cannot load javax.faces.FacesServlet 3. Result is , AnnotationDeployer throws a
         * ClassNotFoundException
         */
        if (this.skip(name)
                || name.startsWith("javax.faces.") && URLClassLoaderFirst.shouldSkipJsf(getParent(), name)) {
    ...
    private static final class ResourceComparator implements Comparator<URL> {
        private static final boolean FORCE_MAVEN_FIRST = "true"
                .equals(SystemInstance.get().getProperty("openejb.classloader.force-maven", "false"));
