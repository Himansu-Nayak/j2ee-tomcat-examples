...
public class FinderFactory {
    ...
    private static volatile boolean MODULE_LIMITED = "true"
            .equalsIgnoreCase(SystemInstance.get().getProperty("openejb.finder.module-scoped", "false"));
                        ...
                        for (final HandlerChain handlerChain : port.getHandlerChains().getHandlerChain()) {
                            for (final Handler handler : handlerChain.getHandler()) {
                                if (handler.getHandlerClass() != null) {
                                    try {
                                        finderClasses
                                                .addAll(ancestors(classLoader.loadClass(handler.getHandlerClass())));
                                    } catch (final ClassNotFoundException e) {
                                        // no-op
                                    }
                                }
    ...
    public IAnnotationFinder create(final DeploymentModule module) throws Exception {
        ...
        if (module instanceof WebModule) {
            ...
            final Collection<URL> urls = NewLoaderLogic
                    .applyBuiltinExcludes(new UrlSet(appModule.getAdditionalLibraries())).getUrls();
    ...
    private OpenEJBAnnotationFinder fallbackAnnotationFinder(DeploymentModule module) {
        final OpenEJBAnnotationFinder finder = new OpenEJBAnnotationFinder(
                new ClassesArchive(ensureMinimalClasses(module)));
    ...
    public static class ModuleLimitedFinder implements IAnnotationFinder {
        ...
        public List<Class<?>> findAnnotatedClasses(final Class<? extends Annotation> annotation) {
            try {
                return filter(delegate.findAnnotatedClasses(annotation),
        ...
        private RuntimeException handleException(final TypeNotPresentException tnpe,
                ...
                for (final Object info : infos) {
                    if (info instanceof AnnotationFinder.ClassInfo) {
                        ...
                        try {
                            ...
                            throw new OpenEJBRuntimeException("Missing type for annotation " + annotation.getName()
                                    + " on class " + classInfo.getName(), tnpe2);
        ...
        public List<Constructor> findAnnotatedConstructors(final Class<? extends Annotation> annotation) {
            return filter(delegate.findAnnotatedConstructors(annotation),
                    new ConstructorPredicate(getAnnotatedClassNames()));
        }
        ...
        public List<Class<?>> findClassesInPackage(final String packageName, final boolean recursive) {
            return filter(delegate.findClassesInPackage(packageName, recursive),
                    new ClassPredicate<>(getAnnotatedClassNames()));
        }
        ...
        public List<Annotated<Method>> findMetaAnnotatedMethods(final Class<? extends Annotation> annotation) {
            return filter(delegate.findMetaAnnotatedMethods(annotation),
                    new AnnotatedMethodPredicate(getAnnotatedClassNames()));
        }

        @Override
        public List<Annotated<Field>> findMetaAnnotatedFields(final Class<? extends Annotation> annotation) {
            return filter(delegate.findMetaAnnotatedFields(annotation),
                    new AnnotatedFieldPredicate(getAnnotatedClassNames()));
        }

        @Override
        public List<Annotated<Class<?>>> findMetaAnnotatedClasses(final Class<? extends Annotation> annotation) {
            try {
                return filter(delegate.findMetaAnnotatedClasses(annotation),
                        new AnnotatedClassPredicate(getAnnotatedClassNames()));
            } catch (final TypeNotPresentException tnpe) {
                throw handleException(tnpe, annotation);
            }
        }
    ...
    public static class OpenEJBAnnotationFinder extends AnnotationFinder {
        private static final String[] JVM_SCANNING_CONFIG = SystemInstance.get()
                .getProperty("openejb.scanning.xbean.jvm", "java.").split(",");
