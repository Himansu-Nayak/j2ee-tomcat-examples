...
public class DeploymentsResolver implements DeploymentFilterable {
    ...
    private static final String EXCLUDE_INCLUDE_ORDER = SystemInstance.get().getOptions()
            .get("openejb.exclude-include.order", "include-exclude");
    private static final String[] ignoreDirs = SystemInstance.get()
            .getProperty("openejb.ignore.directories", ".svn,_svn,cvs,.git,.hg").split(",");
    ...
    /**
     * The algorithm of OpenEJB deployments class-path inclusion and exclusion is implemented as follows: 1- If the
     * string value of the resource URL matches the include class-path pattern Then load this resource 2- If the string
     * value of the resource URL matches the exclude class-path pattern Then ignore this resource 3- If the include and
     * exclude class-path patterns are not defined Then load this resource
     * <p/>
     * The previous steps are based on the following points: 1- Include class-path pattern has the highest priority This
     * helps in case both patterns are defined using the same values. This appears in step 1 and 2 of the above
     * algorithm. 2- Loading the resource is the default behaviour in case of not defining a value for any class-path
     * pattern This appears in step 3 of the above algorithm.
     */
    public static List<URL> loadFromClasspath(final ClassLoader classLoader) {
        ...
        try {
            ...
            if (size == 0 && searchResult.include.length() > 0) {
                logger.warning("No classpath URLs matched.  Current settings: " + CLASSPATH_EXCLUDE + "='"
                        + searchResult.exclude + "', " + CLASSPATH_INCLUDE + "='" + searchResult.include + "'");
                return jarList;
                ...
                if (size < 50 && willScrape) {
                    logger.info("Inspecting classpath for applications: " + size
                            + " urls. Consider adjusting your exclude/include.  " + "Current settings: "
                            + CLASSPATH_EXCLUDE + "='" + searchResult.exclude + "', " + CLASSPATH_INCLUDE + "='"
                            + searchResult.include + "'");
                } else if (willScrape) {
            ...
            processUrls("DeploymentsResolver1", searchResult.urls, classLoader, searchResult.requireDescriptors,
                    jarList);
            ...
            if (!searchResult.filterDescriptors) {
                unchecked = NewLoaderLogic
                        .applyBuiltinExcludes(searchResult.prefiltered.exclude(searchResult.prefiltered));
            ...
            if (time < 1000) {
                logger.debug("Searched " + urlSize + " classpath urls in " + time + " milliseconds.  Average "
                        + time / urlSize + " milliseconds per url.");
            } else if (time < 4000 || urlSize < 3) {
                logger.info("Searched " + urlSize + " classpath urls in " + time + " milliseconds.  Average "
                        + time / urlSize + " milliseconds per url.");
            } else if (time < 10000) {
                logger.warning("Searched " + urlSize + " classpath urls in " + time + " milliseconds.  Average "
                        + time / urlSize + " milliseconds per url.");
                logger.warning("Consider adjusting your " + CLASSPATH_EXCLUDE + " and " + CLASSPATH_INCLUDE
                        + " settings.  Current settings: exclude='" + searchResult.exclude + "', include='"
                        + searchResult.include + "'");
            } else {
                logger.fatal("Searched " + urlSize + " classpath urls in " + time + " milliseconds.  Average "
                        + time / urlSize + " milliseconds per url.  TOO LONG!");
                logger.fatal("ADJUST THE EXCLUDE/INCLUDE!!!.  Current settings: " + CLASSPATH_EXCLUDE + "='"
                        + searchResult.exclude + "', " + CLASSPATH_INCLUDE + "='" + searchResult.include + "'");
            ...
            logger.warning("Unable to search classpath for modules: Received Exception: " + e1.getClass().getName()
                    + " " + e1.getMessage(), e1);
        }
     ...
     */
    @Deprecated
    public static void loadFromClasspath(final FileUtils ignored, final List<URL> jarList,
     ...
     */
    @Deprecated
    public static void processUrls(final String caller, final List<URL> urls, final ClassLoader classLoader,
            final Set<RequireDescriptors> requireDescriptors, final FileUtils ignored, // don't use it, it will be removed since we already suppose it is null in several places
            final List<URL> jarList) {
        processUrls(caller, urls, classLoader, requireDescriptors, jarList);
    }

    public static void processUrls(final String caller, final List<URL> urls, final ClassLoader classLoader,
            final Set<RequireDescriptors> requireDescriptors, final List<URL> jarList) {
        for (final URL url : urls) {
            ...
            try {
                ...
                logger.warning("Unable to determine the module type of " + url.toExternalForm() + ": Exception: "
                        + e.getMessage(), e);
            ...
            for (final URL url : set.getUrls()) {
                try {
                    if ("file".equals(url.getProtocol())
                            && copy.contains(new URL("jar:" + url.toExternalForm() + "!/"))) {
                        copy.remove(url);
                    }
