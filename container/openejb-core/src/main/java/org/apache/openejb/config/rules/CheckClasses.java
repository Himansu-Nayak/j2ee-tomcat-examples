...
public class CheckClasses extends ValidationBase {
    ...
    private void checkInterface(final ClassLoader loader, final RemoteBean b, final Class<?> beanClass, String tag,
        ...
        for (final Class<? extends Annotation> annotation : beanOnlyAnnotations) {
            ...
            if (interfce.isAnnotationPresent(annotation)) {
                warn(b, "interface.beanOnlyAnnotation", annotation.getSimpleName(), interfce.getName(),
                        b.getEjbClass());
            }

            for (final Method method : finder.findAnnotatedMethods(annotation)) {
                warn(b, "interfaceMethod.beanOnlyAnnotation", annotation.getSimpleName(), interfce.getName(),
                        method.getName(), b.getEjbClass());
            }
        }
    ...
    private void check_hasDependentClasses(final RemoteBean b, final String className, final String type) {
        try {
            ...
            /*
             * # 0 - Referring Class name # 1 - Dependent Class name # 2 - Element (home, ejb-class, remote) # 3 - Bean
             * name
             */
    ...
    private Class<?> lookForClass(final ClassLoader loader, final String clazz, final String type,
            final String ejbName) {
        try {
            return loadClass(loader, clazz);
        } catch (final OpenEJBException e) {
            /*
             * # 0 - Class name # 1 - Element (home, ejb-class, remote) # 2 - Bean name
             */
            ...
            /*
             * # 0 - Class name # 1 - Element (home, ejb-class, remote) # 2 - Bean name # 3 - Misslocated Class name
             */
    ...
    protected Class<?> loadClass(final ClassLoader cl, final String clazz) throws OpenEJBException {
        try {
            ...
            throw new OpenEJBException(
                    new Messages("org.apache.openejb.util.resources").format("cl0007", clazz, module.getJarLocation()),
                    cnfe);
        }
    }
}
