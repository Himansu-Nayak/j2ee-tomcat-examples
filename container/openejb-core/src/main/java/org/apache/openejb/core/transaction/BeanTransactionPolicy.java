...
/**
 * BeanTransactionPolicy is an extension to the TransactionPolicy which provides operations for controlling a JEE bean
 * managed transaction.
 */
public interface BeanTransactionPolicy extends TransactionPolicy {
    ...
    /**
     * Syspends the bean managed transaction. This is mainly used by stateful session beans which are required to
     * maintain the bean managed transaction between calls. After this method completes the transaction will be
     * suspended or if there is a problem it will be rolled back.
     *
     * @return the syspended transaction token or null if no transaction was active
     * @throws SystemException
     *             if there was a problem suspending the transaction
     */
    SuspendedTransaction suspendUserTransaction() throws SystemException;

    /**
     * Resumes the transaction contained in the suspended transaction token. After this method completes the transaction
     * will be resumes or if there is a problem it will be rolled back.
     *
