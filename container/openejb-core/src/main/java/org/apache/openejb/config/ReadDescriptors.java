...
public class ReadDescriptors implements DynamicDeployer {
    ...
    private static final boolean ROOT_URL_FROM_WEBINF = SystemInstance.get().getOptions()
            .get("openejb.jpa.root-url-from-webinf", false);
        ...
        { // xml
            ...
            if (url != null) {
                try {
                    ...
                    logger.warning(
                            "can't read " + url.toString() + " to load resources for module " + module.toString(), e);
                }
            }
        }
        { // json
    ...
    public static void checkDuplicatedByBeansXml(final Beans beans, final Beans complete) {
        ...
        checkDuplicatedByBeansXml(beans.getAlternativeStereotypes(),
                complete.getDuplicatedAlternatives().getStereotypes());
    ...
    private static <A extends Keyable<String>> void mergeOnlyMissingEntries(final Map<String, A> existing,
            final Collection<A> news) {
    ...
    public static ApplicationClient readApplicationClient(final URL url) throws OpenEJBException {
        ...
        try {
            ...
            throw new OpenEJBException(
                    "Encountered unknown error parsing the application-client.xml file: " + url.toExternalForm(), e);
        }
    ...
    private static boolean isEmptyEjbJar(final InputStream is)
            throws IOException, ParserConfigurationException, SAXException {
        return isEmpty(is, "ejb-jar");
    }

    private static boolean isEmpty(final InputStream is, final String rootElement)
            throws IOException, ParserConfigurationException, SAXException {
    ...
    public static Webservices readWebservices(final URL url) throws OpenEJBException {
        try {
            ...
            throw new OpenEJBException(
                    "Encountered unknown error parsing the webservices.xml file: " + url.toExternalForm(), e);
        }
    }
    ...
    public static JavaWsdlMapping readJaxrpcMapping(final URL url) throws OpenEJBException {
        ...
        try {
            ...
            throw new OpenEJBException(
                    "Encountered unknown error parsing the JaxRPC mapping file: " + url.toExternalForm(), e);
        }
        return wsdlMapping;
    }
    ...
    public static Connector readConnector(final URL url) throws OpenEJBException {
        ...
        try {
            ...
            try {
                final Connector10 connector10 = (Connector10) JaxbJavaee.unmarshalJavaee(Connector10.class,
                        IO.read(url));
    ...
    public static WebApp readWebApp(final URL url) throws OpenEJBException {
        ...
        try {
            ...
            throw new OpenEJBException("Encountered unknown error parsing the web.xml file: " + url.toExternalForm(),
                    e);
        }
        return webApp;
    }
    ...
    public static TldTaglib readTldTaglib(final URL url) throws OpenEJBException {
        ...
        try {
            ...
            final String message = "Encountered unknown error parsing the JSP tag library definition file: "
                    + url.toExternalForm();
            logger.warning(message);
            logger.debug(message, e);
        }
    ...
    public static FacesConfig readFacesConfig(final URL url) throws OpenEJBException {
        try {
            ...
            throw new OpenEJBException(
                    "Encountered unknown error parsing the faces configuration file: " + url.toExternalForm(), e);
        }
    }
    ...
    public AppModule deploy(final AppModule appModule) throws OpenEJBException {
        ...
        if (persistenceUrls != null) {
            for (final Object persistenceUrl : persistenceUrls) {
                ...
                try {
                    ...
                    DeploymentLoader.logger
                            .error("Unable to load Persistence Unit from EAR: " + appModule.getJarLocation()
                                    + ", module: " + moduleName + ". Exception: " + e1.getMessage(), e1);
                }
            }
        }
        ...
        if (persistenceFragmentUrls != null) {
            for (final URL persistenceFragmentUrl : persistenceFragmentUrls) {
                try {
                    final PersistenceFragment persistenceFragment = JaxbPersistenceFactory
                            .getPersistence(PersistenceFragment.class, persistenceFragmentUrl);
                    // merging
                    for (final PersistenceUnitFragment fragmentUnit : persistenceFragment
                            .getPersistenceUnitFragment()) {
                        for (final PersistenceModule persistenceModule : appModule.getPersistenceModules()) {
                            ...
                            for (final PersistenceUnit unit : persistence.getPersistenceUnit()) {
                                ...
                                if (!persistenceFragment.getVersion().equals(persistence.getVersion())) {
                                    logger.error(
                                            "persistence unit version and fragment version are different, fragment will be ignored");
                                    continue;
                                }

                                if ("file".equals(persistenceFragmentUrl.getProtocol())) {
                                    persistenceModule.getWatchedResources()
                                            .add(URLs.toFile(persistenceFragmentUrl).getAbsolutePath());
                                }

                                for (final String clazz : fragmentUnit.getClazz()) {
                                    if (!unit.getClazz().contains(clazz)) {
                                        logger.info("Adding class " + clazz + " to persistence unit "
                                                + fragmentUnit.getName());
                                ...
                                for (final String mappingFile : fragmentUnit.getMappingFile()) {
                                    if (!unit.getMappingFile().contains(mappingFile)) {
                                        logger.info("Adding mapping file " + mappingFile + " to persistence unit "
                                                + fragmentUnit.getName());
                                ...
                                for (final String jarFile : fragmentUnit.getJarFile()) {
                                    if (!unit.getJarFile().contains(jarFile)) {
                                        logger.info("Adding jar file " + jarFile + " to persistence unit "
                                                + fragmentUnit.getName());
                                ...
                                if (fragmentUnit.isExcludeUnlistedClasses()) {
                                    ...
                                    logger.info("Excluding unlisted classes for persistence unit "
                                            + fragmentUnit.getName());
                    ...
                    DeploymentLoader.logger.error("Unable to load Persistence Unit Fragment from EAR: "
                            + appModule.getJarLocation() + ", fragment: " + persistenceFragmentUrl.toString()
    ...
    private void readValidationConfigType(final Module module) throws OpenEJBException {
        ...
        if (value != null) {
            try {
                final ValidationConfigType validationConfigType = JaxbOpenejb.unmarshal(ValidationConfigType.class,
                        value.get(), false);
    ...
    private void readOpenejbJar(final EjbModule ejbModule) throws OpenEJBException {
        ...
        if (source != null) {
            try {
                ...
                try {
                    final JAXBElement element = (JAXBElement) JaxbOpenejbJar2.unmarshal(OpenejbJarType.class,
                            source.get());
                    ...
                    if (e instanceof SAXException) {
                        throw new OpenEJBException(
                                "Cannot parse the openejb-jar.xml. Xml content written to: " + filePath, e);
                    } else if (e instanceof JAXBException) {
                        throw new OpenEJBException(
                                "Cannot unmarshall the openejb-jar.xml. Xml content written to: " + filePath, e);
    ...
    private void readAppClient(final ClientModule clientModule, final AppModule appModule) throws OpenEJBException {
        ...
        if (data instanceof ApplicationClient) {
            ...
            if (!clientModule.isEjbModuleGenerated()) {
                DeploymentLoader.logger.debug("No application-client.xml found assuming annotations present: "
    ...
    public void readEjbJar(final EjbModule ejbModule, final AppModule appModule) throws OpenEJBException {
        ...
        if (data != null) {
            ...
            DeploymentLoader.logger.debug("No ejb-jar.xml found assuming annotated beans present: "
    ...
    private void readCmpOrm(final EjbModule ejbModule) throws OpenEJBException {
        ...
        if (data != null && !(data instanceof EntityMappings)) {
            if (data instanceof URL) {
                final URL url = (URL) data;
                try {
                    final EntityMappings entitymappings = (EntityMappings) JaxbJavaee
                            .unmarshalJavaee(EntityMappings.class, IO.read(url));
                    ...
                    throw new OpenEJBException(
                            "Cannot unmarshall the openejb-cmp-orm.xml file: " + url.toExternalForm(), e);
                } catch (final IOException e) {
                    throw new OpenEJBException("Cannot read the openejb-cmp-orm.xml file: " + url.toExternalForm(), e);
                } catch (final Exception e) {
                    throw new OpenEJBException(
                            "Encountered unknown error parsing the openejb-cmp-orm.xml file: " + url.toExternalForm(),
                            e);
                }
            }
        }
    ...
    private void readConnector(final ConnectorModule connectorModule, final AppModule appModule)
        ...
        if (data instanceof Connector) {
            ...
            DeploymentLoader.logger.debug("No ra.xml found assuming annotated beans present: "
                    + appModule.getJarLocation() + ", module: " + connectorModule.getModuleId());
    ...
    private void readWebApp(final WebModule webModule, final AppModule appModule) throws OpenEJBException {
        ...
        if (data instanceof WebApp) {
            ...
            DeploymentLoader.logger.debug("No web.xml found assuming annotated beans present: "
                    + appModule.getJarLocation() + ", module: " + webModule.getModuleId());
        ...
        if (urls != null) {
            for (final URL rawUrl : urls) {
                if (rawUrl != null) {
                    ...
                    try {
                        final WebFragment webFragment = WebFragment.class
                                .cast(JaxbJavaee.unmarshal(WebFragment.class, url.get(), false));
                        ...
                        mergeOnlyMissingEntries(webModule.getWebApp().getJMSConnectionFactoriesMap(),
                                webFragment.getJMSConnectionFactories());
