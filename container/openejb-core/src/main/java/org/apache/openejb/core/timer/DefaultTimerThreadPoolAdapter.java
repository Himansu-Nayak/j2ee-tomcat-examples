...
public class DefaultTimerThreadPoolAdapter implements ThreadPool {
    ...
    public int blockForAvailableThreads() {
        if (this.threadPoolExecutorUsed) {
            ...
            synchronized (this.threadAvailableLock) {
                while (threadPoolExecutor.getMaximumPoolSize() - threadPoolExecutor.getActiveCount() < 1
    ...
    public synchronized void initialize() throws SchedulerConfigException {
        ...
        if (timerExecutor != null) {
            ...
            this.executor = new ExecutorBuilder().size(threadCount).prefix("EjbTimerPool")
        ...
        if (!this.threadPoolExecutorUsed) {
            logger.warning("Unrecognized ThreadPool implementation [" + this.executor.getClass().getName()
                    + "] is used, EJB Timer service may not work correctly");
        }
    }
