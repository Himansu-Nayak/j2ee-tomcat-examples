...
public class ThreadSingletonServiceImpl implements ThreadSingletonService {
    ...
    public static void exit(final Object oldContext) {
        if (oldContext != null && !(oldContext instanceof WebBeansContext)) {
            throw new IllegalArgumentException("ThreadSingletonServiceImpl can only be used with WebBeansContext, not "
    ...
    /**
     * Generally contexts.get() is enough since we set the current context from a request (see webbeanslistener) but
     * sometimes matching the classloader is better (manager webapps of tomcat deploys for instance) so here the
     * algorithm: 1) try to match with the classloader 2) if not matched try to use the threadlocal 3) (shouldn't
     * happen) simply return the biggest webbeancontext
     *
     * @param cl
     *            the key (generally TCCL)
    ...
    public static WebBeansContext get(final ClassLoader cl) {
        ...
        if (context == null) {
            ...
            if (context == null) {
                ...
                final List<AppContext> appContexts = SystemInstance.get().getComponent(ContainerSystem.class)
                        .getAppContexts();
    ...
    public void initialize(final StartupObject startupObject) {
        if (lazyInit == null) { // done here cause Cdibuilder trigger this class loading and that's from Warmup so we can't init too early config
            synchronized (this) {
                if (lazyInit == null) {
                    lazyInit = new Object();
                    cachedApplicationScoped = "true".equalsIgnoreCase(
                            SystemInstance.get().getProperty("openejb.cdi.applicationScope.cached", "true").trim());
                    cachedRequestScoped = "true".equalsIgnoreCase(
                            SystemInstance.get().getProperty("openejb.cdi.requestScope.cached", "true").trim());
                    cachedSessionScoped = "true".equalsIgnoreCase(
                            SystemInstance.get().getProperty("openejb.cdi.sessionScope.cached", "true").trim());
                }
            }
        }
        ...
        properties.setProperty("org.apache.webbeans.proxy.mapping.javax.enterprise.context.ApplicationScoped",
                cachedApplicationScoped ? ApplicationScopedBeanInterceptorHandler.class.getName()
        ...
        properties.setProperty("org.apache.webbeans.proxy.mapping.javax.enterprise.context.RequestScoped",
                tomee && cachedRequestScoped ? RequestScopedBeanInterceptorHandler.class.getName()
                        : defaultNormalScopeHandlerClass);

        properties.setProperty("org.apache.webbeans.proxy.mapping.javax.enterprise.context.SessionScoped",
                tomee && cachedSessionScoped ? SessionScopedBeanInterceptorHandler.class.getName()
                        : defaultNormalScopeHandlerClass);
        ...
        try {
            ...
            if (webBeansContext.getOpenWebBeansConfiguration().supportsConversation()
                    && "org.apache.webbeans.jsf.DefaultConversationService".equals(webBeansContext
                            .getOpenWebBeansConfiguration().getProperty(ConversationService.class.getName()))) {
    ...
    private void setConfiguration(final OpenWebBeansConfiguration configuration) {
        ...
        setProperty(configuration, OpenWebBeansConfiguration.TRANSACTION_SERVICE,
        ...
        setProperty(configuration, OpenWebBeansConfiguration.VALIDATOR_SERVICE,
                OpenEJBValidatorService.class.getName());
        setProperty(configuration, ResourceInjectionService.class.getName(),
