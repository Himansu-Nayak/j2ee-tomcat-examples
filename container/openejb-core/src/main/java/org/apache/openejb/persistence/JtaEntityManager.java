...
/**
 * The JtaEntityManager is a wrapper around an entity manager that automatically creates and closes entity managers for
 * each transaction in which it is accessed. This implementation supports both transaction and extended scoped JTA
 * entity managers.
 * </p>
 ...
 * will be thrown when entity manger is used. It is important that a component adds extended scoped entity managers to
 * the JtaEntityManagerRegistry when the component is entered and removes them when exited. If this registration is not
 * preformed, an IllegalStateException will be thrown when entity manger is accessed.
 */
public class JtaEntityManager implements EntityManager, Serializable {
    ...
    private static final Method CREATE_NAMED_QUERY_FROM_NAME = Reflections.findMethod("createNamedQuery",
            EntityManager.class, String.class);
    private static final Method CREATE_QUERY_FROM_NAME = Reflections.findMethod("createQuery", EntityManager.class,
            String.class);
    private static final Method CREATE_NATIVE_FROM_NAME = Reflections.findMethod("createNativeQuery",
            EntityManager.class, String.class);
    private static final Method CREATE_NAMED_QUERY_FROM_NAME_CLASS = Reflections.findMethod("createNamedQuery",
            EntityManager.class, String.class, Class.class);
    private static final Method CREATE_QUERY_FROM_NAME_CLASS = Reflections.findMethod("createQuery",
            EntityManager.class, String.class, Class.class);
    private static final Method CREATE_QUERY_FROM_CRITERIA = Reflections.findMethod("createQuery", EntityManager.class,
            CriteriaQuery.class);
    private static final Method CREATE_NATIVE_FROM_NAME_CLASS = Reflections.findMethod("createNativeQuery",
            EntityManager.class, String.class, Class.class);
    private static final Method CREATE_NATIVE_FROM_NAME_MAPPING = Reflections.findMethod("createNativeQuery",
            EntityManager.class, String.class, String.class);
    ...
    public JtaEntityManager(final String unitName, final JtaEntityManagerRegistry registry,
        ...
        this.synchronizationType = !isJPA21(entityManagerFactory) || synchronizationType == null ? null
        ...
        this.timer = localTimerConfig == null ? (globalTimerConfig == null || Boolean.parseBoolean(globalTimerConfig))
                : Boolean.parseBoolean(localTimerConfig.toString());
        ...
        final String wrapConfig = ReloadableEntityManagerFactory.class.isInstance(entityManagerFactory)
                ? ReloadableEntityManagerFactory.class.cast(entityManagerFactory).getUnitProperties()
                        .getProperty("openejb.jpa.query.wrap-no-tx", "true")
                : "true";
    ...
    /**
     * This method assures that a non-extended entity managers has an acive transaction. This is required for some
     * operations on the entity manager.
     *
     * @throws TransactionRequiredException
     *             if non-extended and a transaction is not active
     */
    ...
    /**
     * Closes a non-extended entity manager if no transaction is active. For methods on an entity manager that do not
     * require an active transaction, a temp entity manager is created for the operation and then closed.
     *
     * @param entityManager
    ...
    <T> T createQuery(final Class<T> expected, final EntityManager entityManager, final Method method,
            final Object... args) {
        try {
    ...
    /**
     * close throws an IllegalStateException if the em is container managed otherwise (emf.newEM()) it is delegated to
     * the user
     */
    public void close() {
        throw new IllegalStateException(
                "PersistenceUnit(name=" + unitName + ") - entityManager.close() call - See JPA 2.0 section 7.9.1",
    ...
    public static class Timer {
        ...
        public void stop() {
            ...
            em.logger.debug(
                    "PersistenceUnit(name=" + em.unitName + ") - entityManager." + operation + " - " + time + "ms");
        }
    }
}
