...
public class DataSourceFactory {
    ...
    private static final Map<String, String> KNOWN_CREATORS = new TreeMap<String, String>(
    ...
    public static CommonDataSource create(final String name, final boolean configuredManaged, final Class impl,
            final String definition, final Duration maxWaitTime, final Duration timeBetweenEvictionRuns,
        ...
        final String handler = SystemInstance.get().getOptions().get(GLOBAL_HANDLER_PROPERTY,
                (String) properties.remove(HANDLER_PROPERTY));
        ...
        final String logPackages = SystemInstance.get().getProperty(GLOBAL_LOG_SQL_PACKAGE_PROPERTY,
                (String) properties.remove(LOG_SQL_PACKAGE_PROPERTY));
        ...
        boolean useContainerLoader = "true"
                .equalsIgnoreCase(SystemInstance.get().getProperty("openejb.resources.use-container-loader", "true"))
                && impl.getClassLoader() == DataSourceFactory.class.getClassLoader();
        ...
        if (useContainerLoader) {
            ...
            try {
                useContainerLoader = basicChecksThatDataSourceCanBeCreatedFromContainerLoader(properties,
                        containerLoader);
            } finally {
        ...
        try {
            ...
            if (createDataSourceFromClass(impl)) { // opposed to "by driver"
                ...
                if (managed) {
                    final XAResourceWrapper xaResourceWrapper = SystemInstance.get()
                            .getComponent(XAResourceWrapper.class);
            ...
            if (doResetOnError || logSql || flushable) { // will get proxied
                ...
                if (ExecutionContext.isContextSet()) {
                    ...
                    if (stack.size() > 0) {
                        ...
                        existingResettableHandler = ResettableDataSourceHandler.class
                                .cast(objectRecipe.getProperty("resettableHandler"));
                        flushableDataSourceHandler = FlushableDataSourceHandler.class
                                .cast(objectRecipe.getProperty("flushableHandler"));
                ...
                if (doResetOnError) { // needs to be done after flushable
                    ...
                    resettableDataSourceHandler = existingResettableHandler != null ? existingResettableHandler
                ...
                if (flushable || doResetOnError) {
                    if (flushableDataSourceHandler == null) {
                        ...
                        final Map<String, Object> recipeProps = new HashMap<>(
                                objectRecipe == null ? new HashMap<String, Object>() : objectRecipe.getProperties());
                        recipeProps.remove("properties");

                        flushConfig = new FlushableDataSourceHandler.FlushConfig(recipeProps);
                        flushableDataSourceHandler = new FlushableDataSourceHandler(ds, flushConfig,
    ...
    private static CommonDataSource wrapIfNeeded(final String handler, final CommonDataSource ds)
        ...
        if (handler != null) {
            try {
                ...
                try {
                    instance = InvocationHandler.class
                            .cast(handlerClass.getConstructor(DataSource.class).newInstance(ds));
                } catch (final InvocationTargetException e) {
                    throw new IllegalStateException(e.getCause());
                } catch (final NoSuchMethodException e) {
                    try {
                        instance = InvocationHandler.class
                                .cast(handlerClass.getConstructor(CommonDataSource.class).newInstance(ds));
    ...
    public static CommonDataSource makeSerializableFlushableDataSourceProxy(final CommonDataSource ds,
            final InvocationHandler handler) {
        return (CommonDataSource) Proxy.newProxyInstance(Thread.currentThread().getContextClassLoader(),
                new Class<?>[] { DataSource.class.isInstance(ds) ? DataSource.class : XADataSource.class,
                        Serializable.class, Flushable.class },
                handler);
    }

    private static boolean basicChecksThatDataSourceCanBeCreatedFromContainerLoader(final Properties properties,
    ...
    private static CommonDataSource wrapWithHandler(final CommonDataSource ds, final InvocationHandler instance) {
        return (CommonDataSource) Proxy.newProxyInstance(
                Thread.currentThread().getContextClassLoader(), new Class<?>[] {
                        DataSource.class.isInstance(ds) ? DataSource.class : XADataSource.class, Serializable.class },
                instance);
    }
    ...
    private static void convert(final Properties properties, final Duration duration, final String key,
            final String oldKey) {
    ...
    private static boolean createDataSourceFromClass(final Class<?> impl) {
        return isDataSource(impl)
                && !SystemInstance.get().getOptions().get("org.apache.openejb.resource.jdbc.hot.deploy", false);
    }
    ...
    public static boolean knows(final Object object) {
        return object instanceof CommonDataSource
                && creatorByDataSource.containsKey(CommonDataSource.class.cast(realInstance(object)));
    }
