...
public class UpdateChecker {
    ...
    // config
    private String repoUrl = SystemInstance.get().getOptions().get("openejb.version.check.repo.url",
    ...
    public boolean usesLatest() {
        if (artifact().contains(TOMEE_ARTIFACT)) {
            final InputStream is = getClass().getClassLoader()
                    .getResourceAsStream("META-INF/maven/org.apache.openejb/tomee-catalina/pom.properties");
            if (is != null) {
    ...
    public String message() {
        ...
        return new StringBuilder("you are using the version ").append(current).append(", our latest stable version ")
                .append(latest).append(" is available on ").append(repoUrl).toString();
    }
