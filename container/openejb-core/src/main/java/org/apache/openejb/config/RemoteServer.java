...
public class RemoteServer {
        ...
        if (ok) {
            try {
                ...
                if (verbose) {
                    ...
                    final String systemInfo = "Java " + javaVersion + "; " + System.getProperty("os.name") + "/"
                ...
                if (!tomcat) {
                    ...
                    if (!addedArgs
                            .containsKey("-Dorg.apache.tomcat.util.http.ServerCookie.ALLOW_HTTP_SEPARATORS_IN_V0")) {
                    ...
                    final StringBuilder cp = new StringBuilder(bootstrapJar.getAbsolutePath()).append(ps)
                            .append(juliJar.getAbsolutePath());
                ...
                // kill3UNIXDebug();
                final ProcessBuilder pb = new ProcessBuilder(args).redirectOutput(ProcessBuilder.Redirect.INHERIT)
                        .redirectError(ProcessBuilder.Redirect.INHERIT).directory(home.getAbsoluteFile());
                ...
                throw (RuntimeException) new OpenEJBRuntimeException(
                        "Cannot start the server.  Exception: " + e.getClass().getName() + ": " + e.getMessage())
                                .initCause(e);
            }
    ...
    public void kill3UNIX() { // debug purpose only
        ...
        try {
            ...
            new ProcessBuilder("kill", "-3", Integer.toString(pid)).redirectOutput(ProcessBuilder.Redirect.INHERIT)
    ...
    private void waitForServerShutdown() throws Exception {
        ...
        if (!b) {
            ...
            System.out.println("SEVERE: Failed to shutdown TomEE running on port " + portStartup
                    + " using shutdown port: " + portShutdown);
        }
    }
     ...
     *
     * @param attempts
     *            How many times to try to send the message before giving up
