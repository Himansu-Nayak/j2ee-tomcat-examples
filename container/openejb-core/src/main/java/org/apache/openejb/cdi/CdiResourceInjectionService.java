...
public class CdiResourceInjectionService implements ResourceInjectionService {
    private final Logger logger = Logger.getInstance(LogCategory.OPENEJB.createChild("cdi"),
            CdiResourceInjectionService.class);
    ...
    private void fillInjectionProperties(final ObjectRecipe objectRecipe, final Object managedBeanInstance) {
        ...
        if (ear) { // let it be contextual, ie use webapp context (env-entries...) to create ear libs interceptors...
            try {
                ...
                comps = CdiResourceInjectionService.class
                        .cast(webBeansContext.getService(ResourceInjectionService.class)).compContexts;
        ...
        for (final BeanContext beanContext : comps) {
            for (final Injection injection : beanContext.getInjections()) {
                ...
                try {
                    ...
                    if (!found) {
                        logger.warning("Injection data not found in JNDI context: jndiName='" + injection.getJndiName()
                                + "', target=" + injection.getTarget().getName() + "/" + injection.getName());
                    }
                }
     ...
     */
    public <T> void writeExternal(final Bean<T> bean, final T actualResource, final ObjectOutput out)
            throws IOException {
        //do nothing
    }
