...
public class ActiveMQ5Factory implements BrokerFactoryHandler {
    ...
    private static String cleanUpUri(final String schemeSpecificPart, final Map<String, String> parameters,
    ...
    private static PersistenceAdapter createPersistenceAdapter(final String clazz, final String prefix,
            final Map<String, String> params) throws IllegalAccessException, InvocationTargetException,
                ...
                if (m.getName().startsWith("set") && m.getParameterTypes().length == 1
                    ...
                    if (field != null) {
                        try {
                            ...
                            throw new IllegalArgumentException("can't convert " + field + " for " + m.getName(),
    ...
    public synchronized BrokerService createBroker(final URI brokerURI) throws Exception {
        ...
        if (null == broker || !broker.isStarted()) {
            ...
            final URISupport.CompositeData compositeData = URISupport
                    .parseComposite(new URI(brokerURI.getRawSchemeSpecificPart()));
            ...
            if ("true".equals(params.remove("usekahadb"))) {
                persistenceAdapter = createPersistenceAdapter(
                        "org.apache.activemq.store.kahadb.KahaDBPersistenceAdapter", "kahadb", params);
            } else if ("true".equals(params.remove("useleveldb"))) {
                persistenceAdapter = createPersistenceAdapter(
                        "org.apache.activemq.store.leveldb.LevelDBPersistenceAdapter", "leveldb", params);
            ...
            final URI uri = new URI(
                    cleanUpUri(brokerURI.getRawSchemeSpecificPart(), compositeData.getParameters(), params));
            ...
            if (persistenceAdapter != null) {
                ...
                broker.setPersistenceFactory(
                        new ProvidedPersistenceAdapterPersistenceAdapterFactory(persistenceAdapter));
                ...
                if (notXbean) {
                    ...
                    if (value != null) {
                        ...
                        if (DataSource.class.isInstance(value)) {
                            ...
                            try {
                                final ContainerSystem containerSystem = SystemInstance.get()
            ...
            final Thread start = new Thread("ActiveMQFactory start and checkpoint") {
                ...
                public void run() {
                    ...
                    try {
                        ...
                        if (!bs.isStarted()) {
                            Logger.getInstance(LogCategory.OPENEJB_STARTUP, ActiveMQ5Factory.class)
                                    .getChildLogger("service").info("Starting ActiveMQ BrokerService");
                            bs.start();
                        }
                        ...
                        Logger.getInstance(LogCategory.OPENEJB_STARTUP, ActiveMQ5Factory.class)
                                .getChildLogger("service").info("Starting ActiveMQ checkpoint");
            ...
            /*
             * An application may require immediate access to JMS. So we need to block here until the service has
             * started. How long ActiveMQ requires to actually create a broker is unpredictable.
             *
             * A broker in OpenEJB is usually a wrapper for an embedded ActiveMQ server service. The broker
             * configuration allows the definition of a remote ActiveMQ server, in which case startup is not an issue as
             * the broker is basically a client.
             *
             * If the broker is local and the message store contains millions of messages then the startup time is
             * obviously going to be longer as these need to be indexed by ActiveMQ.
             *
            ...
            if (null != throwable) {
                Logger.getInstance(LogCategory.OPENEJB_STARTUP, ActiveMQ5Factory.class).getChildLogger("service")
                        .error("ActiveMQ failed to start broker", throwable);
            } else if (started.get()) {
                Logger.getInstance(LogCategory.OPENEJB_STARTUP, ActiveMQ5Factory.class).getChildLogger("service")
    ...
    private BrokerService newDefaultBroker(final URI uri) throws Exception {
        ...
        if (!params.isEmpty()) {
            String msg = "There are " + params.size() + " Broker options that couldn't be set on the BrokerService."
                    + " Check the options are spelled correctly." + " Unknown parameters=[" + params + "]."
    ...
    private void tomeeConfig(final BrokerService broker) {
        ...
        broker.setUseLoggingForShutdownErrors(
                Logger.getInstance(LogCategory.OPENEJB_STARTUP, ActiveMQ5Factory.class).isErrorEnabled());
    }
