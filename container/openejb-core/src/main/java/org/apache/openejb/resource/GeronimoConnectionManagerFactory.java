...
public class GeronimoConnectionManagerFactory {
    ...
    public void setConnectionMaxWaitTime(final Duration connectionMaxWait) {
        ...
        final long milleseconds = TimeUnit.MILLISECONDS.convert(connectionMaxWait.getTime(),
    ...
    public GenericConnectionManager create() {
        ...
        if (validationIntervalMs >= 0 && mcf instanceof ValidatingManagedConnectionFactory) {
            ...
            mgr = new ValidatingGenericConnectionManager(txSupport, poolingSupport, null, new AutoConnectionTracker(),
                    tm, mcf, name, classLoader, validationIntervalMs);
        } else {
            mgr = new GenericConnectionManager(txSupport, poolingSupport, null, new AutoConnectionTracker(), tm, mcf,
                    name, classLoader);
        }
    ...
    private PoolingSupport createPoolingSupport() {
        ...
        if (partitionStrategy == null || "none".equalsIgnoreCase(partitionStrategy)) {
            ...
            // unpartitioned pool
            return new SinglePool(poolMaxSize, poolMinSize, connectionMaxWaitMilliseconds, connectionMaxIdleMinutes,
                    allConnectionsEqual, !allConnectionsEqual, assumeOneMatch);
            ...
            return new PartitionedPool(poolMaxSize, poolMinSize, connectionMaxWaitMilliseconds,
                    connectionMaxIdleMinutes, allConnectionsEqual, !allConnectionsEqual, assumeOneMatch, true, false);
        } else if ("by-subject".equalsIgnoreCase(partitionStrategy)) {

            // partition by caller subject
            return new PartitionedPool(poolMaxSize, poolMinSize, connectionMaxWaitMilliseconds,
                    connectionMaxIdleMinutes, allConnectionsEqual, !allConnectionsEqual, assumeOneMatch, false, true);
        }
    ...
    private static class ValidatingGenericConnectionManager extends GenericConnectionManager {
        ...
        public ValidatingGenericConnectionManager(final TransactionSupport txSupport,
                final PoolingSupport poolingSupport, final SubjectSource o,
            ...
            do {
                if (current instanceof AbstractSinglePoolConnectionInterceptor) {
                    try {
                        final Field resizeLock = AbstractSinglePoolConnectionInterceptor.class
                                .getDeclaredField("resizeLock");
        ...
        private class ValidatingTask extends TimerTask {
            ...
            public ValidatingTask(final ConnectionInterceptor stack, final ReadWriteLock lock, final Object pool,
                ...
                if (!SinglePoolConnectionInterceptor.class.isInstance(stack)
                    ...
                    log.info("stack " + stack
                            + " currently not supported, only AutoConnectionTracker ref will be used for validation");
                }
            }
            ...
            public void run() {
                ...
                try {
                    ...
                    try {
                        final Set<ManagedConnection> invalids = ValidatingManagedConnectionFactory.class
                                .cast(getManagedConnectionFactory()).getInvalidConnections(connections.keySet());
                        if (invalids != null) {
    ...
    private class SimpleRecoverableTransactionManager implements RecoverableTransactionManager {
        ...
        public void unregisterNamedXAResourceFactory(final String namedXAResourceFactoryName) {
            if ((name == null && namedXAResourceFactoryName == null)
                    || (name != null && name.equals(namedXAResourceFactoryName))) {
                return;
            }
            throw new UnsupportedOperationException();
        }
