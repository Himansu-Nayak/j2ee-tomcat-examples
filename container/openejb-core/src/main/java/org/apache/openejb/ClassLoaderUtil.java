...
public class ClassLoaderUtil {
    ...
    public static URLClassLoader createClassLoaderFirst(final String appId, final URL[] urls,
    ...
    /**
     * Dirty hack to force closure of file handles in the Oracle VM URLClassLoader Any URLClassLoader passed into this
     * method will be unusable after the method completes.
     *
     * @param cl
     *            ClassLoader of expected type URLClassLoader (Silent failure)
     */
    ...
    public static URLClassLoader createTempClassLoader(final String appId, final URL[] rawUrls,
            final ClassLoader parent) {
        ...
        final ClassLoaderConfigurer configurer1 = QuickJarsTxtParser
                .parse(new File(appId, "META-INF/" + QuickJarsTxtParser.FILE_NAME));
        final ClassLoaderConfigurer configurer2 = QuickJarsTxtParser
                .parse(new File(appId, "WEB-INF/" + QuickJarsTxtParser.FILE_NAME));
        ...
        if (configurer1 == null && configurer2 == null && configurer3 == null) {
            ...
            final CompositeClassLoaderConfigurer configurer = new CompositeClassLoaderConfigurer(configurer1,
                    configurer2, configurer3);
    ...
    /**
     * Due to several different implementation changes in various JDK releases the code here is not as straight forward
     * as reflecting debug items in your current runtime. There have even been breaking changes between 1.6 runtime
     * builds, let alone 1.5.
     * <p/>
     ...
     * <p/>
     * If you want to look at the mess that leads up to this then follow the source code changes made to the class
     * sun.net.www.protocol.jar.JarFileFactory over several years.
     *
     * @param jarLocation
     *            String
     * @param attempt
     ...
     * setAccessable to obtain access to the Sun cache. The cache Class synchronizes upon itself for access to the cache
     * Map. This method completely clears the class loader cache which will impact preformance of object serialization.
     *
     * @param clazz
     *            the name of the class containing the cache field
     * @param fieldName
    ...
    public static void cleanOpenJPACache(final ClassLoader classLoader) {
        if (classLoader != ClassLoader.getSystemClassLoader()) {
            try {
                final Class<?> pcRegistryClass = ClassLoaderUtil.class.getClassLoader()
                        .loadClass("org.apache.openjpa.enhance.PCRegistry");
