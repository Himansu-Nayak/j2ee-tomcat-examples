...
class AppInfoBuilder {
    ...
    private static final boolean USE_EAR_AS_CONTEXT_ROOT_BASE = SystemInstance.get().getOptions()
            .get("openejb.ear.use-as-webcontext-base", false);
    ...
    public AppInfo build(final AppModule appModule) throws OpenEJBException {
        ...
        for (final EjbModule ejbModule : appModule.getEjbModules()) {
            try {
                ...
                final Map<String, EjbDeployment> deploymentsByEjbName = ejbModule.getOpenejbJar()
                ...
                for (final EnterpriseBeanInfo bean : ejbJarInfo.enterpriseBeans) {
                     ...
                     *
                     * A connector with two or more inbound adapter is registered, causing two containers named with the
                     * format: <moduleId>-<message listener interface>
                     *
                     * This code adjusts the container id for the associated MDBs by sticking the message listener
                     * interface on the end.
                     *
                     */
                    if (bean instanceof MessageDrivenBeanInfo && !containerIds.contains(d.getContainerId())
                            && !skipMdb(bean)) {
                    ...
                    if (!containerIds.contains(d.getContainerId()) && !skipMdb(bean)) {
                        final String msg = new Messages("org.apache.openejb.util.resources")
                                .format("config.noContainerFound", d.getContainerId(), d.getEjbName());
                        logger.fatal(msg);
        ...
        if (appModule.getApplication() != null) {
            ...
            jndiEncInfoBuilder.build(appModule.getApplication(), appInfo.appId, null, appModule.getModuleUri(),
                    new JndiEncInfo(), new JndiEncInfo());
        }
        ...
        for (final EjbModule ejbModule : appModule.getEjbModules()) {
            ...
            for (final EnterpriseBeanInfo beanInfo : ejbJar.enterpriseBeans) {
                ...
                jndiEncInfoBuilder.build(enterpriseBean, beanInfo.ejbName, ejbJar.moduleName, ejbModule.getModuleUri(),
                        ejbJar.moduleJndiEnc, beanInfo.jndiEnc);
    ...
    private void buildClientModules(final AppModule appModule, final AppInfo appInfo,
            final JndiEncInfoBuilder jndiEncInfoBuilder) throws OpenEJBException {
        for (final ClientModule clientModule : appModule.getClientModules()) {
            ...
            jndiEncInfoBuilder.build(applicationClient, clientModule.getJarLocation(), clientInfo.moduleId,
                    clientModule.getModuleUri(), clientInfo.jndiEnc, clientInfo.jndiEnc);
            appInfo.clients.add(clientInfo);
        }
    }

    private void buildWebModules(final AppModule appModule, final JndiEncInfoBuilder jndiEncInfoBuilder,
            final AppInfo appInfo) throws OpenEJBException {
        for (final WebModule webModule : appModule.getWebModules()) {
            ...
            jndiEncInfoBuilder.build(webApp, webModule.getJarLocation(), webAppInfo.moduleId, webModule.getModuleUri(),
                    webAppInfo.jndiEnc, webAppInfo.jndiEnc);
        ...
        for (final ConnectorModule connectorModule : appModule.getConnectorModules()) {
            ...
            if (inbound != null) {
                for (final MessageListener messageListener : inbound.getMessageAdapter().getMessageListener()) {
                    ...
                    final MdbContainerInfo mdbContainerInfo = this.configFactory.configureService(container,
    ...
    private String getId(final AdminObject adminObject, final ResourceAdapter resourceAdapter,
    ...
    private String getId(final MessageListener messageListener, final InboundResourceadapter inbound,
            final ConnectorModule connectorModule) {
        final String id;
    ...
    private String getId(final ConnectionDefinition connection, final OutboundResourceAdapter outbound,
            final ConnectorModule connectorModule) {
        final String id;
    ...
    private String findRelatedWebApp(final AppModule appModule, final String rootUrl) {
        for (final WebModule webModule : appModule.getWebModules()) {
            final List<URL> pXmls = (List<URL>) webModule.getAltDDs()
                    .get(DeploymentLoader.EAR_WEBAPP_PERSISTENCE_XML_JARS);
            if (pXmls != null) {
    ...
    private List<PortInfo> configureWebservices(final Webservices webservices) {
        ...
        for (final WebserviceDescription desc : webservices.getWebserviceDescription()) {
            ...
            for (final PortComponent port : desc.getPortComponent()) {
                ...
                final List<HandlerChainInfo> handlerChains = ConfigurationFactory
    ...
    private void configureWebserviceScurity(final List<PortInfo> infoList, final Object altDD) {
        ...
        for (final PortInfo portInfo : infoList) {
            ...
            if (deployment.getProperties().getProperty("webservice.security.transportGarantee") != null) {
                portInfo.transportGuarantee = deployment.getProperties()
                        .getProperty("webservice.security.transportGarantee");
            } else {
    ...
    public static class PersistenceProviderProperties {
        ...
        /**
         * @param appModule
         *            the app module with its config and its temp classloader, take care to only use getResource here
         * @param info
        ...
        private static void apply(final AppModule appModule, final PersistenceUnitInfo info) {
            ...
            if ("org.hibernate.ejb.HibernatePersistence".equals(info.provider)
                    || "org.hibernate.jpa.HibernatePersistenceProvider".equals(info.provider)) {
                ...
                if (prefix != null) {
                    if (info.properties.containsKey(HIBERNATE_EJB_NAMING_STRATEGY_PROP)) {
                        logger.warning("can't statisfy table prefix since you provided a "
                                + HIBERNATE_EJB_NAMING_STRATEGY_PROP + " property");
                    } else {
                ...
                if (className == null || className.startsWith("org.hibernate.transaction")
                    ...
                    if (classLoader.getResource(ClassLoaderUtil
                            .resourceName("org.hibernate.engine.transaction.jta.platform.spi.JtaPlatform")) == null) {
                        // previous hibernate 4
                        value = MakeTxLookup.HIBERNATE_NEW_FACTORY;

                        if (classLoader.getResource(ClassLoaderUtil
                                .resourceName("org.hibernate.service.jta.platform.spi.JtaPlatform")) == null) {
                    ...
                    if (classLoader.getResource(ClassLoaderUtil.resourceName(value)) != null) {
                        ...
                        logger.debug(
                                "Adjusting PersistenceUnit(name=" + info.name + ") property to " + key + "=" + value);
                    } else {
                ...
                if (className == null || className.startsWith("oracle.toplink.transaction")) {
                    ...
                    logger.debug("Adjusting PersistenceUnit(name=" + info.name + ") property to " + lookupProperty + "="
                            + openejbLookupClass);
                }
                ...
                if (className == null || className.startsWith("org.eclipse.persistence.transaction")) {
                    if (classLoader.getResource(ClassLoaderUtil.resourceName(OPENEJB_TARGET_SERVER)) != null) {
                        ...
                        logger.debug("Adjusting PersistenceUnit(name=" + info.name + ") property to "
                                + ECLIPSELINK_TARGET_SERVER + "=" + OPENEJB_TARGET_SERVER);
                    } else {
                        logger.debug("Can't adjusting PersistenceUnit(name=" + info.name + ") property to "
                                + ECLIPSELINK_TARGET_SERVER + "=" + OPENEJB_TARGET_SERVER + ", using default one");
                    }
                }
                ...
                if (prefix != null) {
                    if (info.properties.containsKey(ECLIPSELINK_SESSION_CUSTOMIZER)) {
                        logger.warning("can't statisfy table prefix since you provided a "
                                ...
                                + " property, add a call to org.apache.openejb.jpa.integration.eclipselink.PrefixSessionCustomizer");
                    } else {
                ...
                for (final String key : singletonList(
                ...
                if (prefix != null) {
                    ...
                    if (mapping != null && !"org.apache.openjpa.jdbc.meta.MappingRepository".equals(mapping)) {
                        throw new OpenEJBRuntimeException(
                                "can't honor table prefixes since you provided a custom mapping repository: "
                                        + mapping);
                    }
                ...
                for (final String key : keys) {
                    if (key.matches("openjpa.Connection(DriverName|URL|UserName|Password)")) {
                        ...
                        logger.warning("Removing PersistenceUnit(name=" + info.name + ") property " + key + "=" + o
                                + "  [not valid in a container environment]");
                    } else { // try to convert it if necessary
                        final JPAPropertyConverter.Pair pair = JPAPropertyConverter.toOpenJPAValue(key,
                                info.properties.getProperty(key), info.properties);
                        if (pair != null && !info.properties.containsKey(pair.getKey())) {
                            logger.info("Converting PersistenceUnit(name=" + info.name + ") property " + key + "="
        ...
        private static void override(final Properties appProperties, final PersistenceUnitInfo info,
            ...
            final Properties overrides = ConfigurationFactory.getOverrides(propertiesToCheckForOverridings, prefix,
                    "PersistenceUnit");

            for (final Map.Entry<Object, Object> entry : overrides.entrySet()) {
                final String property = (String) (prefix.equalsIgnoreCase(info.name) ? entry.getKey()
                        : prefix + "." + entry.getKey());
