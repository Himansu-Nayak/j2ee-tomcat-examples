 ...
 * <p/>
 * A pop() call that returns null is considered successful. A null indicates that the calling code has a permit to
 * create a poolable object and call {@link Pool#push(Object)}. This is the only situation in which that method may be
 * called.
 * <p/>
 * To simply fill the pool without a corresponding pop(), the add() method must be used. This method will attempt to
 * aquire a permit to add to the pool.
 *
 * @version $Rev$ $Date$
 */
...
public class Pool<T> {
    ...
    public Pool(final int max, final int min, final boolean strict, final long maxAge, final long idleTimeout,
            long sweepInterval, final Executor executor, final Supplier<T> supplier, final boolean replaceAged,
            final double maxAgeOffset, final boolean garbageCollection, final boolean replaceFlushed) {
        if (min > max) {
    ...
    public Pool start() {
        ...
        final ScheduledFuture<?> scheduledFuture = scheduledExecutorService.scheduleAtFixedRate(sweeper, 0,
                this.sweepInterval, MILLISECONDS);
    ...
    public void stop() {
        ...
        if (future != null && this.future.compareAndSet(future, null) && !future.isDone() && !future.isCancelled()
                && !future.cancel(false)) {
            Logger.getLogger(Pool.class.getName()).log(Level.WARNING,
                    "Pool scheduler task termination timeout expired");
        }
        ...
        if (scheduler != null && this.scheduler.compareAndSet(scheduler, null)) {
            ...
            try {
                if (!scheduler.awaitTermination(10, SECONDS)) { // should last something like 0s max since we killed the task
                    Logger.getLogger(Pool.class.getName()).log(Level.WARNING,
                            "Pool scheduler termination timeout expired");
                }
    ...
    private Executor createExecutor() {
        final ThreadPoolExecutor threadPoolExecutor = new ThreadPoolExecutor(3, 10, 60L, TimeUnit.SECONDS,
                new LinkedBlockingQueue<Runnable>(2),
                new DaemonThreadFactory("org.apache.openejb.util.Pool", hashCode()));
    ...
    private void greater(final String maxName, final long max, final String minName, final long min) {
        throw new IllegalArgumentException(minName + " cannot be greater than " + maxName + ": " + minName + "=" + min
                + ", " + maxName + "=" + max);
    }
     ...
     *            unit of time dicated by the timeout
     * @param record
     *            should this be reflected in the stats
     * @return an entry from the pool or null indicating permission to create and push() an instance into the pool
     * @throws InterruptedException
     *             vm level thread interruption
     * @throws IllegalStateException
     ...
     *             if no instance could be obtained within the timeout
     */
    private Entry pop(final long timeout, final TimeUnit unit, final boolean record)
    ...
    /**
     * Never call this method without having successfully called {@link #pop(long, TimeUnit)} beforehand.
     * <p/>
    ...
    /**
     * Used when a call to pop() was made that returned null indicating that the caller has a permit to create an object
     * for this pool, but the caller will not be exercising that permit and wishes intstead to return "null" to the
     * pool.
     */
    public void discard() {
        discard(null);
    ...
    /**
     * This internal method allows us to "swap" the status of two entries before returning them to the pool.
     * <p/>
    ...
    public static class Builder<T> {
        ...
        public Pool<T> build() {
            ...
            final Pool pool = new Pool(max, min, strict, maxAge.getTime(MILLISECONDS),
                    idleTimeout.getTime(MILLISECONDS), interval.getTime(MILLISECONDS), executor, supplier, replaceAged,
                    maxAgeOffset, this.garbageCollection, replaceFlushed);
            if (scheduledExecutorService != null) {
    ...
    public final class Entry {
        ...
        /**
         * Constructor is private so that it is impossible for an Entry object to exist without there being a
         * corresponding permit issued for the object wrapped by this Entry.
         * <p/>
         * This helps ensure that when an Entry is returned to the pool it is always safe to call
         * {@link Semaphore#release()} which increases the permit size by one.
         *
         * @param obj
         *            object that this Entry will wrap
         * @param offset
         *            creation time offset, used for maxAge
         */
        private Entry(final T obj, final long offset) {
            ...
            this.soft = garbageCollection ? new SoftReference<Instance>(instance)
        ...
        public String toString() {
            final long now = now();
            return "Entry{" + "min=" + (hard.get() != null) + ", age=" + (now - created) + ", idle=" + (now - used)
                    + ", bean=" + soft.get() + '}';
        }
