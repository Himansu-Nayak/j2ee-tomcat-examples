...
public class HandlerResolverImpl implements HandlerResolver {
    ...
    public HandlerResolverImpl(final List<HandlerChainData> handlerChains, final Collection<Injection> injections,
    ...
    public void destroyHandlers() {
        final List<InjectionProcessor<Handler>> handlerInstances = new ArrayList<InjectionProcessor<Handler>>(
    ...
    private List<Handler> buildHandlers(final PortInfo portInfo, final HandlerChainData handlerChain) {
        ...
        for (final HandlerData handler : handlerChain.getHandlers()) {
                ...
                if (bm.isInUse()) {
                    try {
                        ...
                        if (bean != null) { // proxy so faster to do it
                            ...
                            final Handler instance = Handler.class
                                    .cast(bm.getReference(bean, bean.getBeanClass(), creationalContext));

                            // hack for destroyHandlers()
                            handlers.add(instance);
                            handlerInstances.add(new InjectionProcessor<Handler>(instance,
                                    Collections.<Injection> emptySet(), null) {
                                @Override
            ...
            try { // old way
                ...
                final InjectionProcessor<Handler> processor = new InjectionProcessor<Handler>(handlerClass, injections,
                        handler.getPostConstruct(), handler.getPreDestroy(), unwrap(context));
