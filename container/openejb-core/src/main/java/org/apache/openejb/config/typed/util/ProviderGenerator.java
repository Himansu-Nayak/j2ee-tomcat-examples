    ...
    public static void main(final String[] args) throws Exception {
        ...
        for (final ServiceProvider provider : providers) {
            ...
            final File file = new File(
                    "/Users/dblevins/work/all/trunk/openejb/container/openejb-core/src/main/java/org/apache/openejb/config/typed/"
                            + builder + ".java");
            ...
            out.println(template("@XmlAccessorType(XmlAccessType.FIELD)\n" + "@XmlRootElement(name = \"${name}\")\n"
                    + "public class ${builder} extends ${service} {\n").apply("builder", builder, "service", service,
                            "name", name));
            ...
            for (final Map.Entry<Object, Object> entry : provider.getProperties().entrySet()) {
                ...
                out.println(template("    @XmlAttribute\n" + "    private ${type} ${key} = ${value};").apply("builder",
                        builder, "key", key, "value", asValue(type, value), "type", type));
            ...
            for (final Map.Entry<Object, Object> entry : provider.getProperties().entrySet()) {
                ...
                out.println(template("    public ${builder} with${Key}(${type} ${key}) {\n"
                        + "        this.${key} = ${key};\n" + "        return this;\n" + "    }\n").apply("builder",
                                builder, "key", lcFirstKey, "Key", ucFirstKey, "value", value, "type", type));
                ...
                if (Duration.class.getName().equals(type)) {
                    ...
                    out.println(template("    public void set${Key}(long time, TimeUnit unit) {\n"
                            + "        set${Key}(new Duration(time, unit));\n" + "    }\n").apply("key", lcFirstKey,
                                    "Key", ucFirstKey, "value", value, "type", type));
                }
                ...
                if ("long".equals(type) && s.contains("time")) {
                    ...
                    final Pattern pattern = Pattern.compile("(millis(econds)?|seconds|minutes|hours)",
                    ...
                    out.println(template("    public ${builder} with${Key2}(long time, TimeUnit unit) {\n"
                            + "        return with${Key}(TimeUnit.${unit}.convert(time, unit));\n" + "    }\n").apply(
                                    "builder", builder, "key2", lcFirstKey2, "Key2", ucFirstKey2, "key", lcFirstKey,
                                    "Key", ucFirstKey, "value", value, "unit", unit.name(), "type", type));

                    out.println(template("    public void set${Key2}(long time, TimeUnit unit) {\n"
                            + "        set${Key}(TimeUnit.${unit}.convert(time, unit));\n" + "    }\n").apply("key2",
                                    lcFirstKey2, "Key2", ucFirstKey2, "key", lcFirstKey, "Key", ucFirstKey, "value",
                                    value, "unit", unit.name(), "type", type));
                }
            }
