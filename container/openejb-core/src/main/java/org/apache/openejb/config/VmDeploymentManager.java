...
public class VmDeploymentManager implements DeploymentManager {
    ...
    private static Set<TargetModuleID> toTargetModuleIds(final Collection<AppInfo> deployedApps,
            final ModuleType allowedModuleType) {
    ...
    private static TargetModuleID toTargetModuleId(final AppInfo appInfo, final ModuleType allowedModuleType) {
        ...
        for (final WebAppInfo webAppInfo : appInfo.webApps) {
            final TargetModuleIDImpl clientModuleId = new TargetModuleIDImpl(DEFAULT_TARGET, webAppInfo.moduleId,
    ...
    @Override
    public TargetModuleID[] getAvailableModules(final ModuleType moduleType, final Target[] targetList)
    ...
    @Override
    public TargetModuleID[] getNonRunningModules(final ModuleType moduleType, final Target[] targetList)
            throws TargetException {
        if (!connected) {
            throw new IllegalStateException("Deployment manager is disconnected");
        }
    ...
    @Override
    public TargetModuleID[] getRunningModules(final ModuleType moduleType, final Target[] targetList)
            throws TargetException {
        if (!connected) {
            throw new IllegalStateException("Deployment manager is disconnected");
        }
    ...
    @Override
    public ProgressObject distribute(final Target[] targetList, final InputStream moduleStream,
            final InputStream planStream) {
        if (!connected) {
            throw new IllegalStateException("Deployment manager is disconnected");
        }
    ...
    @Override
    public ProgressObject redeploy(final TargetModuleID[] moduleIDList, final File moduleArchive,
            final File deploymentPlan) {
        throw new UnsupportedOperationException("redeploy is not supported");
    }

    @Override
    public ProgressObject redeploy(final TargetModuleID[] moduleIDList, final InputStream moduleArchive,
    ...
    @Override
    public void setDConfigBeanVersion(final DConfigBeanVersionType version)
    ...
    @Override
    public DeploymentConfiguration createConfiguration(final DeployableObject deployableObject)
    ...
    public static class TargetModuleIDImpl implements TargetModuleID, Comparable, Serializable {
        ...
        public boolean equals(final Object o) {
            ...
            return target.equals(targetModuleID.target) && moduleId.equals(targetModuleID.moduleId);
        }
