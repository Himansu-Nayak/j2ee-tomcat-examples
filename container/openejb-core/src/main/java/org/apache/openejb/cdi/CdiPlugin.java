...
public class CdiPlugin extends AbstractOwbPlugin implements OpenWebBeansJavaEEPlugin, OpenWebBeansEjbPlugin {
    ...
    private static void validateObserverMethods(final CdiEjbBean<?> bean,
            final Map<ObserverMethod<?>, AnnotatedMethod<?>> methods) {
        ...
        for (final Map.Entry<ObserverMethod<?>, AnnotatedMethod<?>> m : methods.entrySet()) {
            ...
            if (!Modifier.isStatic(method.getModifiers()) && doResolveViewMethod(bean, method) == null) {
                throw new WebBeansConfigurationException("@Observes " + method + " neither in the ejb view of ejb "
        ...
        for (final ProducerMethodBean<?> m : methods) {
            ...
            if (doResolveViewMethod(bean, method) == null) {
                throw new WebBeansConfigurationException(
                        "@Produces " + method + " not in the ejb view of ejb " + beanContext.getEjbName());
            }
        }
    }
    ...
    private static void validateScope(final CdiEjbBean<?> bean) {
        ...
        if (BeanType.SINGLETON.equals(beanType) && !Dependent.class.equals(scope)
                    ...
                    for (final Annotation[] a : m.getParameterAnnotations()) {
                        for (final Annotation ann : a) {
                            if (ann.annotationType().equals(Disposes.class) && doResolveViewMethod(bean, m) == null) {
                                throw new WebBeansConfigurationException(
                                        "@Disposes is forbidden on non business EJB methods");
                            }
                        }
                    }
                }
    ...
    public boolean isEEComponent(final Class<?> impl) {
        return Servlet.class.isAssignableFrom(impl) || Filter.class.isAssignableFrom(impl)
                || ServletContextListener.class.isAssignableFrom(impl) || JspTag.class.isAssignableFrom(impl);
    }
    ...
    public <T> BeanAttributes<T> createBeanAttributes(final AnnotatedType<T> type) {
        return new CdiEjbBean.EJBBeanAttributesImpl(findBeanContext(webBeansContext, type.getJavaClass()),
    ...
    public void stop() throws OpenEJBException {
        ...
        try {
            ...
            final CdiResourceInjectionService injectionServices = (CdiResourceInjectionService) webBeansContext
                    .getService(ResourceInjectionService.class);
    ...
    @Override
    public Object getSessionBeanProxy(final Bean<?> inBean, final Class<?> interfce,
        ...
        synchronized (inBean) { // singleton for the app so safe to sync on it
            ...
            if (webBeansContext.getBeanManagerImpl().isNormalScope(scopeClass)) {
                final BeanContext beanContext = cdiEjbBean.getBeanContext();
                final Provider provider = webBeansContext.getNormalScopeProxyFactory()
                        .getInstanceProvider(beanContext.getClassLoader(), cdiEjbBean);
                ...
                if (!beanContext.isLocalbean()) {
                    ...
                    try {
                        instance = ProxyManager.newProxyInstance(interfaces.toArray(new Class<?>[interfaces.size()]),
                    ...
                    final NormalScopeProxyFactory normalScopeProxyFactory = webBeansContext
                            .getNormalScopeProxyFactory();
                    final Class<?> proxyClass = normalScopeProxyFactory.createProxyClass(beanContext.getClassLoader(),
    ...
    @Override
    public <T> Bean<T> defineSessionBean(final Class<T> clazz, final BeanAttributes<T> attributes,
        ...
        if (annotatedType.isAnnotationPresent(Specializes.class)) {
            ...
            if (!businessLocalInterfaces.isEmpty()) {
                throw new DefinitionException(
                        "You can only specialize another EJB with at least the same API: " + clazz);
            }
        }
        final CdiEjbBean<T> bean = new OpenEJBBeanBuilder<T>(bc, webBeansContext, annotatedType, attributes)
                .createBean(clazz, !annotatedType.isAnnotationPresent(Vetoed.class));
        ...
        if (bean.isEnabled()) {
            observerMethods = new ObserverMethodsBuilder<T>(webBeansContext, bean.getAnnotatedType())
        ...
        final Set<ProducerFieldBean<?>> producerFields = new ProducerFieldBeansBuilder(bean.getWebBeansContext(),
                bean.getAnnotatedType()).defineProducerFields(bean);
        final Set<ProducerMethodBean<?>> producerMethods = new ProducerMethodBeansBuilder(bean.getWebBeansContext(),
                bean.getAnnotatedType()).defineProducerMethods(bean, producerFields);
        ...
        for (final ProducerMethodBean<?> producerMethod : producerMethods) {
            final AnnotatedMethod<?> method = webBeansContext.getAnnotatedElementFactory()
                    .newAnnotatedMethod(producerMethod.getCreatorMethod(), annotatedType);
            webBeansUtil.inspectDeploymentErrorStack(
                    "There are errors that are added by ProcessProducer event observers for "
        ...
        for (final ProducerFieldBean<?> producerField : producerFields) {
            ...
            annotatedFields.put(producerField,
                    webBeansContext.getAnnotatedElementFactory().newAnnotatedField(producerField
                            .getCreatorField(),
        ...
        final GProcessSessionBean event = new GProcessSessionBean(Bean.class.cast(bean), annotatedType, bc.getEjbName(),
                bean.getEjbType());
