...
/**
 * ClassFinder searches the classpath of the specified classloader for packages, classes, constructors, methods, or
 * fields with specific annotations.
 * <p/>
 * For security reasons ASM is used to find the annotations. Classes are not loaded unless they match the requirements
 * of a called findAnnotated* method. Once loaded, these classes are cached.
 * <p/>
 * The getClassesNotLoaded() method can be used immediately after any find* method to get a list of classes which
 * matched the find requirements (i.e. contained the annotation), but were unable to be loaded.
 *
 * @version $Rev$ $Date$
 */
public class AnnotationFinder {
    ...
    /**
     * Creates a ClassFinder that will search the urls in the specified classloader excluding the urls in the
    ...
    /**
     * Creates a ClassFinder that will search the urls in the specified classloader excluding the urls in the 'exclude'
     * classloader.
     *
     * @param classLoader
     *            source of classes to scan
    ...
    private static Collection<URL> getUrls(final ClassLoader classLoader, final boolean excludeParent)
            throws IOException {
        return AnnotationFinder.getUrls(classLoader, excludeParent ? classLoader.getParent() : null);
    }

    private static Collection<URL> getUrls(final ClassLoader classLoader, final ClassLoader excludeParent)
            throws IOException {
     ...
     * <p/>
     * The list will only contain entries of classes whose byte code matched the requirements of last invoked find*
     * method, but were unable to be loaded and included in the results.
     * <p/>
     * The list returned is unmodifiable. Once obtained, the returned list will be a live view of the results from the
     * last findAnnotated* method call.
     * <p/>
    ...
    public class Visitor extends ClassVisitor {
        ...
        @Override
        public void visit(final int i, final int i1, final String string, final String string1, final String string2,
        ...
        @Override
        public FieldVisitor visitField(final int i, final String string, final String string1, final String string2,
                final Object object) {
            throw notFoundException;
        }

        @Override
        public MethodVisitor visitMethod(final int i, final String string, final String string1, final String string2,
                final String[] strings) {
            throw notFoundException;
        }
