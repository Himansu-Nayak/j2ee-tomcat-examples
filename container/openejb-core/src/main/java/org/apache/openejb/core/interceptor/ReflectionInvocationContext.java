...
public class ReflectionInvocationContext implements InvocationContext {
    ...
    public void setParameters(final Object[] parameters) {
        ...
        if (parameters.length != this.parameters.length) {
            throw new IllegalArgumentException("Expected " + this.parameters.length + " parameters, but only got "
        ...
        for (int i = 0; i < parameters.length; i++) {
            ...
            if (parameter == null) {
                if (parameterType.isPrimitive()) {
                    throw new IllegalArgumentException("Expected parameter " + i + " to be primitive type "
                            + parameterType.getName() + ", but got a parameter that is null");
                }
            } else {
                ...
                if (!actual.isAssignableFrom(given)) {
                    throw new IllegalArgumentException(
                            "Expected parameter " + i + " to be of type " + parameterType.getName()
    ...
    /**
     * Business method interceptors can only throw exception allowed by the target business method. Lifecycle
     * interceptors can only throw RuntimeException.
     *
     * @param e
     *            the invocation target exception of a reflection method invoke
     * @return the cause of the exception
     * @throws AssertionError
     *             if the cause is not an Exception or Error.
     */
    ...
    public String toString() {
        ...
        return "InvocationContext(operation=" + operation + ", target=" + target.getClass().getName() + ", method="
                + methodName + ")";
    }
