...
public class EntityContainer implements RpcContainer {
    ...
    public EntityContainer(final Object id, final SecurityService securityService, final int poolSize)
            throws OpenEJBException {
        this.containerID = id;
        this.securityService = securityService;
        entrancyTracker = new EntrancyTracker(
    ...
    public Object invoke(final Object deployID, InterfaceType type, final Class callInterface, final Method callMethod,
        ...
        try {
            ...
            final boolean authorized = type == InterfaceType.TIMEOUT
    ...
    protected Object invoke(final InterfaceType type, final Method callMethod, final Method runMethod,
            final Object[] args, final ThreadContext callContext) throws OpenEJBException {
        final BeanContext beanContext = callContext.getBeanContext();
        final TransactionPolicy txPolicy = createTransactionPolicy(beanContext.getTransactionType(callMethod, type),
                callContext);
    ...
    public void ejbLoad_If_No_Transaction(final ThreadContext callContext, final EntityBean bean) throws Exception {
        ...
        if (orginalOperation == Operation.BUSINESS || orginalOperation == Operation.REMOVE) {
            ...
            final BeanContext beanContext = callContext.getBeanContext();
            final TransactionPolicy txPolicy = beanContext.getTransactionPolicyFactory()
                    .createTransactionPolicy(TransactionType.Supports);
            try {
                ...
                throw new ApplicationException(new NoSuchObjectException(
                        "Entity not found: " + callContext.getPrimaryKey())/* .initCause(e) */);
    ...
    protected ProxyInfo createEJBObject(final Method callMethod, final Object[] args, final ThreadContext callContext,
            final InterfaceType type) throws OpenEJBException {
        final BeanContext beanContext = callContext.getBeanContext();

        callContext.setCurrentOperation(Operation.CREATE);

        /*
         * According to section 9.1.5.1 of the EJB 1.1 specification, the "ejbPostCreate(...) method executes in the
         * same transaction context as the previous ejbCreate(...) method."
         *
         * For this reason the TransactionScopeHandler methods usally preformed by the invoke( ) operation must be
         * handled here along with the call explicitly. This ensures that the afterInvoke() is not processed between the
         * ejbCreate and ejbPostCreate methods to ensure that the ejbPostCreate executes in the same transaction context
         * of the ejbCreate. This would otherwise not be possible if container-managed transactions were used because
         * the TransactionScopeManager would attempt to commit the transaction immediately after the ejbCreate and
         * before the ejbPostCreate had a chance to execute. Once the ejbPostCreate method execute the super classes
         * afterInvoke( ) method will be executed committing the transaction if its a CMT.
         */
    ...
    protected Object findMethod(final Method callMethod, final Object[] args, final ThreadContext callContext,
            final InterfaceType type) throws OpenEJBException {
        final BeanContext beanContext = callContext.getBeanContext();
        ...
        /*
         * Find operations return either a single primary key or a collection of primary keys. The primary keys are
         * converted to ProxyInfo objects.
         */
    ...
    protected Object homeMethod(final Method callMethod, final Object[] args, final ThreadContext callContext,
            final InterfaceType type) throws OpenEJBException {
        final BeanContext beanContext = callContext.getBeanContext();
    ...
    protected void removeEJBObject(final Method callMethod, final Object[] args, final ThreadContext callContext,
            final InterfaceType type) throws OpenEJBException {
    ...
    private void handleException(final TransactionPolicy txPolicy, Throwable e, final ThreadContext callContext,
            final EntityBean bean) throws OpenEJBException {
        ...
        if (type == ExceptionType.SYSTEM) {
            ...
            if (bean != null) {
                try {
                    ...
                    logger.error(
                            "The instance manager encountered an unkown system exception while trying to discard the entity instance with primary key "
