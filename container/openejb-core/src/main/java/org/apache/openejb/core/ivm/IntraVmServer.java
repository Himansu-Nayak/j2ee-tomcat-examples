...
public class IntraVmServer implements ApplicationServer {
    ...
    public EJBMetaData getEJBMetaData(final ProxyInfo pi) {
        ...
        final IntraVmMetaData metaData = new IntraVmMetaData(beanContext.getHomeInterface(),
                beanContext.getRemoteInterface(), beanContext.getComponentType());
    ...
    public EJBObject getEJBObject(final ProxyInfo pi) {
        return (EJBObject) EjbObjectProxyHandler.createProxy(pi.getBeanContext(), pi.getPrimaryKey(),
                pi.getInterfaceType(), pi.getInterfaces(), pi.getInterface());
    }

    public Object getBusinessObject(final ProxyInfo pi) {
        return EjbObjectProxyHandler.createProxy(pi.getBeanContext(), pi.getPrimaryKey(), pi.getInterfaceType(),
                pi.getInterfaces(), pi.getInterface());
    }
