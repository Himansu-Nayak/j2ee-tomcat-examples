    ...
    private static Object decryptIfNeeded(final String replace, final boolean acceptCharArray) {
        if (replace.startsWith(CIPHER_PREFIX)) {
            ...
            try {
                ...
                try {
                    cipher = PasswordCipher.class
                            .cast(Thread.currentThread().getContextClassLoader().loadClass(algo).newInstance());
            ...
            return acceptCharArray && SafePasswordCipher.class.isInstance(cipher)
                    ? SafePasswordCipher.class.cast(cipher).decryptAsCharArray(input) : cipher.decrypt(input);
        }
        return replace;
    }
