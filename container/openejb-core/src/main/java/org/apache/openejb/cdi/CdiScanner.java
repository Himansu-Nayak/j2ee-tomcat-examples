...
import org.apache.openejb.cdi.transactional.*;
...
public class CdiScanner implements ScannerService {
    ...
    private static final Class<?>[] TRANSACTIONAL_INTERCEPTORS = new Class<?>[] { MandatoryInterceptor.class,
            NeverInterceptor.class, NotSupportedInterceptor.class, RequiredInterceptor.class,
    ...
    public void init(final Object object) {
        ...
        for (final EjbJarInfo ejbJar : appInfo.ejbJars) {
            ...
            final boolean filterByClassLoader = "true"
                    .equals(ejbJar.properties.getProperty(OPENEJB_CDI_FILTER_CLASSLOADER,
            ...
            for (final BeansInfo.BDAInfo bda : beans.bdas) {
                ...
                infoByBda.put(bda, handleBda(startupObject, classLoader, comparator, beans, scl, filterByClassLoader,
            ...
            for (final BeansInfo.BDAInfo bda : beans.noDescriptorBdas) {
                ...
                handleBda(startupObject, classLoader, comparator, beans, scl, filterByClassLoader, beanArchiveService,
                        openejb, bda);
            }
            ...
            if ("true".equalsIgnoreCase(SystemInstance.get().getProperty("openejb.cdi.debug", "false"))) {
                ...
                logger.info("CDI beans for " + startupObject.getAppInfo().appId + (startupObject.getWebContext() != null
    ...
    private BeanArchiveService.BeanArchiveInformation handleBda(final StartupObject startupObject,
            final ClassLoader classLoader, final ClassLoaderComparator comparator, final BeansInfo beans,
            final ClassLoader scl, final boolean filterByClassLoader, final BeanArchiveService beanArchiveService,
            final boolean openejb, final BeansInfo.BDAInfo bda) {
        ...
        final boolean scanModeAnnotated = BeanArchiveService.BeanDiscoveryMode.ANNOTATED
                .equals(information.getBeanDiscoveryMode());
        ...
        if (!noScan) {
            ...
            for (final String name : bda.managedClasses) {
                ...
                if (scanModeAnnotated) {
                    ...
                    if (!filterByClassLoader || comparator.isSame(loader)
    ...
    private boolean isBean(final Class clazz) {
        try {
            for (final Annotation a : clazz.getAnnotations()) {
                ...
                if (beanManager.isScope(annotationType) || beanManager.isStereotype(annotationType)
                        || beanManager.isInterceptorBinding(annotationType) || Decorator.class == a.annotationType()) {
                    return true;
                }
            }
     ...
     * @param classLoader
     *            classloader to (try to) load it from
