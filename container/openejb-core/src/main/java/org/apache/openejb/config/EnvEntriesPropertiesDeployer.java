...
/**
 * Read in all the properties for an app's META-INF/env-entries.properties file and create <env-entry> tags for each in
 * the component's enc.
 *
 * @version $Rev$ $Date$
 */
public class EnvEntriesPropertiesDeployer implements DynamicDeployer {
    ...
    private static final Logger log = Logger.getInstance(LogCategory.OPENEJB_STARTUP_CONFIG,
    ...
    private void apply(final JndiConsumer bean, final EnvEntry newEntry, final String componentName) {
        ...
        if (entry != null) {
            if (SystemInstance.get().getOptions().get("envprops.override", false)) {
                log.debug("envprops.override", componentName, entry.getName(), entry.getEnvEntryValue(),
