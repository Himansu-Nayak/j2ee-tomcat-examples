...
import org.apache.openejb.quartz.*;
...
public class EjbTimerServiceImpl implements EjbTimerService, Serializable {
    ...
    public EjbTimerServiceImpl(final BeanContext deployment, final TransactionManager transactionManager,
            final TimerStore timerStore, final int retryAttempts) {
    ...
    public static synchronized Scheduler getDefaultScheduler(final BeanContext deployment) {
        ...
        synchronized (deployment.getId()) { // should be done only once so no perf issues
            ...
            if (scheduler == null || newInstance) {
                final boolean useTccl = "true"
                        .equalsIgnoreCase(properties.getProperty(OPENEJB_QUARTZ_USE_TCCL, "false"));
                ...
                try {
                    ...
                    final JobDetail job = JobBuilder.newJob(EjbTimeoutJob.class)
                            .withIdentity(OPENEJB_TIMEOUT_JOB_NAME, OPENEJB_TIMEOUT_JOB_GROUP_NAME).storeDurably(true)
    ...
    private static void defaultQuartzConfiguration(final Properties properties, final BeanContext deployment,
            final boolean newInstance, final boolean tccl) {
        ...
        if (!tccl) {
            final String driverDelegate = properties
                    .getProperty("org.apache.openejb.quartz.jobStore.driverDelegateClass");
            if (driverDelegate != null && StdJDBCDelegate.class.getName().equals(driverDelegate)) {
                properties.put("org.apache.openejb.quartz.jobStore.driverDelegateClass",
                        PatchedStdJDBCDelegate.class.getName());
            } else if (driverDelegate != null) {
                log.info("You use " + driverDelegate
                        + " driver delegate with quartz, ensure it doesn't use ObjectInputStream otherwise your custom TimerData can induce some issues");
            }

            // adding our custom persister
            if (properties.containsKey("org.apache.openejb.quartz.jobStore.class")
                    && !properties.containsKey("org.apache.openejb.quartz.jobStore.driverDelegateInitString")) {
                try {
                    final Class<?> clazz = EjbTimerServiceImpl.class.getClassLoader()
                            .loadClass(properties.getProperty("org.apache.openejb.quartz.jobStore.class"));
        ...
        if (defaultThreadPool.equals(properties.get(StdSchedulerFactory.PROP_THREAD_POOL_CLASS))) {
            if (properties.containsKey("org.apache.openejb.quartz.threadPool.threadCount")
                ...
                log.info(
                        "Found property 'org.apache.openejb.quartz.threadPool.threadCount' for default thread pool, please use '"
                                + DefaultTimerThreadPoolAdapter.OPENEJB_TIMER_POOL_SIZE + "' instead");
                properties.put(DefaultTimerThreadPoolAdapter.OPENEJB_TIMER_POOL_SIZE,
                        properties.getProperty("org.apache.openejb.quartz.threadPool.threadCount"));
            }
            if (properties.containsKey("org.quartz.threadPool.threadCount")
                ...
                log.info("Found property 'org.quartz.threadPool.threadCount' for default thread pool, please use '"
                        + DefaultTimerThreadPoolAdapter.OPENEJB_TIMER_POOL_SIZE + "' instead");
                properties.put(DefaultTimerThreadPoolAdapter.OPENEJB_TIMER_POOL_SIZE,
                        properties.getProperty("org.quartz.threadPool.threadCount"));
            }
        }

        // to ensure we can shutdown correctly, default doesn't support such a configuration
        if (!properties.getProperty(StdSchedulerFactory.PROP_JOB_STORE_CLASS, RAMJobStore.class.getName())
                .equals(RAMJobStore.class.getName())) {
            properties.put("org.apache.openejb.quartz.jobStore.makeThreadsDaemons",
                    properties.getProperty("org.apache.openejb.quartz.jobStore.makeThreadsDaemon", "true"));
        }
    }
    ...
    private static int putAll(final Properties a, final Properties b) {
        ...
        for (final Map.Entry<Object, Object> entry : b.entrySet()) {
            ...
            if (key.startsWith("org.quartz.") || key.startsWith("org.apache.openejb.quartz.")
    ...
    private static void shutdown(final Scheduler s) throws OpenEJBRuntimeException {
        ...
        try {
            if (null != s && !s.isShutdown() && s.isStarted()) {
                ...
                long timeout = SystemInstance.get().getOptions().get(QuartzResourceAdapter.OPENEJB_QUARTZ_TIMEOUT,
                        10000L);
    ...
    public void schedule(final TimerData timerData) throws TimerStoreException {
        ...
        if (trigger instanceof AbstractTrigger) { // is the case
            ...
            throw new OpenEJBRuntimeException(
                    "the trigger was not an AbstractTrigger - Should not be possible: " + trigger);
        }
    ...
    /**
     * Call back from TimerData and ejbTimeout when a timer has been cancelled (or is complete) and should be removed
     * from stores.
     *
     * @param timerData
    ...
    @Override
    public Timer createTimer(final Object primaryKey, final Method timeoutMethod, final long duration,
        ...
        try {
            final TimerData timerData = timerStore.createSingleActionTimer(this, (String) deployment.getDeploymentID(),
                    primaryKey, timeoutMethod, expiration, timerConfig);
            initializeNewTimer(timerData);
            return timerData.getTimer();
    ...
    @Override
    public Timer createTimer(final Object primaryKey, final Method timeoutMethod, final long initialDuration,
            final long intervalDuration, final TimerConfig timerConfig)
        ...
        try {
            final TimerData timerData = timerStore.createIntervalTimer(this, (String) deployment.getDeploymentID(),
                    primaryKey, timeoutMethod, initialExpiration, intervalDuration, timerConfig);
            initializeNewTimer(timerData);
            return timerData.getTimer();
        } catch (final TimerStoreException e) {
            throw new EJBException(e);
    ...
    @Override
    public Timer createTimer(final Object primaryKey, final Method timeoutMethod, final Date expiration,
            final TimerConfig timerConfig) throws IllegalArgumentException, IllegalStateException, EJBException {
        if (expiration == null) {
    ...
    @Override
    public Timer createTimer(final Object primaryKey, final Method timeoutMethod, final Date initialExpiration,
            final long intervalDuration, final TimerConfig timerConfig)
                    throws IllegalArgumentException, IllegalStateException, EJBException {
    ...
    @Override
    public Timer createTimer(final Object primaryKey, final Method timeoutMethod,
            final ScheduleExpression scheduleExpression, final TimerConfig timerConfig) {
        ...
        checkState();
        try {
            final TimerData timerData = timerStore.createCalendarTimer(this, (String) deployment.getDeploymentID(),
                    primaryKey, timeoutMethod, scheduleExpression, timerConfig, false);
            initializeNewTimer(timerData);
            return timerData.getTimer();
        } catch (final TimerStoreException e) {
            throw new EJBException(e);
    ...
    public void ejbTimeout(final TimerData timerData) {
        ...
        try {
            ...
            for (int tries = 0; tries < 1 + retryAttempts; tries++) {
                ...
                try {
                    ...
                    thread.setContextClassLoader(
                            deployment.getClassLoader() != null ? deployment.getClassLoader() : loader);

                    SetAccessible.on(ejbTimeout);
                    container.invoke(deployment.getDeploymentID(), InterfaceType.TIMEOUT,
            ...
            log.warning("Failed to execute ejbTimeout on " + timerData.getDeploymentId() + " successfully within "
    ...
    /**
     * The timer task registered with the java.util.Timer. The run method of this class simply adds an execution of the
     * ejbTimeout method to the thread pool. It is important to use the thread pool, since the java.util.Timer is single
     * threaded.
     */
    /*
     * private class EjbTimeoutTimerTask extends TimerTask { private final TimerData timerData;
     * 
     * public EjbTimeoutTimerTask(TimerData timerData) { this.timerData = timerData; }
     * 
     * public void run() { threadPool.execute(new Runnable() { public void run() { ejbTimeout(timerData); } }); } }
     */
