...
public abstract class EjbHomeProxyHandler extends BaseEjbProxyHandler {
    ...
    public EjbHomeProxyHandler(final BeanContext beanContext, final InterfaceType interfaceType,
            final List<Class> interfaces, final Class mainInterface) {
    ...
    protected static EjbHomeProxyHandler createHomeHandler(final BeanContext beanContext,
            final InterfaceType interfaceType, final List<Class> interfaces, final Class mainInterface) {
        ...
        default:
            throw new IllegalStateException(
                    "Component type does not support rpc interfaces: " + beanContext.getComponentType());
        }
    }

    public static Object createHomeProxy(final BeanContext beanContext, final InterfaceType interfaceType) {
        return createHomeProxy(beanContext, interfaceType, null,
                interfaceType.isRemote() ? beanContext.getRemoteInterface() : beanContext.getLocalInterface());
    }

    public static Object createHomeProxy(final BeanContext beanContext, final InterfaceType interfaceType,
            final List<Class> objectInterfaces, final Class mainInterface) {
        if (!interfaceType.isHome()) {
            throw new IllegalArgumentException("InterfaceType is not a Home type: " + interfaceType);
        }

        try {
            final EjbHomeProxyHandler handler = createHomeHandler(beanContext, interfaceType, objectInterfaces,
                    mainInterface);
    ...
    public Object createProxy(final Object primaryKey, final Class mainInterface) {
        try {
            ...
            final EjbObjectProxyHandler handler = newEjbObjectHandler(getBeanContext(), primaryKey, objectInterfaceType,
            ...
            if ((InterfaceType.LOCALBEAN.equals(objectInterfaceType)
                ...
                return LocalBeanProxyFactory
                        .constructProxy(handler.getBeanContext().get(BeanContext.ProxyClass.class).getProxy(), handler);
            } else {
                ...
                return ProxyManager.newProxyInstance(proxyInterfaces.toArray(new Class[proxyInterfaces.size()]),
                        handler);
            }
    ...
    protected abstract EjbObjectProxyHandler newEjbObjectHandler(BeanContext beanContext, Object pk,
            InterfaceType interfaceType, List<Class> interfaces, Class mainInterface);

    @Override
    protected Object _invoke(final Object proxy, final Class interfce, final Method method, final Object[] args)
            throws Throwable {
        ...
        try {
            ...
            if (operation == null) {
                ...
                default:
                    throw new OpenEJBRuntimeException(
                            "Inconsistent internal state: value " + operation + " for operation " + methodName);
                }
            }

            if (logger.isDebugEnabled()) {
                logger.debug("EjbHomeProxyHandler: finished invoking method " + method.getName() + ". Return value:"
                        + retValue);
            }
            ...
            throw cause;
            /*
             * Application exceptions must be reported dirctly to the client. They do not impact the viability of the
             * proxy.
             */
            ...
            throw exc;
            /*
             * A system exception would be highly unusual and would indicate a sever problem with the container system.
             */
            ...
            logger.debug("EjbHomeProxyHandler: finished invoking method " + method.getName() + " with exception:" + t,
                    t);
            throw t;
        }
    }

    protected Object homeMethod(final Class interfce, final Method method, final Object[] args, final Object proxy)
            throws Throwable {
        ...
        if (beanContext.isAsynchronous(method)) {
            return beanContext.getModuleContext().getAppContext().getAsynchronousPool()
    ...
    private Object homeMethodInvoke(final Class interfce, final Method method, final Object[] args)
            throws OpenEJBException {
        return container.invoke(deploymentID, interfaceType, interfce, method, args, null);
    }

    protected Object create(final Class interfce, final Method method, final Object[] args, final Object proxy)
            throws Throwable {
        ...
        final ProxyInfo proxyInfo = (ProxyInfo) container.invoke(deploymentID, interfaceType, interfce, method, args,
                null);
    ...
    protected Object getEJBMetaData(final Method method, final Object[] args, final Object proxy) throws Throwable {
        ...
        final IntraVmMetaData metaData = new IntraVmMetaData(getBeanContext().getHomeInterface(),
                getBeanContext().getRemoteInterface(), getBeanContext().getPrimaryKeyClass(),
    ...
    public ProxyInfo getProxyInfo() {
        ...
        return new ProxyInfo(getBeanContext(), null, getBeanContext().getInterfaces(interfaceType), interfaceType,
                getMainInterface());
    }
    ...
    protected Object _writeReplace(final Object proxy) throws ObjectStreamException {
        /*
         * If the proxy is being copied between bean instances in a RPC call we use the IntraVmArtifact
         */
        if (IntraVmCopyMonitor.isIntraVmCopyOperation()) {
            return new IntraVmArtifact(proxy);
            /*
             * If the proxy is referenced by a stateful bean that is being passivated by the container we allow this
             * object to be serialized.
             */
        } else if (IntraVmCopyMonitor.isStatefulPassivationOperation()) {
            return proxy;
            /*
             * If the proxy is being copied between class loaders we allow this object to be serialized.
             */
        } else if (IntraVmCopyMonitor.isCrossClassLoaderOperation()) {
            return proxy;
            /*
             * If the proxy is serialized outside the core container system, we allow the application server to handle
             * it.
             */
    ...
    protected Object removeWithHandle(final Class interfce, final Method method, final Object[] args,
        ...
        /*
         * This operation takes care of invalidating all the EjbObjectProxyHanders associated with the same RegistryId.
         * See this.createProxy().
         */
        if (stub != null) {
    ...
    private static enum MethodType {
        CREATE, FIND, HOME_HANDLE, META_DATA, REMOVE
    }
}
