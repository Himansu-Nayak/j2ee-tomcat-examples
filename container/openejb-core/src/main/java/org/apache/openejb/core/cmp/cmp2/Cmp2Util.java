...
public class Cmp2Util {
    ...
    public static <Bean extends EntityBean> Bean getEntityBean(final EJBLocalObject proxy) {
        ...
        if (!(handler.container instanceof CmpContainer)) {
            throw new IllegalArgumentException("Proxy is not connected to a CMP container but is conect to "
                    + handler.container.getClass().getName());
        }
    ...
    public static <Proxy extends EJBLocalObject> Proxy getEjbProxy(final BeanContext beanContext,
            final EntityBean entity) {
        if (entity == null) {
            return null;
        }
        ...
        if (!(beanContext.getContainer() instanceof CmpContainer)) {
            throw new IllegalArgumentException("Proxy is not connected to a CMP container but is conect to "
                    + beanContext.getContainer().getClass().getName());
        }

        final Proxy proxy = (Proxy) EjbObjectProxyHandler.createProxy(beanContext, primaryKey,
                InterfaceType.EJB_LOCAL_HOME, beanContext.getLocalInterface());
        return proxy;
    }
