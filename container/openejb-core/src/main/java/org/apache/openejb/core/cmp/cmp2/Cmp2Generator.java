...
public class Cmp2Generator implements Opcodes {
    ...
    /**
     * Constructor for a Cmp2Generator. This validates the initial EJB state information and prepares for the code
     * generation process.
     *
     * @param cmpImplClass
     *            The name of the implementation class we're generating.
     * @param beanClass
     *            The bean implementation class that is our starting point for code generation.
     * @param pkField
     *            The name of the primary key field (optional if the primary key class is given).
     * @param primKeyClass
     *            The optional primary key class for complex primary keys.
     * @param cmpFields
     *            The list of fields that are managed using cmp.
     */
    public Cmp2Generator(final String cmpImplClass, final Class beanClass, final String pkField,
            final Class<?> primKeyClass, final String[] cmpFields) {
        ...
        for (final String cmpFieldName : cmpFields) {
            ...
            if (getter == null) {
                throw new IllegalArgumentException(
                        "No such property " + cmpFieldName + " defined on bean class " + beanClassName);
            }
        ...
        if (pkField != null) {
            ...
            if (this.pkField == null) {
                throw new IllegalArgumentException(
                        "No such property " + pkField + " defined on bean class " + beanClassName);
            }
        } else {
            this.pkField = null;
        }
     ...
     * @return The string name of the getter method for the property.
     */
    ...
    /**
     * Add a field to the list of fields defined as CMR fields. Note that if the field is also defined as a CMP field,
     * it will be removed from the normal CMP list.
     *
     * @param cmrField
     *            The new CMR field definition pulled from the EJB metadata.
     */
    ...
    /**
     * Perform the generation step for a CMP Entity Bean. This uses the accumulated meta data and the base bean class to
     * generate a subclass with the automatically generated bits of OpenEJB infrastructure hooks.
     *
    ...
    public byte[] generate() {
        ...
        cw.visit(V1_5, ACC_PUBLIC + ACC_SUPER, implClassName, null, beanClassName,
                new String[] { "org/apache/openejb/core/cmp/cmp2/Cmp2Entity", "javax/ejb/EntityBean" });

        // public static Object deploymentInfo;
        {
            final FieldVisitor fv = cw.visitField(ACC_PUBLIC + ACC_STATIC, "deploymentInfo", "Ljava/lang/Object;", null,
                    null);
            fv.visitEnd();
        }
        ...
        if (Object.class.equals(primKeyClass)) {
            final FieldVisitor fv = cw.visitField(ACC_PRIVATE, UNKNOWN_PK_NAME, UNKNOWN_PK_TYPE.getDescriptor(), null,
                    null);
            fv.visitEnd();
        }
    ...
    private void createOpenEJB_addCmr() {
        final MethodVisitor mv = cw.visitMethod(ACC_PUBLIC, "OpenEJB_addCmr",
    ...
    private void createOpenEJB_removeCmr() {
        final MethodVisitor mv = cw.visitMethod(ACC_PUBLIC, "OpenEJB_removeCmr",
    ...
    /**
     * Add a CMP field to the class. The field is created with private scope.
     *
     * @param cmpField
     *            The Cmp field defined in the metadata.
     */
    private void createField(final CmpField cmpField) {
        final FieldVisitor fv = cw.visitField(ACC_PRIVATE, cmpField.getName(), cmpField.getDescriptor(), null, null);
        fv.visitEnd();
    }

    /**
     * Generate a concrete getter field for a CMP field. At this point, we're just generating a simple accessor for the
     * field, given the type. The JPA engine when it makes this implementation class a managed class define whatever
     * additional logic might be required.
     *
     * @param cmpField
    ...
    /**
     * Get the getter method for this CMP field. This will be either get<Name> or is<Name> depending on what abstract
     * method is defined on the base bean class.
     *
     * @param propertyName
    ...
    /**
     * Generate a concrete setter field for a CMP field. At this point, we're just generating a simple accessor for the
     * field, given the type. The JPA engine when it makes this implementation class a managed class define whatever
     * additional logic might be required.
     *
     * @param cmpField
     *            The CMP field backing this setter method.
     */
    private void createSetter(final CmpField cmpField) {
        final String methodName = setterName(cmpField.getName());
        final MethodVisitor mv = cw.visitMethod(ACC_PUBLIC, methodName, "(" + cmpField.getDescriptor() + ")V", null,
                null);
        mv.visitCode();
        mv.visitVarInsn(ALOAD, 0);
    ...
    /**
     * Create a simple internal method for obtaining the primary key. There are 2 possibilities for handling the primary
     * key here:
     * <p/>
     * 1) There is a defined primary key field. The contents of that field are returned.
     * <p/>
     * 2) The primary key is provided by the container. This is a long value stored in a private, generated field. This
     * field is returned as a generated wrappered Long.
     * <p/>
     * 3) A primary key class has been provided. An instance of this class is instantiated, and code is generated that
     * will copy all of the CMP fields from the EJB into the primary key instance.
     */
    private void createSimplePrimaryKeyGetter() {
        final MethodVisitor mv = cw.visitMethod(ACC_PUBLIC, "OpenEJB_getPrimaryKey", "()Ljava/lang/Object;", null,
                null);
        mv.visitCode();
        ...
        if (pkField != null) {
            ...
            for (final Field field : primKeyClass.getFields()) {
                ...
                if (!cmpField.getType().getClassName().equals(field.getType().getName())) {
                    throw new IllegalArgumentException(
                            "Primary key " + cmpField.getName() + " is type " + cmpField.getType().getClassName()
                                    + " but CMP field is type " + field.getType().getName());
                }
    ...
    /**
     * Create the CMR fields defined for this object. This creates a pair of fields for each CMR field. The first field
     * is the real field containing the object data. The second field will be an accessor object that's instantiated
     * when the fields are first initialized. The accessor field gets created with the same name and "Cmr" concatenated
     * to the end of the field name.
     *
     * @param cmrField
     *            The CMR field descriptor.
     */
    private void createCmrFields(final CmrField cmrField) {
        FieldVisitor fv = cw.visitField(ACC_PRIVATE, cmrField.getName(), cmrField.getDescriptor(),
                cmrField.getGenericSignature(), null);
        fv.visitEnd();

        fv = cw.visitField(ACC_PRIVATE + ACC_TRANSIENT, cmrField.getName() + "Cmr", cmrField.getAccessorDescriptor(),
                cmrField.getAccessorGenericSignature(), null);
        fv.visitEnd();
    }

    /**
     * Initialize the CMR fields associated with a CMR definition. This initializes two fields per CMR defined field: 1)
     * The CMR field itself (which might be initialized to an instance of a defined type) and 2) the appropriate CMD
     * accessor that handles the different types of relationship.
     *
     * @param mv
     *            The method context we're initializing in.
     * @param cmrField
     *            The CMR field to process.
     */
    private void initCmrFields(final MethodVisitor mv, final CmrField cmrField) {
        ...
        // invoke
        mv.visitMethodInsn(INVOKESPECIAL, cmrField.getAccessorInternalName(), "<init>",
    ...
    /**
     * Create a getter method for the CMR field. This will used the accessor object initialized into the name + "Cmr"
     * field that was already generated and initialized at object creation. The accessor object manages the object
     * relationship.
     *
     * @param cmrField
    ...
    private void createCmrGetter(final CmrField cmrField) {
        ...
        final MethodVisitor mv = cw.visitMethod(ACC_PUBLIC, methodName, "()" + cmrField.getProxyDescriptor(), null,
                null);
        mv.visitCode();
        mv.visitVarInsn(ALOAD, 0);
        ...
        mv.visitMethodInsn(INVOKEVIRTUAL, cmrField.getAccessorInternalName(), "get",
                cmrField.getCmrStyle().getGetterDescriptor(), false);
    ...
    /**
     * Generate a setter method for a CMR field. The setter method will delegate the setting responsibility to the
     * accessor object store the associated Cmr field.
     *
     * @param cmrField
     *            The field we're generating the setter for.
     */
    private void createCmrSetter(final CmrField cmrField) {
        ...
        final MethodVisitor mv = cw.visitMethod(ACC_PUBLIC, methodName, "(" + cmrField.getProxyDescriptor() + ")V",
                null, null);
        mv.visitCode();
        ...
        if (cmrField.getCmrStyle() != CmrStyle.SINGLE) {
            ...
            mv.visitFieldInsn(GETFIELD, implClassName, cmrField.getName(), cmrField.getDescriptor());
            mv.visitVarInsn(ALOAD, 1);
            mv.visitMethodInsn(INVOKEVIRTUAL, cmrField.getAccessorInternalName(), "set",
                    cmrField.getCmrStyle().getSetterDescriptor(), false);
            mv.visitInsn(RETURN);
        } else {
    ...
    /**
     * Generate the OpenEJB_deleted() logic for a CMR field. This handles the cascade from the referencing object to the
     * CMR fields for the object. This method generates the CMR logic inline inside the object OpenEJB_deleted() method.
     *
     * @param mv
     *            The method context we're operating within.
     * @param cmrField
     *            The CMD field containing the deleted value.
     */
    private void createOpenEJB_deleted(final MethodVisitor mv, final CmrField cmrField) {
        ...
        mv.visitVarInsn(ALOAD, 0);
        mv.visitFieldInsn(GETFIELD, implClassName, cmrField.getName(), cmrField.getDescriptor());
        mv.visitMethodInsn(INVOKEVIRTUAL, cmrField.getAccessorInternalName(), "deleted",
                cmrField.getCmrStyle().getDeletedDescriptor(), false);
    }

    /**
     * Generate the OpenEJB_addCmr logic for an individual CMR field. Each CMR field has a test against the property
     * name, which is passed to the wrappering addCmr method. This results in a series of if blocks for each defined CMD
     * property.
     *
     * @param mv
     *            The method we're generating within.
     * @param cmrField
     *            The CMR field definition.
     */
    private void createOpenEJB_addCmr(final MethodVisitor mv, final CmrField cmrField) {
        ...
        if (cmrField.getCmrStyle() != CmrStyle.SINGLE) {
            ...
            mv.visitMethodInsn(INVOKEINTERFACE, cmrField.getCmrStyle().getCollectionType().getInternalName(), "add",
    ...
    /**
     * Emit the remove logic for an individual CMR field. Like the addCmr logic, each field is guarded by an if test on
     * the property name.
     *
     * @param mv
     * @param cmrField
    ...
    private void createOpenEJB_removeCmr(final MethodVisitor mv, final CmrField cmrField) {
        ...
        if (cmrField.getCmrStyle() != CmrStyle.SINGLE) {
            ...
            mv.visitFieldInsn(GETFIELD, implClassName, cmrField.getName(), cmrField.getDescriptor());
            mv.visitVarInsn(ALOAD, 2);
            mv.visitMethodInsn(INVOKEINTERFACE, cmrField.getCmrStyle().getCollectionType().getInternalName(), "remove",
                    "(Ljava/lang/Object;)Z", true);
            mv.visitInsn(POP);
    ...
    /**
     * Generate a concrete implementation of an abstract ejbSelectxxxx method.
     *
     * @param selectMethod
    ...
    private void createSelectMethod(final Method selectMethod) {
        ...
        final MethodVisitor mv = cw.visitMethod(ACC_PUBLIC, selectMethod.getName(),
        ...
        mv.visitMethodInsn(INVOKESTATIC, Type.getInternalName(executeMethod.getDeclaringClass()),
                executeMethod.getName(), Type.getMethodDescriptor(executeMethod), false);
    ...
    public void createSetEntityContext() {
        final MethodVisitor mv = cw.visitMethod(ACC_PUBLIC, "setEntityContext", "(Ljavax/ejb/EntityContext;)V", null,
                null);
        mv.visitCode();
        mv.visitInsn(RETURN);
        mv.visitMaxs(0, 2);
    ...
    /**
     * Helper class to handle common type conversions in generated code.
     */
    private static final class Convert {
        ...
        /**
         * Generate code to performing boxing of primitive types into a wrapper class instance.
         *
         * @param mv
         *            The method currently being emitted.
         * @param from
         *            The class we're converting from.
         */
        ...
        /**
         * Handle a conversion from one object type to another. If There are 3 possible conversions:
         * <p/>
         * 1) The to class is Object. This can be handled without conversion. This option is a NOP. 2) The to class is a
         * reference type (non-primitive). This conversion is a cast operation (which might fail at run time). 3) The to
         * class is a primitive type. This is an unboxing operation.
         *
         * @param mv
         *            The method currently being constructed.
         * @param to
                ...
                final Convert conversion = getConversion(to);
                {
                    if (conversion == null) {
                        throw new NullPointerException(
                                "unsupported conversion for EJB select return type " + to.getName());
                    }
                }
        ...
        public void primitiveToObject(final MethodVisitor mv) {
            mv.visitMethodInsn(INVOKESTATIC, objectType.getInternalName(), toObject.getName(),
                    Type.getMethodDescriptor(toObject), false);
        }

        public void objectToPrimitive(final MethodVisitor mv) {
            mv.visitTypeInsn(CHECKCAST, objectType.getInternalName());
            mv.visitMethodInsn(INVOKEVIRTUAL, objectType.getInternalName(), toPrimitive.getName(),
                    Type.getMethodDescriptor(toPrimitive), false);
        }
