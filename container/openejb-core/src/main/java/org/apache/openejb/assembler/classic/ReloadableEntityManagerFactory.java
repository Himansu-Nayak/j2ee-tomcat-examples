...
public class ReloadableEntityManagerFactory implements EntityManagerFactory, Serializable {
    ...
    public ReloadableEntityManagerFactory(final ClassLoader cl, final EntityManagerFactoryCallable callable,
    ...
    public void createDelegate() {
        ...
        try {
            ...
            LOGGER.info("assembler.buildingPersistenceUnit", unitInfoImpl.getPersistenceUnitName(),
                    unitInfoImpl.getPersistenceProviderClassName(), String.valueOf(time));
    ...
    private String logCriteriaQueryJpqlLevel(final Properties props) {
        return SystemInstance.get().getOptions().get(OPENEJB_JPA_CRITERIA_LOG_JPQL_LEVEL,
                props.getProperty(OPENEJB_JPA_CRITERIA_LOG_JPQL_LEVEL, "INFO"));
    }

    private boolean logCriteriaQueryJpql(final Properties prop) {
        return SystemInstance.get().getOptions().get(OPENEJB_JPA_CRITERIA_LOG_JPQL,
                Boolean.parseBoolean(prop.getProperty(OPENEJB_JPA_CRITERIA_LOG_JPQL, "false")))
                || SystemInstance.get().getOptions().get(OPENEJB_JPA_CRITERIA_LOG_JPQL_LEVEL,
                        prop.getProperty(OPENEJB_JPA_CRITERIA_LOG_JPQL_LEVEL, null)) != null;
    }
    ...
    public void unregister() throws OpenEJBException {
        if (objectName != null) {
            ...
            try {
                ...
                throw new OpenEJBException("can't unregister the mbean for the entity manager factory " + getPUname(),
                        e);
            }
        }
    }
    ...
    public static class JMXReloadableEntityManagerFactory {
        ...
        public void dump(final String file) {
            final PersistenceUnitInfoImpl info = reloadableEntityManagerFactory.entityManagerFactoryCallable
                    .getUnitInfo();
        ...
        public TabularData getJarFiles() {
            return buildTabularData("jarfile", "jar file type", reloadableEntityManagerFactory.getJarFileUrls(),
                    Info.URL);
        }
        ...
        private TabularData buildTabularData(final String typeName, final String typeDescription, final List<?> list,
                final Info info) {
