...
public class DeploymentLoader implements DeploymentFilterable {
    public static final Logger logger = Logger.getInstance(LogCategory.OPENEJB_STARTUP_CONFIG,
    ...
    private static final Collection<String> KNOWN_DESCRIPTORS = Arrays.asList("app-ctx.xml", "module.properties",
            "application.properties", "web.xml", "ejb-jar.xml", "openejb-jar.xml", "env-entries.properties",
            "beans.xml", "ra.xml", "application.xml", "application-client.xml", "persistence-fragment.xml",
            "persistence.xml", "validation.xml", NewLoaderLogic.EXCLUSION_FILE);
    ...
    public static void addWebModuleDescriptors(final URL baseUrl, final WebModule webModule, final AppModule appModule)
    ...
    private static void addWebPersistenceDD(final String name, final Map<String, Object> otherDD,
    ...
    public static EjbModule addWebModule(final WebModule webModule, final AppModule appModule) throws OpenEJBException {
...

        {
            ...
            List<URL> persistenceXmls = pXml == null ? null
                    : (List.class.isInstance(pXml) ? (List<URL>) pXml : new ArrayList<>(asList(URL.class.cast(pXml))));
        ...
        final EjbModule webEjbModule = new EjbModule(webModule.getClassLoader(), webModule.getModuleId(),
        ...
        try {
            ...
            throw new OpenEJBException("Unable to create annotation scanner for web module " + webModule.getModuleId(),
                    e);
        }
    ...
    /**
     * If the web.xml is metadata-complete and there is no ejb-jar.xml then per specification we use the web.xml
     * metadata-complete setting to imply the same for EJBs.
     *
     * @param webModule
     *            WebModule
     * @param ejbModule
    ...
    private static void addWebservices(final WsModule wsModule) throws OpenEJBException {
        final boolean webservicesEnabled = SystemInstance.get().getOptions()
                .get(ConfigurationFactory.WEBSERVICES_ENABLED, true);
    ...
    protected static ConnectorModule createConnectorModule(final String appId, final String rarPath,
            final ClassLoader parentClassLoader, final String moduleId) throws OpenEJBException {
        return createConnectorModule(appId, rarPath, parentClassLoader, moduleId, null);
    }

    protected static ConnectorModule createConnectorModule(final String appId, final String rarPath,
            final ClassLoader parentClassLoader, final String moduleId, final URL raXmlUrl) throws OpenEJBException {
    ...
    protected static void addWebFragments(final WebModule webModule, final Collection<URL> urls)
            throws OpenEJBException {
        if (urls == null) {
            return;
        }
    ...
    @SuppressWarnings({ "unchecked" })
    protected static Collection<URL> addPersistenceUnits(final AppModule appModule, final URL... urls)
            throws OpenEJBException {
    ...
    private static Map<String, URL> getDescriptors(final ResourceFinder finder, final boolean log)
            throws OpenEJBException {
        try {
    ...
    public static Map<String, URL> mapDescriptors(final ResourceFinder finder) throws IOException {
    ...
    public static Application unmarshal(final URL url) throws OpenEJBException {
        try {
            ...
            throw new OpenEJBException("Encountered error parsing the application.xml file: " + url.toExternalForm(),
                    e);
        }
    }
    ...
    /**
     * @param jarFile
     *            the app file (war, jar, ear)
     * @param config
     *            potentially some more config, mainly used when linking to another system like tomcat to enrich the
     *            conf we can guess
    ...
    public AppModule load(final File jarFile, final ExternalConfiguration config) throws OpenEJBException {
        ...
        try {
            ...
            try {
                doNotUseClassLoader = ClassLoaderUtil.createClassLoader(jarPath, new URL[] { baseUrl },
                        getOpenEJBClassLoader());
                moduleClass = discoverModuleType(baseUrl, ClassLoaderUtil.createTempClassLoader(doNotUseClassLoader),
                        true);
            } catch (final Exception e) {
                throw new UnknownModuleTypeException(
                        "Unable to determine module type for jar: " + baseUrl.toExternalForm(), e);
            }
            ...
            if (EjbModule.class.equals(moduleClass)) {
                ...
                final ClassLoader classLoader = ClassLoaderUtil.createTempClassLoader(jarPath, urls,
                        getOpenEJBClassLoader());
            ...
            if (ClientModule.class.equals(moduleClass)) {
                ...
                final ClientModule clientModule = createClientModule(baseUrl, jarLocation, getOpenEJBClassLoader(),
                        null);
            ...
            if (ConnectorModule.class.equals(moduleClass)) {
                ...
                if (connectorModule != null) {
                    ...
                    if (!"true".equalsIgnoreCase(
                            SystemInstance.get().getProperty("openejb.connector." + id + ".skip-default", "false"))) {
                    ...
                    if (aliases != null) {
                        for (final String alias : aliases.split(",")) {
                            final ConnectorModule aliasModule = createConnectorModule(jarLocation, jarLocation,
                                    getOpenEJBClassLoader(), alias);
                    ...
                    final AppModule appModule = new AppModule(
                            connectorModules.toArray(new ConnectorModule[connectorModules.size()]));
            ...
            if (WebModule.class.equals(moduleClass)) {
                ...
                final WebModule webModule = createWebModule(file.getAbsolutePath(), baseUrl, getOpenEJBClassLoader(),
                ...
                final AppModule appModule = new AppModule(webModule.getClassLoader(), file.getAbsolutePath(),
            ...
            if (PersistenceModule.class.equals(moduleClass)) {
                final String jarLocation = URLs.toFilePath(baseUrl);
                final ClassLoader classLoader = ClassLoaderUtil.createTempClassLoader(jarPath, new URL[] { baseUrl },
                        getOpenEJBClassLoader());
    ...
    protected AppModule createAppModule(final File jarFile, final String jarPath) throws OpenEJBException {
        ...
        try {
            ...
            if (applicationXmlUrl != null) {
                ...
                createApplicationFromFiles(appId, tmpClassLoader, ejbModules, clientModules, resouceModules, webModules,
                        files);
            }
            ...
            try {
                ...
                logger.warning("Cannot load libs from '" + application.getLibraryDirectory() + "' : " + e.getMessage(),
                        e);
            }
            ...
            for (final String moduleName : ejbModules.keySet()) {
                try {
                    ...
                    logger.error("Unable to load EJBs from EAR: " + appId + ", module: " + moduleName + ". Exception: "
                            + e.getMessage(), e);
                }
            }
            ...
            for (final String moduleName : clientModules.keySet()) {
                try {
                    ...
                    if (ClassLoaderUtil.isUrlCached(appModule.getJarLocation(), clientUrl)) {
                        try {
                            clientUrl = ClassLoaderUtil.getUrlCachedName(appModule.getJarLocation(), clientUrl).toURI()
                                    .toURL();
    ...
    private void createApplicationFromFiles(final String appId, final ClassLoader tmpClassLoader,
            final Map<String, URL> ejbModules, final Map<String, URL> clientModules,
            final Map<String, URL> resouceModules, final Map<String, URL> webModules, final HashMap<String, URL> files)
                    throws OpenEJBException {
        for (final Map.Entry<String, URL> entry : files.entrySet()) {
            ...
            try {
                detectAndAddModuleToApplication(appId, tmpClassLoader, ejbModules, clientModules, resouceModules,
                        webModules, entry);
                ...
                throw new OpenEJBException(
                        "Unable to determine the module type of " + entry.getKey() + ": Exception: " + e.getMessage(),
                        e);
            }
        }
    }

    private void detectAndAddModuleToApplication(final String appId, final ClassLoader tmpClassLoader,
            final Map<String, URL> ejbModules, final Map<String, URL> clientModules,
            final Map<String, URL> resouceModules, final Map<String, URL> webModules,
            final Map.Entry<String, URL> entry) throws IOException, UnknownModuleTypeException {
        final ClassLoader moduleClassLoader = ClassLoaderUtil.createTempClassLoader(appId,
                new URL[] { entry.getValue() }, tmpClassLoader);

        final Class<? extends DeploymentModule> moduleType = discoverModuleType(entry.getValue(), moduleClassLoader,
                true);
    ...
    protected ClientModule createClientModule(final URL clientUrl, final String absolutePath,
            final ClassLoader appClassLoader, final String moduleName) throws OpenEJBException {
    ...
    protected ClientModule createClientModule(final URL clientUrl, final String absolutePath,
        ...
        final ClientModule clientModule = new ClientModule(applicationClient, appClassLoader, absolutePath, mainClass,
                moduleName);
    ...
    protected EjbModule createEjbModule(final URL baseUrl, final String jarPath, final ClassLoader classLoader)
            throws OpenEJBException {
    ...
    private WebModule createWebModule(final String jar, final URL warUrl, final ClassLoader parentClassLoader,
            final String contextRoot, final String moduleName, final ExternalConfiguration config)
                    throws OpenEJBException {
        return createWebModule(jar, URLs.toFilePath(warUrl), parentClassLoader, contextRoot, moduleName, config);
    }

    public void addWebModule(final AppModule appModule, final URL warUrl, final ClassLoader parentClassLoader,
            final String contextRoot, final String moduleName) throws OpenEJBException {
        final WebModule webModule = createWebModule(appModule.getJarLocation(), URLs.toFilePath(warUrl),
                parentClassLoader, contextRoot, moduleName, null);
        addWebModule(webModule, appModule);
    }

    public WebModule createWebModule(final String appId, final String warPath, final ClassLoader parentClassLoader,
            final String contextRoot, final String moduleName, final ExternalConfiguration config)
                    throws OpenEJBException {
        ...
        final String externalRepos = systemInstance
                .getProperty("tomee." + warFile.getName().replace(".war", "") + ".externalRepositories");
        ...
        final ClassLoaderConfigurer configurer = QuickJarsTxtParser
                .parse(new File(warFile, "WEB-INF/" + QuickJarsTxtParser.FILE_NAME));
        if (configurer != null) {
        ...
        final ClassLoader warClassLoader = ClassLoaderUtil.createTempClassLoader(appId, webUrlsArray,
                parentClassLoader);
        ...
        final WebModule webModule = new WebModule(webApp, contextRoot, warClassLoader, warFile.getAbsolutePath(),
                moduleName);
                ...
                synchronized (this) {
                    if (containerUrls == null) {
                        try {
                            ...
                            final boolean skipContainerFolders = "true".equalsIgnoreCase(SystemInstance.get()
                                    .getProperty("openejb.scan.webapp.container.skip-folder", "true"));
                            ...
                            while (it.hasNext()) { // remove lib/
                                ...
                                if ((skipContainerFolders && file.isDirectory())
                                        ...
                                        || name.endsWith("tomcat-websocket.jar") || name.startsWith("commons-jcs-")
                                        || name.startsWith("xx-arquillian-tomee")
                                        || ("lib".equals(name) && file.isDirectory()
                                                && new File(System.getProperty("openejb.base", "-"))
    ...
    public Class<? extends DeploymentModule> discoverModuleType(final URL baseUrl, final ClassLoader classLoader,
            final boolean searchForDescriptorlessApplications) throws IOException, UnknownModuleTypeException {
    ...
    public Class<? extends DeploymentModule> discoverModuleType(final URL baseUrl, final ClassLoader classLoader,
        ...
        final Class<? extends DeploymentModule> cls = checkAnnotations(baseUrl, classLoader, scanPotentialEjbModules,
        ...
        final Class<? extends DeploymentModule> defaultType = (Class<? extends DeploymentModule>) SystemInstance.get()
                .getOptions().get("openejb.default.deployment-module", (Class<?>) null);
        if (defaultType != null) {
