...
public final class OpenEjbContainer extends EJBContainer {
    ...
    private static boolean isSingleClose() {
        return OPENEJB_EJBCONTAINER_CLOSE_SINGLE
                .equals(SystemInstance.get().getProperty(OPENEJB_EJBCONTAINER_CLOSE, "by-invocation"));
    }
    ...
    public static class Provider implements EJBContainerProvider {
        ...
        private static boolean isOtherProvider(final Map<?, ?> properties) {
            ...
            return provider != null && !provider.equals(OpenEjbContainer.class)
                    && !provider.equals(OpenEjbContainer.class.getName()) && !"openejb".equals(provider);
        }
        ...
        public EJBContainer createEJBContainer(Map<?, ?> map) {
            ...
            if (instance != null || OpenEJB.isInitialized()) {
                if (!isSingleClose()) {
                    logger().info(
                            "EJBContainer already initialized.  Call ejbContainer.close() to allow reinitialization");
                }
                return instance;
            }
            ...
            try {
                ...
                if (SystemInstance.get().getComponent(ParentClassLoaderFinder.class) == null) {
                    ...
                    SystemInstance.get().setComponent(ParentClassLoaderFinder.class,
                            new ProvidedClassLoaderFinder(tccl));
                }
        ...
        private AppModule load(final Map<?, ?> map, final ConfigurationFactory configurationFactory)
                throws MalformedURLException, OpenEJBException {
        ...
        private AppModule load(final Map<?, ?> map) {
...

            {
                ...
                if (application != null) {
                    final AppModule newModule = new AppModule(appModule.getClassLoader(), appModule.getModuleId(),
                            application, false);
        ...
        private InvalidModulesPropertyException invalidModulesValue(final Object value) {
            ...
            return new InvalidModulesPropertyException(String.format("Invalid '%s' value '%s'. Valid values are: %s",
                    EJBContainer.MODULES, type, Join.join(", ", spec)));
        }
