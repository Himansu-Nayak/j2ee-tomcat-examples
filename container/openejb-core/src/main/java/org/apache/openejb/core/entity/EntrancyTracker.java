...
public class EntrancyTracker {
    /**
     * Thread local used to track the insances in the current call stack so we can determine if an nonreentrant instance
     * is being reentered.
     */
    ...
    public void enter(final BeanContext beanContext, final Object primaryKey) throws ApplicationException {
        ...
        if (!inCall.add(key)) {
            final ApplicationException exception = new ApplicationException(new RemoteException(
                    "Attempted reentrant access. " + "Bean " + deploymentId + " is not reentrant and instance "
                            + primaryKey + " has already been entered : " + inCall));
