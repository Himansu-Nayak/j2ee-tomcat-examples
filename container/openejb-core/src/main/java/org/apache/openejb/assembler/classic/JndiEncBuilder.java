 ...
 * to be what we assume them to be. We need to come up with a different internal naming structure for the global JNDI
 * and finally create the default which will be the default symlinked version of all the components.
 */
public class JndiEncBuilder {
    public static final Logger logger = Logger.getInstance(LogCategory.OPENEJB_STARTUP,
            JndiEncBuilder.class.getPackage().getName());
    ...
    public JndiEncBuilder(final JndiEncInfo jndiEnc, final Collection<Injection> injections, final String moduleId,
            final URI moduleUri, final String uniqueId, final ClassLoader classLoader, final Properties properties)
                    throws OpenEJBException {
        this(jndiEnc, injections, null, moduleId, moduleUri, uniqueId, classLoader, properties);
    }

    public JndiEncBuilder(final JndiEncInfo jndiEnc, final Collection<Injection> injections,
            final String transactionType, final String moduleId, final URI moduleUri, final String uniqueId,
            final ClassLoader classLoader, final Properties properties) throws OpenEJBException {
    ...
    public Map<String, Object> buildMap(final JndiScope scope) throws OpenEJBException {
        ...
        final JtaEntityManagerRegistry jtaEntityManagerRegistry = SystemInstance.get()
        ...
        for (final EjbReferenceInfo referenceInfo : jndiEnc.ejbReferences) {
            ...
            if (referenceInfo.location != null) {
                ...
                final String jndiName = "openejb/Deployment/"
                        + JndiBuilder.format(referenceInfo.ejbDeploymentId, referenceInfo.interfaceClassName,
                                referenceInfo.localbean ? InterfaceType.LOCALBEAN : InterfaceType.BUSINESS_REMOTE);
        ...
        for (final EjbReferenceInfo referenceInfo : jndiEnc.ejbLocalReferences) {
            ...
            if (referenceInfo.location != null) {
                ...
                final String jndiName = "openejb/Deployment/"
                        + JndiBuilder.format(referenceInfo.ejbDeploymentId, referenceInfo.interfaceClassName,
                                referenceInfo.localbean ? InterfaceType.LOCALBEAN : InterfaceType.BUSINESS_LOCAL);
                reference = new IntraVmJndiReference(jndiName);
            }
            bindings.put(normalize(referenceInfo.referenceName), reference);
        }
        ...
        for (final EnvEntryInfo entry : jndiEnc.envEntries) {
            ...
            try {
                ...
                throw new IllegalArgumentException(
                        "The env-entry-value for entry " + entry.referenceName + " was not recognizable as type "
                                + entry.type + ". Received Message: " + e.getLocalizedMessage(),
                        e);
            } catch (final MalformedURLException e) {
                throw new IllegalArgumentException(
                        "URL for reference " + entry.referenceName + " was not a valid URL: " + entry.value, e);
            }
        }
        ...
        for (final PersistenceContextReferenceInfo contextInfo : jndiEnc.persistenceContextRefs) {
            ...
            try {
                ...
                throw new OpenEJBException("PersistenceUnit '" + contextInfo.unitId + "' not found for EXTENDED ref '"
        ...
        for (final ServiceReferenceInfo referenceInfo : jndiEnc.serviceRefs) {
            ...
            if (!client) {
                final Reference reference = new JaxWsServiceReference(referenceInfo.id, referenceInfo.serviceQName,
                        serviceClass, referenceInfo.portQName, referenceClass, wsdlUrl, portRefs, handlerChains,
                        injections, properties);
                bindings.put(normalize(referenceInfo.referenceName), reference);
            } else {
                final ServiceRefData serviceRefData = new ServiceRefData(referenceInfo.id, referenceInfo.serviceQName,
                        serviceClass, referenceInfo.portQName, referenceClass, wsdlUrl, handlerChains, portRefs);
    ...
    private void addSpecialCompBindings(final Map<String, Object> bindings) {
        ...
        bindings.put("comp/ValidatorFactory",
                new IntraVmJndiReference(Assembler.VALIDATOR_FACTORY_NAMING_CONTEXT + uniqueId));
