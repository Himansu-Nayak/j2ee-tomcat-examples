...
public class InterceptorBindingBuilder {
    ...
    public static final Logger logger = Logger.getInstance(LogCategory.OPENEJB_STARTUP,
            InterceptorBindingBuilder.class.getPackage().getName());
    ...
    public void build(final BeanContext beanContext, final EnterpriseBeanInfo beanInfo) {
        ...
        if (beanInfo instanceof StatelessBeanInfo || beanInfo instanceof MessageDrivenBeanInfo) {
            /*
             * 4.3.10.2 and 4.5.8 If the stateless session bean or MDB instance has an ejbCreate method, the container
             * treats the ejbCreate method as the instance’s PostConstruct method, and, in this case, the PostConstruct
             * annotation (or deployment descriptor metadata) can only be applied to the bean’s ejbCreate method.
             */
        ...
        while (clazz != null && clazz != Object.class) {
            for (final Method method : clazz.getDeclaredMethods()) {
                final List<InterceptorData> methodInterceptors = createInterceptorDatas(method, beanInfo.ejbName,
                        this.bindings);
        ...
        final List<InterceptorData> callbackInterceptorDatas = createInterceptorDatas(null, beanInfo.ejbName,
                this.packageAndClassBindings);
    ...
    private List<InterceptorData> createInterceptorDatas(final Method method, final String ejbName,
            final List<InterceptorBindingInfo> bindings) {
        ...
        for (final InterceptorBindingInfo info : methodBindings) {
            ...
            for (final String interceptorClassName : classes) {
                ...
                if (interceptorData == null) {
                    logger.warning("InterceptorBinding references non-existent (undeclared) interceptor: "
    ...
    private List<InterceptorBindingInfo> processBindings(final Method method, final String ejbName,
            final List<InterceptorBindingInfo> bindings) {
    ...
    private boolean implies(final Method method, final String ejbName, final Level level,
            final InterceptorBindingInfo info) {
     ...
     * <p/>
     * - @PostConstruct <any-scope> void <method-name>(InvocationContext) - @PreDestroy <any-scope> void
     * <method-name>(InvocationContext) - @PrePassivate <any-scope> void <method-name>(InvocationContext)
     * - @PostActivate <any-scope> void <method-name>(InvocationContext) - @AroundInvoke <any-scope> Object
     * <method-name>(InvocationContext) throws Exception - @AroundTimeout <any-scope> Object
     * <method-name>(InvocationContext) throws Exception
     *
     * @param clazz
     * @param callbackInfos
     *            the raw CallbackInfo objects
     * @param callbacks
    ...
    private void toMethods(final Class<?> clazz, final List<CallbackInfo> callbackInfos, final Set<Method> callbacks) {
        ...
        for (final CallbackInfo callbackInfo : callbackInfos) {
            try {
                ...
                logger.warning("Interceptor method not found (skipping): public Object " + callbackInfo.method
                        + "(InvocationContext); in class " + clazz.getName());
            }
        }
     ...
     * Used for getting the java.lang.reflect.Method objects for the following callbacks:
     * <p/>
     * - @PostConstruct <any-scope> void <method-name>() - @PreDestroy <any-scope> void <method-name>() - @PrePassivate
     * <any-scope> void <method-name>() - @PostActivate <any-scope> void <method-name>() - @AfterBegin <any-scope> void
     * <method-name>() - @BeforeCompletion <any-scope> void <method-name>() - @AfterCompletion <any-scope> void
     * <method-name>(boolean)
     * <p/>
     * These apply to the bean class only, interceptor methods use InvocationContext as a parameter. The toMethods
     * method is used for those.
     *
     * @param clazz
     * @param callbackInfos
     * @param callbacks
     */
    private void toCallback(final Class<?> clazz, final List<CallbackInfo> callbackInfos, final Set<Method> callbacks,
        ...
        for (final CallbackInfo callbackInfo : callbackInfos) {
            ...
            try {
                ...
                final String message = "Bean Callback method not found (skipping): public void " + callbackInfo.method
    ...
    /**
     * Used by toMethods and toCallbacks to find the nearest java.lang.reflect.Method with the given name and
     * parameters. Callbacks can be private so class.getMethod() cannot be used. Searching starts by looking in the
     * specified class, if the method is not found searching continues with the immediate parent and continues
     * recurssively until the method is found or java.lang.Object is reached. If the method is not found a
     ...
     *             if the method is not found in this class or any of its parent classes
     */
