...
public class ApplicationComposerDeployer implements DynamicDeployer {
        ...
        for (final EjbModule ejbModule : appModule.getEjbModules()) {
            ...
            for (final Class<?> clazz : ejbModule.getFinder().findAnnotatedClasses(ApplicationComposer.class)) {
                ...
                final AnnotationFinder finder = new AnnotationFinder(
                        new ClassesArchive(org.apache.openejb.util.Classes.ancestors(clazz)));
    ...
    private Object configureModule(final AppModule appModule, final EjbModule ejbModule, final Class<?> clazz,
            Object instance, final Method m) {
        final int modifiers = m.getModifiers();
        if (!Modifier.isPublic(modifiers)) {
            ...
            if (EjbJar.class.isInstance(result)) {
                ...
                if (!persistence.getPersistenceUnit().isEmpty()) {
                    appModule.getPersistenceModules()
                            .add(new PersistenceModule(appModule, rootUrl(ejbModule), persistence));
                }
                ...
                appModule.addPersistenceModule(
                        new PersistenceModule(appModule, rootUrl(ejbModule), new Persistence(unit)));
    ...
    private void configureClasses(final WebModule web, final EjbModule ejbModule,
        ...
        final Archive finalArchive = classes.excludes().length > 0
                ? new FilteredArchive(archive, Filters.invert(Filters.prefixes(classes.excludes()))) : archive;
    ...
    private void configureDescriptor(final AppModule appModule, final Descriptor descriptor) {
        ...
        try {
            appModule.getAltDDs().put(descriptor.name(),
