...
public class InitEjbDeployments implements DynamicDeployer {
    ...
    public synchronized AppModule deploy(final AppModule appModule) throws OpenEJBException {
        ...
        for (final EjbModule ejbModule : appModule.getEjbModules()) {
            contextData.put("host", ejbModule.isWebapp() ? findHost(ejbModule.getModuleId(), appModule.getWebModules())
                    : appModule.uniqueHostIfExists());
    ...
    private EjbModule deploy(final EjbModule ejbModule, final Map<String, String> contextData,
            final Set<String> abstractSchemaNames) throws OpenEJBException {
            ...
            if (ejbDeployment == null) {
                ...
                if (annotation != null && isDefined(annotation.id())) {
                    ...
                    if (!(bean instanceof ManagedBean) || !((ManagedBean) bean).isHidden()) {
                        logger.info("Auto-deploying ejb " + bean.getEjbName() + ": EjbDeployment(deployment-id="
                ...
                if (annotation != null && isDefined(annotation.id())) {
                    ...
                    ejbDeployment.setDeploymentId(formatDeploymentId(bean, contextData, template));
                    logger.info("Auto-assigning deployment-id for ejb " + bean.getEjbName()
    ...
    private org.apache.openejb.api.EjbDeployment getEjbDeploymentAnnotation(final EjbModule ejbModule,
