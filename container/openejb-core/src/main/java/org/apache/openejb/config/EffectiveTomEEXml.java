...
public final class EffectiveTomEEXml {
    ...
    private static CommandLine parseCommand(final String[] args) throws SystemExitException {
        ...
        options.addOption(
                OptionBuilder.hasArg(true).withLongOpt("path").withDescription("[openejb|tomee].xml path").create("p"));
            ...
            for (final String config : asList("tomee.xml", "openejb.xml")) {
                xml = new File( // we shouldnt go to catalina.base, just a fallback
                        System.getProperty("openejb.base",
                                System.getProperty("openejb.home", System.getProperty("catalina.base", "missing"))),
                        config);
                if (xml.isFile()) {
                    break;
                }
