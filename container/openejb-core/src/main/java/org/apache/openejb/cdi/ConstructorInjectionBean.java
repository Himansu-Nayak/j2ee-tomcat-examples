...
public class ConstructorInjectionBean<T> extends InjectionTargetBean<T> { // TODO: see InjectableConstructor
    ...
    public ConstructorInjectionBean(final WebBeansContext webBeansContext, final Class<T> returnType,
            final AnnotatedType<T> at) {
        this(webBeansContext, returnType, at, null);
    }

    public ConstructorInjectionBean(final WebBeansContext webBeansContext, final Class<T> returnType,
            final AnnotatedType<T> at, final Boolean passivationCapable) {
        super(webBeansContext, WebBeansType.DEPENDENT, at,
                BeanAttributesBuilder.forContext(webBeansContext).newBeanAttibutes(at).build(), returnType);
        try {
            INJECTION_TARGET_FIELD.set(this,
                    new ConstructorInjectionTarget<T>(getAnnotatedType(), getInjectionPoints(), getWebBeansContext()));
    ...
    private static final class ConstructorInjectionTarget<T> extends InjectionTargetImpl<T> {
        public ConstructorInjectionTarget(final AnnotatedType<T> annotatedType, final Set<InjectionPoint> points,
