...
public class EjbJarBuilder {
    ...
    public HashMap<String, BeanContext> build(final EjbJarInfo ejbJar, final Collection<Injection> appInjections,
        ...
        final Context moduleJndiContext = new JndiEncBuilder(ejbJar.moduleJndiEnc, moduleInjections, null,
                ejbJar.moduleName, ejbJar.moduleUri, ejbJar.uniqueId, classLoader, context.getProperties())
        ...
        final ModuleContext moduleContext = !ejbJar.properties.containsKey("openejb.test.module")
                ? new ModuleContext(ejbJar.moduleName, ejbJar.moduleUri, ejbJar.uniqueId, context, moduleJndiContext,
                        classLoader)
                : new ModuleTestContext(ejbJar.moduleName, ejbJar.moduleUri, ejbJar.uniqueId, context,
        ...
        for (final EnterpriseBeanInfo ejbInfo : ejbJar.enterpriseBeans) {
            ...
            try {
                final EnterpriseBeanBuilder deploymentBuilder = new EnterpriseBeanBuilder(ejbInfo, moduleContext,
                        moduleInjections);
                ...
                if (container == null) {
                    throw new IllegalStateException("Container does not exist: " + ejbInfo.containerId
                            + ".  Referenced by deployment: " + bean.getDeploymentID());
                }
                ...
                throw new OpenEJBException("Error building bean '" + ejbInfo.ejbName + "'.  Exception: " + e.getClass()
                        + ": " + e.getMessage(), e);
            } finally {
