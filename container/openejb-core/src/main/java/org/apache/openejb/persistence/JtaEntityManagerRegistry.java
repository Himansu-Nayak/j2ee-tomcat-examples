...
/**
 * The JtaEntityManagerRegistry tracks JTA entity managers for transaction and extended scoped entity managers. A single
 * instance of this object should be created and shared by all JtaEntityManagers in the server instance. Failure to do
 * this will result in multiple entity managers being created for a single persistence until, and that will result in
 * cache incoherence.
 */
public class JtaEntityManagerRegistry {

    private static final Logger logger = Logger.getInstance(LogCategory.OPENEJB.createChild("persistence"),
    ...
    /**
     * Creates a JtaEntityManagerRegistry using the specified transactionSynchronizationRegistry for the registry if
     * transaction associated entity managers.
     */
     ...
     * entered and removes them when exited. If this registration is not performed, an IllegalStateException will be
     * thrown when entity manger is fetched.
     *
     * @param entityManagerFactory
     *            the entity manager factory from which an entity manager is required
     * @param properties
     *            the properties passed to the entity manager factory when an entity manager is created
     * @param extended
     *            is the entity manager an extended context
     * @param unitName
     * @return the new entity manager
     * @throws IllegalStateException
     *             if the entity manger is extended and there is not an existing entity manager instance already
     *             registered
     */
    @Geronimo
    public EntityManager getEntityManager(final EntityManagerFactory entityManagerFactory, final Map properties,
            final boolean extended, final String unitName, final SynchronizationType synchronizationType)
                    throws IllegalStateException {
    ...
    /**
     * Adds the entity managers for the specified component to the registry. This should be called when the component is
     * entered.
     *
     * @param deploymentId
     ...
     *             if an entity manager is already registered with the transaction for one of the supplied entity
     *             manager factories; for EJBs this should be caught and rethown as an EJBException
     */
    public void addEntityManagers(final String deploymentId, final Object primaryKey,
            final Map<EntityManagerFactory, EntityManagerTracker> entityManagers)
                    throws EntityManagerAlreadyRegisteredException {
     ...
     */
    public Map<EntityManagerFactory, EntityManagerTracker> removeEntityManagers(final String deploymentId,
    ...
    /**
     * Notifies the registry that a user transaction has been started or the specified component. When a transaction is
     * started for a component with registered extended entity managers, the entity managers are enrolled in the
     * transaction.
     *
     * @param deploymentId
     *            the id of the component
    ...
    /**
     * This object is used track all EntityManagers inherited in order to effectively close it when the latest Extended
     * persistence context is no more accessed.
     */
    ...
    private class ExtendedRegistry {
        ...
        private void addEntityManagers(final InstanceId instanceId,
                final Map<EntityManagerFactory, EntityManagerTracker> entityManagers)
                        throws EntityManagerAlreadyRegisteredException {
            if (instanceId == null) {
            ...
            if (isTransactionActive()) {
                for (final Map.Entry<EntityManagerFactory, EntityManagerTracker> entry : entityManagers.entrySet()) {
                    ...
                    if (oldEntityManager != null) {
                        throw new EntityManagerAlreadyRegisteredException(
        ...
        private EntityManagerTracker getInheritedEntityManager(final EntityManagerFactory entityManagerFactory) {
            ...
            for (final Map<EntityManagerFactory, EntityManagerTracker> entityManagers : entityManagersByDeploymentId
                    .values()) {
        ...
        private void transactionStarted(final InstanceId instanceId) {
            ...
            final Map<EntityManagerFactory, EntityManagerTracker> entityManagers = entityManagersByDeploymentId
                    .get(instanceId);
            if (entityManagers == null) {
                return;
            }
