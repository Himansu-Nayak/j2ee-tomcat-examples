...
public class CmpJarBuilder {
    ...
    private static synchronized JarOutputStream openJarFile(final CmpJarBuilder instance) throws IOException {
        ...
        try {
            ...
            Logger.getInstance(LogCategory.OPENEJB_STARTUP, CmpJarBuilder.class)
                    .warning("Failed to create temp jar file in: " + dir, e);
        ...
        Logger.getInstance(LogCategory.OPENEJB_STARTUP, CmpJarBuilder.class)
                .debug("Using temp jar file: " + instance.jarFile);
    ...
    /**
     * Generate the CMP jar file associated with this deployed application. The generated jar file will contain
     * generated classes and metadata that will allow the JPA engine to manage the bean persistence.
     *
     * @throws IOException
     */
    private void generate() throws IOException {
        ...
        try {
            ...
            if (appInfo.cmpMappingsXml != null) {
                ...
                addJarEntry(jarOutputStream, "META-INF/openejb-cmp-generated-orm.xml",
                        appInfo.cmpMappingsXml.getBytes());
            }
        } catch (final Throwable e) {
    ...
    /**
     * Test if an application contains and CMP beans that need to be mapped to the JPA persistence engine. This will
     * search all of the ejb jars contained within the application looking for Entity beans with a CONTAINER persistence
     * type.
     *
     * @return true if the application uses container managed beans, false if none are found.
     */
    ...
    /**
     * Generate a class file for a CMP bean, writing the byte data for the generated class into the jar file we're
     * constructing.
     *
     ...
     * @throws IOException
     */
    private Entry generateClass(final JarOutputStream jarOutputStream, final EntityBeanInfo entityBeanInfo)
            throws IOException {
        // don't generate if there is aleady an implementation class
        final String cmpImplClass = CmpUtil.getCmpImplClassName(entityBeanInfo.abstractSchemaName,
        ...
        if (entityBeanInfo.primKeyClass != null) {
            try {
                ...
                throw (IOException) new IOException(
                        "Could not find entity primary key class " + entityBeanInfo.primKeyClass).initCause(e);
            }
        }
        ...
        if (entityBeanInfo.cmpVersion != 2) {
            ...
            final Cmp2Generator cmp2Generator = new Cmp2Generator(cmpImplClass, beanClass, entityBeanInfo.primKeyField,
                    primKeyClass,
            ...
            for (final CmrFieldInfo cmrFieldInfo : entityBeanInfo.cmrFields) {
                ...
                final CmrField cmrField = new CmrField(cmrFieldInfo.fieldName, cmrFieldInfo.fieldType,
                        CmpUtil.getCmpImplClassName(roleSource.abstractSchemaName, roleSource.ejbClass),
                        roleSource.local, cmrFieldInfo.mappedBy.fieldName, cmrFieldInfo.synthetic);
     ...
     *            The target jar file.
     * @param fileName
     *            The name we're inserting.
     * @param bytes
     *            The file byte data.
     * @throws IOException
     */
    private void addJarEntry(final JarOutputStream jarOutputStream, String fileName, final byte[] bytes)
            throws IOException {
