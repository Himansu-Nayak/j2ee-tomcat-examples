...
public class DynamicSubclass implements Opcodes {
    ...
    public static Class createSubclass(final Class<?> abstractClass, final ClassLoader cl) {
        ...
        try {
            ...
            throw new InternalError(
                    DynamicSubclass.class.getSimpleName() + ".createSubclass: " + Debug.printStackTrace(e));
        } finally {
            lock.unlock();
        }
    }
    ...
    private static byte[] generateBytes(final Class<?> classToProxy) throws ProxyGenerationException {
        ...
        cw.visitField(ACC_FINAL + ACC_PRIVATE, "this$handler", "Ljava/lang/reflect/InvocationHandler;", null, null)
                .visitEnd();
        ...
        for (final Map.Entry<String, List<Method>> entry : methodMap.entrySet()) {
            ...
            for (final Method method : entry.getValue()) {
                if (Modifier.isAbstract(method.getModifiers())) {
                    final MethodVisitor visitor = LocalBeanProxyFactory.visit(cw, method, proxyClassFileName,
                            "this$handler");
    ...
    private static MethodVisitor visitConstructor(final ClassWriter cw, final String proxyClassFileName,
    ...
    private static void copyMethodAnnotations(final Class<?> classToProxy, final Map<String, MethodVisitor> visitors)
    ...
    private static void copyClassAnnotations(final Class<?> clazz, final ClassVisitor newClass)
            throws ProxyGenerationException {
        try {
            final ClassReader classReader = new ClassReader(readClassFile(clazz));
