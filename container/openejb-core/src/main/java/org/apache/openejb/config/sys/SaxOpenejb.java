...
/**
 * Plain Java code for parsing a tomee.xml or openejb.xml file as JAXB loading is so slow.
 *
 * @version $Rev$ $Date$
 */
class SaxOpenejb extends StackHandler {
    ...
    public static Openejb parse(final InputSource source)
    ...
    private class ImportElement extends DefaultHandler {
        ...
        private void updatePath() {
            if (path != null && path.startsWith(HOME_VAR)) {
                path = SystemInstance.get().getHome().getDirectory().getAbsolutePath()
        ...
        private void merge(final Openejb openejb, final Openejb importedOpenEJB) {
            ...
            if (importedOpenEJB.securityService != null || importedOpenEJB.transactionManager != null
                    || importedOpenEJB.connectionManager != null || importedOpenEJB.proxyFactory != null) { //NOPMD
