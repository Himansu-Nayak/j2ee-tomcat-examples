...
import static org.apache.openejb.core.ivm.IntraVmCopyMonitor.State.*;
...
public abstract class BaseEjbProxyHandler implements InvocationHandler, Serializable {
    ...
    /*
     * The EJB 1.1 specification requires that arguments and return values between beans adhere to the Java RMI copy
     * semantics which requires that the all arguments be passed by value (copied) and never passed as references.
     * However, it is possible for the system administrator to turn off the copy operation so that arguments and return
     * values are passed by reference as performance optimization. Simply setting the
     * org.apache.openejb.core.EnvProps.INTRA_VM_COPY property to FALSE will cause this variable to set to false, and
     * therefor bypass the copy operations in the invoke( ) method of this class; arguments and return values will be
     * passed by reference not value.
     *
     * This property is, by default, always TRUE but it can be changed to FALSE by setting it as a System property or a
     * property of the Property argument when invoking OpenEJB.init(props). This variable is set to that property in the
     * static block for this class.
     */
    ...
    public BaseEjbProxyHandler(final BeanContext beanContext, final Object pk, final InterfaceType interfaceType,
            List<Class> interfaces, Class mainInterface) {
        ...
        if (interfaces == null || interfaces.size() == 0) {
            final InterfaceType objectInterfaceType = interfaceType.isHome() ? interfaceType.getCounterpart()
                    : interfaceType;
        ...
        if (mainInterface == null) {
            throw new IllegalArgumentException("No mainInterface: otherwise di: " + beanContext + " InterfaceType: "
    ...
    /**
     * This method should be called to determine the corresponding business interface class to name as the invoking
     * interface. This method should NOT be called on non-business-interface methods the proxy has such as
     * java.lang.Object or IntraVmProxy.
     *
     * @param method
     *            Method
    ...
    protected Class<?> getInvokedInterface(final Method method) {
        ...
        throw new IllegalStateException(
                "Received method invocation and cannot determine corresponding business interface: method=" + method);
    }
    ...
    private <T> T copy(final IntraVmCopyMonitor.State strategy, final T object)
    ...
    private void isValidReference(final Method method) throws NoSuchObjectException {
        ...
        if (!(Object.class.equals(method.getDeclaringClass()) && method.getName().equals("finalize")
                && method.getExceptionTypes().length == 1 && Throwable.class.equals(method.getExceptionTypes()[0]))) {
            getBeanContext(); // will throw an exception if app has been undeployed.
        }
    }

    /**
     * Renamed method so it shows up with a much more understandable purpose as it will be the top element in the
     * stacktrace
     *
     * @param e
     *            Throwable
    ...
    /**
     * Method instance on proxies that come from a classloader outside the bean's classloader need to be swapped out for
     * the identical method in the bean's classloader.
     *
     * @param method
     *            Method
    ...
    protected boolean equalHandler(final BaseEjbProxyHandler other) {
        return (primaryKey == null ? other.primaryKey == null : primaryKey.equals(other.primaryKey))
                && deploymentID.equals(other.deploymentID) && getMainInterface().equals(other.getMainInterface());
    }
    ...
    protected Object[] copyArgs(final Object[] objects) throws IOException, ClassNotFoundException {
        ...
        /*
         * while copying the arguments is necessary. Its not necessary to copy the array itself, because they array is
         * created by the Proxy implementation for the sole purpose of packaging the arguments for the
         * InvocationHandler.invoke( ) method. Its ephemeral and their for doesn't need to be copied.
         */
    ...
    protected <T> T copyObj(final T object) throws IOException, ClassNotFoundException {
        ...
        if (ooc == int.class || ooc == String.class || ooc == long.class || ooc == boolean.class || ooc == byte.class
                || ooc == float.class || ooc == double.class || ooc == short.class || ooc == Long.class
                || ooc == Boolean.class || ooc == Byte.class || ooc == Character.class || ooc == Float.class
                || ooc == Double.class || ooc == Short.class || ooc == BigDecimal.class) {
            return object;
        }
        ...
        try {
            ...
            throw (IOException) new NotSerializableException(e.getMessage()
                    ...
                    + OPENEJB_LOCALCOPY + "=false system property.").initCause(e);
        }
        ...
        if (tc != null && tc.getBeanContext() != beanContext /* parent bean */
            ...
            try {
                ...
                if (tx != null && tx.getStatus() == Status.STATUS_ACTIVE) {
                    final TransactionSynchronizationRegistry registry = systemInstance
