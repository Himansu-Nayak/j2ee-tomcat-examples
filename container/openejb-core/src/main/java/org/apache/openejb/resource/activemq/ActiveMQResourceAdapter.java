...
public class ActiveMQResourceAdapter extends org.apache.activemq.ra.ActiveMQResourceAdapter {
    ...
    public void setStartupTimeout(final Duration startupTimeout) {
        ...
        this.startupTimeout = String
                .valueOf(TimeUnit.MILLISECONDS.convert(startupTimeout.getTime(), startupTimeout.getUnit()));
    }
    ...
    private void createInternalBroker(final String brokerXmlConfig, final Properties properties) {
        ...
        try {
            ...
            Logger.getInstance(LogCategory.OPENEJB_STARTUP, ActiveMQResourceAdapter.class).getChildLogger("service")
                    .fatal("Failed to start ActiveMQ", e);
        } finally {
    ...
    public void stop() {
        ...
        final Thread stopThread = new Thread("ActiveMQResourceAdapter stop") {
            ...
            public void run() {
                try {
                    ...
                    Logger.getInstance(LogCategory.OPENEJB_STARTUP, ActiveMQResourceAdapter.class)
                            .getChildLogger("service").error("ActiveMQ shutdown failed", t);
                }
            }
        };
        ...
        try {
            ...
            Logger.getInstance(LogCategory.OPENEJB_STARTUP, ActiveMQResourceAdapter.class).getChildLogger("service")
                    .warning("Gave up on ActiveMQ shutdown after " + timeout + "ms", ex);
        }
    }
