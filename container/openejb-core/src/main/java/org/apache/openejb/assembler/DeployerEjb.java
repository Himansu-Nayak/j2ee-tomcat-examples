...
public class DeployerEjb implements Deployer {
    ...
    private static final boolean oldWarDeployer = "old"
            .equalsIgnoreCase(SystemInstance.get().getOptions().get("openejb.deployer.war", "new"));
    private static final String OPENEJB_DEPLOYER_SAVE_DEPLOYMENTS = "openejb.deployer.save-deployments";
    private static final boolean SAVE_DEPLOYMENTS = SystemInstance.get().getOptions()
            .get(OPENEJB_DEPLOYER_SAVE_DEPLOYMENTS, false);
    ...
    public AppInfo deploy(final String inLocation, Properties properties) throws OpenEJBException {
        ...
        if (WebAppDeployer.Helper.isWebApp(file) && !oldWarDeployer) {
            ...
            try {
                final AppInfo appInfo = SystemInstance.get().getComponent(WebAppDeployer.class).deploy(host,
                        contextRoot(properties, file.getAbsolutePath()), file);
                if (appInfo != null) {
    ...
    private synchronized void saveDeployment(final File file, final boolean add) {
        ...
        if (config == null || !config.getParentFile().exists()) {
            LOGGER.info(
                    "Cannot save the added app because the conf folder does not exist, it will not be present on a restart");
            return;
        }
    ...
    public void undeploy(final String moduleId) throws UndeployException, NoSuchApplicationException {
        ...
        if (appInfo == null) {
            ...
            if (appInfo == null) {
                ...
                if (appInfo == null) {
                    appInfo = assembler
                            .getAppInfo(new File(realLocation(moduleId).iterator().next()).getAbsolutePath());
                }
            }
        }
        if (appInfo != null) {
