...
public abstract class AbstractUnivariateStatistic implements UnivariateStatistic {
    ...
    /**
     * This method is used by <code>evaluate(double[], int, int)</code> methods to verify that the input parameters
     * designate a subarray of positive length.
     * <p>
     * <ul>
    ...
    /**
     * This method is used by <code>evaluate(double[], double[], int, int)</code> methods to verify that the begin and
     * length parameters designate a subarray of positive length and the weights are all non-negative, non-NaN, finite,
     * and not all zero.
     * <p>
     * <ul>
    ...
    protected boolean test(final double[] values, final double[] weights, final int begin, final int length) {
        ...
        for (int i = begin; i < begin + length; i++) {
            ...
            if (weights[i] < 0) {
                throw MathRuntimeException.createIllegalArgumentException("negative weight {0} at index {1} ",
                        weights[i], i);
            }
        ...
        if (!containsPositiveWeight) {
            throw MathRuntimeException
                    .createIllegalArgumentException("weight array must contain at least one non-zero value");
        }
