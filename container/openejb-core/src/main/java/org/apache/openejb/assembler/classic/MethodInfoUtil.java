...
public class MethodInfoUtil {
    ...
    /**
     * Finds the nearest java.lang.reflect.Method with the given NamedMethodInfo Callbacks can be private so
     * class.getMethod() cannot be used. Searching starts by looking in the specified class, if the method is not found
     * searching continues with the immediate parent and continues recurssively until the method is found or
     * java.lang.Object is reached. If the method is not found a IllegalStateException is thrown.
     *
     * @param clazz
     * @param methodName
        ...
        while (clazz != null) {
            try {
                ...
                if (noSuchMethod == null) {
                    noSuchMethod = new IllegalStateException(
                            "Callback method does not exist: " + clazz.getName() + "." + info.methodName, e);
                }
    ...
    /**
     * This method splits the MethodPermissionInfo objects so that there is exactly one MethodInfo per
     * MethodPermissionInfo. A single MethodPermissionInfo with three MethodInfos would be expanded into three
     * MethodPermissionInfo with one MethodInfo each.
     * <p/>
    ...
    public static Map<Method, MethodAttributeInfo> resolveAttributes(final List<? extends MethodAttributeInfo> infos,
    ...
    public static Map<ViewMethod, MethodAttributeInfo> resolveViewAttributes(
            final List<? extends MethodAttributeInfo> infos, final BeanContext beanContext) {
    ...
    private static boolean containerMethod(final Method method) {
        return (method.getDeclaringClass() == EJBObject.class || method.getDeclaringClass() == EJBHome.class
                || method.getDeclaringClass() == EJBLocalObject.class
                || method.getDeclaringClass() == EJBLocalHome.class) && !method.getName().equals("remove");
    }
