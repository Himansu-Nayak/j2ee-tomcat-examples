...
public class AutoConfig implements DynamicDeployer, JndiConstants {
    ...
    private static String chooseType(final ClassLoader classLoader, final ResourceInfo info, final String defaultType) {
        ...
        return info.className != null ? ((canLoad(classLoader, info.className) ? info.className : defaultType))
                : defaultType;
    }
    ...
    private static String getVendorUrl(final Properties properties) {
        ...
        if (driver.startsWith("org.apache.derby.jdbc.Embedded")) { // Driver or DataSource
            return String.format("jdbc:derby:%s%s", databaseName,
                    properties.getProperty("connectionAttributes", ";create=true"));
        }

        if (driver.equals("org.apache.derby.jdbc.ClientDriver")) {
            return String.format("jdbc:derby://%s:%s/%s%s", serverName, port, databaseName,
                    properties.getProperty("connectionAttributes", ";create=true"));
        }
    ...
    private void processPersistenceRefs(final JndiConsumer component, final DeploymentModule module,
            final LinkResolver<PersistenceUnit> persistenceUnits, final URI moduleURI) {
    ...
    private PersistenceUnit processPersistenceRef(final LinkResolver<PersistenceUnit> persistenceUnits,
            final PersistenceRef ref, final URI moduleURI, final String componentName,
        ...
        if (unit != null) {
            ...
            if (availableUnits.size() == 0) {
            ...
            } else if ((ref.getPersistenceUnitName() == null || ref.getPersistenceUnitName().length() == 0)
                ...
                validation.fail(componentName, refType + ".noUnitName", refShortName, Join.join(", ", availableUnits),
                        sampleUnitName);
            } else {
                ...
                if (vagueMatches.size() != 0) {
                    ...
                    validation.fail(componentName, refType + ".vagueMatches", refShortName, unitName,
                            possibleUnits.size(), Join.join("\n", possibleUnits));
                } else {
                    validation.fail(componentName, refType + ".noMatches", refShortName, unitName,
                            Join.join(", ", availableUnits));
                }
            }
        }
        return unit;
        ...
        for (final EnterpriseBean bean : ejbModule.getEjbJar().getEnterpriseBeans()) {
            if (bean instanceof MessageDrivenBean) {
                ...
                String destination = properties.getProperty("destinationName",
                        properties.getProperty("destinationLookup"));
                ...
                if ("javax.jms.Topic".equals(destinationType)) {
                    if (Boolean.parseBoolean(SystemInstance.get()
                            .getProperty("openejb.activemq.deploymentId-as-clientId", ejbModule.getProperties()
                                    .getProperty("openejb.activemq.deploymentId-as-clientId", "true")))
                    ...
                    if (!properties.containsKey("subscriptionName")) {
                        mdb.getActivationConfig().addProperty("subscriptionName",
                                ejbDeployment.getDeploymentId() + "_subscription");
                    }
                }
        ...
        for (final EjbModule ejbModule : appModule.getEjbModules()) {
            ...
            if (assembly != null) {
                for (final MessageDestination destination : assembly.getMessageDestination()) {
                    destinationResolver.add(ejbModule.getModuleUri(), destination.getMessageDestinationName(),
                            destination);
                }
            }
        }
        ...
        for (final EjbModule ejbModule : appModule.getEjbModules()) {
            ...
            for (final EnterpriseBean bean : ejbModule.getEjbJar().getEnterpriseBeans()) {
                ...
                if (bean instanceof MessageDrivenBean) {
                    ...
                    if (destination == null) {
                        throw new OpenEJBException("Message destination " + link + " for message driven bean "
                                + mdb.getEjbName() + " not found");
                    }
            ...
            for (final EnterpriseBean bean : ejbModule.getEjbJar().getEnterpriseBeans()) {
                ...
                for (final MessageDestinationRef ref : bean.getMessageDestinationRef()) {
                    ...
                    if (ref.getMappedName() == null && ejbDeployment.getResourceLink(ref.getName()) == null) {
                        final String destinationId = resolveDestinationId(ref, moduleUri, destinationResolver,
                                destinationTypes);
                        if (destinationId != null) {
            ...
            for (final EnterpriseBean bean : ejbModule.getEjbJar().getEnterpriseBeans()) {
                ...
                if (bean instanceof MessageDrivenBean) {
                    ...
                    if (destinationType == null) {
                        ...
                        logger.info("Auto-configuring a message driven bean " + ejbDeployment.getDeploymentId()
                                + " destination " + properties.getProperty("destination") + " to be destinationType "
                                + destinationType);
                    }
    ...
    private String resolveDestinationId(final MessageDestinationRef ref, final URI moduleUri,
        ...
        final MessageDestination destination = destinationResolver.resolveLink(link, moduleUri);
        if (destination == null) {
            throw new OpenEJBException("Message destination " + link + " for message-destination-ref "
                    + ref.getMessageDestinationRefName() + " not found");
        }
    ...
    private void processJndiRefs(final String moduleId, final JndiConsumer jndiConsumer,
            final AppResources appResources, final ClassLoader classLoader) throws OpenEJBException {
        // Resource reference
    ...
    private void deploy(final EjbModule ejbModule, final AppResources appResources) throws OpenEJBException {
        ...
        for (final EnterpriseBean bean : ejbModule.getEjbJar().getEnterpriseBeans()) {
            ...
            final Class<? extends ContainerInfo> containerInfoType = ConfigurationFactory
                    .getContainerInfoType(getType(bean));
            ...
            if (autoCreateResources && bean instanceof MessageDrivenBean) {
                ...
                if (resourceLink != null) {
                    try {
                        final String destinationId = getResourceEnvId(bean.getEjbName(), resourceLink.getResId(),
                                mdb.getMessageDestinationType(), appResources);
                        resourceLink.setResId(destinationId);
    ...
    private void processApplicationContainers(final AppModule module, final AppResources appResources)
        ...
        for (final Container container : module.getContainers()) {
            if (container.getId() == null) {
                throw new IllegalStateException(
                        "a container can't get a null id: " + container.getType() + " from " + module.getModuleId());
            }
    ...
    private void processApplicationResources(final AppModule module) throws OpenEJBException {
        ...
        for (final Resource resource : resources) {
            ...
            final ResourceRef strictRef = new ResourceRef(OPENEJB_RESOURCE_JNDI_PREFIX + originalId,
                    resourceRef.getResType(), resourceRef.getResAuth(), resourceRef.getResSharingScope());
    ...
    private String createContainer(final Class<? extends ContainerInfo> containerInfoType,
            final EjbDeployment ejbDeployment, final EnterpriseBean bean) throws OpenEJBException {
        ...
        logger.info("Auto-creating a container for bean " + ejbDeployment.getDeploymentId() + ": Container(type="
                + getType(bean) + ", id=" + containerInfo.id + ")");
    ...
    private void processResourceRef(final ResourceRef ref, final EjbDeployment ejbDeployment,
            final AppResources appResources, final EjbModule ejbModule) throws OpenEJBException {
        ...
        if (ignoredReferenceTypes.contains(refType)) {
            final ResourceInfo resourceInfo = configFactory
                    .getResourceInfo(refName.replace("java:", "").replace("comp/env/", ""));
            if (resourceInfo != null) {
                ref.setMappedName("jndi:" + (resourceInfo.id.startsWith("java:") ? resourceInfo.id
                        : "openejb:Resource/" + resourceInfo.id));
            }
            return;
        }
        ...
        try {
            ...
            if (link == null) {
                ...
                try {
                    ...
                    if (appModule != null) {
                        final String newId = findResourceId(
                                appModule.getModuleId() + '/' + id.replace("java:", "").replaceAll("^comp/env/", ""),
                                refType, new Properties(), appResources);
                    ...
                    throw new OpenEJBException(
                            "Can't find resource for " + ref.getOrigin() + ". (" + e.getMessage() + ")", e.getCause());
                }
                logger.info("Auto-linking resource-ref '" + refName + "' in bean " + ejbDeployment.getDeploymentId()
                        + " to Resource(id=" + id + ")");
                ...
                final String id = getResourceId(ejbDeployment.getDeploymentId(), link.getResId(), refType,
                        appResources);
                link.setResId(id);
                link.setResRefName(refName);
            }
    ...
    private void processResourceEnvRef(final JndiReference ref, final EjbDeployment ejbDeployment,
            final AppResources appResources, final ClassLoader classLoader) throws OpenEJBException {
        // skip destinations with lookup name
        if (ref.getLookupName() != null) {
            return;
        ...
        if (link == null) {
            ...
            logger.info("Auto-linking resource-env-ref '" + refName + "' in bean " + ejbDeployment.getDeploymentId()
                    + " to Resource(id=" + id + ")");
        ...
        if (classLoader != null) {
            ...
            for (final InjectionTarget injection : injections) {
                try {
                    ...
                    final Class type = IntrospectionSupport.getPropertyType(target,
                            injection.getInjectionTargetName().trim());
                    return type.getName();
    ...
    private void deploy(final AppModule app, final PersistenceModule persistenceModule) throws OpenEJBException {
        ...
        for (final PersistenceUnit unit : persistence.getPersistenceUnit()) {
            ...
            if (unit.getJtaDataSource() == null && unit.getNonJtaDataSource() == null
                    && "true".equalsIgnoreCase(SystemInstance.get().getProperty("openejb.force-unit-type",
                            unit.getProperty("openejb.force-unit-type", "true")))) {
            ...
            final boolean resourceLocal = TransactionType.RESOURCE_LOCAL.equals(unit.getTransactionType())
                    && unit.getJtaDataSource() == null;
            if (resourceLocal && unit.getNonJtaDataSource() == null
                    && isDataSourcePropertiesConfigured(unit.getProperties())) {
                continue;
            }
            ...
            for (final String potentialName : asList(prefix + jtaWithJavaAndSlash, originalJtaDataSource,
                ...
                if (jtaInfo != null) {
                    if (!"false".equalsIgnoreCase(jtaInfo.properties.getProperty("JtaManaged")) // don't test true since it can be missing
                        ...
                        logger.warning(
                                "Found matching datasource: " + jtaInfo.id + " but this one is not a JTA datasource");
                    }
                }
            }

            final String nonJtaWithJavaAndSlash = replaceJavaAndSlash(unit.getNonJtaDataSource());
            for (final String potentialName : asList(prefix + nonJtaWithJavaAndSlash, originalNonJtaDataSource,
                    nonJtaWithJavaAndSlash)) {
                if (potentialName == null) {
            ...
            if (jtaDataSourceId == null && !resourceLocal) {
                ...
                /*
                 * this shouldn't be mandatory anymore since our DataSource has as alias javax.sql.DataSource if
                 * (jtaDataSourceId == null) { jtaDataSourceId =
                 * findResourceId(replaceJavaAndSlash(unit.getJtaDataSource()), "DataSource", required, null); }
                 */
            }
            ...
            if (possibleJta != null && possibleJta.equals(possibleNonJta)) {
                ...
                logger.warning("PeristenceUnit(name=" + unit.getName() + ") invalidly refers to Resource(id="
                        + dataSource.id + ") as both its <jta-data-source> and <non-jta-data-source>.");
            ...
            if (jtaDataSourceId == null && nonJtaDataSourceId == null) {
                ...
                if (jtaDataSourceId != null || nonJtaDataSourceId != null) {
                    ...
                    final ResourceInfo jtaResourceInfo = configFactory.configureService(jtaResource,
                            ResourceInfo.class);
                    final ResourceInfo nonJtaResourceInfo = configFactory.configureService(nonJtaResource,
                            ResourceInfo.class);
                    if (jtaDataSourceId != null && nonJtaDataSourceId == null) {
            ...
            if (jtaDataSourceId != null && nonJtaDataSourceId == null) {
                ...
                if (jtaProperties.containsKey("JtaManaged")) {
                    ...
                    if (nonJtaDataSourceId == null) {
                        ...
                        final Properties overrides = ConfigurationFactory.getSystemProperties(nonJtaResourceInfo.id,
                        ...
                        logAutoCreateResource(nonJtaResourceInfo, "DataSource", unit.getName());
                        logger.info("configureService.configuring", nonJtaResourceInfo.id, nonJtaResourceInfo.service,
                                jtaResourceInfo.id);
            ...
            // hibernate uses the fact that this ds is missing to get a non jta em instead of a JTA one
                    && (!resourceLocal || deduceJtaFromNonJta != null) && (deduceJtaFromNonJta == null
                ...
                if (nonJtaProperties.containsKey("JtaManaged")) {
                    ...
                    if (jtaDataSourceId == null) {
                        ...
                        final Properties overrides = ConfigurationFactory.getSystemProperties(jtaResourceInfo.id,
                        ...
                        logAutoCreateResource(jtaResourceInfo, "DataSource", unit.getName());
                        logger.info("configureService.configuring", jtaResourceInfo.id, jtaResourceInfo.service,
                                nonJtaResourceInfo.id);
    ...
    private boolean isDataSourcePropertiesConfigured(final Properties properties) {
        return "true"
                .equals(SystemInstance.get()
                        .getProperty("openejb.guess.resource-local-datasource-properties-configured", "true"))
                && (properties.containsKey("javax.persistence.jdbc.driver")
                        || properties.containsKey("javax.persistence.jdbc.url"));
    }
    ...
    private void setNonJtaDataSource(final PersistenceUnit unit, final String current) {
        ...
        if (!current.equals(previous)) {
            ...
            logger.info("Adjusting PersistenceUnit " + unit.getName() + " <non-jta-data-source> to Resource ID '"
    ...
    private void setJtaDataSource(final PersistenceUnit unit, final String current) {
        ...
        if (!current.equals(previous)) {
            ...
            logger.info("Adjusting PersistenceUnit " + unit.getName() + " <jta-data-source> to Resource ID '" + current
                    + "' from '" + previous + "'");
    ...
    private void checkUnitDataSourceRefs(final PersistenceUnit unit) throws OpenEJBException {
        ...
        if (invalidNonJta != null) {
            throw new OpenEJBException("PeristenceUnit " + unit.getName()
                    + " <non-jta-data-source> points to a jta managed Resource.  Update Resource \"" + invalidNonJta
        ...
        if (invalidJta != null) {
            throw new OpenEJBException("PeristenceUnit " + unit.getName()
                    + " <jta-data-source> points to a non jta managed Resource.  Update Resource \"" + invalidJta
    ...
    private String getResourceId(final String beanName, final String resourceId, final String type,
            final AppResources appResources) throws OpenEJBException {
        return getResourceId(beanName, resourceId, type, null, appResources);
    }

    private String getResourceId(final String beanName, String resourceId, final String type, final Properties required,
            AppResources appResources) throws OpenEJBException {
        ...
        final String message = "No existing resource found while attempting to Auto-link unmapped resource-ref '"
                + resourceId + "' of type '" + type + "' for '" + beanName + "'.  Looked for Resource(id=" + resourceId
                + ") and Resource(id=" + shortName + ")";
        if (!autoCreateResources) {
    ...
    private String autoCreateResource(final String type, final Properties required, final String beanName)
            throws OpenEJBException {
        final String resourceId;
        resourceId = ServiceUtils.getServiceProviderId(type, required);
        if (resourceId == null) {
            throw new OpenEJBException("No provider available for resource-ref '" + resourceId + "' of type '" + type
                    + "' for '" + beanName + "'.");
        }
        final ResourceInfo resourceInfo = configFactory.configureService(resourceId, ResourceInfo.class);
    ...
    private String firstMatching(final String prefix, final String type, final Properties required,
    ...
    private String findResourceId(final String resourceId, final String type, final Properties required,
            final AppResources appResources) {
        if (resourceId == null) {
            return null;
        }
    ...
    private String getResourceEnvId(final String beanName, String resourceId, final String type,
            AppResources appResources) throws OpenEJBException {
        if (resourceId == null) {
            return null;
        }
        ...
        if (providerId == null) {
            ...
            throw new OpenEJBException("No provider available for resource-env-ref '" + resourceId + "' of type '"
                    + type + "' for '" + beanName + "'.");
        }
    ...
    private String getUsableContainer(final Class<? extends ContainerInfo> containerInfoType, final Object bean,
            final AppResources appResources) {
    ...
    private String matchContainer(final Class<? extends ContainerInfo> containerInfoType, final Object bean,
            final Collection<ContainerInfo> list) {
