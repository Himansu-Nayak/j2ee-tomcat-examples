...
public class JaxWsServiceReference extends Reference {
    ...
    public JaxWsServiceReference(final String id, final QName serviceQName, final Class<? extends Service> serviceClass,
            final QName portQName, final Class<?> referenceClass, final URL wsdlUrl, final List<PortRefData> portRefs,
            final List<HandlerChainData> handlerChains, final Collection<Injection> injections,
    ...
    public Object getObject() throws NamingException {
        ...
        final WebServiceClientCustomizer customizer = SystemInstance.get()
                .getComponent(WebServiceClientCustomizer.class);
            ...
            if (Service.class.equals(serviceClass)) {
                ...
                try {
                    ...
                    throw (NamingException) new NamingException(
                            "Could not instantiate jax-ws service class " + serviceClass.getName()).initCause(e);
                }
            }
        } finally {
        ...
        if (!handlerChains.isEmpty()) {
            final HandlerResolver handlerResolver = new HandlerResolverImpl(handlerChains, injections,
                    new InitialContext());
        ...
        final ServiceRefData serviceRefData = new ServiceRefData(id, serviceQName, serviceClass, portQName,
                referenceClass, wsdlUrl, handlerChains, portRefs);
        ...
        if (portAddressRegistry == null) {
            ...
            if (portAddressRegistry == null) {
                throw new OpenEJBRuntimeException(
                        "No port address registry, it generally means you either didn't activate cxf or don't use tomee+");
            }
        }
