...
public class StackHandler extends DefaultHandler {
    ...
    protected void checkAttributes(final Attributes attributes, final List<String> allowed) throws SAXException {
        ...
        if (invalid.size() > 0) {
            throw new SAXException("Unsupported Attribute(s): " + Join.join(", ", invalid)
                    + ".  Supported Attributes are: " + Join.join(", ", allowed)
                    + ".  If the setting is a configuration property it must be placed inside the element body.");
        }
    }
