...
public class SingletonInstanceManager {
            ...
            if (context.getBean() instanceof SessionBean) {
                ...
                try {
                    ...
                    final InterceptorStack ejbCreate = new InterceptorStack(context.getBean(), create, Operation.CREATE,
                            new ArrayList<InterceptorData>(), new HashMap());
                    ejbCreate.invoke();
                } finally {
    ...
    public void freeInstance(final ThreadContext callContext) {
        ...
        try {
            ...
            Thread.interrupted();
            logger.error(
                    "Singleton shutdown failed because the thread was interrupted: " + beanContext.getDeploymentID(),
                    e);
            return;
        } catch (final ExecutionException e) {
        ...
        try {
            ...
            if (beanContext.getComponentType() == BeanType.SINGLETON) {
                ...
                transactionType = beanContext.isBeanManagedTransaction() ? TransactionType.BeanManaged
    ...
    /**
     * This method has no work to do as all instances are removed from the pool on getInstance(...) and not returned via
     * poolInstance(...) if they threw a system exception.
     *
     * @param callContext
     * @param bean
     */
