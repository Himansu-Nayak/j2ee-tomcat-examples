...
public class StatefulContainerFactory {
    ...
    public void setPreventExtendedEntityManagerSerialization(final boolean preventExtendedEntityManagerSerialization) {
        properties.put("PreventExtendedEntityManagerSerialization",
                Boolean.toString(preventExtendedEntityManagerSerialization));
    }
    ...
    public StatefulContainer create() throws Exception {
        ...
        return new StatefulContainer(id, securityService, cache, accessTimeout,
                "true".equalsIgnoreCase(
                        properties.getProperty("PreventExtendedEntityManagerSerialization", "false").trim()),
                createLockFactory());
    }
    ...
    private LockFactory createLockFactory() {
        ...
        if (lockFactory != null) {
            try {
                return LockFactory.class.cast(StatefulContainerFactory.class.getClassLoader()
                        .loadClass(lockFactory.toString().trim()).newInstance());
    ...
    private void buildCache() throws Exception {
        ...
        // invoke recipe
        /*
         * the cache should be created with container loader to avoid memory leaks ClassLoader classLoader =
         * Thread.currentThread().getContextClassLoader(); if (classLoader == null) getClass().getClassLoader();
         */
