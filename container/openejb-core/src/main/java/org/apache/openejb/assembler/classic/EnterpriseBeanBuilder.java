...
class EnterpriseBeanBuilder {
    ...
    public EnterpriseBeanBuilder(final EnterpriseBeanInfo bean, final ModuleContext moduleContext,
    ...
    public BeanContext build() throws OpenEJBException {
        ...
        // build the enc
        final JndiEncBuilder jndiEncBuilder = new JndiEncBuilder(bean.jndiEnc, injections, transactionType,
                moduleContext.getId(), null, moduleContext.getUniqueId(), moduleContext.getClassLoader(),
                moduleContext.getAppContext() == null ? moduleContext.getProperties()
                        : moduleContext.getAppContext().getProperties());
        ...
        if (BeanType.MESSAGE_DRIVEN != ejbType) {
            deployment = new BeanContext(bean.ejbDeploymentId, compJndiContext, moduleContext, ejbClass, home, remote,
                    localhome, local, proxy, serviceEndpoint, businessLocals, businessRemotes, primaryKey, ejbType,
                    bean.localbean && ejbType.isSession(), bean.passivable);
            ...
            deployment = new BeanContext(bean.ejbDeploymentId, compJndiContext, moduleContext, ejbClass, mdbInterface,
        ...
        if (bean.statefulTimeout != null) {
            deployment.setStatefulTimeout(
                    new Duration(bean.statefulTimeout.time, TimeUnit.valueOf(bean.statefulTimeout.unit)));
        }
            ...
            for (final PersistenceContextReferenceInfo info : statefulBeanInfo.jndiEnc.persistenceContextRefs) {
                if (info.extended) {
                    try {
                        ...
                        final Object o = containerSystem.getJNDIContext()
                                .lookup(PersistenceBuilder.getOpenEJBJndiName(info.unitId));
                        final EntityManagerFactory emf = EntityManagerFactory.class.cast(o);
                        extendedEntityManagerFactories
                                .put(emf,
                                        new BeanContext.EntityManagerConfiguration(info.properties,
                                                JtaEntityManager.isJPA21(emf) && info.synchronizationType != null
                                                        ? SynchronizationType.valueOf(info.synchronizationType)
                                                        : null));
                    } catch (final NamingException e) {
                        throw new OpenEJBException("PersistenceUnit '" + info.unitId + "' not found for EXTENDED ref '"
                                + info.referenceName + "'");
                    }
    ...
    private void bind(final Context compJndiContext, final String s, final Context moduleJndiContext)
    ...
    private Class loadClass(final String className, final String messageCode) throws OpenEJBException {
        ...
        try {
            ...
            if (clazz.getClassLoader() != moduleContext.getClassLoader()) {
                final String message = messages().format("cl0008", className, clazz.getClassLoader(),
                        moduleContext.getClassLoader(), e.getMessage());
