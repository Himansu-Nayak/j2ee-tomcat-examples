...
import org.apache.openejb.jee.sun.*;
...
public class SunConversion implements DynamicDeployer {
    ...
    private SunApplicationClient getSunApplicationClient(final ClientModule clientModule) {
        ...
        if (altDD instanceof String) {
            try {
                altDD = JaxbSun.unmarshal(SunApplicationClient.class,
                        new ByteArrayInputStream(((String) altDD).getBytes()));
            } catch (final Exception e) {
                // todo warn about not being able to parse sun descriptor
            }
        }
    ...
    public void convertModule(final WebModule webModule) {
        ...
        if (webModule.getWebservices() != null) {
            final Map<String, WebserviceDescription> descriptions = webModule.getWebservices()
                    .getWebserviceDescriptionMap();
            for (final org.apache.openejb.jee.sun.WebserviceDescription sunDescription : sunWebApp
                    .getWebserviceDescription()) {
                final WebserviceDescription description = descriptions
                        .get(sunDescription.getWebserviceDescriptionName());
                if (description == null) {
                    continue;
                }

                final String serviceId = extractSerivceId(sunDescription.getWsdlPublishLocation(),
    ...
    public void convertModule(final EjbModule ejbModule, final EntityMappings entityMappings) {
        ...
        mergeEntityMappings(entities, ejbModule.getModuleId(), ejbModule.getEjbJar(), ejbModule.getOpenejbJar(),
                sunEjbJar);
    ...
    private void mergeEjbConfig(final EjbModule ejbModule, final SunEjbJar sunEjbJar) {
        ...
        if (ejbModule.getWebservices() != null) {
            final Map<String, org.apache.openejb.jee.sun.WebserviceDescription> sunDescriptions = sunEjbJar
                    .getEnterpriseBeans().getWebserviceDescriptionMap();
            for (final WebserviceDescription description : ejbModule.getWebservices().getWebserviceDescription()) {
                final org.apache.openejb.jee.sun.WebserviceDescription sunDescription = sunDescriptions
                        .get(description.getWebserviceDescriptionName());
    ...
    private void mergeEntityMappings(final Map<String, EntityData> entities, final String moduleId, final EjbJar ejbJar,
            final OpenejbJar openejbJar, final SunEjbJar sunEjbJar) {
        if (openejbJar == null) {
            return;
        }
        if (sunEjbJar == null) {
        ...
        for (final Ejb ejb : sunEjbJar.getEnterpriseBeans().getEjb()) {
            ...
            if (oneOneFinders != null) {
                for (final Finder finder : oneOneFinders.getFinder()) {
                    ...
                    final String ejbQl = convertToEjbQl(entityData.entity.getName(), cmpFields, finder.getQueryParams(),
                            queryFilter);
    ...
    public void mergeEntityMappings(final Map<String, EntityData> entities, final String moduleId,
            final EjbModule ejbModule, final EntityMappings entityMappings, final SunCmpMapping sunCmpMapping) {
    ...
    public String convertToEjbQl(final String abstractSchemaName, final Collection<String> cmpFields,
            final String queryParams, final String queryFilter) {
    ...
    private String convertToEjbQlFilter(final Collection<String> cmpFields, final List<List<String>> queryParams,
