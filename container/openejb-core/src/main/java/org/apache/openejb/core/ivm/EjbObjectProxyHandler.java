...
public abstract class EjbObjectProxyHandler extends BaseEjbProxyHandler {
    ...
    public EjbObjectProxyHandler(final BeanContext beanContext, final Object pk, final InterfaceType interfaceType,
            final List<Class> interfaces, final Class mainInterface) {
        super(beanContext, pk, interfaceType, interfaces, mainInterface);
    }

    public static Object createProxy(final BeanContext beanContext, final Object primaryKey,
            final InterfaceType interfaceType, final Class mainInterface) {
        return createProxy(beanContext, primaryKey, interfaceType, null, mainInterface);
    }

    public static Object createProxy(final BeanContext beanContext, final Object primaryKey,
            InterfaceType interfaceType, final List<Class> interfaces, final Class mainInterface) {
        if (!interfaceType.isHome()) {
            interfaceType = interfaceType.getCounterpart();
        }
        final EjbHomeProxyHandler homeHandler = EjbHomeProxyHandler.createHomeHandler(beanContext, interfaceType,
    ...
    public Object _invoke(final Object p, final Class interfce, final Method m, final Object[] a) throws Throwable {
        ...
        try {
            if (logger.isDebugEnabled()) {
                logger.debug("EjbObjectProxyHandler: invoking method " + methodName + " on " + deploymentID
                        + " with identity " + primaryKey);
            }
            ...
            exc = ire.getRootCause() != null ? ire.getRootCause()
                    : new RemoteException("InvalidateReferenceException: " + ire);
            throw exc;
            /*
            ...
            if (logger.isDebugEnabled()) {
                if (exc == null) {
                    ...
                    logger.debug(
                            "EjbObjectProxyHandler: finished invoking method " + methodName + ". Return value:" + ret);
                } else {
                    logger.debug(
                            "EjbObjectProxyHandler: finished invoking method " + methodName + " with exception " + exc);
                }
            }
        }
    }
    ...
    protected Object businessMethod(final Class<?> interfce, final Method method, final Object[] args,
    ...
    protected Object synchronizedBusinessMethod(final Class<?> interfce, final Method method, final Object[] args)
