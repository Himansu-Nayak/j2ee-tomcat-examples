 ...
 * </p>
 * <p>
 * The internal storage array starts with capacity determined by the <code>initialCapacity</code> property, which can be
 * set by the constructor. The default initial capacity is 16. Adding elements using {@link #addElement(double)} appends
 * elements to the end of the array. When there are no open entries at the end of the internal storage array, the array
 * is expanded. The size of the expanded array depends on the <code>expansionMode</code> and
 * <code>expansionFactor</code> properties. The <code>expansionMode</code> determines whether the size of the array is
 * multiplied by the <code>expansionFactor</code> (MULTIPLICATIVE_MODE) or if the expansion is additive (ADDITIVE_MODE
 * -- <code>expansionFactor</code> storage locations added). The default <code>expansionMode</code> is
 ...
 * addressable elements (the <code>numElements</code> property) and if the difference is too large, the internal array
 * is contracted to size <code>numElements + 1.</code> The determination of when the internal storage array is
 * "too large" depends on the <code>expansionMode</code> and <code>contractionFactor</code> properties. If the
 * <code>expansionMode</code> is <code>MULTIPLICATIVE_MODE</code>, contraction is triggered when the ratio between
 * storage array length and <code>numElements</code> exceeds <code>contractionFactor.</code> If the
 * <code>expansionMode</code> is <code>ADDITIVE_MODE,</code> the number of excess storage locations is compared to
 * <code>contractionFactor.</code>
 * </p>
 * <p>
 * To avoid cycles of expansions and contractions, the <code>expansionFactor</code> must not exceed the
 * <code>contractionFactor.</code> Constructors and mutators for both of these properties enforce this requirement,
 * throwing IllegalArgumentException if it is violated.
 * </p>
 *
...
public class ResizableDoubleArray implements DoubleArray, Serializable {
    ...
    /**
     * The contraction criteria determines when the internal array will be contracted to fit the number of elements
     * contained in the element array + 1.
     */
    protected float contractionCriteria = 2.5f;

    /**
     * The expansion factor of the array. When the array needs to be expanded, the new array size will be
     * <code>internalArray.length * expansionFactor</code> if <code>expansionMode</code> is set to MULTIPLICATIVE_MODE,
     * or <code>internalArray.length + expansionFactor</code> if <code>expansionMode</code> is set to ADDITIVE_MODE.
     */
    ...
    /**
     * The initial capacity of the array. Initial capacity is not exposed as a property as it is only meaningful when
    ...
    /**
     * The number of addressable elements in the array. Note that this has nothing to do with the length of the internal
     * storage array.
     */
    protected int numElements;

    /**
     * The position of the first addressable element in the internal storage array. The addressable elements in the
     * array are <code>
    ...
    /**
     * <p>
     * Create a ResizableArray with the specified initial capacity and expansion factor. The remaining properties take
     * default values:
     * <ul>
     * <li><code>expansionMode = MULTIPLICATIVE_MODE</code></li>
     ...
     * <code>expansionMode</code> will default to <code>MULTIPLICATIVE_MODE.</code>
     * </p>
     * <p>
     * Throws IllegalArgumentException if the following conditions are not met:
     * <ul>
    ...
    /**
     * Copy constructor. Creates a new ResizableDoubleArray that is a deep, fresh copy of the original. Needs to acquire
     * synchronization lock on original. Original may not be null; otherwise a NullPointerException is thrown.
     *
     * @param original
     *            array to copy
     * @since 2.0
    ...
    /**
     * Substitutes <code>value</code> for the most recently added value. Returns the value that has been replaced. If
     * the array is empty (i.e. if {@link #numElements} is zero), a MathRuntimeException is thrown.
     *
     * @param value
    ...
    public synchronized double substituteMostRecentElement(final double value) {
        if (numElements < 1) {
            throw MathRuntimeException
                    .createArrayIndexOutOfBoundsException("cannot substitute an element from an empty array");
        }
    ...
    /**
     * Checks the expansion factor and the contraction criteria and throws an IllegalArgumentException if the
     * contractionCriteria is less than the expansionCriteria
     *
     * @param expansion
    ...
    protected void checkContractExpand(final float contraction, final float expansion) {
        ...
        if (expansion <= 1.0) {
            throw MathRuntimeException.createIllegalArgumentException("expansion factor smaller than one ({0})",
                    expansion);
        }
    }
    ...
    /**
     * Contracts the storage array to the (size of the element set) + 1 - to avoid a zero length array. This function
     * also resets the startIndex to zero.
     */
     ...
     * <code>discardFrontElements(2)</code> will cause the first two elements to be discarded, leaving 3,4 in the array.
     * Throws illegalArgumentException if i exceeds numElements.
     *
     * @param i
     ...
     * if the array contains the elements 1,2,3,4, invoking
     * <code>discardMostRecentElements(2)</code> will cause the last two elements to be discarded, leaving 1,2 in the
     * array. Throws illegalArgumentException if i exceeds numElements.
     *
     * @param i
     ...
     * depending on the value of <code>front</code>. For example, if the array contains the elements 1,2,3,4, invoking
     * <code>discardExtremeElements(2,false)</code> will cause the last two elements to be discarded, leaving 1,2 in the
     * array. For example, if the array contains the elements 1,2,3,4, invoking
     * <code>discardExtremeElements(2,true)</code> will cause the first two elements to be discarded, leaving 3,4 in the
     * array. Throws illegalArgumentException if i exceeds numElements.
     *
     * @param i
     *            the number of elements to discard from the front/end of the array
     * @param front
     *            true if elements are to be discarded from the front of the array, false if elements are to be
     *            discarded from the end of the array
     * @throws IllegalArgumentException
     *             if i is greater than numElements.
     * @since 2.0
     */
    private synchronized void discardExtremeElements(final int i, final boolean front) {
        if (i > numElements) {
            throw MathRuntimeException.createIllegalArgumentException(
                    "cannot discard {0} elements from a {1} elements array", i, numElements);
        } else if (i < 0) {
            throw MathRuntimeException
                    .createIllegalArgumentException("cannot discard a negative number of elements ({0})", i);
        } else {
    ...
    /**
     * The contraction criteria defines when the internal array will contract to store only the number of elements in
     * the element array. If the <code>expansionMode</code> is <code>MULTIPLICATIVE_MODE</code>, contraction is
     * triggered when the ratio between storage array length and <code>numElements</code> exceeds
     * <code>contractionFactor</code>. If the <code>expansionMode</code> is <code>ADDITIVE_MODE</code>, the number of
     * excess storage locations is compared to <code>contractionFactor.</code>
     *
    ...
    public synchronized double getElement(final int index) {
        if (index >= numElements) {
            throw MathRuntimeException.createArrayIndexOutOfBoundsException(
                    "the index specified: {0} is larger than the current maximal index {1}", index, numElements - 1);
        } else if (index >= 0) {
            return internalArray[startIndex + index];
        } else {
            throw MathRuntimeException.createArrayIndexOutOfBoundsException(
                    "elements cannot be retrieved from a negative array index {0}", index);
        }
    }

    /**
     * Returns a double array containing the elements of this <code>ResizableArray</code>. This method returns a copy,
     * not a reference to the underlying array, so that changes made to the returned array have no effect on this
     ...
     * <code>expansionFactor</code> (MULTIPLICATIVE_MODE) or if the expansion is additive (ADDITIVE_MODE --
     * <code>expansionFactor</code> storage locations added). The default <code>expansionMode</code> is
     * MULTIPLICATIVE_MODE and the default <code>expansionFactor</code> is 2.0.
     *
    ...
    /**
     * The <code>expansionMode</code> determines whether the internal storage array grows additively (ADDITIVE_MODE) or
     * multiplicatively (MULTIPLICATIVE_MODE) when it is expanded.
     *
    ...
    /**
     * Sets the <code>expansionMode</code>. The specified value must be one of ADDITIVE_MODE, MULTIPLICATIVE_MODE.
     *
     * @param expansionMode
    ...
    public void setExpansionMode(final int expansionMode) {
        if (expansionMode != MULTIPLICATIVE_MODE && expansionMode != ADDITIVE_MODE) {
            throw MathRuntimeException.createIllegalArgumentException(
                    "unsupported expansion mode {0}, supported modes are {1} ({2}) and {3} ({4})", expansionMode,
                    MULTIPLICATIVE_MODE, "MULTIPLICATIVE_MODE", ADDITIVE_MODE, "ADDITIVE_MODE");
        }
        synchronized (this) {
    ...
    /**
     * Notice the package scope on this method. This method is simply here for the JUnit test, it allows us check if the
     * expansion is working properly after a number of expansions. This is not meant to be a part of the public
    ...
    /**
     * This function allows you to control the number of elements contained in this array, and can be used to
     * "throw out" the last n values in an array. This function will also expand the internal array as needed.
     *
     * @param i
    ...
    /**
     * Returns the internal storage array. Note that this method returns a reference to the internal storage array, not
     * a copy, and to correctly address elements of the array, the <code>startIndex</code> is required (available via
     * the {@link #start} method). This method should only be used in cases where copying the internal array is not
     * practical. The {@link #getElements} method should be used in all other cases.
     *
     ...
     * , the <code>numElements</code> property is increased to <code>index +1</code> and additional storage is allocated
     * (if necessary) for the new element and all (uninitialized) elements between the new element and the previous end
     * of the array).
     *
     * @param index
    ...
    public synchronized void setElement(final int index, final double value) {
        if (index < 0) {
            throw MathRuntimeException
                    .createArrayIndexOutOfBoundsException("cannot set an element at a negative index {0}", index);
        }
    ...
    protected void setInitialCapacity(final int initialCapacity) {
        if (initialCapacity > 0) {
            ...
            throw MathRuntimeException.createIllegalArgumentException("initial capacity ({0}) is not positive",
                    initialCapacity);
        }
    }
    ...
    /**
     * Returns the starting index of the internal array. The starting index is the position of the first addressable
     * element in the internal storage array. The addressable elements in the array are <code>
     * internalArray[startIndex],...,internalArray[startIndex + numElements -1]
     * </code>
     *
    ...
    /**
     * Returns a copy of the ResizableDoubleArray. Does not contract before the copy, so the returned object is an exact
     * copy of this.
     *
    ...
    /**
     * Returns true iff object is a ResizableDoubleArray with the same properties as this and an identical internal
     * storage array.
     *
     * @param object
