...
import org.apache.xbean.finder.*;
...
public class ApplicationComposers {
    ...
    private static final Class[] MODULE_TYPES = { IAnnotationFinder.class, ClassesArchive.class, AppModule.class,
            WebModule.class, EjbModule.class, Application.class, WebApp.class, EjbJar.class, EnterpriseBean.class,
            Persistence.class, PersistenceUnit.class, Connector.class, Beans.class, Class[].class, Class.class,
            Resources.class };
    ...
    private static String implicitRootUrl(final PersistenceRootUrl annotation) {
        ...
        try {
            final URL url = DeploymentLoader.altDDSources(DeploymentLoader.mapDescriptors(finder), false)
                    .get("persistence.xml");
            if (url == null) {
                return "";
            }
        ...
        if (descriptors != null) {
            ...
            if (descriptors instanceof Descriptors) {
                for (final Descriptor descriptor : ((Descriptors) descriptors).value()) {
                    ...
                    try {
                        dds.put(descriptor.name(),
                                resource == null ? new File(descriptor.path()).toURI().toURL() : resource);
                ...
                if (descriptors instanceof org.apache.openejb.junit.Descriptors) {
                    for (final org.apache.openejb.junit.Descriptor descriptor : ((org.apache.openejb.junit.Descriptors) descriptors)
                            .value()) {
                        final URL resource = loader.getResource(descriptor.path());
                        try {
                            dds.put(descriptor.name(),
    ...
    private void validate() {
        ...
        if (isContainer()) {
            final Map<Object, List<Method>> annotatedConfigurationMethods = findAnnotatedMethods(
                    new HashMap<Object, List<Method>>(), Configuration.class);
            {
                int nbProp = 0;
                int nbOpenejb = 0;
            ...
            int injectorSize = 0;
            for (final List<Method> m : findAnnotatedMethods(new HashMap<Object, List<Method>>(),
                    org.apache.openejb.junit.MockInjector.class).values()) {
                injectorSize += m.size();
            }
            for (final List<Method> m : findAnnotatedMethods(new HashMap<Object, List<Method>>(), MockInjector.class)
                    .values()) {
                injectorSize += m.size();
            }
            if (injectorSize > 1) {
            ...
            for (final List<Method> l : findAnnotatedMethods(new HashMap<Object, List<Method>>(), Component.class)
                    .values()) {
                components.addAll(l);
            }
            for (final List<Method> l : findAnnotatedMethods(new HashMap<Object, List<Method>>(),
                    org.apache.openejb.junit.Component.class).values()) {
                components.addAll(l);
            }
        ...
        if (isApplication()) {
            ...
            for (final List<Method> l : findAnnotatedMethods(new HashMap<Object, List<Method>>(), Descriptors.class)
                    .values()) {
                descriptors.addAll(l);
            }
            for (final List<Method> l : findAnnotatedMethods(new HashMap<Object, List<Method>>(),
                    org.apache.openejb.junit.Descriptors.class).values()) {
                descriptors.addAll(l);
            }
            ...
            for (final List<Method> l : findAnnotatedMethods(new HashMap<Object, List<Method>>(), Classes.class)
                    .values()) {
                classes.addAll(l);
            }
            for (final List<Method> l : findAnnotatedMethods(new HashMap<Object, List<Method>>(),
                    org.apache.openejb.junit.Classes.class).values()) {
                classes.addAll(l);
            }
            ...
            for (final List<Method> l : findAnnotatedMethods(new HashMap<Object, List<Method>>(), Jars.class)
                    .values()) {
            ...
            for (final List<Method> l : findAnnotatedMethods(new HashMap<Object, List<Method>>(), Module.class)
                    .values()) {
                moduleMethods.addAll(l);
            }
            for (final List<Method> l : findAnnotatedMethods(new HashMap<Object, List<Method>>(),
                    org.apache.openejb.junit.Module.class).values()) {
                moduleMethods.addAll(l);
            }
            ...
            if (appModules > 1) {
                final String gripe = "Test class should have no more than one @Module method that returns "
    ...
    private Map<Object, List<Method>> findAnnotatedMethods(final Map<Object, List<Method>> map,
    ...
    public void deployApp(final Object inputTestInstance) throws Exception {
        ...
        final ManagedBean testBean;
        {
            ...
            final FinderFactory.OpenEJBAnnotationFinder finder = new FinderFactory.OpenEJBAnnotationFinder(
                    new ClassesArchive(ancestors(testClass)));
        ...
        final Map<String, URL> additionalDescriptors = descriptorsToMap(
                testClass.getAnnotation(org.apache.openejb.junit.Descriptors.class));
        ...
        for (final Map.Entry<Object, List<Method>> methods : moduleMethods.entrySet()) {
            ...
            for (final Method method : methods.getValue()) {
                ...
                final org.apache.openejb.junit.Classes classesAnnotationOld = method
                        .getAnnotation(org.apache.openejb.junit.Classes.class);
                ...
                if (classesAnnotation != null) {
                    ...
                    cdi = isCdi(classesAnnotation.cdi(), cdiInterceptors, cdiAlternatives, cdiStereotypes,
                            cdiDecorators);
                ...
                if (obj instanceof WebApp) { // will add the ejbmodule too
                    ...
                    webModulesNb++;
                    addWebApp(appModule, testBean, additionalDescriptors, method.getAnnotation(Descriptors.class),
                            method.getAnnotation(JaxrsProviders.class), webApp, globalJarsAnnotation, jarsAnnotation,
                            classes, excludes, cdiInterceptors, cdiAlternatives, cdiDecorators, cdiStereotypes, cdi,
                            innerClassesAsBean, defaultConfig);
                    ...
                    if (cdi) {
                        ejbModule.setBeans(
                                beans(new Beans(), cdiDecorators, cdiInterceptors, cdiAlternatives, cdiStereotypes));
                    }
                    ...
                    appModule.addPersistenceModule(new PersistenceModule(appModule,
                    ...
                    appModule.addPersistenceModule(new PersistenceModule(appModule,
                            implicitRootUrl(method.getAnnotation(PersistenceRootUrl.class)), new Persistence(unit)));
                    notBusinessModuleNumber++;
                    ...
                    if (cdi) {
                        ejbModule.setBeans(
                                beans(beans, cdiDecorators, cdiInterceptors, cdiAlternatives, cdiStereotypes));
                    }
                    Collection<File> files = findFiles(jarsAnnotation);
                    if (defaultConfig) {
                        (files == null ? files = new LinkedList<>() : files).add(jarLocation(testClass));
        ...
        if (classClasses != null) {
            ...
            addWebApp(appModule, testBean, additionalDescriptors, null, null, webapp, globalJarsAnnotation, null,
                    classClasses.value(), classClasses.excludes(), classClasses.cdiInterceptors(),
                    classClasses.cdiAlternatives(), classClasses.cdiDecorators(), classClasses.cdiStereotypes(),
        ...
        if (application != null) {
            final AppModule newModule = new AppModule(appModule.getClassLoader(), appModule.getModuleId(), application,
                    false);
        ...
        // config for the app
        for (final Map.Entry<Object, List<Method>> method : findAnnotatedMethods(new HashMap<Object, List<Method>>(),
                ApplicationConfiguration.class).entrySet()) {
        ...
        final BeanContext context = SystemInstance.get().getComponent(ContainerSystem.class)
                .getBeanContext(testClass.getName());
    ...
    private boolean isCdi(final boolean cdi, final Class<?>[] cdiInterceptors, final Class<?>[] cdiAlternatives,
            final Class<?>[] cdiStereotypes, final Class<?>[] cdiDecorators) {
        return cdi || isNotNullOrEmpty(cdiAlternatives) || isNotNullOrEmpty(cdiDecorators)
                || isNotNullOrEmpty(cdiInterceptors) || isNotNullOrEmpty(cdiStereotypes);
    }
    ...
    private void addWebApp(final AppModule appModule, final ManagedBean testBean,
            final Map<String, URL> additionalDescriptors, final Descriptors descriptors, final JaxrsProviders providers,
            final WebApp webapp, final Jars globalJarsAnnotation, final Jars jarsAnnotation,
            final Class<?>[] cdiClasses, final String[] excludes, final Class<?>[] cdiInterceptors,
            final Class<?>[] cdiAlternatives, final Class<?>[] cdiDecorators, final Class<?>[] cdiStereotypes,
        ...
        if (autoConfig) {
            ...
            try {
                webModule.getAltDDs().putAll(DeploymentLoader
                        .mapDescriptors(new ResourceFinder("", webModule.getClassLoader(), thisJar.toURI().toURL())));
                webModule.getAltDDs().putAll(DeploymentLoader
                        .getWebDescriptors(new File(thisJar.getParentFile().getParentFile(), "src/main/webapp")));
            } catch (final IOException e) {
    ...
    private void enrich(final Object inputTestInstance, final BeanContext context)
        ...
        try {
            final InjectionProcessor processor = new InjectionProcessor(inputTestInstance, context.getInjections(),
    ...
    @SuppressWarnings("unchecked")
    private void setComponent(final Object testInstance, final Method method)
    ...
    private IAnnotationFinder finderFromClasses(final DeploymentModule module, final Class<?>[] value,
            final Collection<File> others, final String[] excludes) {
        final Collection<Archive> archives = new ArrayList<>(1 + (others == null ? 0 : others.size()));

        final Filter filter = excludes == null || excludes.length == 0 ? null
                : Filters.invert(Filters.prefixes(excludes));
        ...
        if (others != null) {
            ...
            for (final File f : others) {
                try {
                    final Archive archive = f.isDirectory() ? new FileArchive(classLoader, f)
                            : new JarArchive(classLoader, f.toURI().toURL());
    ...
    private void initFilteredServiceManager(final String[] services) {
        ...
        try {
            ...
            throw new IllegalStateException(
                    "Failed initializing FilteredServiceManager with services " + Arrays.toString(services), e);
        }
    }
    ...
    public void startContainer(final Object instance) throws Exception {
        ...
        final org.apache.openejb.junit.EnableServices annotationOld = testClass
                .getAnnotation(org.apache.openejb.junit.EnableServices.class);
        ...
        if (cdiExtensions != null) {
            SystemInstance.get().setComponent(LoaderService.class,
                    new ExtensionAwareOptimizedLoaderService(cdiExtensions.value()));
        }
        ...
        for (final Map.Entry<Object, List<Method>> method : findAnnotatedMethods(new HashMap<Object, List<Method>>(),
                Component.class).entrySet()) {
            for (final Method m : method.getValue()) {
                setComponent(method.getKey(), m);
            }
        }
        for (final Map.Entry<Object, List<Method>> method : findAnnotatedMethods(new HashMap<Object, List<Method>>(),
                org.apache.openejb.junit.Component.class).entrySet()) {
            for (final Method m : method.getValue()) {
                setComponent(method.getKey(), m);
            }
        }
    ...
    public void handleLifecycle(final Class<?> type, final Object appInstance)
        ...
        if (!appContext.getWebContexts().isEmpty()) {
            beforeDestroyAfterRunnables.add(new Runnable() {
                ...
                public void run() {
                    try {
                        final Object sessionManager = SystemInstance.get().getComponent(ParentClassLoaderFinder.Helper
                                .get().loadClass("org.apache.openejb.server.httpd.session.SessionManager"));
                        if (sessionManager != null) {
                            final Class<?>[] paramTypes = { WebContext.class };
                            for (final WebContext web : appContext.getWebContexts()) {
                                Reflections.invokeByReflection(sessionManager, "destroy", paramTypes,
                                        new Object[] { web });
                            }
                        }
                    } catch (final Throwable e) {
