...
public class IntraVmHandle implements Serializable, HomeHandle, Handle {
    ...
    protected Object writeReplace() throws ObjectStreamException {
        /*
         * If the handle is being copied between bean instances in a RPC call we use the IntraVmArtifact
         */
        if (IntraVmCopyMonitor.isIntraVmCopyOperation()) {
            return new IntraVmArtifact(this);
            /*
             * If the handle is referenced by a stateful bean that is being passivated by the container, we allow this
            ...
            return this;
            /*
             * If the handle is serialized outside the core container system, we allow the application server to handle
             * it.
             */
        } else {
            ...
            if (theProxy instanceof EJBObject) {
                ...
                throw new OpenEJBRuntimeException(
                        "Invalid proxy type. Handles are only supported by EJBObject types in EJB 1.1");
            }
        }
    }
