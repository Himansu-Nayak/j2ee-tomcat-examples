...
public class GeronimoTransactionManagerFactory {
    ...
    public static GeronimoTransactionManager create(Integer defaultTransactionTimeoutSeconds, // Deprecated, use defaultTransactionTimeout
            final Duration defaultTransactionTimeout, final boolean txRecovery, final byte[] tmId,
            final String bufferClassName, final int bufferSizeKb, final boolean checksumEnabled,
            final boolean adler32Checksum, Integer flushSleepTimeMilliseconds, // Deprecated, use flushSleepTime
            final Duration flushSleepTime, final String logFileDir, final String logFileExt, final String logFileName,
            final int maxBlocksPerFile, final int maxBuffers, final int maxLogFiles, final int minBuffers,
        ...
        if (flushSleepTimeMilliseconds == null) {
            flushSleepTimeMilliseconds = (int) TimeUnit.MILLISECONDS.convert(flushSleepTime.getTime(),
        ...
        if (defaultTransactionTimeoutSeconds == null) {
            defaultTransactionTimeoutSeconds = (int) TimeUnit.SECONDS.convert(defaultTransactionTimeout.getTime(),
        ...
        if (txRecovery) {
            ...
            txLog = new HOWLLog(bufferClassName == null ? "org.objectweb.howl.log.BlockLogBuffer" : bufferClassName,
                    ...
                    maxLogFiles, minBuffers, threadsWaitingForceThreshold, xidFactory,
                    SystemInstance.get().getBase().getDirectory("."));
            ((HOWLLog) txLog).doStart();
        }

        final GeronimoTransactionManager geronimoTransactionManager = new DestroyableTransactionManager(
                defaultTransactionTimeoutSeconds, xidFactory, txLog);
        final ObjectNameBuilder jmxName = new ObjectNameBuilder("openejb.management").set("j2eeType",
                "TransactionManager");
    ...
    public static class DestroyableTransactionManager extends GeronimoTransactionManager
            implements DestroyableResource {
        public DestroyableTransactionManager(final int defaultTransactionTimeoutSeconds, final XidFactory xidFactory,
                final TransactionLog transactionLog) throws XAException {
        ...
        public void destroyResource() {
            ...
            try {
                ...
                final ExponentialtIntervalRetryScheduler rs = ExponentialtIntervalRetryScheduler.class
                        .cast(f.get(this));
    ...
    public static final class TransactionManagerMBean {
        ...
        public TransactionManagerMBean(final GeronimoTransactionManager transactionManager,
                final Duration defaultTransactionTimeout, final TransactionLog txLog) {
