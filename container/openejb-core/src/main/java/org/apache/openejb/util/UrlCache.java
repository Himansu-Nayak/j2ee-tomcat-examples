...
public class UrlCache {
        ...
        while (!locationStack.isEmpty()) {
            ...
            if (file != null) {
                try {
                    ...
                    logger.error(
                            "Error caching url. Original jar file will be used which may result in a file lock: url="
                                    + url,
                            e);
                    cachedUrls.add(url);
                }
    ...
    private synchronized File cacheUrl(final String appId, URL url) {
        ...
        if (success) {
            ...
            logger.error(
                    "Unable to copy jar into URL cache directory. Original jar file will be used which may result in a file lock: file="
                            + sourceFile);
            return null;
        }
    }
    ...
    private List<URL> getManifestClassPath(final URL codeSource, final File location) {
        try {
            ...
            for (final StringTokenizer tokenizer = new StringTokenizer(manifestClassPath, " "); tokenizer
                    .hasMoreTokens();) {
