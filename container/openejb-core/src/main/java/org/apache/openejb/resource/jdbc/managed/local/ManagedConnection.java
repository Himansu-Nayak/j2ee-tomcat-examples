...
public class ManagedConnection implements InvocationHandler {
    ...
    public ManagedConnection(final CommonDataSource ds, final TransactionManager txMgr,
            final TransactionSynchronizationRegistry txRegistry, final String user, final String password) {
                ...
                if (connection == null && delegate == null) {
                    ...
                    if (xaConnection == null) {
                        try {
                            ...
                            final Logger logger = Logger.getInstance(LogCategory.OPENEJB_RESOURCE_JDBC,
    ...
    protected Object newConnection() throws SQLException {
        final Object connection = DataSource.class.isInstance(key.ds)
                ? (key.user == null ? DataSource.class.cast(key.ds).getConnection()
                        : DataSource.class.cast(key.ds).getConnection(key.user, key.pwd))
                : (key.user == null ? XADataSource.class.cast(key.ds).getXAConnection()
    ...
    private Object invokeUnderTransaction(final Connection delegate, final Method method, final Object[] args)
            throws Exception {
        final String mtdName = method.getName();
        if ("setAutoCommit".equals(mtdName) || "commit".equals(mtdName) || "rollback".equals(mtdName)
                || "setSavepoint".equals(mtdName) || "setReadOnly".equals(mtdName)) {
    ...
    private static final class Key {
        ...
        public boolean equals(final Object o) {
            ...
            return (ds == key.ds || ds.equals(key.ds)) && !(user != null ? !user.equals(key.user) : key.user != null)
