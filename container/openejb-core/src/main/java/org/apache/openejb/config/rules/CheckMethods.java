        ...
        for (int i = 0; i < interfaceMethods.length; i++) {
            ...
            try {
                ...
                if (differentArgs.size() > 0) {
                    fail(b, "no.busines.method.args", interfaceMethods[i].getName(), interfaceMethods[i].toString(),
                            "local", intrface.getName(), beanClass.getName(), differentArgs.size());
                }
                if (differentCase.size() > 0) {
                    fail(b, "no.busines.method.case", interfaceMethods[i].getName(), interfaceMethods[i].toString(),
                            "local", intrface.getName(), beanClass.getName(), differentCase.size());
                }
                if (differentArgs.size() == 0 && differentCase.size() == 0) {
                    fail(b, "no.busines.method", interfaceMethods[i].getName(), interfaceMethods[i].toString(), "local",
    ...
    private void check_remoteInterfaceMethods(final RemoteBean b) {
        ...
        for (int i = 0; i < interfaceMethods.length; i++) {
            ...
            try {
                ...
                if (differentArgs.size() > 0) {
                    fail(b, "no.busines.method.args", interfaceMethods[i].getName(), interfaceMethods[i].toString(),
                            "remote", intrface.getName(), beanClass.getName(), differentArgs.size());
                }
                if (differentCase.size() > 0) {
                    fail(b, "no.busines.method.case", interfaceMethods[i].getName(), interfaceMethods[i].toString(),
                            "remote", intrface.getName(), beanClass.getName(), differentCase.size());
                }
                if (differentArgs.size() == 0 && differentCase.size() == 0) {
                    fail(b, "no.busines.method", interfaceMethods[i].getName(), interfaceMethods[i].toString(),
                            "remote", intrface.getName(), beanClass.getName());
                }
            }
        }
    }
        ...
        for (int i = 0; i < homeMethods.length; i++) {
            ...
            try {
                ...
                if (b instanceof EntityBean) {
                    ...
                    fail(b, "entity.no.ejb.create", b.getEjbClass(), entity.getPrimKeyClass(), ejbCreateName.toString(),
                            paramString);
    ...
    public void check_unusedPostCreateMethods(final RemoteBean b) {
        ...
        for (final Method postCreate : bean.getMethods()) {
            ...
            try {
                ...
                warn(b, "unused.ejbPostCreate", b.getEjbClass(), postCreate.getName(), paramString,
                        ejbCreate.toString());
