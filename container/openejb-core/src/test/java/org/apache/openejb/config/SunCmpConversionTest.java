...
public class SunCmpConversionTest extends TestCase {
    ...
    private EntityMappings convert(final String prefix) throws Exception {
        return convert(prefix + "ejb-jar.xml", prefix + "sun-ejb-jar.xml", prefix + "sun-cmp-mappings.xml",
                prefix + "orm.xml");
    }

    private EntityMappings convert(final String ejbJarFileName, final String sunEjbJarFileName,
            final String sunCmpMappingsFileName, final String expectedFileName) throws Exception {
        InputStream in = getClass().getClassLoader().getResourceAsStream(ejbJarFileName);
        final EjbJar ejbJar = (EjbJar) JaxbJavaee.unmarshalJavaee(EjbJar.class,
                new ByteArrayInputStream(readContent(in).getBytes()));

        // create and configure the module
        final EjbModule ejbModule = new EjbModule(getClass().getClassLoader(), "TestModule", null, ejbJar,
                new OpenejbJar());
        ...
        // add the altDD
        ejbModule.getAltDDs().put("sun-cmp-mappings.xml",
                getClass().getClassLoader().getResource(sunCmpMappingsFileName));
        ...
        if (expectedFileName != null) {
            ...
            final Set<String> generatedPks = new HashSet<String>(
                    Arrays.asList("BasicCmp2", "AOBasicCmp2", "EncCmp2", "Cmp2RmiIiop"));
            ...
            for (final Entity entity : cmpMappings.getEntity()) {
                if (generatedPks.contains(entity.getName())) {
                    entity.getAttributes().getId().get(0)
                            .setGeneratedValue(new GeneratedValue(GenerationType.IDENTITY));
                }
            }
