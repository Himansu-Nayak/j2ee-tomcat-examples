...
public @interface MetaTest {
    ...
    public static class $ extends Statement {
        ...
        private void testAsAllBeanTypes(final MetaTest annotation) throws Throwable {
            ...
            if (classes.length == 0)
                classes = new Class[] { SingletonBean.class, StatelessBean.class, StatefulBean.class, ManagedBean.class,
            ...
            for (final Class<? extends EnterpriseBean> beanType : classes) {
                ...
                try {
                    ...
                    final List<ContainerTransaction> expectedList = expected.getAssemblyDescriptor()
                            .getContainerTransaction();
                    final List<ContainerTransaction> actualList = actual.getAssemblyDescriptor()
                            .getContainerTransaction();

                    assertEquals(expectedList.size(), actualList.size());
                    final String expectedXml = toString(expected);
                    final String actualXml = toString(actual).replaceAll("Actual", "Expected").replaceAll("actual",
                            "expected");
        ...
        private boolean isSpecificBeanType(final MetaTest annotation) {
            final Class<? extends Annotation>[] annotations = new Class[] { javax.ejb.Singleton.class,
                    javax.ejb.Stateless.class, javax.ejb.Stateful.class, javax.ejb.MessageDriven.class };
