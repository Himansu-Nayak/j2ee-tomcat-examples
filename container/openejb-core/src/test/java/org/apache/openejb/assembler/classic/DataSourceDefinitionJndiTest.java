...
public class DataSourceDefinitionJndiTest {
    ...
    public void checkGlobal() throws Exception {
        final DataSource ds = (DataSource) SystemInstance.get().getComponent(ContainerSystem.class).getJNDIContext()
                .lookup("java:global/foo");
        check(ds, "dsdjt1");
    }

    private void check(final DataSource ds, final String name)
        ...
        final Method getInnermostDelegateInternal = DelegatingConnection.class
                .getDeclaredMethod("getInnermostDelegateInternal");
    ...
    @DataSourceDefinitions({
            @DataSourceDefinition(name = "java:global/foo", className = "org.hsqldb.jdbc.JDBCDataSource", user = "sa", password = "", url = "jdbc:hsqldb:mem:dsdjt1"),
            @DataSourceDefinition(name = "java:app/foo", className = "org.hsqldb.jdbc.JDBCDataSource", user = "sa", password = "", url = "jdbc:hsqldb:mem:dsdjt2") })
    @Stateless
