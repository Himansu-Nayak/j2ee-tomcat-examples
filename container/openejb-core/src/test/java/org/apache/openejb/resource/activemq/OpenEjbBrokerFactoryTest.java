...
public class OpenEjbBrokerFactoryTest extends TestCase {
    private final int brokerPort = NetworkUtil
            .getNextAvailablePort(new int[] { 7272 /* try to not use default AMQ port */, 0 });
    ...
    private String getBrokerUri(final String brokerUri) throws URISyntaxException {
    ...
    public void testBrokerDoubleCreate() throws Exception {
        BrokerService broker = BrokerFactory
                .createBroker(new URI(getBrokerUri("broker:(tcp://localhost:" + brokerPort + ")?useJmx=false")));
        stopBroker(broker);
    ...
    public void testNoDataSource() throws Exception {
        final BrokerService broker = BrokerFactory
                .createBroker(new URI(getBrokerUri("broker:(tcp://localhost:" + brokerPort + ")?useJmx=false")));
