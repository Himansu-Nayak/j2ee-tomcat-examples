...
public class GetAllTimersTest {
    ...
    @Module
    public EjbJar jar() {
        return new EjbJar().enterpriseBean(new SingletonBean(Query.class).localBean())
    ...
    private void checkList(final Collection<Timer> in) {
        final List<Timer> list = new ArrayList<>(in);
    ...
    public static class TimerBean {
        ...
        @PostConstruct
        public void run() {
            ts.createSingleActionTimer(new Date(System.currentTimeMillis() + 100000L),
                    new TimerConfig(getClass().getSimpleName(), false));
        }
