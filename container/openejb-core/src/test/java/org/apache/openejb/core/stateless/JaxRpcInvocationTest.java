...
/**
 * The point of this test case is to verify that OpenEJB is accurately performing it's part of a WebServiceProvider to
 * OpenEJB invocation as it relates to JAX-RPC.
 * <p/>
 * In the agreement between OpenEJB and the Web Service Provider, the Web Service Provider must supply the
 * MessageContext and an Interceptor as the arguments of the standard container.invoke method call.
 * <p/>
 * OpenEJB must ensure the MessageContext is exposed via the SessionContext.getMessageContext and ensure that the
 * interceptor is added to the chain just after the other interceptors and before the bean method itself is invoked.
 *
 * @version $Rev$ $Date$
 */
public class JaxRpcInvocationTest extends TestCase {
    ...
    public void testWebServiceInvocations() throws Exception {
        ...
        assembler.createSecurityService(config.configureService(SecurityServiceInfo.class, "PseudoSecurityService",
                null, "PseudoSecurityService", null));
        ...
        final String value = (String) container.invoke("EchoBean", InterfaceType.SERVICE_ENDPOINT,
                echoMethod.getDeclaringClass(), echoMethod, args, null);
    ...
    public static enum Call {
        WebServiceProvider_Invoke_BEFORE, EjbInterceptor_Invoke_BEFORE, Bean_Invoke_BEFORE, Bean_Invoke, Bean_Invoke_AFTER, EjbInterceptor_Invoke_AFTER, WebServiceProvider_Invoke_AFTER,
    }
    ...
    public static class EchoBean {
        ...
        public Object invoke(final InvocationContext context) throws Exception {
            ...
            /**
             * For JAX-RPC invocations the JAX-RPC MessageContex must be available in the javax.ejb.SessionContext via
             * the getMessageContext method. As per the agreement between OpenEJB and the Web Service Provider the
             * MessageContex should have been passed into the container.invoke method and the container should then
             * ensure it's available via the SessionContext for the duration of this call.
             */
            ...
            org.junit.Assert.assertTrue("the Web Service Provider's message context should be used",
                    messageContext instanceof FakeMessageContext);
    ...
    /**
     * This interceptor is here to ensure that the container still invokes interceptors normally for web serivce
     * invocations and to also guarantee that the Web Service Provider's interceptor (which is a special OpenEJB
     * concept) is invoked *after* all the normal ejb interceptors.
     */
    ...
    /**
     * This object would be implemented by the Web Service Provider per the JAX-RPC spec and supplied to us in the
     * container.invoke method per the OpenEJB-WebServiceProvider agreement
     */
    ...
    /**
     * This object would be supplied by the Web Service Provider as per the OpenEJB-WebServiceProvider agreement and
     * serves two purposes:
     * <p/>
     * 1. Executing the Handler Chain (as required by the JAX-RPC specification) in the context of the EJB Container (as
     * required by the EJB and J2EE WebServices specifications)
     * <p/>
     * 2. Unmarshalling the method arguments from the SOAP message after the handlers in the Handler Chain have had a
     * chance to modify the argument values via the SAAJ tree.
     * <p/>
     * The Interceptor instance given to OpenEJB is constructed and created by the Web Service Provider and should
     * contain all the data it requires to complete it's part of the agreement.
     * <p/>
     * OpenEJB will not perform any injection on this object and the interceptor will be discarded so that the Web
     * Service Provider may pass in a new Interceptor instance on every web service invocation.
     * <p/>
     * The Web Service Provider may pass in any object to serve the roll of the Interceptor as long as it has
     * an @AroundInvoke method using the method signature:
     * <p/>
     * public Object <METHOD-NAME> (InvocationContext ctx) throws Exception
     * <p/>
     * Unlike typical EJB Interceptor around invoke methods, the @AroundInvoke annotation must be used and is not
     * optional, and the method must be public.
     */
