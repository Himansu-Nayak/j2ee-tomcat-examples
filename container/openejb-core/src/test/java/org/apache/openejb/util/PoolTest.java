...
public class PoolTest extends TestCase {
    ...
    private static void await(final CountDownLatch latch, final int timeout, final TimeUnit seconds)
    ...
    private static <T> List<Pool<T>.Entry> getMin(final List<Pool<T>.Entry> entries) {
        ...
        for (final Pool<T>.Entry entry : entries) {
            if (entry != null && entry.hasHardReference())
                list.add(entry);
        }
        return list;
    }
    ...
    /**
     * Tests the idle timeout as well as the Thread pool used to invoke the discard/create jobs.
     *
     * @throws Exception
     *             exception
     */
     ...
     * <p/>
     * The pool should naturally balance itself out by promoting regular instances to "min" instance as things are
     * popped and pushed to and from the pool.
     *
     * @throws Exception
     *             exception
     */
    ...
    /**
     * When an item is in use, it should still be flushed upon return to the pool if the pool was flushed
     * <p/>
     * Active items should still be flushed the moment they become inactive (returned to the pool).
     *
     * @throws Exception
     */
    ...
    /**
     * When an item is in use, it should still be expired upon return to the pool if the item has lived too long
     * <p/>
     * Active items that have lived too long should still be expired the moment they become inactive (returned to the
     * pool).
     *
     * @throws Exception
     */
