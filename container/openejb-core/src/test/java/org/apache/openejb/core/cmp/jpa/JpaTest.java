...
public class JpaTest extends TestCase {
    ...
    static {
        try {
            final Class<?> classRedefinerClass = ParentClassLoaderFinder.Helper.get()
                    .loadClass("org.apache.openjpa.enhance.ClassRedefiner");
    ...
    private static void set(final Object instance, final String parameterName, final Class type, final Object value)
            throws Exception {
        try {
    ...
    public void setUp() throws Exception {
        ...
        MockInitialContextFactory
                .install(Collections.singletonMap("java:comp/TransactionSynchronizationRegistry", transactionManager));
    ...
    public void testJta() throws Exception {
        ...
        final Object jpaTestObject = getClass().getClassLoader()
                .loadClass("org.apache.openejb.core.cmp.jpa.JpaTestObject").newInstance();
    ...
    private EntityManagerFactory createEntityManagerFactory() throws Exception {
        ...
        /*
         * Agent.getInstrumentation().addTransformer(new ClassFileTransformer() { public byte[] transform(ClassLoader
         * loader, String className, Class<?> classBeingRedefined, ProtectionDomain protectionDomain, byte[]
         * classfileBuffer) { if (!className.equals("org/apache/openejb/core/cmp/jpa/Employee")) { return null; } byte[]
         * newBytes = addNewField(classfileBuffer); return newBytes; } });
         */
        ...
        final PersistenceProvider persistenceProvider = (PersistenceProvider) getClass().getClassLoader()
                .loadClass(PERSISTENCE_PROVIDER).newInstance();
    ...
    private void initializeDatabase(final DataSource dataSource) throws SQLException {
        // employee
        createTable(dataSource, "employee",
                "CREATE TABLE employee ( id IDENTITY PRIMARY KEY, first_name VARCHAR(255), last_name VARCHAR(255))");
        execute(dataSource, "INSERT INTO employee (first_name, last_name) VALUES ('David', 'Blevins')");

        // bill
        createTable(dataSource, "bill",
                "CREATE TABLE bill ( billNumber BIGINT NOT NULL, billVersion BIGINT NOT NULL, billRevision BIGINT NOT NULL, billDescription VARCHAR(255) )");
        execute(dataSource,
                "INSERT INTO bill (billNumber, billVersion, billRevision, billDescription) VALUES (1, 0, 0, 'Basic Model')");

        // embedded bill
        createTable(dataSource, "embeddedBill",
                "CREATE TABLE embeddedBill ( billNumber BIGINT NOT NULL, billVersion BIGINT NOT NULL, billRevision BIGINT NOT NULL, billDescription VARCHAR(255) )");
        execute(dataSource,
                "INSERT INTO embeddedBill (billNumber, billVersion, billRevision, billDescription) VALUES (2, 0, 0, 'Advanced Model')");

        // relationship bean
        createTable(dataSource, "OneToOneA", "CREATE TABLE OneToOneA(A1 INTEGER, A2 VARCHAR(255))");
        createTable(dataSource, "OneToOneB",
                " CREATE TABLE OneToOneB(B1 INTEGER, B2 VARCHAR(255), B3 INTEGER, B4 VARCHAR(255), FKA1 INTEGER)");
    ...
    private void createTable(final DataSource dataSource, final String tableName, final String create)
            throws SQLException {
        try {
    ...
    public static class FieldAdderClassVisitor extends ClassVisitor {
        ...
        public void visitEnd() {
            ...
            cv.visitField(ACC_PRIVATE + ACC_TRANSIENT, "newField" + System.currentTimeMillis(), "Ljava/lang/String;",
                    null, null);
            cv.visitEnd();
        }
    }
