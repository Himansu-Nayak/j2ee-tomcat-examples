...
public class SimpleCmpTest {
    ...
    public EjbJar jar() throws OpenEJBException {
        return ReadDescriptors
                .readEjbJar(new ByteArrayInputStream(
                        ("<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n" + "<ejb-jar xsi:schemaLocation=\""
                                ...
                                + HotelEJBLocalObject.class.getName() + "</local>\n" + "<ejb-class>"
                                + HotelBean.class
                                        .getName()
                                ...
                                + TestBeanEJBLocalObject.class.getName() + "</local>\n" + "<ejb-class>"
                                + TestBean.class.getName() + "</ejb-class>\n"
                                + "<session-type>Stateless</session-type>\n"
                                + "<transaction-type>Container</transaction-type>\n" + "<ejb-local-ref>\n"
                                + "<ejb-ref-name>ejb/HotelEJBLocalHome</ejb-ref-name>\n"
                                + "<ejb-ref-type>Entity</ejb-ref-type>\n" + "<local-home>"
                                + HotelEJBLocalHome.class.getName() + "</local-home>\n" + "<local>"
                                + HotelEJBLocalObject.class.getName() + "</local>\n" + "</ejb-local-ref>\n"
                                + "</session>\n" + "</enterprise-beans>\n" + "</ejb-jar>").getBytes()));
    }
    ...
    public static class TestBean implements SessionBean {
        ...
        public void check(final String pk) {
            ...
            try {
                final Object registry = handler.getBeanContext().get(Thread.currentThread().getContextClassLoader() // private so use reflection
    ...
    public static abstract class HotelBean implements EntityBean {
        ...
        public void ejbPostCreate(String str1, String str2) throws CreateException {
        }
