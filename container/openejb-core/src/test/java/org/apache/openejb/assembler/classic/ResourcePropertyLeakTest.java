...
/**
 * This test ensures that additional properties are not leaked into the properties map when creating a resource using
 * class-name, without a provider or type. When using "SkipImpliedAttributes" only the properties the user specified in
 * their configuration should be available in the map.
 *
 * With "SkipImpliedAttributes" set to false (the default), ServiceId (the id of the resource itself) and
 * transactionManager (the Geronimo Transaction Manager) are the only additional properties that should be available for
...
public class ResourcePropertyLeakTest {
    ...
    public void testResourceProperties() throws NamingException {
        ...
        final MyResource r1 = (MyResource) SystemInstance.get().getComponent(ContainerSystem.class).getJNDIContext()
                .lookup("openejb/Resource/r1");
        ...
        final MyResource r2 = (MyResource) SystemInstance.get().getComponent(ContainerSystem.class).getJNDIContext()
                .lookup("openejb/Resource/r2");
    ...
    public Properties config() {
        return new PropertiesBuilder()
                ...
                .p("r1.prop1", "value1").p("r1.prop2", "value2")
                .p("r2", "new://Resource?class-name=org.apache.openejb.assembler.classic.ResourcePropertyLeakTest$MyResource")
                .p("r2.SkipImplicitAttributes", "true").p("r2.prop1", "value1").p("r2.prop2", "value2").build();
    }
