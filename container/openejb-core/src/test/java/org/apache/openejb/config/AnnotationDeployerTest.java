...
public class AnnotationDeployerTest {
    ...
    public void applicationExceptionInheritanceTest() throws Exception {
        ...
        org.apache.openejb.jee.ApplicationException appEx = assemblyDescriptor
                .getApplicationException(BusinessException.class);
    ...
    private EjbModule testModule() {
        ...
        ejbModule.setFinder(new ClassFinder(AnnotationDeployerTest.class, BusinessException.class, Exception.class,
                GenericInterface.class, InterceptedSLSBean.class, MyMainClass.class, TestLocalBean.class,
    ...
    public void badMainClassFormatTest() throws Exception {
        ...
        final ClientModule clientModule = new ClientModule(null, app.getClassLoader(), app.getJarLocation(), null,
                null);
    ...
    public void testResourceAdapter() throws Exception {
        ...
        final List<org.apache.openejb.jee.SecurityPermission> securityPermission = connector.getResourceAdapter()
        ...
        final List<org.apache.openejb.jee.AuthenticationMechanism> authenticationMechanism = connector
                .getResourceAdapter().getOutboundResourceAdapter().getAuthenticationMechanism();
        Assert.assertEquals("authMechanism", authenticationMechanism.get(0).getAuthenticationMechanismType());
        Assert.assertEquals(CredentialInterface.GenericCredential.toString(),
                authenticationMechanism.get(0).getCredentialInterface());
        Assert.assertEquals("description", authenticationMechanism.get(0).getDescription());

        Assert.assertEquals(TransactionSupportType.NO_TRANSACTION,
                connector.getResourceAdapter().getOutboundResourceAdapter().getTransactionSupport());
        Assert.assertEquals(true,
                connector.getResourceAdapter().getOutboundResourceAdapter().isReauthenticationSupport());

        Assert.assertEquals(Connection.class.getName(), connector.getResourceAdapter().getOutboundResourceAdapter()
                .getConnectionDefinition().get(0).getConnectionInterface());
        Assert.assertEquals(ConnectionImpl.class.getName(), connector.getResourceAdapter().getOutboundResourceAdapter()
                .getConnectionDefinition().get(0).getConnectionImplClass());
        Assert.assertEquals(ConnectionFactory.class.getName(), connector.getResourceAdapter()
                .getOutboundResourceAdapter().getConnectionDefinition().get(0).getConnectionFactoryInterface());
        Assert.assertEquals(ConnectionFactoryImpl.class.getName(), connector.getResourceAdapter()
                .getOutboundResourceAdapter().getConnectionDefinition().get(0).getConnectionFactoryImplClass());

        Assert.assertEquals(TestActivation.class.getName(), connector.getResourceAdapter().getInboundResourceAdapter()
                .getMessageAdapter().getMessageListener().get(0).getActivationSpec().getActivationSpecClass());
        Assert.assertEquals(TestMessageListener.class.getName(), connector.getResourceAdapter()
                .getInboundResourceAdapter().getMessageAdapter().getMessageListener().get(0).getMessageListenerType());

        Assert.assertEquals(TestAdminObject.class.getName(),
                connector.getResourceAdapter().getAdminObject().get(0).getAdminObjectClass());
        Assert.assertEquals(TestAdminObjectInterface.class.getName(),
                connector.getResourceAdapter().getAdminObject().get(0).getAdminObjectInterface());
    }

    private ConnectorModule testConnectorModule() {
        final Connector connector = new Connector();
        final ConnectorModule connectorModule = new ConnectorModule(connector);
        connectorModule.setFinder(new ClassFinder(TestConnector.class, TestManagedConnectionFactory.class,
                TestActivation.class, TestAdminObject.class));
    ...
    public void findRestClasses() throws Exception {
        ...
        webModule.setFinder(
                new AnnotationFinder(new ClassesArchive(RESTClass.class, RESTMethod.class, RESTApp.class)).link());
    ...
    @javax.resource.spi.Connector(description = "description", displayName = "displayName", smallIcon = "smallIcon", largeIcon = "largeIcon", vendorName = "vendorName", eisType = "eisType", version = "version", licenseDescription = {
            "licenseDescription" }, licenseRequired = true, authMechanisms = {
                    @AuthenticationMechanism(authMechanism = "authMechanism", credentialInterface = CredentialInterface.GenericCredential, description = {
                            "description" }) }, reauthenticationSupport = true, securityPermissions = {
                                    @SecurityPermission(permissionSpec = "permissionSpec", description = "description") }, transactionSupport = TransactionSupportLevel.NoTransaction, requiredWorkContexts = {
                                            TestWorkContext.class })
    public static class TestConnector implements ResourceAdapter {

        public void endpointActivation(final MessageEndpointFactory mef, final ActivationSpec spec)
    ...
    public static class TestManagedConnectionFactory implements ManagedConnectionFactory {
        ...
        public ManagedConnection matchManagedConnections(final Set managedConnections, final Subject subject,
                final ConnectionRequestInfo connectionRequestInfo) throws ResourceException {
            return null;
        }
