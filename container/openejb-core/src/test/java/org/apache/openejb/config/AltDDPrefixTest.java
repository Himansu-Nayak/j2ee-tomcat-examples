...
public class AltDDPrefixTest extends TestCase {
    ...
    /**
     * This module has only a "test.ejb-jar.xml" file and no equivalent non-test ejb-jar.xml file. It should still get
     * discovered even though there is not an ejb-jar.xml file and only a test.ejb-jar.xml file
     *
     * @throws Exception
     */
    ...
    public void testMultitplePrefixes() throws Exception {
        ...
        // 4 + ComponentName
        assertEquals("EjbJar.enterpriseBeans.get(0).jndiEnc.envEntries.size()", 5,
                ejbJar.enterpriseBeans.get(0).jndiEnc.envEntries.size());

    }

    /**
     * OPENEJB-1059: altdd does not work with a persistence.xml when no embedded in EjbModule/Client module.
     *
     * @throws Exception
