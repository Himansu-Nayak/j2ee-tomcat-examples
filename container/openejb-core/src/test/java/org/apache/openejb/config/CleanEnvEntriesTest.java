...
public class CleanEnvEntriesTest extends TestCase {
    ...
    public void test() throws Exception {
        ...
        singletonBean.getEnvEntry().add(new EnvEntry().name("message").type(String.class).value("hello")
        ...
        singletonBean.getEnvEntry().add(new EnvEntry().name("value-but-no-type1").value("10")
                .injectionTarget(Blue.class, "number").injectionTarget(Orange.class, "number") // attempt to confuse the type
        );

        singletonBean.getEnvEntry()
                .add(new EnvEntry().name("value-but-no-type2").value("D").injectionTarget(Blue.class, "letter"));

        singletonBean.getEnvEntry()
                .add(new EnvEntry().name("value-but-no-type3").value("2").injectionTarget(Blue.class, "vague") // short
