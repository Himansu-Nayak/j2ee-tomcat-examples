...
public class EntityManagerPropogationTest extends TestCase {
    ...
    private void _testEMClose() throws Exception {
        ...
        final PleaseCloseMyExtendedEmBean checkExtendedWorks = (PleaseCloseMyExtendedEmBean) ctx
                .lookup("PleaseCloseMyExtendedEmLocalBean");
    ...
    /**
     * Test that the extended persistence context can survive the removal of the parent
     *
     * @throws Exception
     */
    ...
    /**
     * Test that two Stateful session bean siblings do not share the same extended persistence context
     * <p/>
     * A stateful session bean must be a child in order for the context to be propogated to that bean.
     *
     * @throws Exception
     */
    ...
    private void addStatefulBean(final EjbJar ejbJar, final Class<?> ejbClass, final String name,
    ...
    private void addStatelessBean(final EjbJar ejbJar, final Class<?> ejbClass, final String name,
            final String reference) {
    ...
    /**
     * The root stateful session bean is essentially the "owner" of the persistence context and all children downstream
     * with an EXTENDED persistence context should share the same persistence context and underlying EntityManager
     * <p/>
     * Transactions are dissabled to ensure that we aren't relying on the JTA propogation that also exists with an
