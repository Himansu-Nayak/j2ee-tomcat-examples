...
import static org.apache.openejb.config.rules.ValidationAssertions.*;
...
public class InvokeMethod extends Statement {
    ...
    public void evaluate() throws Throwable {
        ...
        final String outputDescriptors = SystemInstance.get().getProperty(OutputGeneratedDescriptors.OUTPUT_DESCRIPTORS,
                "false");
        try {
            ...
            if (!isEmpty(expectedKeys)) {
                if (vc != null && expectedKeys.get(KeyType.FAILURE).isEmpty()
                        && expectedKeys.get(KeyType.ERROR).isEmpty()) {
    ...
    /**
     * Tests to see if the keys specified in the @Keys annotation are also available in the
     * org.apache.openejb.config.rules.Messages.properties file. If there are any invalid keys, then it throws an
     * exception and causes the test to error out. If all the keys are valid, then it returns those keys. This list of
     * keys can then be compared with all the Keys in org.apache.openejb.config.rules.Messages.properties and one can
     * then find out the "test coverage" of the keys i.e. this tool can be used to find keys for which tests have not
     * yet been written
     *
     * @return
     * @throws Exception
    ...
    private Map<Integer, List<String>> validateKeys() throws Exception {
        ...
        if (wrongKeys.isEmpty()) {
            ...
            throw new Exception(
                    "The following keys listed in the @Keys annotation on the method " + testMethod.getName() + "() of "
