...
/**
 * Note: to make this test work under JavaSE 6 you should add geronimo-annotation_1.1_spec in your endorsed dir. It is
 * automatically done with maven.
 */
...
public class DataSourceDefinitionTest {
    ...
    public void assertDatasourceDefinitions() throws Exception {
        assertDataSourceDefinitionValues(multipleDatasources.getMega(), "org.hsqldb.jdbc.JDBCDataSource", "foo1",
                "bar1");
        assertDataSourceDefinitionValues(multipleDatasources.getGiga(), "org.hsqldb.jdbc.JDBCDataSource", "foo2",
                "bar2");
    }

    private void assertDataSourceDefinitionValues(final DataSource dataSource, final String clazz, final String user,
    ...
    private void assertEqualsByReflection(final String message, final Object value, final String name,
    ...
    @DataSourceDefinition(name = "java:comp/env/superDS", className = "org.hsqldb.jdbc.JDBCDataSource", user = "sa", password = "", url = "jdbc:hsqldb:mem:superDS", properties = {
            "poolPreparedStatements = true", "minIdle = 2", "maxOpenPreparedStatements = 20" })
    @Singleton
    ...
    @DataSourceDefinitions({
            @DataSourceDefinition(name = "java:comp/env/superMegaDS", className = "org.hsqldb.jdbc.JDBCDataSource", user = "sa", password = "", url = "jdbc:hsqldb:mem:superDS"),
            @DataSourceDefinition(name = "java:comp/env/superGigaDS", className = "org.hsqldb.jdbc.JDBCDataSource", user = "sa", password = "", url = "jdbc:hsqldb:mem:superDS"
    ...
    public static class Movies {
        ...
        public Movies(final DataSource movieDatabase) throws SQLException {
            ...
            final PreparedStatement stmt = connection
                    .prepareStatement("CREATE TABLE movie ( director VARCHAR(255), title VARCHAR(255), year integer)");
            stmt.execute();
            stmt.close();
            connection.close();
        ...
        public void addMovie(final Movie movie) throws Exception {
            ...
            try {
                final PreparedStatement sql = conn
