...
public class ActiveMQResourceAdapterTest extends TestCase {
    ...
    public void testSchedulerSupport() throws Exception {
        ...
        final String brokerAddress = NetworkUtil.getLocalAddress("broker:(tcp://",
                ")?useJmx=false&schedulerSupport=true");
        resourceAdapter.setBrokerXmlConfig(brokerAddress);
        resourceAdapter.start(null);
        assertTrue(Boolean.class
                .cast(Reflections.get(ActiveMQFactory.getBrokers().iterator().next(), "schedulerSupport")));
        resourceAdapter.stop();

        resourceAdapter.setBrokerXmlConfig(
                NetworkUtil.getLocalAddress("broker:(tcp://", ")?useJmx=false&schedulerSupport=false"));
        resourceAdapter.start(null);
        assertFalse(Boolean.class
                .cast(Reflections.get(ActiveMQFactory.getBrokers().iterator().next(), "schedulerSupport")));
        resourceAdapter.stop();
    }
}
