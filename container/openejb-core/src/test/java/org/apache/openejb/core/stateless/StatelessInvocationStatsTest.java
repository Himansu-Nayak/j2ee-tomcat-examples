 ...
 * javax.management.MBeanOperationInfo[description=, name=FilterAttributes, returnType=void,
 * signature=[javax.management.MBeanParameterInfo[description="", name=excludeRegex, type=java.lang.String,
 * descriptor={}], javax.management.MBeanParameterInfo[description="", name=includeRegex, type=java.lang.String,
 ...
 * javax.management.MBeanAttributeInfo[description=, name=someMethod().GeometricMean, type=double, read-only,
 * descriptor={}] javax.management.MBeanAttributeInfo[description=, name=someMethod().Kurtosis, type=double, read-only,
 * descriptor={}] javax.management.MBeanAttributeInfo[description=, name=someMethod().Max, type=double, read-only,
 * descriptor={}] javax.management.MBeanAttributeInfo[description=, name=someMethod().Mean, type=double, read-only,
 * descriptor={}] javax.management.MBeanAttributeInfo[description=, name=someMethod().Min, type=double, read-only,
 * descriptor={}] javax.management.MBeanAttributeInfo[description=, name=someMethod().Percentile01, type=double,
 * read-only, descriptor={}] javax.management.MBeanAttributeInfo[description=, name=someMethod().Percentile10,
 * type=double, read-only, descriptor={}] javax.management.MBeanAttributeInfo[description=,
 * name=someMethod().Percentile25, type=double, read-only, descriptor={}]
 * javax.management.MBeanAttributeInfo[description=, name=someMethod().Percentile50, type=double, read-only,
 * descriptor={}] javax.management.MBeanAttributeInfo[description=, name=someMethod().Percentile75, type=double,
 * read-only, descriptor={}] javax.management.MBeanAttributeInfo[description=, name=someMethod().Percentile90,
 * type=double, read-only, descriptor={}] javax.management.MBeanAttributeInfo[description=,
 ...
 * javax.management.MBeanAttributeInfo[description=, name=someMethod().StandardDeviation, type=double, read-only,
 * descriptor={}] javax.management.MBeanAttributeInfo[description=, name=someMethod().Sum, type=double, read-only,
 * descriptor={}] javax.management.MBeanAttributeInfo[description=, name=someMethod().Sumsq, type=double, read-only,
 * descriptor={}] javax.management.MBeanAttributeInfo[description=, name=someMethod().Variance, type=double, read-only,
 * descriptor={}] javax.management.MBeanOperationInfo[description=, name=someMethod().setSampleSize, returnType=void,
 * signature=[javax.management.MBeanParameterInfo[description=, name=p1, type=int, descriptor={}]], impact=unknown,
 * descriptor={}] javax.management.MBeanOperationInfo[description=, name=someMethod().sortedValues, returnType=[D,
 * signature=[], impact=unknown, descriptor={}] javax.management.MBeanOperationInfo[description=,
 * name=someMethod().values, returnType=[D, signature=[], impact=unknown, descriptor={}]
 * <p/>
 * Attribute values that should be tested: javax.management.MBeanAttributeInfo[description=, name=InvocationCount,
 * type=long, read-only, descriptor={}] javax.management.MBeanAttributeInfo[description=, name=InvocationTime,
 * type=long, read-only, descriptor={}]
 * <p/>
 * Atribute values that should be tested per method:
 * <p/>
 * javax.management.MBeanAttributeInfo[description=, name=someMethod().Count, type=long, read-only, descriptor={}]
 * javax.management.MBeanOperationInfo[description=, name=someMethod().values, returnType=[D, signature=[],
 * impact=unknown, descriptor={}] javax.management.MBeanAttributeInfo[description=, name=someMethod().Max, type=double,
 * read-only, descriptor={}]
 * <p/>
 * We should test that all expected attributes are there, but when testing each method's values are updated correctly we
 * only need to check that count is correct, values.length == count, max is as expected. To determine that max is as
 * expected one technique would be to make each method sleep for a different amount of time and test that the max is at
 * least that length.
 *
 * @version $Rev$ $Date$
 */
public class StatelessInvocationStatsTest extends TestCase {
    ...
    /**
     * This whole method is a template, feel free to split it anyway you like Fine to have one big
     *
     * @throws Exception
     */
    public void testBasic() throws Exception {
        ...
        System.setProperty(javax.naming.Context.INITIAL_CONTEXT_FACTORY,
                org.apache.openejb.core.LocalInitialContextFactory.class.getName());
        ...
        final ObjectName invocationsName = new ObjectName(
                "openejb.management:J2EEServer=openejb,J2EEApplication=<empty>,EJBModule=StatsModule,StatelessSessionBean=CounterBean,j2eeType=Invocations,name=CounterBean");
        ...
        for (final Map.Entry<String, Object> entry : actualValues.entrySet()) {
            ...
            if (!value.equals(real)) { // tolerating a 1 wide range
                Logger.getLogger(StatelessInvocationStatsTest.class.getName()).log(Level.WARNING, "Test tolerance: "
                        + entry.getKey() + " => " + entry.getValue() + "/" + expectedValues.get(entry.getKey()));
        ...
        expectedOperations.add(new MBeanOperationInfo("FilterAttributes",
        ...
        for (final String s : methods) {
            expectedOperations.add(new MBeanOperationInfo(s + ".setSampleSize", "", invocationParameters2, "void",
                    MBeanOperationInfo.UNKNOWN));
            expectedOperations.add(new MBeanOperationInfo(s + ".sortedValues", "", new MBeanParameterInfo[0], "[D",
                    MBeanOperationInfo.UNKNOWN));
            expectedOperations.add(new MBeanOperationInfo(s + ".values", "", new MBeanParameterInfo[0], "[D",
                    MBeanOperationInfo.UNKNOWN));
        }
    ...
    public void testInvocation() throws Exception {
        ...
        final ObjectName invocationsName = new ObjectName(
                "openejb.management:J2EEServer=openejb,J2EEApplication=<empty>,EJBModule=StatsInvocModule,StatelessSessionBean=CounterBean,j2eeType=Invocations,name=CounterBean");
        ...
        for (final MBeanAttributeInfo info : invocationsMBeanInfo.getAttributes()) {
            ...
            if (info.getName().equals("waitSecs().GeometricMean") || info.getName().equals("waitSecs().Max")
                    || info.getName().equals("waitSecs().Mean") || info.getName().equals("waitSecs().Min")
                    ...
                    || info.getName().equals("waitSecs().Percentile99") || info.getName().equals("waitSecs().Sum")) {
    ...
    /**
     * convenience method for checking out a specific number of instances. Can be used like so:
     * <p/>
     * // checkout some instances from the pool CountDownLatch startingPistol = checkout(bean, 7);
     * <p/>
     * // Look at pool stats ...
     * <p/>
     * // Release them all back into the pool startingPistol.countDown();
     *
     * @param bean
     * @param count
     * @return
