...
public class Addressing$JAXB extends JAXBObject<Addressing> {

    public Addressing$JAXB() {
        super(Addressing.class, null,
                new QName("http://java.sun.com/xml/ns/javaee".intern(), "addressingType".intern()),
                AddressingResponses$JAXB.class);
    }
    ...
    public static void writeAddressing(final XoXMLStreamWriter writer, final Addressing addressing,
            final RuntimeContext context) throws Exception {
        _write(writer, addressing, context);
    }

    public final static Addressing _read(final XoXMLStreamReader reader, RuntimeContext context) throws Exception {
        ...
        if (xsiType != null) {
            if (("addressingType" != xsiType.getLocalPart())
        ...
        for (final XoXMLStreamReader elementReader : reader.getChildElements()) {
            if (("enabled" == elementReader.getLocalName())
                    && ("http://java.sun.com/xml/ns/javaee" == elementReader.getNamespaceURI())) {
                // ELEMENT: enabled
                addressing.enabled = ("1".equals(elementReader.getElementAsString())
                        || "true".equals(elementReader.getElementAsString()));
            } else if (("required" == elementReader.getLocalName())
                    && ("http://java.sun.com/xml/ns/javaee" == elementReader.getNamespaceURI())) {
                // ELEMENT: required
                addressing.required = ("1".equals(elementReader.getElementAsString())
                        || "true".equals(elementReader.getElementAsString()));
            } else if (("responses" == elementReader.getLocalName())
                    && ("http://java.sun.com/xml/ns/javaee" == elementReader.getNamespaceURI())) {
                // ELEMENT: responses
                final AddressingResponses responses = parseAddressingResponses(elementReader, context,
                ...
                context.unexpectedElement(elementReader, new QName("http://java.sun.com/xml/ns/javaee", "enabled"),
                        ...
                        new QName("http://java.sun.com/xml/ns/javaee", "responses"));
            }
        }
    ...
    public final Addressing read(final XoXMLStreamReader reader, final RuntimeContext context) throws Exception {
        return _read(reader, context);
    }
