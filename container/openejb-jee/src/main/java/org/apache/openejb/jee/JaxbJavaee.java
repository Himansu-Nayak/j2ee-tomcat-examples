...
public class JaxbJavaee {
    ...
    public static <T> void marshal(final Class<T> type, final Object object, final OutputStream out)
     ...
     *             is the SAX parser can not be configured
     * @throws SAXException
     *             if there is an xml problem
     * @throws JAXBException
     *             if the xml cannot be marshalled into a T.
     */
    public static <T> Object unmarshalJavaee(final Class<T> type, final InputStream in)
     ...
     *             if the xml cannot be marshalled into a T.
     */
    public static <T> Object unmarshal(final Class<T> type, final InputStream in, final boolean validate)
            throws ParserConfigurationException, SAXException, JAXBException {
     ...
     *             if the xml cannot be marshalled into a T.
     */
    public static <T> Object unmarshalTaglib(final Class<T> type, final InputStream in)
            throws ParserConfigurationException, SAXException, JAXBException {
        final InputSource inputSource = new InputSource(in);
     ...
     *             if the xml cannot be marshalled into a T.
     */
    public static <T> Object unmarshalHandlerChains(final Class<T> type, final InputStream in)
            throws ParserConfigurationException, SAXException, JAXBException {
        final InputSource inputSource = new InputSource(in);
        ...
        final JaxbJavaee.HandlerChainsNamespaceFilter xmlFilter = new JaxbJavaee.HandlerChainsNamespaceFilter(
                parser.getXMLReader());
        xmlFilter.setContentHandler(unmarshaller.getUnmarshallerHandler());
    ...
    /**
     * validate the inputStream, which should be a Java EE standard deployment descriptor against its schema type Note,
     * this method will use the new Java EE 6 schema to validate the old descriptors after changing their namespace and
     * version.
     *
     * @param type
     * @param in
     ...
     * @throws IOException
     */
    public static void validateJavaee(final JavaeeSchema type, final InputStream in)
        ...
        final Schema schema = schemaFactory.newSchema(new Source[] { new StreamSource(xmlSchemaURL.openStream()),
    ...
    public static class TaglibNamespaceFilter extends XMLFilterImpl {
        ...
        @Override
        public void startElement(final String uri, String localName, final String qname, final Attributes atts)
                throws SAXException {
    ...
    public static class Javaee6SchemaFilter extends XMLFilterImpl {
        ...
        private Attributes fixVersion(final String localName, final Attributes atts) {
            ...
            if (localName.equals("application") && atts.getIndex("version") != -1
            ...
            if (localName.equals("application-client") && atts.getIndex("version") != -1
                    && !atts.getValue(atts.getIndex("version")).equals("6")) {
                final AttributesImpl newAtts = new AttributesImpl(atts);
                newAtts.setValue(newAtts.getIndex("version"), "6");
                return newAtts;
