...
/**
 * To implement your own context, you need to create an implementation of TestContext which would configure the test
 * when instructed. You can then use whatever method of configuration you choose.
 */
public class OpenEjbTestContext implements TestContext {
    ...
    /**
     * Constructs the configuration needed to create the InitialContext. This will be determined from the class/method
        ...
        if (!loadedConfig) {
            ...
            if (env.size() == 0) {
                throw new IOException(
                        "Context configuration failed to load, so OpenEJB won't load either. Specify configuration "
    ...
    /**
     * Loads the configuration file specified in the {@link org.apache.openejb.junit.ContextConfig} annotation into the
     * specified Properties instance
     *
     * @param env
     * @param contextConfig
     * @return true if any properties were loaded
     */
    protected boolean loadConfigFile(final Properties env, final ContextConfig contextConfig) throws IOException {
        // properties file
        if (contextConfig.configFile().length() > 0) {
            final InputStream in = clazz.getResourceAsStream(contextConfig.configFile());
            if (in == null) {
                throw new FileNotFoundException(
                        "Cannot find resource '" + contextConfig.configFile() + "' in classpath: " + clazz.getName());
            }
            env.load(in);
    ...
    /**
     * Performs any non-OpenEJB type injections on the test object. It will "prefer" a setter, and therefore I made it
     * work on private fields as well. So if you are injecting to a private field and wish to have some control over it,
     * create a setter according to the JavaBeans idioms.
     * <p/>
     * If the setter isn't found OR it fails, then an attempt will be made to set it directly, and a message will be
    ...
    private void performInjections(final Object testObj) throws Exception {
        for (final Field field : clazz.getDeclaredFields()) {
            ...
            try {
                ...
                System.err.println(
                        "Failed to perform setter injection on: " + clazz.getCanonicalName() + "." + field.getName());
                e.printStackTrace();
            }
            ...
            try {
                ...
                throw new OpenEJBRuntimeException(
                        "Failed to inject on: " + clazz.getCanonicalName() + "." + field.getName(), e);
            }
        }
    }
    ...
    protected Object getInjectionValue(final Field field) throws Exception {
        ...
        if (field.isAnnotationPresent(TestResource.class)) {
            ...
            if (resourceType == null) {
                throw new IllegalArgumentException("Null TestResource type '" + resourceType + "' on field: "
    ...
    /**
     * Override to perform custom resource types injection. This method will be called when whatever value was specified
     * in the {@link TestResource} annotation wasn't understood by the {@link #performInjections(java.lang.Object) }
     * method. This method will be called, supplying the annotation, and you can then interpret and create the value to
     * be injected. By default this method just returns null.
     * <p/>
     * You can use this to inject values into annotated fields which contain custom values in their names.
     *
