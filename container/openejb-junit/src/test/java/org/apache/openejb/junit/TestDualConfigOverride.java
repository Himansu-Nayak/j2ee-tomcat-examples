...
/**
 * Here we test class level dual config, with method level overrides. We check some properties in all tests (even when
 * not in it's config), to ensure overrides work properly, and also that context sharing doesn't happen.
 */
@RunWith(OpenEjbRunner.class)
@ContextConfig(configFile = "/META-INF/test-config.properties", properties = {
...
public class TestDualConfigOverride {
    ...
    @Test
    @ContextConfig(configFile = "/META-INF/test-config-method.properties")
    ...
    @Test
    @ContextConfig(properties = { @Property("junit.test-property-override3=New Method Property Value 3"),
    ...
    @Test
    @ContextConfig(configFile = "/META-INF/test-config-method.properties", properties = {
