...
public class Agent {
    ...
    /**
     * Gets the instrumentation instance. You must have java.lang.ReflectPermission(suppressAccessChecks) to call this
     * method
     *
    ...
    /**
     * Try to find the openejb-javaagent jar, and if not found create a new jar file for the sole purpose of specifying
     * an Agent-Class to load into the JVM.
     */
    private static String getAgentJar() throws IOException {
        final URL resource = Agent.class.getClassLoader()
                .getResource(Agent.class.getName().replace('.', '/') + ".class");
        if (resource == null) {
