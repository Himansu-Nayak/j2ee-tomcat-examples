...
public class Embedder {
    public static final String ADDITIONAL_LIB_FOLDER = SystemInstance.get().getOptions()
            .get("openejb.additional.lib.folder", "additionallib");
    ...
    public Object init(final Properties properties) throws Exception {
        ...
        try {
            ...
            throw (IllegalStateException) new IllegalStateException("Signatures for Loader are no longer correct.")
                    .initCause(e);
    ...
    private Class forcefulLoad(final ClassPath classPath, final ClassLoader classLoader) throws Exception {
        try {
            ...
            throw new Exception(
                    "Could not load OpenEJB libraries. Exception: " + e2.getClass().getName() + " " + e2.getMessage());
        }
        ...
        try {
            ...
            try {
                ...
                throw new Exception("Could not load class '" + className + "' after embedding libraries. Exception: "
                        + le.getClass().getName() + " " + le.getMessage());
            }
        } catch (final Exception e2) {
            throw new Exception("Could not load class '" + className + "' after embedding libraries. Exception: "
