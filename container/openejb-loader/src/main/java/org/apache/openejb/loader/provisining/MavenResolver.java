...
public class MavenResolver implements ArchiveResolver, ProvisioningResolverAware {
    ...
    private static String m2Home() {
        ...
        String home = "";
        File f = new File(
                properties.getProperty("openejb.m2.home", System.getProperty("user.home") + "/.m2/repository/"));

        if (f.exists()) {
            home = f.getAbsolutePath();
        } else {
            f = new File(
                    properties.getProperty("openejb.m2.home", System.getProperty("user.home") + "/.m2/settings.xml"));
            if (f.exists()) {
                try {
    ...
    private String mvnArtifactPath(final String toParse, final String snapshotBase) throws MalformedURLException {
        ...
        final String base = snapshotBase == null || snapshotBase.isEmpty() ? ""
                : (snapshotBase + (!snapshotBase.endsWith("/") ? "/" : ""));
