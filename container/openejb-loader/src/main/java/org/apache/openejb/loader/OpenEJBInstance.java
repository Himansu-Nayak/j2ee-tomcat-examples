    ...
    private Class<?> loadOpenEJBClass() throws Exception {
        ...
        try {
            ...
            try {
                ...
                throw new Exception("Could not load OpenEJB libraries. Exception: " + e2.getClass().getName() + " "
                        + e2.getMessage());
            }
            try {
                return classLoader.loadClass("org.apache.openejb.OpenEJB");
            } catch (final Exception e2) {
                throw new Exception("Could not load OpenEJB class after embedding libraries. Exception: "
