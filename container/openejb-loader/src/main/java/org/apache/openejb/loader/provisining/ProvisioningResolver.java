...
public class ProvisioningResolver {
        ...
        if (SystemInstance.isInitialized()) {
            ...
            if (userOnes != null) {
                for (final String u : userOnes.split(" *, *")) {
                    ...
                    try {
                        final ArchiveResolver instance = ArchiveResolver.class
                                .cast(ProvisioningResolver.class.getClassLoader().loadClass(c));
    ...
    public void addResolver(final ArchiveResolver resolver) {
        if (resolvers.put(resolver.prefix(), resolver) != null) {
            Logger.getLogger(ProvisioningResolver.class.getName())
                    .warning("Overriding resolver " + resolver.prefix() + " with " + resolver);
        }
    }
        ...
        if (rawLocation.endsWith("*.jar")) {
            ...
            if (dir.exists()) {
                ...
                throw new IllegalArgumentException(
                        dir.getAbsolutePath() + " doesn't exist, *.jar is only supported locally");
            }
        }
    ...
    private String protocol(final String rawLocation) {
        ...
        if (sep < 1) {
            throw new IllegalArgumentException(
                    "Not a local file but no prefix specified: " + rawLocation + ". So can't resolve");
        }
    ...
    public String toString() {
        return "ProvisioningResolver{" + "resolvers=" + resolvers + ", fallbackResolver=" + fallbackResolver + '}';
    }
