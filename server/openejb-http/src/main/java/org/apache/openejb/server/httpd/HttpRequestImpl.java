...
public class HttpRequestImpl implements HttpRequest {
            ...
            if (cookie != null) {
                ...
                for (String c : cookies) {
                    ...
                    if (current.startsWith("EJBSESSIONID=")) {
                        final SessionManager.SessionWrapper sessionWrapper = SystemInstance.get()
                                .getComponent(SessionManager.class)
                                .findSession(current.substring("EJBSESSIONID=".length()));
                        session = sessionWrapper == null ? null : sessionWrapper.session;
                    } else if (current.startsWith("JSESSIONID=")) {
                        final SessionManager.SessionWrapper sessionWrapper = SystemInstance.get()
                                .getComponent(SessionManager.class)
                                .findSession(current.substring("JSESSIONID=".length()));
                        session = sessionWrapper == null ? null : sessionWrapper.session;
                    }
                }
            }
    ...
    private boolean readRequestLine(DataInput in) throws IOException {
        ...
        try {
            ...
            throw new IOException(
                    "Could not read the HTTP Request Line :" + e.getClass().getName() + " : " + e.getMessage());
        }
        if (line == null) {
            return false;
        }
    ...
    private void parseMethod(StringTokenizer lineParts) throws IOException {
        ...
        try {
            ...
            throw new IOException(
                    "Could not parse the HTTP Request Method :" + e.getClass().getName() + " : " + e.getMessage());
        }
        ...
        /*
         * if (token.equalsIgnoreCase("GET")) { method = Method.GET.name(); } else if (token.equalsIgnoreCase("POST")) {
         * method = Method.POST.name(); } else if (token.equalsIgnoreCase("PUT")) { method = Method.PUT.name(); } else
         * if (token.equalsIgnoreCase("DELETE")) { method = Method.DELETE.name(); } else if
         * (token.equalsIgnoreCase("HEAD")) { method = Method.HEAD.name(); } else if (token.equalsIgnoreCase("OPTIONS"))
         * { method = Method.HEAD.name(); } else if (token.equalsIgnoreCase("PATCH")) { method = Method.PATCH.name(); }
         * else { method = Method.UNSUPPORTED.name(); throw new IOException("Unsupported HTTP Request Method :" +
         * token); }
         */
    }
     ...
     *
     * @param query
     *            the URL (or query) parameters to be parsed
     */
     ...
     *
     * @param in
     *            the data input sent from the browser
     * @throws java.io.IOException
     *             if an exeption is thrown
     */
    private void readHeaders(DataInput in) throws IOException {
        ...
        while (true) {
            ...
            try {
                ...
                throw new IOException("Could not read the HTTP Request Header Field :" + e.getClass().getName() + " : "
                        + e.getMessage());
            }
        ...
        if (host != null) {
            ...
            try {
                uri = new URI(uri.getScheme(), uri.getUserInfo(), hostName, port, uri.getPath(), uri.getQuery(),
                        uri.getFragment());
     ...
     *
     * @param in
     *            the data input with the body of the page
     * @throws java.io.IOException
     *             if an exception is thrown
     */
    private void readBody(DataInput in) throws IOException {
        ...
        if (hasBody && contentType != null
                && (contentType.startsWith(FORM_URL_ENCODED) || contentType.startsWith(MULTIPART_FORM_DATA))) {
            String rawParams;
            ...
            try {
                ...
                throw (IOException) new IOException("Could not read the HTTP Request Body: " + e.getMessage())
                        .initCause(e);
            }
    ...
    public AsyncContext getAsyncContext() {
        ...
        return new OpenEJBAsyncContext(this /* TODO */, HttpResponse.class.cast(getAttribute("openejb_response")),
                contextPath);
    }
    ...
    public AsyncContext startAsync(final ServletRequest servletRequest, final ServletResponse servletResponse) {
        setAttribute("openejb_async", "true");
        final OpenEJBAsyncContext asyncContext = new OpenEJBAsyncContext(
                HttpServletRequest.class.cast(servletRequest) /* TODO */, servletResponse, contextPath);
        asyncContext.internalStartAsync();
        asyncStarted = true;
        final WebBeansContext webBeansContext = WebBeansContext.currentInstance();
        return webBeansContext != null ? new EEFilter.AsynContextWrapper(asyncContext, servletRequest, webBeansContext)
                : asyncContext;
    }
    ...
    public ServletContext getServletContext() { // we need a not null value but it is not intended to be used in standalone for now
        if (context == null) {
            context = (ServletContext) Proxy.newProxyInstance(Thread.currentThread().getContextClassLoader(),
                    SERVLET_CONTEXT_INTERFACES, SERVLET_CONTEXT_HANDLER);
        }
        return context;
    }
    ...
    public void init() {
        ...
        listeners = LightweightWebAppBuilderListenerExtractor.findByTypeForContext(contextPath,
    ...
    private static class SimpleDispatcher implements RequestDispatcher {
        ...
        public void forward(final ServletRequest request, final ServletResponse response)
            ...
            try {
                SystemInstance.get().getComponent(HttpListenerRegistry.class).onMessage(httpRequest,
    ...
    protected class SessionInvalidateListener extends ServletSessionAdapter {
        ...
        public SessionInvalidateListener(final javax.servlet.http.HttpSession session,
                final BeginWebBeansListener end) {
            super(session);
            listener = end;
        }
