...
public class SessionManager {
    ...
    public void initEviction() {
        ...
        final Duration duration = new Duration(
                SystemInstance.get().getProperty("openejb.http.eviction.duration", "1 minute"));
        ...
        es.scheduleWithFixedDelay(new Runnable() {
            ...
            public void run() {
                for (final SessionWrapper data : new ArrayList<>(sessions.values())) {
                    ...
                    if (session.getMaxInactiveInterval() > 0 && session.getLastAccessedTime()
                            + TimeUnit.SECONDS.toMillis(session.getMaxInactiveInterval()) < System
                                    .currentTimeMillis()) {
                        doDestroy(data);
    ...
    public static class SessionWrapper extends HttpSessionEvent {
        ...
        public SessionWrapper(final BeginWebBeansListener begin, final EndWebBeansListener end,
                final HttpSession session, final WebContext app) {
            super(session);
            this.begin = begin;
            this.end = end;
