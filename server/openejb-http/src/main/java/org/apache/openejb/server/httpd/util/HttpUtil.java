...
public final class HttpUtil {
    ...
    public static void addDefaultsIfAvailable(final WebContext wc) {
        try {
            ...
            SystemInstance.get().getComponent(ServletContext.class).setAttribute("org.apache.tomcat.InstanceManager",
                    wc.getClassLoader().loadClass("org.apache.tomee.catalina.JavaeeInstanceManager")
        ...
        try {
            ...
            if ("javax.faces.webapp.FacesServlet".equals(classname)) {
                try {
                    ...
                    final Class<?> mfListenerClass = wc.getClassLoader()
                            .loadClass("org.apache.myfaces.webapp.StartupServletContextListener");

                    final ServletContextListener servletContextListener = ServletContextListener.class
                            .cast(mfListenerClass.newInstance());
                    servletContext.setAttribute("javax.enterprise.inject.spi.BeanManager",
                            new InjectableBeanManager(wc.getWebBeansContext().getBeanManagerImpl()));
    ...
    private static ClassLoader setClassLoader(final WebContext wc, final Thread thread) {
        ...
        thread.setContextClassLoader(
                wc.getClassLoader() == null ? wc.getAppContext().getClassLoader() : wc.getClassLoader());
        return old;
    }
    ...
    public static void removeServlet(final String mapping, final WebContext wc) {
        ...
        final Servlet servlet = ((ServletListener) registry.removeHttpListener(pattern(wc.getContextRoot(), mapping)))
                .getDelegate();
        servlet.destroy();
        wc.destroy(servlet);
        if (servlet.getClass().equals("org.apache.jasper.servlet.JspServlet")) {
            SystemInstance.get().getComponent(ServletContext.class)
                    .removeAttribute("org.apache.tomcat.InstanceManager");
        }
    }

    public static boolean addFilter(final String classname, final WebContext wc, final String mapping,
        ...
        try {
            listener = new FilterListener((Filter) wc.newInstance(wc.getClassLoader().loadClass(classname)),
                    wc.getContextRoot());
