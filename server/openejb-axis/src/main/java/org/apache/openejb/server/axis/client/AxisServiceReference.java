...
public class AxisServiceReference extends Reference {
    ...
    private Object createServiceInterfaceProxy(String serviceInterfaceClassName, Map seiPortNameToFactoryMap,
        ...
        if (initialize) {
            ...
            try {
                ...
                throw (NamingException) new NamingException(
                        "Could not load service interface class " + serviceInterfaceClassName).initCause(e);
            }
            ...
            // get constructor
            this.serviceConstructor = FastClass.create(this.enhancedServiceClass)
                    .getConstructor(SERVICE_CONSTRUCTOR_TYPES);
        }
        ...
        try {
            ...
            throw (NamingException) new NamingException("Could not construct service instance")
        ...
        if (initialize) {
            for (Iterator iterator = seiPortNameToFactoryMap.values().iterator(); iterator.hasNext();) {
                ...
                try {
                    ...
                    throw (NamingException) new NamingException(
                            "Could not load service interface class; " + e.getMessage()).initCause(e);
                }
            }
        }
