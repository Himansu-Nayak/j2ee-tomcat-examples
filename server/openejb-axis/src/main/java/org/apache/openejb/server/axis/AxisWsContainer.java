...
public class AxisWsContainer implements HttpListener {
    ...
    public AxisWsContainer(final URL wsdlURL, final SOAPService service, final Map wsdlMap,
    ...
    protected void doService(final HttpRequest req, final HttpResponse res) throws Exception {
        ...
        try {
            try {
                final String characterEncoding = (String) requestMessage
            ...
            if (messageContext.getOperation() != null) {
                if (messageContext.getOperation().getMep() == OperationType.ONE_WAY) {
                    ...
                    responseMessage = handleException(messageContext, null,
                            new ServerRuntimeException("No response for non-one-way operation"));
                }
            } else if (responseMessage == null) {
                res.setStatus(HttpServletResponse.SC_ACCEPTED);
                return;
    ...
    public void getWsdl(final HttpRequest request, final HttpResponse response) throws Exception {
        ...
        if (query == null || !query.toLowerCase().startsWith("wsdl")) {
            throw new IllegalStateException(
                    "request must contain a  wsdl or WSDL parameter: " + request.getParameters());
        }
        ...
        final URI updated = new URI(realLocation.getScheme(), realLocation.getUserInfo(), realLocation.getHost(),
                realLocation.getPort(), null, //try null for no path
                null, null);
    ...
    private void printServiceInfo(final HttpResponse response, final String serviceName) throws IOException {
        ...
        final StringBuffer output = new StringBuffer("<h1>").append(serviceName).append("</h1>\n");
