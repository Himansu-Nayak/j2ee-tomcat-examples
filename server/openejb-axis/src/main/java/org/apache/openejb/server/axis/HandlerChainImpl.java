...
public class HandlerChainImpl extends ArrayList implements javax.xml.rpc.handler.HandlerChain {
        ...
        for (int i = 0; i < handlerInfos.size(); i++) {
            ...
            try {
                ...
                throw new JAXRPCException(
                        "Unable to initialize handler class: " + handlerInfo.getHandlerClass().getName(), e);
            }
        }
    }
    ...
    public boolean handleRequest(MessageContext context) {
        ...
        if (!snapshot.equals(context)) {
            throw new IllegalStateException(
                    "The soap message operation or arguments were illegally modified by the HandlerChain");
        }
        return true;
    }
     ...
     * Handlers cannot:
     * <p/>
     * - re-target a request to a different component. - change the operation - change the message part types - change
