...
public class HeavyweightOperationInfoBuilder {
     ...
     * <p/>
     * wrapped-element tag is used WSDL message with a single part part uses the 'element' attribute to point to an
     * elemement in the types section the element type and the element's name match the operation name
     */
    ...
    public HeavyweightOperationInfoBuilder(BindingOperation bindingOperation,
            ServiceEndpointMethodMapping methodMapping, JavaWsdlMapping mapping, XmlSchemaInfo schemaInfo)
        ...
        if (methodMapping.getWrappedElement() != null) {
            ...
            SOAPOperation soapOperation = JaxRpcServiceInfoBuilder.getExtensibilityElement(SOAPOperation.class,
                    bindingOperation.getExtensibilityElements());
            String styleString = soapOperation.getStyle();
            if (styleString == null) {
                SOAPBinding soapBinding = JaxRpcServiceInfoBuilder.getExtensibilityElement(SOAPBinding.class,
                        bindingInput.getExtensibilityElements());
                styleString = soapBinding.getStyle();
            }

            SOAPBody soapBody = JaxRpcServiceInfoBuilder.getExtensibilityElement(SOAPBody.class,
                    bindingInput.getExtensibilityElements());
        ...
        if (outputMessage != null && bindingStyle.isWrapped()) {
            ...
            if (!outParamNames.equals(expectedOutParams)) {
                throw new OpenEJBException(
                        "Not all wrapper children were mapped to parameters or a return value for operation "
                                + operationName);
            }
        } else if (null != outputMessage) {
            if (!outParamNames.equals(outputMessage.getParts().keySet())) {
                throw new OpenEJBException(
                        "Not all output message parts were mapped to parameters or a return value for operation "
                                + operationName);
            }
        }
    ...
    private JaxRpcParameterInfo[] mapParameters() throws OpenEJBException {
        ...
        for (int i = 0; i < parameterInfos.length; i++) {
            if (parameterInfos[i] == null) {
                throw new OpenEJBException(
                        "There is no mapping for parameter number " + i + " for operation " + operationName);
            }
        }
        ...
        if (bindingStyle.isWrapped()) {
            ...
            if (!inParamNames.equals(inputMessage.getParts().keySet())) {
                throw new OpenEJBException(
                        "Not all input message parts were mapped for operation name" + operationName);
            }
        }
    ...
    private JaxRpcParameterInfo mapParameter(MethodParamPartsMapping paramMapping) throws OpenEJBException {
        ...
        if ((mode == Mode.OUT || mode == Mode.INOUT) && outputMessage == null) {
            throw new OpenEJBException("Mapping for output parameter " + wsdlMessagePartName
                    + " found, but no output message for operation " + operationName);
        }
        ...
        if (mode == Mode.IN || mode == Mode.INOUT) {
            ...
            if (!wsdlMessageQName.equals(inputMessage.getQName())) {
                throw new OpenEJBException(
                        "QName of input message: " + inputMessage.getQName() + " does not match mapping message QName: "
            ...
            if (bindingStyle.isWrapped()) {
                ...
                if (!name.getLocalPart().equals(operationName)) {
                    throw new OpenEJBException("message " + inputMessage.getQName()
                            + " refers to a global element named " + name.getLocalPart()
                            + ", which is not equal to the operation name " + operationName);
                }
                ...
                if (part == null) {
                    throw new OpenEJBException("No part for wsdlMessagePartName " + wsdlMessagePartName
            ...
            if (mode == Mode.INOUT) {
                if (bindingStyle.isWrapped()) {
                    ...
                    if (inParameter.xmlType != outParameter.xmlType) {
                        throw new OpenEJBException("The wrapper children " + wsdlMessagePartName
                    ...
                    if (!part.getName().equals(outPart.getName())) {
                        throw new OpenEJBException(
                                "Mismatched input part name: " + part.getName() + " and output part name: "
                                        + outPart.getName() + " for INOUT parameter for wsdlMessagePartName "
                    ...
                    if (!(part.getElementName() == null ? outPart.getElementName() == null
                        ...
                        throw new OpenEJBException("Mismatched input part element name: " + part.getElementName()
                                + " and output part element name: " + outPart.getElementName()
                                + " for INOUT parameter for wsdlMessagePartName " + wsdlMessagePartName
                                + " for operation " + operationName);
                    }
                    if (!(part.getTypeName() == null ? outPart.getTypeName() == null
                            : part.getTypeName().equals(outPart.getTypeName()))) {
                        throw new OpenEJBException("Mismatched input part type name: " + part.getTypeName()
                                + " and output part type name: " + outPart.getTypeName()
                                + " for INOUT parameter for wsdlMessagePartName " + wsdlMessagePartName
                                + " for operation " + operationName);
                    }
                } else {
            ...
            if (!wsdlMessageQName.equals(outputMessage.getQName())) {
                throw new OpenEJBException("QName of output message: " + outputMessage.getQName()
                        + " does not match mapping message QName: " + wsdlMessageQName + " for operation "
                        + operationName);
            }
    ...
    private void mapReturnType() throws OpenEJBException {
        ...
        if (!wsdlReturnValueMapping.getWsdlMessage().equals(outputMessage.getQName())) {
            throw new OpenEJBException(
                    "OutputMessage has QName: " + outputMessage.getQName() + " but mapping specifies: "
                            + wsdlReturnValueMapping.getWsdlMessage() + " for operation " + operationName);
        }
        ...
        if (wsdlReturnValueMapping.getWsdlMessagePartName() != null) {
            ...
            if (outParamNames.contains(wsdlMessagePartName)) {
                throw new OpenEJBException("output message part " + wsdlMessagePartName
                        + " has both an INOUT or OUT mapping and a return value mapping for operation "
                        + operationName);
            }
    ...
    private JaxRpcFaultInfo mapFaults(Fault fault) throws OpenEJBException {
        ...
        if (exceptionMapping == null) {
            throw new OpenEJBException("No exception mapping for fault " + fault.getName() + " and fault message "
                    + message.getQName() + " for operation " + operationName);
        }
        ...
        if (part.getElementName() != null) {
            ...
            if (elementInfo == null) {
                throw new OpenEJBException("Can not find element: " + part.getElementName() + ", known elements: "
                        + schemaInfo.elements.keySet());
            }
            faultTypeInfo = schemaInfo.types.get(elementInfo.xmlType);
            if (faultTypeInfo == null) {
                throw new OpenEJBException("Can not find type " + elementInfo.xmlType + " for element "
                        + elementInfo.qname + ", known types: " + schemaInfo.types.keySet());
            }
            ...
            if (faultTypeInfo == null) {
                throw new OpenEJBException(
                        "Can not find type: " + part.getTypeName() + ", known elements: " + schemaInfo.types.keySet());
            }
        ...
        if (exceptionMapping.getConstructorParameterOrder() != null) {
            if (faultTypeInfo.simpleBaseType != null) {
                throw new OpenEJBException(
                        "ConstructorParameterOrder can only be set for complex types, not " + faultTypeInfo.qname);
            }
            ...
            for (int i = 0; i < constructorParameterOrder.getElementName().size(); i++) {
                ...
                if (paramElementInfo == null) {
                    throw new OpenEJBException("Can not find element " + paramName + " in fault type "
                            + faultTypeInfo.qname + ", known elements: " + elements.keySet());
                }
    ...
    private XmlElementInfo getWrapperChild(Part part, String wsdlMessagePartName) throws OpenEJBException {
        ...
        XmlTypeInfo wrapperType = schemaInfo.types.get(wrapperElement.xmlType);
        if (wrapperType == null) {
            throw new OpenEJBException("Can not find type " + wrapperElement.xmlType + " for element "
                    + wrapperElement.qname + ", known types: " + schemaInfo.types.keySet());
        }
        ...
        throw new OpenEJBException("Global element named " + elementName + " does not define a child element named "
                + wsdlMessagePartName + " required by the operation " + operationName);
    }
}