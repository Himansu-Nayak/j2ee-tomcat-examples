...
import org.apache.openejb.server.axis.assembler.*;
...
public class JavaServiceDescBuilder {
    ...
    public JavaServiceDesc createServiceDesc() throws OpenEJBException {
        ...
        try {
            ...
            throw new OpenEJBException(
                    "Unable to load the service endpoint interface " + serviceInfo.serviceEndpointInterface, e);
        }
    ...
    private OperationDesc buildOperationDesc(JaxRpcOperationInfo operationInfo, Class serviceEndpointInterface)
        ...
        try {
            ...
            throw new OpenEJBException("Mapping references non-existent method in service-endpoint: "
                    + operationInfo.javaMethodName + "(" + args + ")");
        }
    ...
    private ParameterDesc buildParameterDesc(JaxRpcParameterInfo parameterInfo) throws OpenEJBException {
        ...
        ParameterDesc parameterDesc = new ParameterDesc(parameterInfo.qname, mode, parameterInfo.xmlType, javaType,
                inHeader, outHeader);
    ...
    private void registerType(JaxRpcTypeInfo type, TypeMapping typeMapping) throws OpenEJBException {
        ...
        default:
            if (type.simpleBaseType != null) {
                ...
                if (null != clazz) {
                    ...
                    deserializerFactoryClass = SOAP_TYPE_MAPPING.getDeserializer(clazz, type.simpleBaseType, null)
                            .getClass();
                } else {
                    clazz = JAXRPC_TYPE_MAPPING.getClassForQName(type.simpleBaseType, null, null);
                    if (null != clazz) {
                        // Built in XML schema type
                        serializerFactoryClass = JAXRPC_TYPE_MAPPING.getSerializer(clazz, type.simpleBaseType)
                                .getClass();
                        deserializerFactoryClass = JAXRPC_TYPE_MAPPING.getDeserializer(clazz, type.simpleBaseType, null)
                                .getClass();
                    }
                }
            }
        ...
        SerializerFactory serializerFactory = BaseSerializerFactory.createFactory(serializerFactoryClass, javaType,
                type.qname);
        DeserializerFactory deserializerFactory = BaseDeserializerFactory.createFactory(deserializerFactoryClass,
                javaType, type.qname);
