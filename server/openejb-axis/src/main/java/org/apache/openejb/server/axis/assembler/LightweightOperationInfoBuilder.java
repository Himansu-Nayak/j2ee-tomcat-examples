...
public class LightweightOperationInfoBuilder {
    ...
    public JaxRpcOperationInfo buildOperationInfo() throws OpenEJBException {
        ...
        if (methodParamTypes.length != inputParts.size()) {
            throw new OpenEJBException("mismatch in parameter counts: method has " + methodParamTypes.length
                    + " whereas the input message has " + inputParts.size());
        }
        ...
        if (outputMessage != null && outputMessage.getParts().size() > 1) {
            throw new OpenEJBException(
                    "Lightweight mapping has at most one part in the (optional) output message, not: "
                            + outputMessage.getParts().size());
        }
