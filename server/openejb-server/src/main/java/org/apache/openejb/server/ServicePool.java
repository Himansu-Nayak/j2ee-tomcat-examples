...
public class ServicePool extends ServerServiceFilter {
    ...
    public ServicePool(final ServerService next, final Properties properties) {
        /**
         * Defaults. This suggests that 10 core threads should cope with up to 19 runnables (threads + queue, whereby
         * queue = threads - 1). Any more than 19 runnables will spawn a thread to cope if the thread count is less than
         * 150. If 150 threads are processing runnables and the queue is full then block and wait for a slot for up to
         * 10 seconds before rejecting the runnable. If a thread remains idle for more than 1 minute then it will be
         * removed.
         */
    ...
    public ServicePool(final ServerService next, final Options properties) {
        this(next, properties.get("threadsCore", 10), properties.get("threads", 150), properties.get("queue", 0),
                properties.get("block", true), properties.get("keepAliveTime", KEEP_ALIVE_TIME),
    ...
    public ServicePool(final ServerService next, int threadCore, int threads, int queue, final boolean block,
            long keepAliveTime, boolean forceClose) {
        super(next);
        ...
        /**
         * This thread pool starts with 'threadCore' core threads and can grow to the limit defined by 'threads'. If a
         * pool thread is idle for more than 1 minute it will be discarded, until the core size is reached. It can
         * accept upto the number of runnables defined by 'queue' + 'threadCore' before the pool will grow, so the
         * 'queue' should ideally be less than 'threadsCore', but certainly less than 'threads'. If the 'queue' and
         * 'threads' are full then an attempt is made to add the runnable to the queue for 10 seconds. Failure to add to
         * the queue in this time will either result in a logged rejection, or if 'block' is true then a final attempt
         * is made to run the runnable in the current thread (the service thread).
         */
        ...
        threadPool = new ThreadPoolExecutor(threadCore, threads, keepAliveTime, TimeUnit.MILLISECONDS,
                new LinkedBlockingQueue<Runnable>(queue), new ThreadFactory() {
                    ...
                    public Thread newThread(final Runnable r) {
                        final Thread t = new Thread(r,
                                "OpenEJB." + ServicePool.this.getName() + "." + i.incrementAndGet());
                        t.setDaemon(true);
                        t.setUncaughtExceptionHandler(new Thread.UncaughtExceptionHandler() {
                            @Override
                            public void uncaughtException(final Thread t, final Throwable e) {
                                log.error("ServicePool '" + ServicePool.this.getName() + "': Uncaught error in: "
                                        + t.getName(), e);
                            }
                        });
                    ...
                    public void rejectedExecution(final Runnable r, final ThreadPoolExecutor tpe) {
                        ...
                        if (log.isWarningEnabled()) {
                            log.warning(String.format(
                                    "ServicePool '" + ServicePool.this.getName()
                                            + "' with (%1$s) threads is at capicity (%2$s) for queue (%3$s) on process: %4$s"
                        ...
                        if (!offer) {
                            ...
                            if (block) {
                                try {
                                    ...
                                    log.warning("ServicePool '" + ServicePool.this.getName()
                                            + "' forced execution on the current server thread: " + r
                                    ...
                                    log.error("ServicePool '" + ServicePool.this.getName()
                                            + "' failed to run a process in the current server thread: " + r);
                                }
                            } else {
                                log.error("ServicePool '" + ServicePool.this.getName()
        ...
        if (log.isInfoEnabled()) {
            log.info(String.format(
                    "Created ServicePool '%1$s' with (%2$s) core threads, limited to (%3$s) threads with a queue of (%4$s)",
                    getName(), c, t, q));
        }
    }
        ...
        final Runnable ctxCL = new Runnable() {
            ...
            public void run() {
                ...
                try {
                    ...
                    final String msg = "ServicePool '" + ServicePool.this.getName() + "': Security error: "
                            + e.getMessage();
                    ...
                    final String msg = "ServicePool '" + ServicePool.this.getName() + "': Unexpected IO error: "
                            + e.getMessage();
                    if (log.isDebugEnabled()) {
                        log.debug(msg, e);
                    } else {
                    ...
                    final String msg = "ServicePool '" + ServicePool.this.getName() + "': Unexpected error: "
                            + e.getMessage();
                    if (log.isDebugEnabled()) {
                        log.error(msg, e);
                    } else {
    ...
    public class Pool {
        ...
        public void setCorePoolSize(final int corePoolSize) {
            ...
            if (log.isInfoEnabled()) {
                log.info(String.format("Set ServicePool '" + ServicePool.this.getName() + "' core threads to (%1$s)",
                        corePoolSize));
            }
        }
        ...
        public void setMaximumPoolSize(final int maximumPoolSize) {
            ...
            if (log.isInfoEnabled()) {
                log.info(String.format("Set ServicePool '" + ServicePool.this.getName() + "' maximum threads to (%1$s)",
                        maximumPoolSize));
            }
        }
        ...
        public void allowCoreThreadTimeOut(final boolean value) {
            ...
            if (log.isInfoEnabled()) {
                log.info(String.format(
                        "Set ServicePool '" + ServicePool.this.getName() + "' allow core thread timeout to (%1$s)",
                        value));
            }
        }
        ...
        public void setKeepAliveTime(final long time) {
            ...
            if (log.isInfoEnabled()) {
                log.info(String.format(
                        "Set ServicePool '" + ServicePool.this.getName() + "' keep alive time to (%1$s) nanoseconds",
                        time));
            }
        }
    }
