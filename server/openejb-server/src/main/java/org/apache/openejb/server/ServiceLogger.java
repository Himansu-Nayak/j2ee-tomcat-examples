...
public class ServiceLogger extends ServerServiceFilter {
    ...
    public void init(final Properties props) throws Exception {
        ...
        this.logger = Logger.getInstance(LogCategory.OPENEJB_SERVER.createChild("service." + getName()),
    ...
    public void service(final Socket socket) throws ServiceException, IOException {
        ...
        try {
            ...
            if (this.debug) {
                this.logger.debug(
                        "[request] for '" + name + "' by '" + address + "' took " + (System.nanoTime() - start) + "ns");
            }
        } catch (Exception e) {
            if (this.debug) {
                ...
                this.logger.debug("[Request failed] for '" + name + "' by '" + address + "' : " + e.getMessage(), e);
                    ...
                    && (errors < Integer.getInteger("openejb.max.log.socket.errors", 10))) {
                errors++;
