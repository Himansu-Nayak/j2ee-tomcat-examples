...
public class SimpleServiceManager extends ServiceManager {
    ...
    public synchronized void start(final boolean block) throws ServiceException {
        ...
        for (int i = 0; i < daemons.length; i++) {
            ...
            if (errors[i] == null) {
                ...
                logger.fatal("Service Start Failed: " + d.getName() + " " + d.getIP() + " " + d.getPort() + ": "
                        + errors[i].getMessage());
                if (display) {
        ...
        /*
         * This will cause the user thread (the thread that keeps the vm alive) to go into a state of constant waiting.
         * Each time the thread is woken up, it checks to see if it should continue waiting.
         *
         * To stop the thread (and the VM), just call the stop method which will set 'stop' to true and notify the user
         * thread.
         */
        try {
            while (!stopped) {
            ...
            logger.fatal("Unable to keep the server thread alive. Received exception: " + t.getClass().getName() + " : "
                    + t.getMessage());
        }
    ...
    public static class SimpleServiceFinder implements ServiceFinder {
        ...
        @Override
        public Map<String, Properties> mapAvailableServices(final Class interfase)
                throws IOException, ClassNotFoundException {
            final Map<String, Properties> service = resourceFinder
                    .mapAvailableProperties(ServerService.class.getName());
