...
public final class CxfUtil {
    ...
    private static Bus initDefaultBus() {
        ...
        try { // create the bus reusing cxf logic but skipping factory lookup
            ...
            CXFBusFactory.setDefaultBus(Bus.class.cast(Proxy.newProxyInstance(CxfUtil.class.getClassLoader(),
                    new Class<?>[] { Bus.class }, new ClientAwareBusHandler())));
    ...
    public static void configureEndpoint(final AbstractEndpointFactory svrFactory,
            final ServiceConfiguration configuration, final String prefix) {
        ...
        // endpoint properties
        final Properties properties = ServiceInfos.serviceProperties(availableServices,
                beanConfig.getProperty(prefix + ENDPOINT_PROPERTIES));
        if (properties != null) {
        ...
        if ("true".equalsIgnoreCase(
                beanConfig.getProperty(debugKey, SystemInstance.get().getOptions().get(debugKey, "false")))) {
    ...
    public static void configureInterceptors(final InterceptorProvider abip, final String prefix,
            final Collection<ServiceInfo> availableServices, final Properties beanConfig) {
        // interceptors
    ...
    public static List<Feature> createFeatures(final Collection<ServiceInfo> availableServices,
    ...
    public static List<Interceptor<? extends Message>> createInterceptors(
            final Collection<ServiceInfo> availableServices, final String ids) {
    ...
    public static void configureBus() {
        ...
        final Properties busProperties = ServiceInfos.serviceProperties(serviceInfos,
                properties.getProperty(BUS_PREFIX + ENDPOINT_PROPERTIES));
    ...
    private static class ClientAwareBusHandler implements InvocationHandler {
        ...
        public Object invoke(final Object proxy, final Method method, final Object[] args) throws Throwable {
            ...
            if ("getExtension".equals(method.getName()) && args != null && args.length == 1
                    && ClassLoader.class.equals(args[0])) {
                final ClassLoader extensionLoader = ClassLoader.class.cast(method.invoke(bus, args));
                if (CxfContainerClassLoader.class.isInstance(extensionLoader)
                        && !CxfContainerClassLoader.class.cast(extensionLoader).hasTccl()) {
                    return null;
                }
    ...
    public static class LifecycleManager {
        ...
        public void destroy(@Observes final AssemblerBeforeApplicationDestroyed ignored) {
            ...
            if ("true".equalsIgnoreCase(
                    systemInstance.getProperty("openejb.cxf.monitoring.jmx.clear-on-undeploy", "true"))) {
