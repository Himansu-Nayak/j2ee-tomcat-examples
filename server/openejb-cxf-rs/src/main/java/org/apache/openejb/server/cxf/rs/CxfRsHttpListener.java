...
public class CxfRsHttpListener implements RsHttpListener {
    ...
    public static final boolean TRY_STATIC_RESOURCES = "true"
            .equalsIgnoreCase(SystemInstance.get().getProperty("openejb.jaxrs.static-first", "true"));
    ...
    private static final boolean FAIL_ON_CONSTRAINED_TO = "true"
            .equalsIgnoreCase(SystemInstance.get().getProperty("openejb.jaxrs.fail-on-constrainedto", "true"));
    ...
    private static void addMandatoryProviders(final Collection<Object> instances,
        ...
        if (!shouldSkipProvider(ValidationExceptionMapper.class.getName())) {
            ...
            final String level = SystemInstance.get().getProperty("openejb.cxf.rs.bval.log.level",
    ...
    public InputStream findStaticContent(final HttpServletRequest request, final String[] welcomeFiles)
    ...
    public boolean serveStaticContent(final HttpServletRequest request, final HttpServletResponse response,
    ...
    public void deploySingleton(final String contextRoot, final String fullContext, final Object o,
            final Application appInstance, final Collection<Object> additionalProviders,
            final ServiceConfiguration configuration) {
        deploy(contextRoot, o.getClass(), fullContext, new SingletonResourceProvider(o), o, appInstance, null,
                additionalProviders, configuration, null);
    }

    @Override
    @Deprecated // we could drop it now I think
    public void deployPojo(final ClassLoader loader, final String contextRoot, final String fullContext,
            final Class<?> loadedClazz, final Application app, final Collection<Injection> injections,
            final Context context, final WebBeansContext owbCtx, final Collection<Object> additionalProviders,
            final ServiceConfiguration configuration) {
        deploy(contextRoot, loadedClazz, fullContext,
                new OpenEJBPerRequestPojoResourceProvider(loader, loadedClazz, injections, context, owbCtx), null, app,
                null, additionalProviders, configuration, owbCtx);
    }

    @Override
    @Deprecated // we could drop it now I think
    public void deployEJB(final String contextRoot, final String fullContext, final BeanContext beanContext,
        ...
        deploy(contextRoot, beanContext.getBeanClass(), fullContext,
                new NoopResourceProvider(beanContext.getBeanClass(), proxy), proxy, null,
                new OpenEJBEJBInvoker(Collections.singleton(beanContext)), additionalProviders, configuration,
                beanContext.getWebBeansContext());
    }

    private void deploy(final String contextRoot, final Class<?> clazz, final String address, final ResourceProvider rp,
            final Object serviceBean, final Application app, final Invoker invoker,
            final Collection<Object> additionalProviders, final ServiceConfiguration configuration,
            final WebBeansContext webBeansContext) {
        final ClassLoader oldLoader = Thread.currentThread().getContextClassLoader();
        Thread.currentThread().setContextClassLoader(CxfUtil.initBusLoader());
        try {
            final JAXRSServerFactoryBean factory = newFactory(address, createServiceJmxName(clazz.getClassLoader()),
                    createEndpointName(app));
    ...
    private List<Object> providers(final Collection<ServiceInfo> services, final Collection<Object> additionalProviders,
        ...
        for (final Object o : additionalProviders) {
            if (o instanceof Class<?>) {
                ...
                final Collection<Object> instance = ServiceInfos.resolve(services, new String[] { name },
                        OpenEJBProviderFactory.INSTANCE);
    ...
    public void deployApplication(final Application application, final String prefix, final String webContext,
            final Collection<Object> additionalProviders, final Map<String, EJBRestServiceInfo> restEjbs,
            final ClassLoader classLoader, final Collection<Injection> injections, final Context context,
            final WebBeansContext owbCtx, final ServiceConfiguration serviceConfiguration) {
        final ClassLoader oldLoader = Thread.currentThread().getContextClassLoader();
        Thread.currentThread().setContextClassLoader(CxfUtil.initBusLoader());
        try {
            final JAXRSServerFactoryBean factory = newFactory(prefix, createServiceJmxName(classLoader),
                    createEndpointName(application));
            ...
            for (final Class<?> clazz : application.getClasses()) {
                if (!additionalProviders.contains(clazz) && !clazz.isInterface()) {
                    ...
                    if (restServiceInfo != null) {
                        ...
                        factory.setResourceProvider(clazz,
                                new NoopResourceProvider(restServiceInfo.context.getBeanClass(), proxy));
                    } else {
                        factory.setResourceProvider(clazz, new OpenEJBPerRequestPojoResourceProvider(classLoader, clazz,
                ...
                if (!additionalProviders.contains(o)) {
                    ...
                    if (restServiceInfo != null) {
                        ...
                        if (owbCtx != null && owbCtx.getBeanManagerImpl().isInUse()) {
                            final CdiSingletonResourceProvider provider = new CdiSingletonResourceProvider(classLoader,
                                    clazz, o, injections, context, owbCtx);
            ...
            try {
                ...
                final ServerProviderFactory spf = ServerProviderFactory.class
                        .cast(server.getEndpoint().get(ServerProviderFactory.class.getName()));
    ...
    private void logEndpoints(final Application application, final String prefix,
            final Map<String, EJBRestServiceInfo> restEjbs, final JAXRSServerFactoryBean factory, final String base) {
        ...
        for (final ClassResourceInfo info : resources) {
            ...
            resourcesToLog
                    .add(new Logs.LogResourceEndpointInfo(type, address, clazz, toLog, methodSize, methodStrSize));
        }
        ...
        for (final Logs.LogResourceEndpointInfo resource : resourcesToLog) {
            ...
            final ObjectNameBuilder jmxName = new ObjectNameBuilder("openejb.management").set("j2eeType", "JAX-RS")
                    .set("J2EEServer", "openejb").set("J2EEApplication", base).set("EndpointType", resource.type)
            ...
            LOGGER.info("     Service URI: " + Logs.forceLength(resource.address, addressSize, true) + " -> "
            ...
            for (final Logs.LogOperationEndpointInfo log : resource.operations) {
                LOGGER.info("          " + Logs.forceLength(log.http, resource.methodSize, false) + " "
    ...
    private void configureFactory(final Collection<Object> givenAdditionalProviders,
            final ServiceConfiguration serviceConfiguration, final JAXRSServerFactoryBean factory,
            final WebBeansContext ctx) {
        if (!"true"
                .equalsIgnoreCase(SystemInstance.get().getProperty("openejb.cxf.rs.skip-provider-sorting", "false"))) {
        ...
        // activate bval
        boolean bvalActive = Boolean.parseBoolean(SystemInstance.get().getProperty("openejb.cxf.rs.bval.active",
        ...
        final String staticSubresourceResolution = serviceConfiguration.getProperties()
                .getProperty(CXF_JAXRS_PREFIX + STATIC_SUB_RESOURCE_RESOLUTION_KEY);
        ...
        final boolean ignoreAutoProviders = "true".equalsIgnoreCase(
                SystemInstance.get().getProperty(key, serviceConfiguration.getProperties().getProperty(key, "false")));
        final Collection<Object> additionalProviders = ignoreAutoProviders ? Collections.emptyList()
        ...
        SystemInstance.get()
                .fireEvent(new ExtensionProviderRegistration(
    ...
    private Comparator<?> findProviderComparator(final ServiceConfiguration serviceConfiguration,
            final WebBeansContext ctx) {
        final String comparatorKey = CXF_JAXRS_PREFIX + "provider-comparator";
        final String comparatorClass = serviceConfiguration.getProperties().getProperty(comparatorKey,
        ...
        if (comparatorClass == null) {
            ...
            if (comparator == null) {
                comparator = Comparator.class
                        .cast(ServiceInfos.resolve(serviceConfiguration.getAvailableServices(), comparatorClass));
            }
            if (comparator == null) {
                try {
                    comparator = Comparator.class.cast(
                            Thread.currentThread().getContextClassLoader().loadClass(comparatorClass).newInstance());
                } catch (final Exception e) {
            ...
            for (final Type itf : comparator.getClass().getGenericInterfaces()) {
                ...
                if (Comparator.class == pt.getRawType() && pt.getActualTypeArguments().length > 0) {
                    ...
                    if (ParameterizedType.class.isInstance(t)
                            && ProviderInfo.class == ParameterizedType.class.cast(t).getRawType()) {
                        return comparator;
                    }
                }
            }
    ...
    public static final class DefaultProviderComparator extends ProviderFactory implements Comparator<ProviderInfo<?>> {
        ...
        public int compare(final ProviderInfo<?> o1, final ProviderInfo<?> o2) {
            ...
            if (classLoader1 != classLoader2 && loadersNotNull && !classLoader1.equals(classLoader2)
                ...
                if (MessageBodyWriter.class.isInstance(o1.getProvider())) {
                    final List<MediaType> types1 = JAXRSUtils.sortMediaTypes(
                            JAXRSUtils.getProviderProduceTypes(MessageBodyWriter.class.cast(o1.getProvider())),
                            JAXRSUtils.MEDIA_TYPE_QS_PARAM);
                    final List<MediaType> types2 = JAXRSUtils.sortMediaTypes(
                            JAXRSUtils.getProviderProduceTypes(MessageBodyWriter.class.cast(o2.getProvider())),
                            JAXRSUtils.MEDIA_TYPE_QS_PARAM);
                    ...
                    final List<MediaType> types1 = JAXRSUtils.sortMediaTypes(
                            JAXRSUtils.getProviderConsumeTypes(MessageBodyReader.class.cast(o1.getProvider())), null);
                    final List<MediaType> types2 = JAXRSUtils.sortMediaTypes(
                            JAXRSUtils.getProviderConsumeTypes(MessageBodyReader.class.cast(o2.getProvider())), null);
