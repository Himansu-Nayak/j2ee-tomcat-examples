...
public class OpenEJBEJBInvoker extends JAXRSInvoker {
    ...
    public OpenEJBEJBInvoker(final Collection<BeanContext> restEjbs) {
        for (final BeanContext context : restEjbs) {
            ...
            for (final Collection<InterceptorData> list : Arrays.asList(context.getInterceptorData(),
                    context.getInstanceScopedInterceptors(), context.getCallbackInterceptors())) {
    ...
    @Override
    protected Object performInvocation(final Exchange exchange, final Object serviceObject, final Method m,
