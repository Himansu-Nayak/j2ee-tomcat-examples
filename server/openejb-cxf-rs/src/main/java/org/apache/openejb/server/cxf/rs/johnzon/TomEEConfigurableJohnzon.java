...
public class TomEEConfigurableJohnzon<T> extends ConfigurableJohnzonProvider<T> {
    ...
    public void setConverters(final Collection<Converter<?>> converters) {
        for (final Converter<?> converter : converters) {
            ...
            builder().addAdapter(ParameterizedType.class.cast(type).getActualTypeArguments()[0], String.class,
    ...
    private Type findType(final Object ref, final Class<?> api) { // need to impl adapters directly
        ...
        throw new IllegalArgumentException(
                "Didn't find " + ref + " in interfaces: " + asList(ref.getClass().getGenericInterfaces()));
    }

    private MapperBuilder builder() {
        return localBuilder == null ? (localBuilder = MapperBuilder.class.cast(Reflections.get(this, "builder")))
                : localBuilder;
    }
}
