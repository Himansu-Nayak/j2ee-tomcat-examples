...
public class AdvancedProviderConfigTest {
    ...
    public void check() throws Exception {
        assertEquals("true",
                ClientBuilder.newClient().target("http://127.0.0.1:" + port + "/AdvancedProviderConfigTest")
                        .path("advanced-provider-config").request().accept(MediaType.TEXT_PLAIN_TYPE)
                        .get(String.class));
    }
    ...
    public static class AdvancedBean {
        ...
        public boolean providers() {
            final JAXBElementProvider<?> mbr = JAXBElementProvider.class.cast(providers.getMessageBodyReader(Pojo.class,
                    Pojo.class, new Annotation[0], MediaType.APPLICATION_XML_TYPE));
