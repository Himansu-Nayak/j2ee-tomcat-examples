...
public class ProviderWithConstructorTest {
    ...
    public WebApp war() {
        return new WebApp().contextRoot("app").addServlet("REST Application", Application.class.getName())
    ...
    public void checkServiceWasDeployed() {
        assertEquals("/app", WebClient.create("http://localhost:" + port + "/app").path("/foo")
                .accept("openejb/constructor").get(String.class));
    }
    ...
    public static class ConstructorProvider<T> implements MessageBodyWriter<T> {
        ...
        @Override
        public void writeTo(T t, Class<?> rawType, Type genericType, Annotation[] annotations, MediaType mediaType,
