...
public class SingletonTest {
    ...
    public void checkStateStays() {
        for (int i = 0; i < 3; i++) {
            assertEquals(i, WebClient.create(base.toExternalForm() + "foo/foo").accept(TEXT_PLAIN_TYPE)
    ...
    public void checkCdiInjections() {
        for (int i = 0; i < 3; i++) {
            assertEquals(i, WebClient.create(base.toExternalForm() + "foo/cdi").accept(TEXT_PLAIN_TYPE)
                    .get(Integer.class).intValue());
        }
    }
