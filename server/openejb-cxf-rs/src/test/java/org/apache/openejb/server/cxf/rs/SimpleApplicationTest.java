...
public class SimpleApplicationTest {
    ...
    @Module
    @Classes(cdi = true, value = { MySecondRestClass.class, HookedRest.class, RestWithInjections.class, SimpleEJB.class,
    ...
    public void wadlXML() throws IOException {
        final Response response = WebClient.create(BASE_URL).path("/first/hi").query("_wadl").query("_type", "xml")
                .accept(TEXT_PLAIN_TYPE).get();
         ...
         * base="http://localhost:4204/foo/my-app/"><resource path="/first"><doc>MyFirstRestClass service</doc><resource
         * path="/hi"><method name="GET"><response><representation mediaType="application/octet-stream"><param
         * name="result" style="plain"
    ...
    public void wadlJSON() throws IOException {
        final Response response = WebClient.create(BASE_URL).path("/first/hi").query("_wadl").query("_type", "json")
                .accept(TEXT_PLAIN_TYPE).get();
    ...
    public void expert() throws Exception {
        final Response response = WebClient.create(BASE_URL).path("/expert/still-hi").accept(TEXT_PLAIN_TYPE)
                .post("Pink Floyd");
