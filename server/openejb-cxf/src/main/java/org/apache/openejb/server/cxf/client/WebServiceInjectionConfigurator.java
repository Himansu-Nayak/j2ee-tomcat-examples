...
/**
 * Used to configure a @WebServiceRef. Config uses application.properties. Format is:
 *
 * # global for all clients cxf.jaxws.client.out-interceptors = com.foo.MyInInterceptor # specific
...
public class WebServiceInjectionConfigurator implements JaxWsServiceReference.WebServiceClientCustomizer {
    ...
    public WebServiceFeature[] features(final QName qname, final Properties properties) {
        ...
        for (final String suffix : asList("", (qname == null ? "_" : qname.toString()) + ".")) {
            ...
            if (wsFeatures != null) {
                final Collection<Object> instances = ServiceInfos.resolve(createServiceInfos(properties),
                        wsFeatures.split(" *, *"));
    ...
    private void configure(final Client client, final Properties properties) {
        ...
        for (final String suffix : asList("", client.getEndpoint().getEndpointInfo().getName().toString() + ".")) {
            ...
            configureInterceptors(client, CXF_JAXWS_CLIENT_PREFIX + suffix, lazyServiceInfoList(properties),
                    properties);
        }
    }
    ...
    private List<ServiceInfo> lazyServiceInfoList(final Properties properties) { // don't create service info if not needed, ie no conf
        return List.class.cast(Proxy.newProxyInstance(Thread.currentThread().getContextClassLoader(),
    ...
    private List<ServiceInfo> createServiceInfos(final Properties properties) {
        ...
        final List<ServiceInfo> services = new ArrayList<>(
                config.facilities != null && config.facilities.services != null ? config.facilities.services
                        : Collections.<ServiceInfo> emptyList());
