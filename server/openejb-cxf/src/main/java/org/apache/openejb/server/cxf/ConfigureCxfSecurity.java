...
/**
 * Helper class to extract WSS4J properties from a set of properties. More over, it configures In and Out interceptor to
 * manage WS-Security.
 */
public class ConfigureCxfSecurity {
    ...
    private static final Map<QName, Object> DEFAULT_VALIDATOR_MAP = new HashMap<QName, Object>() {
        {
            put(new QName("http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-wssecurity-secext-1.0.xsd",
                    "UsernameToken"), new OpenEJBLoginValidator());
        }
    };
    ...
    private static Object getValidator(String validator)
    ...
    public static final void setupWSS4JChain(InterceptorProvider endpoint, Map<String, Object> inProps,
    ...
    public static final void configure(Endpoint endpoint, PortData port) {
        ...
        if (p != null && p.containsKey(OPENEJB_ENDPOINT_CONFIGURATOR)) {
            ...
            try {
                final EndpointConfigurator configurator = (EndpointConfigurator) Thread.currentThread()
                        .getContextClassLoader().loadClass(classname).newInstance();
                configurator.configure(endpoint, p);
            } catch (Exception e) {
                LOGGER.error("can't configure endpoint " + endpoint + " with configurator " + classname
    ...
    /**
     * split {<namespace>}<local> = foo useful because in the namespace there is at least a '.' which is a separator for
     * Proeprties and escaping doesn't always work.
     */
