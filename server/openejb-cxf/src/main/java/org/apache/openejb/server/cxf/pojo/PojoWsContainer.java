...
public class PojoWsContainer extends CxfWsContainer {
    ...
    public PojoWsContainer(final ClassLoader loader, final DestinationFactory transportFactory, final Bus bus,
            final PortData port, final Context context, final Class target, final Map<String, Object> bdgs,
    ...
    protected String getFakeUrl() {
        return (Class.class.isInstance(target) ? Class.class.cast(target).getName() : target.getClass().getName()) + "_"
                + hashCode(); // pojo are not like ejbName: unique
    }
    ...
    protected ObjectName registerMBean() {
        final ObjectName name = new ObjectNameBuilder("openejb.management").set("j2eeType", "JAX-WS")
                .set("J2EEServer", "openejb").set("J2EEApplication", null).set("EndpointType", "POJO")
                .set("name", target.getSimpleName()).build();
    ...
    public class WsServiceMBean {
        ...
        public TabularData getHandlers() {
            ...
            return LocalMBeanServer.tabularData("handlers", "The list of handlers",
        ...
        public TabularData getProperties() {
            return LocalMBeanServer.tabularData("properties", "Service configuration properties",
                    "Service configuration properties", port.getProperties());
        }
