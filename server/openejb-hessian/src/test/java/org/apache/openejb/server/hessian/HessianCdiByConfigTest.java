...
public class HessianCdiByConfigTest {
    ...
    public Properties config() {
        return new PropertiesBuilder().p(
                "openejb.hessian." + MyCdiHessianService.class.getName() + "_" + CdiService.class.getName() + ".path",
                "foo").p("httpejbd.port", Integer.toString(port)).build();
    }
    ...
    public void client() throws MalformedURLException {
        ...
        final CdiService client = CdiService.class
                .cast(clientFactory.create(CdiService.class, "http://127.0.0.1:" + port + "/web/hessian/foo"));
