...
public class HessianService implements ServerService, SelfManaging {
    private static final Logger LOGGER = Logger.getInstance(LogCategory.OPENEJB_SERVER.createChild("hessian"),
        ...
        for (final BeanContext beanContext : beanContexts) {
            ...
            try {
                ...
                if (serializerFactory != null) {
                    try {
                        server.serializerFactory(classLoader.loadClass(serializerFactory).newInstance())
                ...
                try {
                    LOGGER.info("Hessian(url="
                            + registry.deploy(classLoader, server, virtualHost, appName(app, beanContext), authMethod,
                                    transportGuarantee, realmName, String.class.cast(beanContext.getDeploymentID()))
        ...
        if (assembler != null) {
            for (final AppInfo appInfo : assembler.getDeployedApplications()) {
                deploy(new AssemblerAfterApplicationCreated(appInfo,
                        SystemInstance.get().getComponent(ContainerSystem.class).getAppContext(appInfo.appId), null));
            }
        }
    }
