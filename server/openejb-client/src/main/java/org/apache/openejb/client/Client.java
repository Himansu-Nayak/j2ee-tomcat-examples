...
public class Client {
    ...
    public static Response request(final Request req, final Response res, final ServerMetaData server)
    ...
    protected Response processRequest(final Request req, final Response res, final ServerMetaData server)
        ...
        final ProtocolMetaData protocolRequest = (null != COMPATIBLE_META_DATA ? COMPATIBLE_META_DATA
                : PROTOCOL_META_DATA);
        ...
        try {
            ...
            try {
                ...
                throw newIOException("Cannot determine server protocol version: Received " + protocolResponse.getSpec()
                        + " : " + e.getMessage(), e);
            }
            ...
            try {
                ...
                throw new RemoteException(
                        "Cannot read the cluster response from the server.  The class for an object being returned is not located in this system:",
                        e);

            } catch (final IOException e) {
                throw newIOException("Cannot read the cluster response from the server (" + protocolResponse.getSpec()
                        + ") : " + e.getMessage(), e);

            } catch (final Throwable e) {
                throw new RemoteException("Error reading cluster response from server (" + protocolResponse.getSpec()
                        + ") : " + e.getMessage(), e);
            }
            ...
            try {
                ...
                throw new RemoteException(
                        "Error reading response from server (" + protocolResponse.getSpec() + ") : " + e.getMessage(),
                        e);
            }
            ...
            if (FINEST) {
                ...
                final String message = String.format("Invocation %sns - %s - Request(%s) - Response(%s)", time,
                        conn.getURI(), req, res);
            ...
            if (e instanceof RetryException || getRetry()) {
                try {
                    ...
                    throw new RemoteFailoverException(
                            "Cannot complete request.  Retry attempted on " + failed.size() + " servers", e);
                }
            } else {
                throw new RemoteException("Cannot read the response from the server (" + protocolRequest.getSpec()
                        + ") : " + e.getMessage(), e);
            }
    ...
    public static class Context {
        ...
        public void setClusterMetaData(final ClusterMetaData updated) {
            ...
            final ClusterMetaDataUpdated clusterMetaDataUpdated = new ClusterMetaDataUpdated(serverMetaData, updated,
                    previous);
