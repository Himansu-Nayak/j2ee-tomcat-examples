...
public class MulticastPulseClient extends MulticastConnectionFactory {
    ...
    private static final int LIMIT = Integer
            .parseInt(System.getProperty(ORG_APACHE_OPENEJB_MULTIPULSE_URI_LIMIT, "50000"));
     ...
     * <p/>
     * Returned URIs are of the format 'mp-{serverhost}:group:scheme://servicehost:port'. The serverhost is prefixed
     * with 'mp-' in case the serverhost is an IP-Address, as RFC 2396 defines scheme must begin with a 'letter'
     *
     * @param forGroup
     *            Specific case sensitive group name or * for all
     * @param schemes
     *            Acceptable scheme list
     * @param host
     ...
     *             On error
     */
    public static Set<URI> discoverURIs(final String forGroup, final Set<String> schemes, final String host,
            final int port, long timeout) throws Exception {
                                    ...
                                    if (null != sa && (sa instanceof InetSocketAddress)) {
                                        ...
                                        if (len > 2048) {
                                            ...
                                            if (log.isLoggable(Level.FINE)) {
                                                log.log(Level.FINE, "Truncating multipulse length {0} to 2048",
                                                        new Object[] { len });
                                            }
                                            len = 2048;
                                        }
                                        ...
                                        if (s.startsWith(MulticastPulseClient.SERVER)) {
                                            ...
                                            for (final String svc : serviceList) {
                                                ...
                                                if (schemes.contains(serviceUri.getScheme())) {
                                                    ...
                                                    final String serverHost = ((InetSocketAddress) response
                                                    ...
                                                    final String svcfull = ("mp-" + serverHost + ":" + group + ":"
                                                            + svc);
                                                        ...
                                                        if (svcfull.contains("0.0.0.0")) {
                                                            for (final String h : hosts) {
                                                                if (!h.replace("[", "").startsWith("2001:0:")) { //Filter Teredo
                                                                    set.add(URI.create(
                                                                            svcfull.replace("0.0.0.0", ipFormat(h))));
                                                                }
                                                            }
    ...
    public static void main(final String[] args) throws Exception {
        ...
        System.out.println(String.format("Using discovery options group=%1$s, host=%2$s, port=%3$s, timeout=%4$s",
                discover, mchost, mcport, timeout));
        System.out.println();
                ...
                while (running.get()) {
                    ...
                    try {
                        uriSet = MulticastPulseClient.discoverURIs(discover,
                                new HashSet<String>(Arrays.asList("ejbd", "ejbds", "http", "https")), mchost, mcport,
                                timeout);
                    } catch (Exception e) {
                    ...
                    if (uriSet != null && size > 0) {
                        ...
                        for (final URI uri : uriSet) {
                            ...
                            if (MulticastPulseClient.isLocalAddress(host, false)
                                ...
                                System.out.println(server + ":" + group + " - " + uriSub.toASCIIString()
    ...
    /**
     * Asynchronous attempt to broadcast a bad URI on our channel. Hopefully the culprit server will hear this and stop
     * sending it.
     *
     * @param uri
        ...
        getExecutorService().submit(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    final byte[] bytes = (MulticastPulseClient.CLIENT + group + MulticastPulseClient.BADURI
                    ...
                    for (final MulticastSocket socket : multicastSockets) {
                        ...
                        try {
                            ...
                            log.log(Level.WARNING, "Failed to broadcast bad URI: " + uri + " on: "
                                    + socket.getInterface().getHostAddress(), e);
                        }
                    }
                } catch (Exception e) {
    ...
    public Connection getConnection(final URI uri) throws IOException {
        ...
        if (knownUris.size() >= LIMIT) {
            ...
            throw new IllegalArgumentException("Unique MultiPulse URI limit of " + LIMIT
                    + " reached. Increase using the system property '" + ORG_APACHE_OPENEJB_MULTIPULSE_URI_LIMIT + "'");
        }
        ...
        for (final URI serviceURI : uriSet) {
            ...
            try {
                ...
                if (java.net.SocketTimeoutException.class.isInstance(e) || SocketException.class.isInstance(e)) {
                    ...
                    MulticastPulseClient.broadcastBadUri(getString(getUriParameters(uri), "group", "default"), tryUri,
