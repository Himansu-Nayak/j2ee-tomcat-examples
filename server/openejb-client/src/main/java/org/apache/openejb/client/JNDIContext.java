...
public class JNDIContext implements InitialContextFactory, Context {
    ...
    public static ThreadPoolExecutor newExecutor(final int threads, final BlockingQueue<Runnable> blockingQueue) {
        /**
         * This thread pool starts with 3 core threads and can grow to the limit defined by 'threads'. If a pool thread
         * is idle for more than 1 minute it will be discarded, unless the core size is reached. It can accept upto the
         * number of processes defined by 'queue'. If the queue is full then an attempt is made to add the process to
         * the queue for 10 seconds. Failure to add to the queue in this time will either result in a logged rejection,
         * or if 'block' is true then a final attempt is made to run the process in the current thread (the service
         * thread).
         */
        ...
        if (userID != null) {
            if (!authWithRequest) {
                ...
                authenticationInfo = new AuthenticationInfo(
                        String.class.cast(env.get("openejb.authentication.realmName")), userID, psswrd.toCharArray());
            }
        }
        if (client == null) {
     ...
     * <p/>
     * TODO: Move the check to a place where it really belongs - ConnectionManager, ConnectionFactory or such This
     * method (class in general) doesn't really know what is required as far as connection details go Assuming that
     * java.net.URI or java.net.URL are going to be used is overly stated
     */
    ...
    public void authenticate(final String userID, final String psswrd) throws AuthenticationException {
        ...
        final AuthenticationRequest req = new AuthenticationRequest(
                String.class.cast(env.get("openejb.authentication.realmName")), userID, psswrd);
    ...
    private void logout() throws AuthenticationException {
        ...
        case ResponseCodes.AUTH_DENIED:
            throw AuthenticationException.class
                    .cast(new AuthenticationException("Can't logout").initCause(response.getDeniedCause()));
