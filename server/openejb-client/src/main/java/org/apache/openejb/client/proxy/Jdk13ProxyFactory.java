...
public class Jdk13ProxyFactory implements ProxyFactory {
    ...
    public void init(final Properties props) {
        ...
        if (version.contains(badVersion)) {
            final String message = "" + "INCOMPATIBLE VM: \n\n"
    ...
    @Override
    public Object setInvocationHandler(final Object proxy, final InvocationHandler handler)
    ...
    @Override
    public Object newProxyInstance(final Class[] interfaces, final InvocationHandler h)
            throws IllegalArgumentException {
        if (interfaces.length < 1) {
            throw new IllegalArgumentException("There must be at least one interface to implement.");
        }
