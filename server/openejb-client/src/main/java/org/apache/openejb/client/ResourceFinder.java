...
/**
 * Note: This class is a copy of the xbean-finder ResourceFinder class. Any changes should be make to the xbean-finder
 * source first and then copied to this class. Also, this class should be kept in sync with the xbean-finder code.
 */
...
public class ResourceFinder {
    ...
    private static void readDirectoryEntries(final URL location, final Map<String, URL> resources)
    ...
    private static void readJarEntries(final URL location, final String basePath, final Map<String, URL> resources)
            throws IOException {
     ...
     * <p/>
     * The list will only contain entries of resources that match the requirements of the last invoked findAvailable* or
     * mapAvailable* methods, but were unable to be loaded and included in their results.
     * <p/>
     * The list returned is unmodifiable and the results of this method will change after each invocation of a
     * findAvailable* or mapAvailable* methods.
     * <p/>
     ...
     * @throws IOException
     *             if a resource pointed out by the uri param could not be find
    ...
    /**
     * Reads the contents of the found URLs as a Strings and returns them. Individual URLs that cannot be read are
     * skipped and added to the list of 'resourcesNotLoaded'
     *
     * @param uri
     *            String
     * @return a list of the content of each resource URL found
     ...
     * Example classpath:
     * <p/>
     * META-INF/serializables/one META-INF/serializables/two META-INF/serializables/three
     * META-INF/serializables/four/foo.txt
     * <p/>
     * ResourceFinder finder = new ResourceFinder("META-INF/"); Map map = finder.mapAvailableStrings("serializables");
     * map.contains("one"); // true map.contains("two"); // true map.contains("three"); // true map.contains("four"); //
     * false
     *
     * @param uri
     *            String
     ...
     * Example classpath:
     * <p/>
     * META-INF/serializables/one META-INF/serializables/two # not readable META-INF/serializables/three
     * META-INF/serializables/four/foo.txt
     * <p/>
     * ResourceFinder finder = new ResourceFinder("META-INF/"); Map map = finder.mapAvailableStrings("serializables");
     * map.contains("one"); // true map.contains("two"); // false map.contains("three"); // true map.contains("four");
     * // false
     *
     * @param uri
     *            String
    ...
    /**
     * Executes {@link #findString(String)} assuming the contents URL found is the name of a class that should be loaded
     * and returned.
     *
     * @param uri
     *            String
     ...
     * <p/>
     * ResourceFinder finder = new ResourceFinder("META-INF/"); Map map = finder.mapAvailableStrings("xmlparsers");
     * map.contains("xerces"); // true map.contains("crimson"); // true Class xercesClass = map.get("xerces"); Class
     * crimsonClass = map.get("crimson");
     *
     * @param uri
     *            String
     * @return Map
    ...
    /**
     * Executes mapAvailableStrings assuming the value of each entry in the map is the name of a class that should be
     * loaded.
     * <p/>
     * Any class that cannot be loaded will be skipped and placed in the 'resourcesNotLoaded' collection.
     * <p/>
    ...
    /**
     * Assumes the class specified points to a file in the classpath that contains the name of a class that implements
     * or is a subclass of the specfied class.
     * <p/>
     * Any class that cannot be loaded will be cause an exception to be thrown.
     * <p/>
     * Example classpath:
     ...
     * <p/>
     * ResourceFinder finder = new ResourceFinder("META-INF/"); Class clazz =
     * finder.findImplementation(java.io.InputStream.class); clazz.getName(); // returns "org.acme.AcmeInputStream"
     *
     * @param interfase
     ...
     * @throws ClassNotFoundException
     *             if the class found is not loadable
     ...
     * Example classpath:
     * <p/>
     * META-INF/java.io.InputStream # contains the classname org.acme.AcmeInputStream META-INF/java.io.InputStream #
     * contains the classname org.widget.NeatoInputStream META-INF/java.io.InputStream # contains the classname
     * com.foo.BarInputStream
     * <p/>
     * ResourceFinder finder = new ResourceFinder("META-INF/"); List classes =
     * finder.findAllImplementations(java.io.InputStream.class); classes.contains("org.acme.AcmeInputStream"); // true
     * classes.contains("org.widget.NeatoInputStream"); // true classes.contains("com.foo.BarInputStream"); // true
     *
     * @param interfase
     *            a superclass or interface
     * @return List
     ...
     * or is a subclass of the specfied class.
     * <p/>
     * Any class that cannot be loaded or are not assignable to the specified class will be skipped and placed in the
    ...
    /**
     * Assumes the class specified points to a directory in the classpath that holds files containing the name of a
     * class that implements or is a subclass of the specfied class.
     * <p/>
     * Any class that cannot be loaded or assigned to the specified interface will be cause an exception to be thrown.
     * <p/>
     * Example classpath:
     * <p/>
     * META-INF/java.net.URLStreamHandler/jar META-INF/java.net.URLStreamHandler/file
     * META-INF/java.net.URLStreamHandler/http
     * <p/>
     * ResourceFinder finder = new ResourceFinder("META-INF/"); Map map =
     * finder.mapAllImplementations(java.net.URLStreamHandler.class); Class jarUrlHandler = map.get("jar"); Class
     * fileUrlHandler = map.get("file"); Class httpUrlHandler = map.get("http");
     *
     * @param interfase
     *            a superclass or interface
     * @return Map
     ...
     * <p/>
     * ResourceFinder finder = new ResourceFinder("META-INF/"); Properties widgetProps =
     * finder.findProperties("widget.properties");
     *
     * @param uri
     *            String
     * @return Properties
     ...
     * <p/>
     * ResourceFinder finder = new ResourceFinder("META-INF/"); List<Properties> driversList =
     * finder.findAvailableProperties("jdbcDrivers"); Properties oracleProps = driversList.get("oracle.properties");
     * Properties mysqlProps = driversList.get("mysql.props"); Properties derbyProps = driversList.get("derby");
     *
     * @param uri
     *            String
     * @return Map
        ...
        for (int i = 0; i < search.length; i++) {
            ...
            try {
                ...
                if (protocol.equals("jar")) {
                    /*
                     * If the connection for currentUrl or resURL is used, getJarFile() will throw an exception if the
                     * entry doesn't exist.
                     */
                    final URL jarURL = ((JarURLConnection) currentUrl.openConnection()).getJarFileURL();
                    final JarFile jarFile;
                    try {
                        final JarURLConnection juc = (JarURLConnection) new URL("jar", "",
