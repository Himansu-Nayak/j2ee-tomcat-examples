...
public class ClientSecurity {
    ...
    /**
     * Login the spedified user using the specified password. This is a global login for the entire Java Virtural
     * Machine. If you would like to have a thread scoped login, use ClientSecurity.login(username, password, true);
     * </p>
     ...
     *             if the username and password combination are not valid or if there is a problem communiating with the
     *             server
     */
    ...
    /**
     * Login the spedified user using the specified password either globally for the entire Java Virtural Machine or
     * scoped to the thread.
     * </p>
     * When using thread scoped login, you should logout in a finally block. This particularly when using thread pools.
     * If a thread is returned to the pool with a login attached to the thread the next user of that thread will inherit
     * the thread scoped login.
     *
     * @param username
     *            the user to login
     * @param password
     *            the password for the user
     * @param threadScoped
     *            if true the login is scoped to the thread; otherwise the login is global for the entire Java Virtural
     *            Machine
     * @throws FailedLoginException
     *             if the username and password combination are not valid or if there is a problem communiating with the
     *             server
     */
    public static void login(final String username, final String password, final boolean threadScoped)
            throws FailedLoginException {
    ...
    /**
     * This is a helper method for login modules. Directly authenticates with the server using the specified username
     * and password returning the identity token for the client. This methods does not store the identity token and the
     * caller must arrange for the to be available to the OpenEJB proxies via an IdentityResolver.
     *
     * @param username
     ...
     */
    public static Object directAuthentication(final String username, final String password, final ServerMetaData server)
            throws FailedLoginException {
        return directAuthentication(null, username, password, server);
    }

    public static Object directAuthentication(final String securityRealm, final String username, final String password,
            final ServerMetaData server) throws FailedLoginException {
        // authenticate
        ...
        try {
            ...
            throw (FailedLoginException) new FailedLoginException("Unable to authenticate with server " + server)
                    .initCause(e);
        }

        // check the response
        if (authRes.getResponseCode() != ResponseCodes.AUTH_GRANTED) {
            throw (FailedLoginException) new FailedLoginException("This principal is not authenticated.")
                    .initCause(authRes.getDeniedCause());
        }
        ...
        if (identityResolver == null) {
            ...
            if (strategy == null) {
                ...
                if (!IdentityResolver.class.isAssignableFrom(identityResolverClass)) {
                    throw new IllegalArgumentException("Client identity strategy '" + strategy + "' " + "class '"
                            + identityResolverClass.getName() + "' does not implement the " + "interface '"
