...
/**
 * Extension of {@link GZIPOutputStream} to workaround for a couple of long standing JDK bugs (
 * <a href="http://bugs.sun.com/bugdatabase/view_bug.do?bug_id=4255743">Bug 4255743</a> and
 * <a href="http://bugs.sun.com/bugdatabase/view_bug.do?bug_id=4813885">Bug 4813885</a>) so the GZIP'd output can be
 * flushed.
 */
public class FlushableGZIPOutputStream extends GZIPOutputStream {
    ...
    public synchronized void flush() throws IOException {
        if (hasLastByte) {
            ...
            /**
             * Now this is tricky: We force the Deflater to flush its data by switching compression level. As yet, a
             * perplexingly simple workaround for http://developer.java.sun.com/developer/bugParade/bugs/4255743.html
             */
            if (!def.finished()) {
    ...
    /*
     * Keep on calling deflate until it runs dry. The default implementation only does it once and can therefore hold
     * onto data when they need to be flushed out.
     */
    @Override
