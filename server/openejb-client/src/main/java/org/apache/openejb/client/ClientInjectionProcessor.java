...
public class ClientInjectionProcessor<T> {
    ...
    public ClientInjectionProcessor(final Class<? extends T> beanClass, final List<Injection> injections,
            final List<CallbackMetaData> postConstructMethods, final List<CallbackMetaData> preDestroyMethods,
        ...
        for (final Injection injection : injections) {
            ...
            try {
                ...
                logger.warning("Injection data not found in JNDI context: jndiName='" + injection.getJndiName()
                        + "', target=" + injection.getTargetClass() + "/" + injection.getName());
            }
        }
