...
public abstract class EJBObjectHandler extends EJBInvocationHandler {
    ...
    /*
     * The registryId is a logical identifier that is used as a key when placing EntityEJBObjectHandler into the
     * BaseEjbProxyHanlder's liveHandleRegistry. EntityEJBObjectHandlers that represent the same bean identity (keyed by
     * the registry id) will be stored together so that they can be removed together when the
     * EJBInvocationHandler.invalidateAllHandlers is invoked. The EntityEJBObjectHandler uses a compound key composed of
     * the entity bean's primary key, deployment id, and container id. This uniquely identifies the bean identity that
     * is proxied by this handler allowing it to be removed with other handlers bound to the same registry id.
     */
    ...
    public EJBObjectHandler(final ThreadPoolExecutor es, final EJBMetaDataImpl ejb, final ServerMetaData server,
            final ClientMetaData client, final JNDIContext.AuthenticationInfo auth) {
        super(ejb, server, client, null, auth);
        this.executor = es;
    }

    public EJBObjectHandler(final ThreadPoolExecutor es, final EJBMetaDataImpl ejb, final ServerMetaData server,
            final ClientMetaData client, final Object primaryKey, final JNDIContext.AuthenticationInfo auth) {
    ...
    public static EJBObjectHandler createEJBObjectHandler(final ThreadPoolExecutor executorService,
            final EJBMetaDataImpl ejb, final ServerMetaData server, final ClientMetaData client,
    ...
    public EJBObjectProxy createEJBObjectProxy() {
        ...
        try {
            ...
            try {
                ejbObject = (EJBObjectProxy) ProxyManager
                        .newProxyInstance(interfaces.toArray(new Class[interfaces.size()]), this);
            } finally {
                if (!parent) {
    ...
    public Object _invoke(final Object p, final Method m, final Object[] a) throws Throwable {
        ...
        try {
            ...
            if (remote) {
                ...
                throw new RemoteException("Unknown Container Exception: " + throwable.getClass().getName() + ": "
                        + throwable.getMessage(), getCause(throwable));
            } else {
                if (throwable instanceof EJBException) {
                    throw throwable;
                }
                throw new EJBException("Unknown Container Exception: " + throwable.getClass().getName() + ": "
    ...
    protected Object getEJBHome(final Method method, final Object[] args, final Object proxy) throws Throwable {
        if (ejbHome == null) {
            ejbHome = EJBHomeHandler.createEJBHomeHandler(executor, ejb, server, client, authenticationInfo)
                    .createEJBHomeProxy();
        }
        return ejbHome;
    }
    ...
    protected Object businessMethod(final Method method, final Object[] args, final Object proxy) throws Throwable {
        ...
        if (ejb.isAsynchronousMethod(method)) {
            try {
                ...
                final AsynchronousCall asynchronousCall = new AsynchronousCall(method, args, proxy, requestId,
                        response);
                if (executor == null) {
    ...
    private Object _businessMethod(final Method method, final Object[] args, final Object proxy, final String requestId)
            throws Throwable {
        final EJBRequest req = new EJBRequest(RequestMethodCode.EJB_OBJECT_BUSINESS_METHOD, ejb, method, args,
    ...
    private Object _businessMethod(final Method method, final Object[] args, final Object proxy, final String requestId,
            final EJBResponse response) throws Throwable {
        final EJBRequest req = new EJBRequest(RequestMethodCode.EJB_OBJECT_BUSINESS_METHOD, ejb, method, args,
                primaryKey, client.getSerializer());
    ...
    private Object getResult(final EJBResponse res) throws ClassNotFoundException {
        ...
        if (client.getSerializer() != null && SerializationWrapper.class.isInstance(result)) {
            ...
            return client.getSerializer().deserialize(wrapper.getData(),
    ...
    private class AsynchronousCall implements Callable {
        ...
        public AsynchronousCall(final Method method, final Object[] args, final Object proxy, final String requestId,
                final EJBResponse response) {
            this.method = method;
            this.args = args;
            this.proxy = proxy;
    ...
    private class FutureAdapter<T> implements Future<T> {
        ...
        public boolean cancel(final boolean mayInterruptIfRunning) {
            /*
             * In EJB 3.1 spec 3.4.8.1.1 a. If a client calls cancel on its Future object, the container will attempt to
             * cancel the associated asynchronous invocation only if that invocation has not already been dispatched.
             * There is no guarantee that an asynchronous invocation can be cancelled, regardless of how quickly cancel
             * is called after the client receives its Future object. If the asynchronous invocation can not be
             * cancelled, the method must return false. If the asynchronous invocation is successfully cancelled, the
             * method must return true.
             *
             * b. The meaning of parameter mayInterruptIfRunning is changed.
             *
             * So, we should never call cancel(true), or the underlying Future object will try to interrupt the target
             * thread.
             */
            ...
            if (executor.getQueue().remove(target)) {
                ...
                if (!target.isDone()) {
                    ...
                    final EJBRequest req = new EJBRequest(RequestMethodCode.FUTURE_CANCEL, ejb, CANCEL,
                            new Object[] { Boolean.valueOf(mayInterruptIfRunning) }, primaryKey,
