...
public class ClientHandlerResolverImpl implements HandlerResolver {
    ...
    public ClientHandlerResolverImpl(final List<HandlerChainMetaData> handlerChains, final List<Injection> injections,
    ...
    public void destroyHandlers() {
        final List<ClientInjectionProcessor<Handler>> handlerInstances = new ArrayList<ClientInjectionProcessor<Handler>>(
    ...
    private List<Handler> buildHandlers(final javax.xml.ws.handler.PortInfo portInfo,
            final HandlerChainMetaData handlerChain) {
        ...
        for (final HandlerMetaData handler : handlerChain.getHandlers()) {
            try {
                final Class<? extends Handler> handlerClass = loadClass(handler.getHandlerClass())
                ...
                final ClientInjectionProcessor<Handler> processor = new ClientInjectionProcessor<Handler>(handlerClass,
                        injections, handler.getPostConstruct(), handler.getPreDestroy(), context);
