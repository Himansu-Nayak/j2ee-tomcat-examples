...
public class SocketConnectionFactory implements ConnectionFactory {
    ...
    private static class Pool {
        ...
        public SocketConnection get() throws IOException {
            ...
            final ConnectionPoolTimeoutException exception = new ConnectionPoolTimeoutException(
                    "No connections available in pool (size " + this.size + ").  Waited for " + this.timeout
        ...
        public String toString() {
            return "Pool{" + "size=" + this.size + ", available=" + this.semaphore.availablePermits() + ", uri="
                    + this.uri + '}';
        }
    }
    ...
    class SocketConnection implements Connection {
        ...
        protected void open(final URI uri) throws IOException {
            ...
            try {
                ...
                throw this.failure(
                        "Cannot connect to server '" + uri.toString()
                                + "'.  Check that the server is started and that the specified serverURL is correct.",
                        e);

            } catch (IOException e) {
                throw this.failure("Cannot connect to server: '" + uri.toString() + "'.  Exception: "
                        + e.getClass().getName() + " : " + e.getMessage(), e);

            } catch (SecurityException e) {
                throw this.failure("Cannot access server: '" + uri.toString()
                        + "' due to security restrictions in the current VM: " + e.getClass().getName() + " : "
                        + e.getMessage(), e);

            } catch (Throwable e) {
                throw this.failure("Cannot  connect to server: '" + uri.toString()
                        + "' due to an unknown exception in the OpenEJB client: " + e.getClass().getName() + " : "
                        + e.getMessage(), e);
            }
        ...
        public InputStream getInputStream() throws IOException {
            ...
            try {
                ...
                throw this.failure(
                        "Cannot open input stream to server, the stream has been corrupted: " + e.getClass().getName(),
                        e);
                ...
                if (this.out == null) {
                    if (!this.gzip) {
                        ...
                        this.out = new BufferedOutputStream(
                                new FlushableGZIPOutputStream(this.socket.getOutputStream()));
                    }
                }
