...
public class MulticastDiscoveryAgent implements DiscoveryAgent, ServerService, SelfManaging {
    ...
    private static final Logger log = Logger.getInstance(
            LogCategory.OPENEJB_SERVER.createChild("discovery").createChild("multicast"),
                    ...
                    try {
                        ...
                        if (failed == null) {
                            ...
                            if (null != message && message.toLowerCase().contains("operation not permitted")) {
                                log.error(
                                        "The 'Operation not permitted' error has been know to be caused by improper firewall/network setup.  "
                                                + "Please make sure that the OS is properly configured to allow multicast traffic over: "
