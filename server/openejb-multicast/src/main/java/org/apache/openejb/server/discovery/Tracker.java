...
public class Tracker {
    ...
    private final Executor executor = new ThreadPoolExecutor(1, 2, 30, TimeUnit.SECONDS,
    ...
    public Tracker(final String group, final long heartRate, final int maxMissedHeartbeats, final long reconnectDelay,
            final long maxReconnectDelay, final int maxReconnectAttempts, final long exponentialBackoff,
            final Logger log, final boolean debug) {
        this.group = group;
    ...
    public void checkServices() {
        ...
        for (final ServiceVitals serviceVitals : discoveredServices.values()) {
            if (serviceVitals.getLastHeartbeat() < expireTime && !isSelf(serviceVitals.service)) {
                ...
                if (debug()) {
                    log.debug("Expired " + serviceVitals.service + String.format(" Timeout{lastSeen=%s, threshold=%s}",
                            serviceVitals.getLastHeartbeat() - now, threshold));
                }
    ...
    public String toString() {
        return "Tracker{" + "group='" + group + '\'' + ", groupPrefix='" + groupPrefix + '\'' + ", heartRate="
                + heartRate + ", maxMissedHeartbeats=" + maxMissedHeartbeats + ", reconnectDelay=" + reconnectDelay
                + ", maxReconnectDelay=" + maxReconnectDelay + ", maxReconnectAttempts=" + maxReconnectAttempts
                + ", exponentialBackoff=" + exponentialBackoff + ", useExponentialBackOff=" + useExponentialBackOff
                + ", registeredServices=" + registeredServices.size() + ", discoveredServices="
    ...
    public static class Builder {
        ...
        public Tracker build() {
            ...
            return new Tracker(group, heartRate, maxMissedHeartbeats, reconnectDelay, maxReconnectDelay,
                    maxReconnectAttempts, exponentialBackoff, logger, debug);
        }
    }
    ...
    public class Service {
        ...
        @Override
        public String toString() {
            return "Service{" + "uri=" + uri + ", broadcastString='" + broadcastString + '\'' + '}';
        }
    }
    ...
    private class ServiceVitals {
        ...
        @Override
        public String toString() {
            return service + "Vitals{" + ", lastHeartBeat=" + lastHeartBeat + ", recoveryTime=" + recoveryTime
                    + ", failureCount=" + failureCount + ", dead=" + dead + '}';
        }
    }
}
