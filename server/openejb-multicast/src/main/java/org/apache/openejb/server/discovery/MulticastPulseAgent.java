...
import org.apache.openejb.server.*;
...
public class MulticastPulseAgent implements DiscoveryAgent, ServerService, SelfManaging {
    ...
    private static final Logger log = Logger.getInstance(
            LogCategory.OPENEJB_SERVER.createChild("discovery").createChild("multipulse"), MulticastPulseAgent.class);
    ...
    /**
     * This agent listens for client pulses on a defined multicast channel. On receipt of a valid pulse the agent
     * responds with its own pulse for a defined amount of time and rate. A client can deliver a pulse as often as
     * required until it is happy of the server response.
     * <p/>
     * Both server and client deliver crafted information payloads.
     * <p/>
     * The client pulse contains OpenEJB.MCP.Client:(group or *)[:BadUri:URI] The server will only respond to a request
     * for it's own group or * The optional :BadUri: is used by clients to notify a server that it is sending out
     * unreachable URI's
     * <p/>
    ...
    /**
     * Attempts to return at least one socket per valid network interface. If no valid interface is found then the array
     * will be empty.
     *
     * @param multicastAddress
     *            A valid multicast address
     * @param port
     *            A valid multicast port
    ...
    private static MulticastSocket[] getSockets(final InetAddress ia, final int port) throws Exception {
        ...
        for (final NetworkInterface ni : getInterfaces()) {
            ...
            try {
                ...
                log.debug(String.format("Created MulticastSocket for '%1$s:%2$s' on network adapter: %3$s",
                        ia.getHostName(), port, ni));
    ...
    public void init(final Properties p) throws Exception {
        ...
        try {
            ...
            log.warning(
                    "Invalid ignore parameter. Should be a lowercase single host or comma seperated list of hosts to ignore like: ignore=host1,host2,ipv4,ipv6");
        }
                                    ...
                                    if (req.startsWith(CLIENT)) {
                                        ...
                                        if (mpg.equals(req) || "*".equals(req)) {
                                            ...
                                            if (null != badUri) {
                                                if (getHosts(agent.ignore).contains(badUri)) {
                                                    ...
                                                    try {
                                                        ...
                                                        if (agent.ignore.add(badUri)) {
                                                            agent.buildPacket();
                                                            log.warning("This server has removed the unreachable host '"
                                                                    + badUri
                                                                    + "' from discovery, you should consider adding"
                                                ...
                                                final String client = ((InetSocketAddress) sa).getAddress()
                                                        .getHostAddress();
                                                ...
                                                if (isLoopBackOnly
                                                    ...
                                                    if (log.isDebugEnabled()) {
                                                        log.debug(String.format(
                                                                "Ignoring remote client %1$s pulse request for group: %2$s - No remote services available",
                                                                client, req));
                                                    }
                                                } else {
                                                    ...
                                                    if (log.isDebugEnabled()) {
                                                        log.debug(String.format(
                                                                "Answering client '%1$s' pulse request for group: '%2$s' on '%3$s'",
    ...
    /**
     * Lists current broadcast hosts as a comma separated list. Used principally for testing.
     *
     * @return String
     */
    ...
    /**
     * Remove a host from the ignore list. Used principally for testing.
     *
     * @param host
     *            String
