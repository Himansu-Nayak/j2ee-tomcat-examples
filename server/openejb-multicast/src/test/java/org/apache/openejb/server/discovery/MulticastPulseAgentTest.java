                                ...
                                if (s.startsWith(MulticastPulseAgent.SERVER)) {
                                    ...
                                    for (final String svc : serviceList) {
                                        ...
                                        if (schemes.contains(serviceUri.getScheme())) {
                                            ...
                                            final String serverHost = ((InetSocketAddress) response.getSocketAddress())
                                                ...
                                                if (fullsvc.contains("0.0.0.0")) {
                                                    for (final String h : hosts) {
                                                        if (!h.replace("[", "").startsWith("2001:0:")) { //Filter Teredo
                                                            set.add(URI
                                                                    .create(fullsvc.replace("0.0.0.0", ipFormat(h))));
                                                        }
                                                    }
                                            ...
                                            System.out.println("Reject service: " + serviceUri.toASCIIString()
                                                    + " - Not looking for scheme: " + serviceUri.getScheme());
                                        }
                                    }
                                }
                            }
        ...
        final Future<?> future = executor.submit(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    final byte[] bytes = (MulticastPulseAgent.CLIENT + forGroup + MulticastPulseAgent.BADURI + host)
                            .getBytes(Charset.forName("UTF-8"));
                    final DatagramPacket request = new DatagramPacket(bytes, bytes.length,
                            new InetSocketAddress(ia, port));

                    final MulticastSocket[] multicastSockets = MulticastPulseAgent
                            .getSockets(MulticastPulseAgentTest.host, port);
                    ...
                    for (int i = 0; i < 5; i++) {
                        for (final MulticastSocket socket : multicastSockets) {
                            ...
                            try {
                                ...
                                System.out.println(
                                        "Failed to broadcast bad URI on: " + socket.getInterface().getHostAddress());
                                e.printStackTrace();
                            }
                        }
                    }
