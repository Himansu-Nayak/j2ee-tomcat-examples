...
public class LightWeightMappingValidator extends WsdlVisitor {
    ...
    protected void visit(Definition definition) {
        if (definition.getServices().values().size() != 1) {
            context.addFailure(
                    new ValidationFailure("A lightweight RPC/Encoded service must contain only one Service"));
        }
    }
    ...
    protected void visit(Output output) {
        ...
        if (outputParts.size() != 0 && outputParts.size() != 1) {
            context.addFailure(
                    new ValidationFailure("The output message must contain zero or one parts: " + output.getName()));
        }
    ...
    protected void visit(Operation operation) {
        if (!operationNames.add(operation.getName())) {
            context.addFailure(
                    new ValidationFailure("No two operations can have the same name: " + operation.getName()));
        }
    }
    ...
    protected void visit(Fault fault) {
        ...
        if (message == null) {
            context.addFailure(new ValidationFailure(
                    "The fault message must contain one part named 'message' : " + fault.getName()));
        } else if (!XSD_STRING.equals(message.getTypeName())) {
            context.addFailure(new ValidationFailure(
                    "The fault message must contain one part of type 'xsd:string' : " + fault.getName()));
        }
    }
    ...
    protected void visit(BindingInput bindingInput) {
        ...
        if (encoding == null || !encoding.equals("encoded")) {
            context.addFailure(new ValidationFailure(
                    "The use attribute of the binding input operation must be 'encoded': " + bindingInput.getName()));
        }
    }
    ...
    protected void visit(BindingOutput bindingOutput) {
        ...
        if (encoding == null || !encoding.equals("encoded")) {
            context.addFailure(new ValidationFailure(
                    "The use attribute of the binding output operation must be 'encoded': " + bindingOutput.getName()));
        }
    }
    ...
    protected void visit(BindingFault bindingFault) {
        ...
        if (encoding == null || !encoding.equals("encoded")) {
            context.addFailure(new ValidationFailure(
                    "The use attribute of the binding fault operation must be 'encoded': " + bindingFault.getName()));
        }
    }
    ...
    protected void visit(Binding binding) {
        ...
        if (soapBinding == null || soapBinding.getStyle() == null || !soapBinding.getStyle().equals("rpc")) {
            context.addFailure(
                    new ValidationFailure("The messaging style of the binding must be rpc: " + binding.getQName()));
        }
    }
