...
public abstract class WsService implements ServerService, SelfManaging {
    ...
    protected abstract HttpListener createEjbWsContainer(URL moduleBaseUrl, PortData port, BeanContext beanContext,
            ServiceConfiguration configuration) throws Exception;

    protected abstract void destroyEjbWsContainer(String deploymentId);

    protected abstract HttpListener createPojoWsContainer(ClassLoader loader, URL moduleBaseUrl, PortData port,
            String serviceId, Class target, Context context, String contextRoot, Map<String, Object> bindings,
            ServiceConfiguration configuration) throws Exception;
        ...
        for (final EjbJarInfo ejbJar : appInfo.ejbJars) {
            ...
            for (final EnterpriseBeanInfo bean : ejbJar.enterpriseBeans) {
                if (bean instanceof StatelessBeanInfo || bean instanceof SingletonBeanInfo) {
                    ...
                    try {
                        final PortData port = WsBuilder.toPortData(portInfo, beanContext.getInjections(), moduleBaseUrl,
                                beanContext.getClassLoader());

                        final HttpListener container = createEjbWsContainer(moduleBaseUrl, port, beanContext,
                        ...
                        if (wsRegistry != null) {
                            ...
                            if ("BASIC".equals(portInfo.authMethod) || "DIGEST".equals(portInfo.authMethod)
                            ...
                            final List<String> addresses = wsRegistry.addWsContainer(container, classLoader, context,
                                    host, location, realm, transport, auth, moduleId);
                            ...
                            if (address != null) {
                                ...
                                portAddressRegistry.addPort(portInfo.serviceId, portInfo.wsdlService, portInfo.portId,
                                        portInfo.wsdlPort, portInfo.seiInterfaceName, address);
                                ...
                                addressesForApp(appInfo.appId).add(new EndpointInfo(address, port.getWsdlService(),
    ...
    public void afterApplicationCreated(final AppInfo appInfo, final WebAppInfo webApp) {
        ...
        for (final ServletInfo servlet : webApp.servlets) {
            ...
            try {
                ...
                final HttpListener container = createPojoWsContainer(classLoader, moduleBaseUrl, port,
                        portInfo.serviceLink, target, context, webApp.contextRoot, bindings, new ServiceConfiguration(
                                PojoUtil.findConfiguration(pojoConfiguration, target.getName()), appInfo.services));
                ...
                if (wsRegistry != null) {
                    ...
                    final List<String> addresses = wsRegistry.setWsContainer(container, classLoader, webApp.contextRoot,
                            host(webApp), servlet, realm, transport, auth, webApp.moduleId);
                    ...
                    portAddressRegistry.addPort(portInfo.serviceId, portInfo.wsdlService, portInfo.portId,
                            portInfo.wsdlPort, portInfo.seiInterfaceName, address);
                    setWsdl(container, address);
                    logger.info("Webservice(wsdl=" + address + ", qname=" + port.getWsdlService() + ") --> Pojo(id="
                            + portInfo.portId + ")");
                    servletAddresses.put(webApp.moduleId + "." + servlet.servletName, address);
                    addressesForApp(webApp.moduleId)
                            .add(new EndpointInfo(address, port.getWsdlService(), target.getName()));
                }
            } catch (final Throwable e) {
            ...
            for (final EjbJarInfo ejbJar : appInfo.ejbJars) {
                ...
                for (final EnterpriseBeanInfo enterpriseBean : ejbJar.enterpriseBeans) {
                    if (enterpriseBean instanceof StatelessBeanInfo || enterpriseBean instanceof SingletonBeanInfo) {
                        ...
                        addressesForApp(appInfo.appId).remove(
                                new EndpointInfo(address, portInfo.wsdlPort, beanContext.getBeanClass().getName()));
    ...
    private String autoAssignWsLocation(final EnterpriseBeanInfo bean, final PortData port,
            final Map<String, String> contextData, final StringTemplate template) {
