    ...
    public void checkZipIsOn() throws Exception {
        ...
        for (final ServerService daemon : SimpleServiceManager.class.cast(ServiceManager.get()).getDaemons()) {
            if (ServiceDaemon.class.isInstance(daemon) && daemon.getName().equals("ejbd")) {
                assertTrue(EjbDaemon.class.cast(Reflections.get(Reflections.get(Reflections.get(Reflections.get(
                        Reflections.get(Reflections.get(Reflections.get(daemon, "next"), "service"), "service"),
                        "service"), "service"), "service"), "server")).isGzip());
                checked++;
            }
        }
        ...
        final Context ctx = new InitialContext(new Properties() {
            {
                ...
                setProperty(Context.PROVIDER_URL,
                        scheme + "://localhost:" + ejbd + "?connectTimeout=1000&readTimeout=1000");
            }
        });
