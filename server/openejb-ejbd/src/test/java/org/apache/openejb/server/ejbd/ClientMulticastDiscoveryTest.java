...
public class ClientMulticastDiscoveryTest extends TestCase {
    ...
    public void test() throws Exception {
        /*
         * System.setProperty("openejb.client.requestretry", "true"); Properties initProps = new Properties();
         * initProps.setProperty("openejb.deployments.classpath.include", "");
         * initProps.setProperty("openejb.deployments.classpath.filter.descriptors", "true"); OpenEJB.init(initProps,
         * new ServerFederation());
         *
         * Assembler assembler = SystemInstance.get().getComponent(Assembler.class); ConfigurationFactory config = new
         * ConfigurationFactory();
         *
         * EjbJar ejbJar = new EjbJar(); ejbJar.addEnterpriseBean(new StatelessBean(Target.class));
         * assembler.createApplication(config.configureApplication(ejbJar));
         *
         *
         * MulticastDiscoveryAgent multicastAgent = new MulticastDiscoveryAgent(); DiscoveryRegistry registry = new
         * DiscoveryRegistry(multicastAgent); SystemInstance.get().setComponent(DiscoveryRegistry.class, registry);
         * this.agent = registry; multicastAgent.start();
         *
         *
         * ServerService red = server(Host.RED); ServerService blue = server(Host.BLUE); ServerService green =
         * server(Host.GREEN);
         *
         * red.start();
         *
         ...
         *
         * Context context = new InitialContext(props); TargetRemote target = (TargetRemote)
         * context.lookup("TargetRemote");
         *
         * assertEquals(Host.RED, target.getHost());
         *
         * red.stop();
         *
         * // Blue is not in the cluster list as it was started // after the last invocation list was sent. // We're
         * testing that the client can fail all // the way out of the cluster list and back to // the original
         * multicast://239.255.2.3:6142 uri. blue.start();
         *
         * assertEquals(Host.BLUE, target.getHost());
         *
         * blue.stop();
         *
         * try { target.getHost(); } catch (EJBException e) { // pass.. no server available }
         */
    }
