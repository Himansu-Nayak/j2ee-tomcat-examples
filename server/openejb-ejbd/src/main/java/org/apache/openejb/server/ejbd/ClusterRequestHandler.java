...
public class ClusterRequestHandler extends RequestHandler implements DiscoveryListener {
    ...
    private static final Logger logger = Logger.getInstance(LogCategory.OPENEJB_SERVER_REMOTE.createChild("cluster"),
            ClusterRequestHandler.class);
    ...
    @Override
    public ClusterResponse processRequest(final ObjectInputStream in, final ProtocolMetaData metaData)
            throws Exception {
    ...
    public void serviceAdded(final URI uri) {
        try {
            ...
            logger.error("serviceAdded: Invalid service URI format.  Expected <group>:<type>:<serverURI> but found '"
                    + uri.toString() + "'");
        }
    }
