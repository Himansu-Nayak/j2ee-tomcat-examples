...
public class EjbDaemon implements org.apache.openejb.spi.ApplicationServer {
    ...
    static final Logger logger = Logger.getInstance(LogCategory.OPENEJB_SERVER_REMOTE,
    ...
    public void init(final Properties props) throws Exception {
        ...
        if (serializer != null) {
            try {
                this.serializer = EJBDSerializer.class
    ...
    public void service(final InputStream rawIn, final OutputStream rawOut) throws IOException {
        ...
        try {
            ...
            case LOGOUT_REQUEST:
                ...
                break;
            default:
                logger.error("\"" + requestType + " " + clientProtocol.getSpec() + "\" FAIL \"Unknown request type "
                        + requestType);
                return;
            }
            ...
            try {
                ...
                default:
                    ...
                    logger.error("\"" + requestType + " " + clientProtocol.getSpec()
                            + "\" FAIL \"Unknown response type " + requestType);
                }
            }
            ...
            final String msg = "\"" + requestType + " " + clientProtocol.getSpec() + "\" FAIL \"Security error - "
                    + e.getMessage() + "\"";
            if (logger.isDebugEnabled()) {
                logger.debug(msg, e);
            } else {
            ...
            final String msg = "\"" + requestType + " " + clientProtocol.getSpec() + "\" FAIL \"Unexpected error - "
                    + e.getMessage() + "\"";
            if (logger.isDebugEnabled()) {
                logger.debug(msg, e);
            } else {
    ...
    public void processEjbResponse(final Response response, final ObjectOutputStream out,
            final ProtocolMetaData metaData) throws Exception {
        ejbHandler.processResponse(response, out, metaData);
    }

    public void processJndiResponse(final Response response, final ObjectOutputStream out,
            final ProtocolMetaData metaData) throws Exception {
        jndiHandler.processResponse(response, out, metaData);
    }

    public void processAuthResponse(final Response response, final ObjectOutputStream out,
            final ProtocolMetaData metaData) throws Exception {
        authHandler.processResponse(response, out, metaData);
    }

    public void processLogoutResponse(final Response response, final ObjectOutputStream out,
            final ProtocolMetaData metaData) throws Exception {
