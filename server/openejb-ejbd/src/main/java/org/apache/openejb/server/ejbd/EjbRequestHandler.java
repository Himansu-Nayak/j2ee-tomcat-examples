...
class EjbRequestHandler extends RequestHandler {
    ...
    private static final Logger logger = Logger.getInstance(LogCategory.OPENEJB_SERVER_REMOTE.createChild("ejb"),
    ...
    public Response processRequest(final ObjectInputStream in, final ProtocolMetaData metaData) throws Exception {
        ...
        try {
            ...
            try {
                ...
                failed = true;
                return setResponseError(res, version, t,
                        "Unable to set the thread call context for this request: " + req);
            }
        ...
        try {
            ...
            if (authentication != null) {
                try {
                    securityToken = securityService.login(authentication.getRealm(), authentication.getUser(),
            ...
            if (res.getResponseCode() != ResponseCodes.AUTH_DENIED) {
                ...
                default:
                    throw new org.apache.openejb.SystemException(
                            "Unexpected request method: " + req.getRequestMethod());
                }
            }
    ...
    @Override
    public void processResponse(final Response response, final ObjectOutputStream out, final ProtocolMetaData metaData)
            throws Exception {
    ...
    protected void doEjbObject_BUSINESS_METHOD(final EJBRequest req, final EJBResponse res) throws Exception {
        ...
        try {
            ...
            Object result = c.invoke(req.getDeploymentId(), InterfaceType.EJB_OBJECT, req.getInterfaceClass(),
                    req.getMethodInstance(), req.getMethodParameters(), req.getPrimaryKey());
    ...
    protected void doEjbHome_METHOD(final EJBRequest req, final EJBResponse res) throws Exception {
        ...
        final Object result = c.invoke(req.getDeploymentId(), InterfaceType.EJB_HOME, req.getInterfaceClass(),
                req.getMethodInstance(), req.getMethodParameters(), req.getPrimaryKey());
    ...
    protected void doEjbHome_CREATE(final EJBRequest req, final EJBResponse res) throws Exception {
        ...
        Object result = c.invoke(req.getDeploymentId(), InterfaceType.EJB_HOME, req.getInterfaceClass(),
                req.getMethodInstance(), req.getMethodParameters(), req.getPrimaryKey());
        ...
        if (result instanceof ProxyInfo) {
            ...
            result = new RemoteException(
                    "The bean is not EJB compliant.  The bean should be created or and exception should be thrown.");
            logger.error(req
                    + "The bean is not EJB compliant.  The bean should be created or and exception should be thrown.");
    ...
    protected void doEjbHome_FIND(final EJBRequest req, final EJBResponse res) throws Exception {
        ...
        if (result instanceof Collection) {
            ...
            res.setResponse(req.getVersion(), ResponseCodes.EJB_OK_FOUND_ENUMERATION,
            ...
            final String message = "The bean is not EJB compliant. " + "The finder method ["
                    + req.getMethodInstance().getName() + "] is declared "
                    + "to return neither Collection nor the Remote Interface, " + "but [" + result.getClass().getName()
                    + "]";
    ...
    protected void doEjbObject_REMOVE(final EJBRequest req, final EJBResponse res) throws Exception {
        ...
        c.invoke(req.getDeploymentId(), InterfaceType.EJB_OBJECT, req.getInterfaceClass(), req.getMethodInstance(),
    ...
    protected void doEjbHome_REMOVE_BY_HANDLE(final EJBRequest req, final EJBResponse res) throws Exception {
        ...
        c.invoke(req.getDeploymentId(), InterfaceType.EJB_HOME, req.getInterfaceClass(), req.getMethodInstance(),
                req.getMethodParameters(), req.getPrimaryKey());
    ...
    private EJBResponse setResponseError(final EJBResponse res, final byte version, final Throwable error,
