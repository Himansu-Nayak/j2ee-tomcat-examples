...
import org.apache.openejb.core.webservices.*;
...
class JndiRequestHandler extends RequestHandler {
    ...
    private static final Logger logger = Logger.getInstance(LogCategory.OPENEJB_SERVER_REMOTE.createChild("jndi"),
    ...
    private void logRequestResponse(final JNDIRequest req, final JNDIResponse res) {
        ...
        logger.debug("JNDI REQUEST: " + req + " (size = " + (null != cis ? cis.getCount() : 0) + "b, remote-ip ="
                + info.ip + ") -- RESPONSE: " + res + " (size = " + (null != cos ? cos.getCount() : 0) + "b)");
    }
    ...
    private void doLookup(final JNDIRequest req, final JNDIResponse res, final String prefix) {
        ...
        try {
            ...
            if (object instanceof Context) {
                ...
                if (DataSourceFactory.knows(object)) {
                    try {
                        ...
                        final DataSourceMetaData dataSourceMetaData = new DataSourceMetaData(cf.getDriverClassName(),
                                cf.getUrl(), cf.getUsername(), cf.getPassword());
            ...
            if (serviceRef != null) {
                ...
                final PortAddressRegistry portAddressRegistry = SystemInstance.get()
                        .getComponent(PortAddressRegistry.class);
            ...
            try {
                ...
                if (object instanceof java.io.Serializable) {
                    ...
                    final NamingException namingException = new NamingException(
                            "Expected an ejb proxy, found unknown object: type=" + object.getClass().getName()
                                    + ", toString=" + object);
                    res.setResult(new ThrowableArtifact(namingException));
                    return;
                }
        ...
        case EJB_HOME: {
            ...
            final EJBMetaDataImpl metaData = new EJBMetaDataImpl(beanContext.getHomeInterface(),
                    beanContext.getRemoteInterface(), beanContext.getPrimaryKeyClass(),
                    beanContext.getComponentType().toString(), deploymentID, -1, convert(proxyInfo.getInterfaceType()),
                    null, beanContext.getAsynchronousMethodSignatures());
        ...
        case BUSINESS_REMOTE: {
            ...
            final EJBMetaDataImpl metaData = new EJBMetaDataImpl(null, null, beanContext.getPrimaryKeyClass(),
                    beanContext.getComponentType().toString(), deploymentID, -1, convert(proxyInfo.getInterfaceType()),
                    proxyInfo.getInterfaces(), beanContext.getAsynchronousMethodSignatures());
        ...
        case LOCALBEAN: {
            res.setResponseCode(ResponseCodes.JNDI_NAMING_EXCEPTION);
            final NamingException namingException = new NamingException(
                    "Not remotable: '" + name + "'. LocalBean classes are not remotable as per the EJB specification.");
            res.setResult(new ThrowableArtifact(namingException));
            break;
        }
        default: {
