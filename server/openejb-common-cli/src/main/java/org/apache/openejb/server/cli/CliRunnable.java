...
public class CliRunnable implements Runnable {
    ...
    static {
        ...
        try {
            ...
            final IAnnotationFinder finder = new AnnotationFinder(new ConfigurableClasspathArchive(
                    new ConfigurableClasspathArchive.FakeModule(loader, Collections.EMPTY_MAP), true,
                    urlSet.getUrls()));
            for (final Annotated<Class<?>> cmd : finder.findMetaAnnotatedClasses(Command.class)) {
                try {
                    ...
                    if (!COMMANDS.containsKey(key)) {
                        ...
                        LOGGER.warning("command " + key + " already exists, this one will be ignored ( "
    ...
    public void run() {
        ...
        try {
            ...
            final StringBuilder builtWelcome = new StringBuilder("Apache OpenEJB ")
                    .append(OpenEjbVersion.get().getVersion()).append("    build: ")
                    .append(OpenEjbVersion.get().getDate()).append("-").append(OpenEjbVersion.get().getTime())
                    .append(lineSep);
            if (tomee) {
            ...
            streamManager.writeOut(builtWelcome.toString().replace("$bind", bind)
                    .replace("$port", Integer.toString(port)).replace("$name", NAME).replace(OS_LINE_SEP, lineSep));
    ...
    private String prompt() {
        ...
        prompt.append(" @ ").append(bind).append(":").append(port).append(PROMPT_SUFFIX);
