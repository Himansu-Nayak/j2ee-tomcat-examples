    ...
    public static File[] resolve(final String pom) {
        if (!CACHE.containsKey(pom)) {
            try {
                ...
                CACHE.put(pom, Maven.resolver().offline(true).loadPomFromFile(pom).importRuntimeAndTestDependencies()
                        .resolve().withTransitivity().asFile());
            } catch (ResolutionException re) { // try on central
                CACHE.put(pom, Maven.resolver().loadPomFromFile(pom).importRuntimeAndTestDependencies().resolve()
