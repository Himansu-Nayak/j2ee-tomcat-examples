...
public class LegacyClientTest {
    ...
    public static void beforeClass() throws Exception {
        logger.info("Retrieving standalone server: " + Repository.guessVersion("org.apache.tomee", "openejb-standalone")
    ...
    private static void assertBalance(final Calculator bean, final int size) {
        ...
        for (final Map.Entry<String, AtomicInteger> entry : entries) {
            ...
            Assert.assertTrue(String.format("%s out of %s is too low", actualInvocations, expectedInvocations),
                    actualInvocations > expectedInvocations * percent);
        }
    }
    ...
    public static class Services {
        ...
        public void assertServices(final long timeout, final TimeUnit unit, final Callable callable, final int delay) {
            ...
            client.start();
            try {
                Assert.assertTrue(String.format("services failed to come online: waited %s %s", timeout, unit),
                        await(timeout, unit));
