...
public class EmployeeBean implements javax.ejb.EntityBean {
    ...
    public Integer ejbCreate(final String fname, final String lname) throws javax.ejb.CreateException {
        try {
            lastName = lname;
            firstName = fname;
            ...
            try {
                ...
                try {
                    ...
                    stmt = con
                            .prepareStatement("select EmployeeID from Employees where FirstName = ? AND LastName = ?");
                    stmt.setString(1, firstName);
                    stmt.setString(2, lastName);
        ...
        try {
            ...
            try {
                final PreparedStatement stmt = con
                        .prepareStatement("update Employees set FirstName = ?, LastName = ? where EmployeeID = ?");
                try {
                    stmt.setString(1, firstName);
                    stmt.setString(2, lastName);
                    stmt.setInt(3, id);
