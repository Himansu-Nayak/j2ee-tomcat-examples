...
public class MdbProxy {
    ...
    public static <T> T newProxyInstance(final Class<T> type, final ConnectionFactory connectionFactory,
            final String requestQueueName) throws JMSException {
    ...
    @SuppressWarnings({ "unchecked" })
    public static <T> T newProxyInstance(final Class<T> type, final ConnectionFactory connectionFactory,
            final Destination requestQueue) throws JMSException {
        ClassLoader classLoader = Thread.currentThread().getContextClassLoader();
        if (classLoader == null)
            classLoader = type.getClassLoader();
        if (classLoader == null)
    ...
    private static class MdbInvocationHandler implements InvocationHandler {
        ...
        public MdbInvocationHandler(final ConnectionFactory connectionFactory, final String requestQueueName)
                throws JMSException {
            // open a connection
        ...
        public MdbInvocationHandler(final ConnectionFactory connectionFactory, final Destination requestQueue)
                throws JMSException {
            ...
            try {
                ...
                if (message == null)
                    throw new NullPointerException(
                            "Did not get a response within " + MdbProxy.MdbInvocationHandler.MAX_RESPONSE_WAIT + "ms");
                ...
                if (!(message instanceof ObjectMessage))
                    throw new IllegalArgumentException(
                            "Expected a ObjectMessage response but got a " + message.getClass().getName());
                ...
                if (!(object instanceof Map)) {
                    if (ObjectMessage.class.isInstance(message))
                        throw new IllegalArgumentException(
                                "Expected a Map contained in the ObjectMessage response but got a "
