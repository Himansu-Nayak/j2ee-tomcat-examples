...
public class StatefulBeanTxTests extends org.apache.openejb.test.NamedTestCase {
     ...
     * <p/>
     * The Container must make the javax.transaction.UserTransaction interface available to the enterprise bean's
     * business method via the javax.ejb.EJBContext interface and under the environment entry java:comp/UserTransaction.
     * When an instance uses the javax.trans-action. UserTransaction interface to demarcate a transaction, the Container
     * must enlist all the resource managers used by the instance between the begin() and commit(),or rollback(),
     * methods with the transaction. When the instance attempts to commit the transaction, the Container is responsible
     * for the global coordination of the transaction commit.
     * </P>
     * <P>
     * --------------------------------------------------------
     * </P>
     ...
     * </P>
     * <p/>
     * Check that a javax.transaction.UserTransaction can be obtained from the environment entry
    ...
    public void test02_java_comp_UserTransaction() {
        try {
            ...
            assertNotNull(
                    "UserTransaction is null. Could not retreive a UserTransaction from the bean's JNDI namespace.", t);
        } catch (final Exception e) {
            fail("Could not retreive a UserTransaction from the bean's JNDI namespace. Received Exception "
     ...
     * <B>11.6.1 Bean-managed transaction demarcation</B>
     * <p/>
     * The Container must throw the java.lang.IllegalStateException if an instance of a bean with bean-managed
     * transaction demarcation attempts to invoke the setRollbackOnly() or getRollbackOnly() method of the
    ...
    /**
     * This test does work for the IntraVM Server, but it fails on the Remote Server. For some reason, when the
     * RollbackException is sent to the client, the server blocks.
     */
     ...
     * <B>11.6.1 Bean-managed transaction demarcation</B>
     * <p/>
     * When an instance attempts to start a transaction using the begin() method of the
     * javax.transaction.UserTransaction interface while the instance has not committed the previous transaction, the
     * Container must throw the javax.transaction.NotSupportedException in the begin() method.
     * </P>
     */
     ...
     * association between the transaction and the instance across multiple client calls until the instance commits or
     * rolls back the transaction. When the client invokes the next business method, the Container must invoke the
     * business method in this transaction context.
     * </P>
     */
     ...
     * <B>11.6.1 Bean-managed transaction demarcation</B>
     * <p/>
     * The actions performed by the Container for an instance with bean-managed transaction are summarized by the
     * following table. T1 is a transaction associated with a client request, T2 is a transaction that is cur-rently
     * associated with the instance (i.e. a transaction that was started but not completed by a previous business
     * method).
     * </P>
     * 
     * <PRE>
     ...
     * </PRE>
     * <p/>
     * If the client request is not associated with a transaction and the instance is not associated with a transaction,
     * the container invokes the instance with an unspecified transaction context.
     * </P>
     * <P>
     * --------------------------------------------------------
     * </P>
     ...
     * If the client's transaction is none and the transaction currently associated with instance none then the
     * transaction associated with the method is none.
     * </P>
     */
     ...
     * </PRE>
     * <p/>
     * If the client is associated with a transaction T1, and the instance is not associated with a transaction, the
     * container suspends the client's transaction association and invokes the method with an unspecified transaction
     * context. The container resumes the client's transaction association (T1) when the method completes.
     * </P>
     * <P>
     * --------------------------------------------------------
     * </P>
     ...
     * </PRE>
     * <p/>
     * If the client request is not associated with a transaction and the instance is already associated with a
     * transaction T2, the container invokes the instance with the transaction that is associated with the instance
     * (T2). This case can never happen for a stateless Session Bean.
     * </P>
     * <P>
     * --------------------------------------------------------
     * </P>
     ...
     * </PRE>
     * <p/>
     * If the client is associated with a transaction T1, and the instance is already associated with a transaction T2,
     * the container suspends the client's transaction association and invokes the method with the transaction context
     * that is associated with the instance (T2). The container resumes the client's transaction association (T1) when
     * the method completes. This case can never happen for a stateless Session Bean.
     * </P>
     * <P>
     * --------------------------------------------------------
     * </P>
