...
public class TestManager {
    ...
    public static void init(String propertiesFileName) throws Exception {
        ...
        try {
            ...
            throw new IllegalArgumentException(
                    "Cannot access the system properties: " + e.getClass().getName() + " " + e.getMessage());
        }
        ...
        if (propertiesFileName == null) {
            try {
                ...
                throw new IllegalArgumentException("Cannot access the system property \"" + TESTSUITE_PROPERTY_FILENAME
    ...
    private static void initServer(final Properties props) {
        try {
            ...
            if (className == null) {
                throw new IllegalArgumentException(
                        "Must specify a test server by setting its class name using the system property \""
                                + TEST_SERVER_CLASSNAME + "\"");
            }
            ...
            if (warn)
                e.printStackTrace();
            if (warn)
                System.out.println("Cannot instantiate or initialize the test server: " + e.getClass().getName() + " "
                        + e.getMessage());
            throw new RuntimeException("Cannot instantiate or initialize the test server: " + e.getClass().getName()
                    + " " + e.getMessage(), e);
        }
    }
    ...
    private static void initDatabase(final Properties props) {
        try {
            ...
            if (className == null)
                throw new IllegalArgumentException(
                        "Must specify a test database by setting its class name  using the system property \""
                                + TEST_DATABASE_CLASSNAME + "\"");
            final ClassLoader cl = getContextClassLoader();
            ...
            if (warn)
                System.out.println("Cannot instantiate or initialize the test database: " + e.getClass().getName() + " "
                        + e.getMessage());
            throw new RuntimeException("Cannot instantiate or initialize the test database: " + e.getClass().getName()
                    + " " + e.getMessage(), e);
        }
    }
    ...
    private static void initJms(final Properties props) {
        try {
            ...
            if (warn)
                System.out.println("Cannot instantiate or initialize the test jms: " + e.getClass().getName() + " "
                        + e.getMessage());
            throw new RuntimeException(
                    "Cannot instantiate or initialize the test jms: " + e.getClass().getName() + " " + e.getMessage(),
                    e);
        }
    }
