...
/**
 * This class tests that all javax.ejb.EJBObject methods work as expected on the EJB 2.1 compatible remote interface of
 * an ejb3 stateless bean. <br>
...
public class StatelessPojoEjbObjectTests extends BasicStatelessTestClient {
    ...
    /**
     * According to the EJB3.0 "Core Contracts and Requirements" specs, section 3.6.4, a session EJBObject supports: 1.
     * Get the session object's remote home interface. 2. Get the session object's handle. 3. Test if the session object
     * is identical with another session object. 4. Remove the session object.
     */
    ...
    /**
     * A method to test the implementation of Stateless Session Bean identity check. See EJB3.0
     * "Core Contracts and Requirements" specification, section 3.4.5.2
     */
    public void test03_isIdentical() {
        BasicStatelessObject otherEJBObject = null;
        try {
            /**
             * This EJBObject reference is created to validate the identity if different EJBObject references of the
             * same interface type of the same session bean.
             */
    ...
    /**
     * This test is 2 in 1, it tests calling remove() on an EJBObject reference and then calling a business method on
     * the same reference after the remove() is successfuly called.
     */
