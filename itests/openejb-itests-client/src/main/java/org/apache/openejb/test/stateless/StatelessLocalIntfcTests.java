...
public class StatelessLocalIntfcTests extends BasicStatelessLocalTestClient {
    ...
    protected void setUp() throws Exception {
        super.setUp();
        final Object obj = initialContext.lookup("client/tests/stateless/BasicStatelessPojoHomeLocal");
    ...
    public void test04_throwSystemException() {
        try {
            ...
            assertTrue(
                    "Nested exception should be an instance of NullPointerException, but exception is "
                            + e.getCausedByException().getClass().getName(),
                    (e.getCausedByException() instanceof NullPointerException));
            return;
        } catch (final Throwable e) {
            fail("Received Exception " + e.getClass() + " : " + e.getMessage());
        }
    ...
    /**
     * After a system exception the intance should be garbage collected but this is invisible to the client as it will
     * just use another stateless session object. All the stateless session objects are equal. Refer 4.5.3 in EJB 3.0
     * core specification. This one seems to fail. we should double-check the spec on this.
     */
    ...
    /*
     * Test an Application Exception that is a runtime exception Throwing an application exception within a transaction
     * will rollback only if that method is marked for rollback Also FinderException, CreateException and
     * RemoveException are application exceptions.
     */
}