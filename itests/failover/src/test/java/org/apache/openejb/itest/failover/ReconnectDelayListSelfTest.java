...
/**
 * This test verifies the situation where none of the servers listed in the 'initialServers' list can be contacted.
 * <p/>
 * A server that is unable to connect to any of its peers should continue trying to connect to servers on the
 * 'initialServers' list. How long to wait between attempts is dictated by the 'reconnectDelay' setting, which is 30
 * seconds by default.
 */
public class ReconnectDelayListSelfTest {
    ...
    public void test() throws Exception {
        ...
        for (final Map.Entry<String, StandaloneServer> entry : servers.entrySet()) {
            ...
            final StandaloneServer.ServerService multipoint = server.getServerService(MULTIPOINT);
            final String value = "localhost:" + red.getServerService(MULTIPOINT).getPort() + ",localhost:"
                    + multipoint.getPort();
